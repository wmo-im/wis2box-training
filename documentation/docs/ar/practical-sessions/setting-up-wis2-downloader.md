...
في هذه الجلسة، ستتعلم كيفية إعداد نسخة من WIS2 Downloader على الجهاز الافتراضي الخاص بالطالب وكيفية التنقل عبر خدماته المختلفة.
...
!!! note "إذا لم يكن أثناء التدريب"
    ...
    الخطوات التالية تطبق فقط إذا كانت المنافذ المذكورة غير متاحة افتراضيًا على الخادم. في أي تكوين، هذه هي المنافذ الوحيدة التي تحتاج إلى الوصول إليها لاستخدام الإمكانيات الكاملة لمكدس WIS2 Downloader.    
...
!!! note "إنشاء النفق على Linux وmacOS"
    ...
    على Linux وmacOS، يمكنك إعداد نفس الأنفاق مباشرة من الطرفية باستخدام العلم `-L` مع أمر SSH الخاص بك:
...
ابدأ تشغيل المكدس الكامل للخدمة:
...
ستجد نفسك في الصفحة الرئيسية التي تم تعيينها افتراضيًا على عرض `Dashboard` الذي يعرض لوحة تحكم Grafana.
...
- **Dashboard** — الصفحة الرئيسية الافتراضية، وهي لوحة تحكم Grafana مدمجة تعرض نشاط التنزيل، حالة الطابور، ومقاييس الخدمة قيد التشغيل. متاحة أيضًا على `http://<WIS2DOWNLOADER_BASE_URL>:3000`.
...
- أولاً، اذهب إلى **Manual Subscribe**.
...
!!! note "تحديث تكوين WIS2 Downloader"
    ...
    لتحديث التكوين، قم بتحرير ملف `.env` وأعد تشغيل المكدس لتطبيق التغييرات:
...
للحصول على القائمة الكاملة لنقاط النهاية المتاحة (قائمة، الحصول على، تحديث الاشتراكات والمزيد)، راجع الوثائق التفاعلية لـ Swagger المتوفرة على `localhost:5002/swagger`.
