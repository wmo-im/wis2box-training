...
!!! abstract "نتائج التعلم"
    ...
    - تكوين مكون إضافي لتحويل بيانات CSV إلى BUFR باستخدام قالب تحويل BUFR مخصص
    - استخدام القوالب المدمجة AWS وDAYCLI لتحويل بيانات CSV إلى BUFR

## المقدمة

تُستخدم ملفات البيانات ذات القيم المفصولة بفواصل (CSV) غالبًا لتسجيل البيانات الرصدية وغيرها من البيانات بتنسيق جدولي. 
معظم مسجلات البيانات المستخدمة لتسجيل مخرجات المستشعرات قادرة على تصدير الملاحظات في ملفات مفصولة، بما في ذلك CSV. 
وبالمثل، عند إدخال البيانات في قاعدة بيانات، من السهل تصدير البيانات المطلوبة في ملفات بتنسيق CSV.

يوفر وحدة `wis2box csv2bufr` أداة سطر أوامر لتحويل بيانات CSV إلى تنسيق BUFR. عند استخدام `csv2bufr`، تحتاج إلى توفير قالب تحويل BUFR يربط أعمدة CSV بالعناصر المقابلة في BUFR. إذا كنت لا ترغب في إنشاء قالب تحويل خاص بك، يمكنك استخدام القوالب المدمجة AWS وDAYCLI لتحويل بيانات CSV إلى BUFR، ولكن ستحتاج إلى التأكد من أن بيانات CSV التي تستخدمها في التنسيق الصحيح لهذه القوالب. إذا كنت ترغب في فك تشفير معلمات غير مدرجة في قوالب AWS وDAYCLI، فستحتاج إلى إنشاء قالب تحويل خاص بك.
...
تأكد من أن `wis2box-stack` قد تم تشغيله باستخدام الأمر التالي:  
`python3 wis2box.py start`

تأكد من أن لديك متصفح ويب مفتوحًا مع واجهة MinIO الخاصة بمثالك عن طريق الذهاب إلى `http://YOUR-HOST:9000`.  
...
لنجرب إنشاء قالب تحويل مخصص جديد باستخدام الأمر `csv2bufr mapping create` باستخدام تسلسل BUFR 301150 بالإضافة إلى عنصر BUFR 012101 كمدخلات.
...
??? success "انقر للكشف عن الإجابة"
    ...
    قالب التحويل الذي أنشأته يربط **5** أعمدة CSV بعناصر BUFR، وهي العناصر الأربعة في التسلسل 301150 بالإضافة إلى العنصر 012101.
...
قالب التحويل الذي أنشأته يفتقد إلى بيانات وصفية مهمة حول الملاحظة التي تم إجراؤها، مثل التاريخ والوقت، وخط العرض وخط الطول للمحطة.

سنقوم الآن بتحديث قالب التحويل وإضافة التسلسلات التالية:
...
!!! question "فحص قالب التحويل المحدث"
    ...
    كم عدد أعمدة CSV التي يتم ربطها الآن بعناصر BUFR؟ ما هو رأس CSV لكل عنصر BUFR يتم ربطه؟

??? success "انقر للكشف عن الإجابة"
    
    قالب التحويل الذي أنشأته الآن يربط **18** عمودًا من CSV بعناصر BUFR:
...
??? success "اضغط لعرض الإجابة"
    ...
    في الإخراج، سترى القيم لعناصر BUFR التي قمت بتعيينها في القالب، على سبيل المثال، سيظهر "airTemperature" كما يلي:
...
انقر على مجموعة البيانات التي أنشأتها في الجلسة العملية السابقة وانقر على "UPDATE" بجانب المكون الإضافي الذي يحمل اسم "CSV data converted to BUFR":
...
!!! hint
    ...
    لاحظ أنه إذا لم ترَ القالب الجديد الذي قمت بإنشائه، حاول تحديث الصفحة أو فتحها في نافذة تصفح خاصة جديدة.
...
??? success "اضغط لعرض الإجابة"
    يتم تمثيل البيانات المفقودة داخل الملف بواسطة خلايا فارغة. في ملف CSV، سيتم ترميز ذلك بـ ``,,``. لاحظ أن هذه خلية فارغة وليست سلسلة نصية بطول صفر، مثل ``,"",``.

!!! hint "البيانات المفقودة"
    من المعروف أن البيانات قد تكون مفقودة لأسباب متنوعة، سواء بسبب عطل في المستشعر أو عدم مراقبة المعلمة. في هذه الحالات، يمكن ترميز البيانات المفقودة كما هو موضح في الإجابة أعلاه، وتظل البيانات الأخرى في التقرير صالحة.
...
قم بإنشاء مجلد جديد في حاوية MinIO يتطابق مع معرف مجموعة البيانات التي أنشأتها مع القالب='weather/surface-weather-observations/synop':
...
راجع المكونات الإضافية للبيانات لمجموعة البيانات. انقر على "UPDATE" بجانب المكون الإضافي الذي يحمل اسم "CSV data converted to BUFR" وسترى أن القالب مضبوط على **DayCLI**:
...
يحتوي ملف المثال على صف واحد من البيانات لكل يوم في الشهر، ويبلغ عن البيانات لمحطة واحدة. قم بتحديث ملف المثال الذي قمت بتنزيله لاستخدام تاريخ ووقت اليوم وقم بتغيير معرفات محطات WIGOS لاستخدام محطة قمت بتسجيلها في wis2box-webapp.
