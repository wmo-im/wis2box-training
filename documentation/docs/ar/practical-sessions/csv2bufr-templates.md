...
!!! abstract "نتائج التعلم"
    ...
    - تكوين مكون البيانات الإضافي لتحويل CSV إلى BUFR لاستخدام قالب تحويل BUFR مخصص
...
وبالمثل، عند إدخال البيانات في قاعدة بيانات، يكون من السهل تصدير البيانات المطلوبة في ملفات بتنسيق CSV.
...
تأكد من أن `wis2box-stack` قد تم تشغيله باستخدام الأمر:  
`python3 wis2box.py start`

تأكد من أن لديك متصفح ويب مفتوح مع واجهة MinIO الخاصة بمثيلك عن طريق الذهاب إلى:  
`http://YOUR-HOST:9000`  
...
تأكد من أن لديك `MQTT Explorer` مفتوحًا ومتصلًا بالوسيط الخاص بك باستخدام بيانات الاعتماد:  
`everyone/everyone`.
...
للعثور على تسلسلات وعناصر BUFR محددة، يمكنك الرجوع إلى جداول BUFR على الرابط:  
...
??? success "اضغط للكشف عن الإجابة"
    ...
    سيعرض الأمر محتوى ملف BUFR بتنسيق يمكن قراءته من قبل البشر.

    ```bash
    bufr_dump -p ./WIGOS_0-20000-0-15015_20250412T210000.bufr4
    ```

    في الإخراج، سترى القيم لعناصر BUFR التي قمت بتعيينها في القالب، على سبيل المثال، ستظهر قيمة "airTemperature" كالتالي:
...
انقر على مجموعة البيانات التي أنشأتها في الجلسة العملية السابقة وانقر على "UPDATE" بجانب المكون الإضافي الذي يحمل الاسم "CSV data converted to BUFR":
...
في الوقت الحالي، احتفظ بالاختيار الافتراضي لقالب AWS (انقر في الزاوية العلوية اليمنى لإغلاق تكوين المكون الإضافي).
...
??? success "اضغط للكشف عن الإجابة"
    يحتوي كل عمود على قطعة واحدة من المعلومات. على سبيل المثال، يتم تقسيم التاريخ إلى السنة، الشهر، واليوم، مما يعكس كيفية تخزين البيانات في BUFR. سيتم تقسيم تاريخ اليوم عبر الأعمدة "year"، "month"، و"day". وبالمثل، يجب تقسيم الوقت إلى "hour" و"minute"، ومعرف محطة WIGOS إلى مكوناته الخاصة.

!!! question
    بالنظر إلى ملف البيانات، كيف يتم ترميز البيانات المفقودة؟
...
!!! hint "البيانات المفقودة"
    من المعروف أن البيانات قد تكون مفقودة لأسباب متنوعة، سواء بسبب عطل في المستشعر أو لأن المعلمة لم يتم ملاحظتها. في هذه الحالات، يمكن ترميز البيانات المفقودة كما هو موضح في الإجابة أعلاه، بينما تظل البيانات الأخرى في التقرير صالحة.
...
<img alt="صورة تعرض واجهة MinIO مع aws-example محمّل" src="/../assets/img/minio-upload-aws-example.png"/></center>

تحقق من لوحة معلومات Grafana على `http://YOUR-HOST:3000` لمعرفة ما إذا كانت هناك أي تحذيرات أو أخطاء. إذا رأيت أيًا منها، حاول إصلاحها وأعد التمرين.
...
راجع المكونات الإضافية للبيانات لمجموعات البيانات. انقر على "UPDATE" بجانب المكون الإضافي الذي يحمل الاسم "CSV data converted to BUFR" وسترى أن القالب مضبوط على **DayCLI**:
...
أغلق تكوين المكون الإضافي وقدم النموذج باستخدام رمز المصادقة الذي قمت بإنشائه في الجلسة العملية السابقة.
...
??? success "اضغط للكشف عن الإجابة"
    يتضمن قالب daycli بيانات وصفية مهمة حول موقع الأدوات وتصنيفات جودة القياس لدرجة الحرارة والرطوبة، أعلام التحكم في الجودة، ومعلومات حول كيفية حساب متوسط درجة الحرارة اليومية.
...
كما في السابق، ستحتاج إلى تحميل البيانات إلى الحاوية 'wis2box-incoming' في MinIO ليتم معالجتها بواسطة محول csv2bufr. هذه المرة ستحتاج إلى إنشاء مجلد جديد في حاوية MinIO يتطابق مع معرف مجموعة البيانات (dataset-id) الخاصة بمجموعة البيانات التي أنشأتها باستخدام القالب `climate/surface-based-observations/daily`، والذي سيكون مختلفًا عن معرف مجموعة البيانات الذي استخدمته في التمرين السابق:

<img alt="Image showing MinIO UI with DAYCLI-example uploaded" src="/../assets/img/minio-upload-daycli-example.png"/></center>

بعد تحميل البيانات، تحقق من عدم وجود أي تحذيرات (WARNINGS) أو أخطاء (ERRORS) في لوحة تحكم Grafana، وتحقق من MQTT Explorer لمعرفة ما إذا كنت تتلقى إشعارات بيانات WIS2.
