...
!!! abstract "نتائج التعلم"
    ...
    - تكوين مكون البيانات الإضافي CSV-to-BUFR لاستخدام قالب تحويل BUFR مخصص
    - استخدام القوالب المدمجة AWS وDAYCLI لتحويل بيانات CSV إلى BUFR

## المقدمة

تُستخدم ملفات البيانات ذات القيم المفصولة بفواصل (CSV) غالبًا لتسجيل البيانات الرصدية وغيرها بتنسيق جدولي. 
تستطيع معظم أجهزة تسجيل البيانات المستخدمة لتسجيل مخرجات المستشعرات تصدير الرصدات في ملفات مفصولة، بما في ذلك CSV.
وبالمثل، عند إدخال البيانات إلى قاعدة بيانات، يكون من السهل تصدير البيانات المطلوبة في ملفات بتنسيق CSV.

يوفر وحدة `wis2box csv2bufr` أداة سطر أوامر لتحويل بيانات CSV إلى تنسيق BUFR. عند استخدام `csv2bufr`، تحتاج إلى توفير قالب تحويل BUFR يقوم بتعيين أعمدة CSV إلى العناصر المقابلة في BUFR. إذا لم ترغب في إنشاء قالب تحويل خاص بك، يمكنك استخدام القوالب المدمجة AWS وDAYCLI لتحويل بيانات CSV إلى BUFR، ولكن ستحتاج إلى التأكد من أن بيانات CSV التي تستخدمها في التنسيق الصحيح لهذه القوالب. إذا كنت ترغب في فك شفرة المعلمات غير المدرجة في قوالب AWS وDAYCLI، فستحتاج إلى إنشاء قالب تحويل خاص بك.
...
تأكد من أن لديك متصفح ويب مفتوحًا مع واجهة MinIO الخاصة بمثالك من خلال الانتقال إلى `http://YOUR-HOST:9000`.
...
### أداة سطر الأوامر لتعيينات csv2bufr
...
- `csv2bufr mappings list` : عرض قوالب التحويل المتوفرة في النظام
...
??? success "انقر للكشف عن الإجابة"
    ...
    يقوم قالب التحويل الذي أنشأته بتعيين **5** أعمدة CSV إلى عناصر BUFR، وهي العناصر الأربعة في تسلسل 301150 بالإضافة إلى عنصر BUFR 012101. 

    يتم تعيين أعمدة CSV التالية إلى عناصر BUFR:

    - **wigosIdentifierSeries** يتم تعيينه إلى `"eccodes_key": "#1#wigosIdentifierSeries"` (عنصر BUFR 001125)
    - **wigosIssuerOfIdentifier** يتم تعيينه إلى `"eccodes_key": "#1#wigosIssuerOfIdentifier` (عنصر BUFR 001126)
    - **wigosIssueNumber** يتم تعيينه إلى `"eccodes_key": "#1#wigosIssueNumber"` (عنصر BUFR 001127)
    - **wigosLocalIdentifierCharacter** يتم تعيينه إلى `"eccodes_key": "#1#wigosLocalIdentifierCharacter"` (عنصر BUFR 001128)
    - **airTemperature** يتم تعيينه إلى `"eccodes_key": "#1#airTemperature"` (عنصر BUFR 012101)

يفتقد قالب التحويل الذي أنشأته بيانات وصفية مهمة حول الرصد الذي تم إجراؤه، مثل تاريخ ووقت الرصد وخط العرض وخط الطول للمحطة.
...
!!! question "فحص قالب التحويل المحدث"
    ...
    كم عدد أعمدة CSV التي يتم الآن تعيينها إلى عناصر BUFR؟ ما هو رأس CSV لكل عنصر BUFR يتم تعيينه؟

??? success "انقر للكشف عن الإجابة"
    
    يقوم قالب التحويل الذي أنشأته الآن بتعيين **18** عمودًا من CSV إلى عناصر BUFR:
    ...
    - **wigosIssueNumber** يتم تعيينه إلى `"eccodes_key": "#1#wigosIssueNumber"` (عنصر BUFR 001127)
    - **wigosLocalIdentifierCharacter** يتم تعيينه إلى `"eccodes_key": "#1#wigosLocalIdentifierCharacter"` (عنصر BUFR 001128)
    - **year** يتم تعيينه إلى `"eccodes_key": "#1#year"` (عنصر BUFR 004001)
    - **month** يتم تعيينه إلى `"eccodes_key": "#1#month"` (عنصر BUFR 004002)
    - **day** يتم تعيينه إلى `"eccodes_key": "#1#day"` (عنصر BUFR 004003)
    - **hour** يتم تعيينه إلى `"eccodes_key": "#1#hour"` (عنصر BUFR 004004)
    - **minute** يتم تعيينه إلى `"eccodes_key": "#1#minute"` (عنصر BUFR 004005)
    - **latitude** يتم تعيينه إلى `"eccodes_key": "#1#latitude"` (عنصر BUFR 005002)
    - **longitude** يتم تعيينه إلى `"eccodes_key": "#1#longitude"` (عنصر BUFR 006002)
    - **heightOfBarometerAboveMeanSeaLevel"** يتم تعيينه إلى `"eccodes_key": "#1#heightOfBarometerAboveMeanSeaLevel"` (عنصر BUFR 007031)
    - **airTemperature** يتم تعيينه إلى `"eccodes_key": "#1#airTemperature"` (عنصر BUFR 012101)
    - **nonCoordinatePressure** يتم تعيينه إلى `"eccodes_key": "#1#nonCoordinatePressure"` (عنصر BUFR 010004)
...
??? success "اضغط للكشف عن الإجابة"
    ...
    في الإخراج ستشاهد القيم لعناصر BUFR التي قمت بتعيينها في القالب، على سبيل المثال، ستظهر "airTemperature" كما يلي:
...
سيكتشف wis2box-webapp تلقائيًا قالب التعيين الذي قمت بإنشائه ويجعله متاحًا في قائمة القوالب لمكون تحويل CSV إلى BUFR.

انقر على مجموعة البيانات التي قمت بإنشائها في الجلسة العملية السابقة وانقر على "UPDATE" بجانب المكون الإضافي باسم "CSV data converted to BUFR":
...
!!! hint
    ...
    لاحظ أنه إذا لم تشاهد القالب الجديد الذي قمت بإنشائه، حاول تحديث الصفحة أو فتحها في نافذة تصفح خاصة جديدة.

في الوقت الحالي، احتفظ بالاختيار الافتراضي لقالب AWS (انقر على الزاوية العلوية اليمنى لإغلاق إعدادات المكون الإضافي).
...
??? success "اضغط للكشف عن الإجابة"
    يحتوي كل عمود على جزء واحد من المعلومات. على سبيل المثال، يتم تقسيم التاريخ إلى السنة والشهر واليوم، مما يعكس كيفية تخزين البيانات في BUFR. سيتم تقسيم تاريخ اليوم عبر الأعمدة "year"، "month"، و"day". وبالمثل، يجب تقسيم الوقت إلى "hour" و"minute"، ومعرف محطة WIGOS إلى مكوناته الخاصة.

!!! question
    عند النظر إلى ملف البيانات، كيف يتم ترميز البيانات المفقودة؟
    
??? success "اضغط للكشف عن الإجابة"
    يتم تمثيل البيانات المفقودة داخل الملف بخلايا فارغة. في ملف CSV، سيتم ترميز ذلك بـ ``,,``. لاحظ أن هذه خلية فارغة وليست سلسلة نصية بطول صفر، مثل ``,"",``.

!!! hint "البيانات المفقودة"
    من المعروف أن البيانات قد تكون مفقودة لأسباب متنوعة، سواء بسبب عطل في المستشعر أو لأن المعلمة لم يتم ملاحظتها. في هذه الحالات، يمكن ترميز البيانات المفقودة كما هو موضح في الإجابة أعلاه، وتظل البيانات الأخرى في التقرير صالحة.
...
إذا قمت بإدخال البيانات بنجاح، يجب أن ترى 3 إشعارات في MQTT Explorer على الموضوع `origin/a/wis2/<centre-id>/data/weather/surface-weather-observations/synop` للمحطات الثلاث التي أبلغت عنها البيانات:
...
انتقل إلى محرر مجموعة البيانات في wis2box-webapp وقم بإنشاء مجموعة بيانات جديدة. استخدم نفس معرف المركز كما في الجلسات العملية السابقة وحدد **Data Type='climate/surface-based-observations/daily'**:
...
راجع المكونات الإضافية للبيانات لمجموعات البيانات. انقر على "UPDATE" بجانب المكون الإضافي باسم "CSV data converted to BUFR" وسترى أن القالب مضبوط على **DayCLI**:
...
أغلق إعدادات المكون الإضافي وأرسل النموذج باستخدام رمز المصادقة الذي قمت بإنشائه في الجلسة العملية السابقة.
...
!!! question
    ما المتغيرات الإضافية التي تم تضمينها في قالب daycli؟

??? success "اضغط للكشف عن الإجابة"
    يتضمن قالب daycli بيانات وصفية مهمة حول موقع الأداة وتصنيفات جودة القياس لدرجة الحرارة والرطوبة، وأعلام مراقبة الجودة، ومعلومات حول كيفية حساب متوسط درجة الحرارة اليومية.
