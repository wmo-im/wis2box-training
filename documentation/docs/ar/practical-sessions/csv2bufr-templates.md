...
تستطيع معظم مسجلات البيانات المستخدمة لتسجيل مخرجات المستشعرات تصدير الملاحظات في ملفات مفصولة، بما في ذلك CSV.
وبالمثل، عند إدخال البيانات إلى قاعدة بيانات، يكون من السهل تصدير البيانات المطلوبة في ملفات بتنسيق CSV.

يوفر وحدة `wis2box csv2bufr` أداة سطر أوامر لتحويل بيانات CSV إلى تنسيق BUFR. عند استخدام `csv2bufr`، تحتاج إلى توفير قالب تحويل BUFR يربط أعمدة CSV بالعناصر المقابلة في BUFR. إذا كنت لا ترغب في إنشاء قالب تحويل خاص بك، يمكنك استخدام القوالب المدمجة AWS وDAYCLI لتحويل بيانات CSV إلى BUFR، ولكن ستحتاج إلى التأكد من أن بيانات CSV التي تستخدمها بتنسيق صحيح لهذه القوالب. إذا كنت ترغب في فك تشفير المعلمات غير المدرجة في قوالب AWS وDAYCLI، فستحتاج إلى إنشاء قالب تحويل خاص بك.
...
تأكد من أن لديك متصفح ويب مفتوح مع واجهة MinIO الخاصة بمثالك عن طريق الذهاب إلى `http://YOUR-HOST:9000`.
...
!!! Note "csv2bufr mapping list"
    ...
    سيعرض الأمر `csv2bufr mapping list` قوالب التحويل المتوفرة في النظام. 
...
دعونا نحاول إنشاء قالب تحويل مخصص جديد باستخدام الأمر `csv2bufr mapping create` باستخدام تسلسل BUFR 301150 بالإضافة إلى عنصر BUFR 012101 كمدخلات.
...
!!! question "فحص قالب التحويل"
    ...
    كم عدد أعمدة CSV التي يتم ربطها بعناصر BUFR؟ ما هو رأس CSV لكل عنصر BUFR يتم ربطه؟
...
وتحقق مرة أخرى من محتوى قالب التحويل:
...
!!! question "فحص قالب التحويل المحدث"
    ...
    كم عدد أعمدة CSV التي يتم الآن ربطها بعناصر BUFR؟ ما هو رأس CSV لكل عنصر BUFR يتم ربطه؟
...
!!! question "تحقق من محتوى ملف BUFR"
    ...
    كيف يمكنك التحقق من محتوى ملف BUFR الذي قمت بإنشائه للتو والتأكد من أنه قد تم ترميز البيانات بشكل صحيح؟
...
??? success "انقر لعرض الإجابة"
    ...
    في الإخراج، سترى القيم الخاصة بعناصر BUFR التي قمت بتعيينها في القالب. على سبيل المثال، ستظهر قيمة "airTemperature" كما يلي:
...
انقر على مجموعة البيانات التي أنشأتها في الجلسة العملية السابقة، ثم انقر على "UPDATE" بجانب المكون الذي يحمل الاسم "CSV data converted to BUFR":
...
في الوقت الحالي، احتفظ بالاختيار الافتراضي لقالب AWS (انقر على الزاوية العلوية اليمنى لإغلاق تكوين المكون).
...
قم بتنزيل المثال الخاص بهذا التمرين من الرابط أدناه:
...
??? success "انقر لعرض الإجابة"
    يحتوي كل عمود على قطعة واحدة من المعلومات. على سبيل المثال، يتم تقسيم التاريخ إلى السنة والشهر واليوم، مما يعكس كيفية تخزين البيانات في BUFR. سيتم تقسيم تاريخ اليوم عبر الأعمدة "year"، "month" و"day". وبالمثل، يجب تقسيم الوقت إلى "hour" و"minute"، ومعرف محطة WIGOS إلى مكوناته الخاصة.

!!! question
    عند النظر إلى ملف البيانات، كيف يتم ترميز البيانات المفقودة؟
    
??? success "انقر لعرض الإجابة"
    يتم تمثيل البيانات المفقودة داخل الملف بواسطة خلايا فارغة. في ملف CSV، يتم ترميز ذلك بواسطة ``,,``. لاحظ أن هذه خلية فارغة وليست سلسلة بطول صفر، مثل ``,"",``.

!!! hint "البيانات المفقودة"
    من المعروف أن البيانات قد تكون مفقودة لأسباب متنوعة، سواء بسبب فشل المستشعر أو عدم مراقبة المعلمة. في هذه الحالات، يمكن ترميز البيانات المفقودة كما هو موضح في الإجابة أعلاه، وتظل البيانات الأخرى في التقرير صالحة.

### تحديث ملف المثال

قم بتحديث ملف المثال الذي قمت بتنزيله لاستخدام تاريخ ووقت اليوم وقم بتغيير معرفات محطات WIGOS لاستخدام المحطات التي قمت بتسجيلها في wis2box-webapp.
...
<img alt="صورة تعرض واجهة MinIO مع تمييز زر إنشاء مسار جديد" src="/../assets/img/minio-create-new-path.png"/>

قم بإنشاء مجلد جديد في دلو MinIO يتطابق مع معرف مجموعة البيانات التي أنشأتها مع القالب='weather/surface-weather-observations/synop':

<img alt="صورة تعرض واجهة MinIO مع تمييز زر إنشاء مسار جديد" src="/../assets/img/minio-create-new-path-metadata_id.png"/>

قم بتحميل ملف المثال الذي قمت بتنزيله إلى المجلد الذي أنشأته في دلو MinIO:

<img alt="صورة تعرض واجهة MinIO مع aws-example الذي تم تحميله" src="/../assets/img/minio-upload-aws-example.png"/></center>
...
إذا قمت بإدخال البيانات بنجاح، يجب أن ترى 3 إشعارات في MQTT Explorer على الموضوع `origin/a/wis2/<centre-id>/data/weather/surface-weather-observations/synop` للمحطات الثلاث التي قمت بالإبلاغ عن بياناتها:
...
انقر على "CONTINUE TO FORM" وأضف وصفًا لمجموعة البيانات الخاصة بك، وقم بتعيين المربع المحيط، وقدم معلومات الاتصال لمجموعة البيانات. بمجرد الانتهاء من ملء جميع الأقسام، انقر على 'VALIDATE FORM' وتحقق من النموذج.

راجع مكونات البيانات لمجموعات البيانات. انقر على "UPDATE" بجانب المكون الذي يحمل الاسم "CSV data converted to BUFR" وسترى أن القالب مضبوط على **DayCLI**:
...
أغلق تكوين المكون وقم بإرسال النموذج باستخدام رمز المصادقة الذي قمت بإنشائه في الجلسة العملية السابقة.
...
يحتوي ملف المثال على صف واحد من البيانات لكل يوم في الشهر، ويبلغ عن بيانات لمحطة واحدة. قم بتحديث ملف المثال الذي قمت بتنزيله لاستخدام تاريخ ووقت اليوم وقم بتغيير معرفات محطات WIGOS لاستخدام محطة قمت بتسجيلها في wis2box-webapp.
...
إذا قمت بإدخال البيانات بنجاح، يجب أن ترى 30 إشعارًا في MQTT Explorer على الموضوع `origin/a/wis2/<centre-id>/data/climate/surface-based-observations/daily` للأيام الثلاثين في الشهر الذي أبلغت عن بياناته:
