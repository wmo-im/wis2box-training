...
تأكد من أن لديك متصفح ويب مفتوح مع واجهة MinIO الخاصة بمثالك عن طريق الذهاب إلى `http://YOUR-HOST:9000`. إذا كنت لا تتذكر بيانات اعتماد MinIO الخاصة بك، يمكنك العثور عليها في ملف `wis2box.env` في دليل `wis2box` على الجهاز الافتراضي الخاص بك.
...
### أداة سطر أوامر csv2bufr mappings
...
??? success "اضغط لعرض الإجابة"
    ...
    في المخرجات، سترى القيم الخاصة بعناصر BUFR التي قمت بتعيينها في القالب، على سبيل المثال، سيظهر "airTemperature" كالتالي:
...
انقر على مجموعة البيانات التي أنشأتها في الجلسة العملية السابقة، ثم انقر على "UPDATE" بجانب المكون الذي يحمل اسم "CSV data converted to BUFR":
...
!!! hint
    ...
    لاحظ أنه إذا لم ترَ القالب الجديد الذي أنشأته، حاول تحديث الصفحة أو فتحها في نافذة تصفح خفي جديدة.
...
??? success "اضغط لعرض الإجابة"
    يتم تمثيل البيانات المفقودة داخل الملف بخلايا فارغة. في ملف CSV، يتم ترميز ذلك بـ ``,,``. لاحظ أن هذه خلية فارغة وليست سلسلة نصية بطول صفري، مثل ``,"",``.

!!! hint "البيانات المفقودة"
    من المعروف أن البيانات قد تكون مفقودة لأسباب متنوعة، سواء بسبب فشل المستشعر أو عدم ملاحظة المعلمة. في هذه الحالات، يمكن ترميز البيانات المفقودة كما هو موضح في الإجابة أعلاه، بينما تظل البيانات الأخرى في التقرير صالحة.
...
انقر على "CONTINUE TO FORM" وأضف وصفًا لمجموعة البيانات الخاصة بك، وحدد المربع المحيط وقدم معلومات الاتصال الخاصة بمجموعة البيانات. بمجرد الانتهاء من ملء جميع الأقسام، انقر على 'VALIDATE FORM' وتحقق من النموذج.

راجع مكونات البيانات لمجموعات البيانات. انقر على "UPDATE" بجانب المكون الذي يحمل اسم "CSV data converted to BUFR" وسترى أن القالب مضبوط على **DayCLI**:
...
أغلق تكوين المكون وقم بإرسال النموذج باستخدام رمز المصادقة الذي أنشأته في الجلسة العملية السابقة.
...
??? success "اضغط لعرض الإجابة"
    يتضمن قالب daycli بيانات وصفية مهمة حول موقع الأدوات وتصنيفات جودة القياس لدرجة الحرارة والرطوبة، علامات التحكم في الجودة، ومعلومات حول كيفية حساب متوسط درجة الحرارة اليومية.
