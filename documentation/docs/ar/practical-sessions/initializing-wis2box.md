...
- دليل على الجهاز المضيف للمشاركة بين الجهاز المضيف وحاويات wis2box، يتم تعريفه بواسطة متغير البيئة `WIS2BOX_HOST_DATADIR`
...
ستتمكن من مراجعة وتحديث ملفات التهيئة بعد اكتمال السكربت.
...
بعد ذلك، سيُطلب منك إدخال عنوان URL الخاص بـ wis2box. هذا هو عنوان URL الذي سيتم استخدامه للوصول إلى تطبيق الويب الخاص بـ wis2box وواجهة API وواجهة المستخدم.
...
يمكنك استخدام خيار إنشاء كلمات مرور عشوائية عند المطالبة بـ `WIS2BOX_WEBAPP_PASSWORD` و`WIS2BOX_STORAGE_PASSWORD` و`WIS2BOX_BROKER_PASSWORD` أو تحديد كلمات المرور الخاصة بك.
...
??? success "اضغط لعرض الإجابة"
    ...
    القيمة الافتراضية لـ WIS2BOX_STORAGE_DATA_RETENTION_DAYS هي 30 يومًا. يمكنك تغيير هذه القيمة إلى عدد أيام مختلف إذا كنت ترغب.
...
??? success "انقر للكشف عن الإجابة"
    ...
    لعرض المجموعات المتاحة حاليًا عبر واجهة برمجة التطبيقات، انقر على `View the collections in this service`:
...
ستظهر نافذة منبثقة تطلب اسم المستخدم وكلمة المرور الخاصة بك. استخدم اسم المستخدم الافتراضي `wis2box-user` وكلمة المرور `WIS2BOX_WEBAPP_PASSWORD` المحددة في ملف `wis2box.env` وانقر على "Sign in":
...
تأكد من النقر على "SAVE" لتخزين تفاصيل الاتصال الخاصة بك.
