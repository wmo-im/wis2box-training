...
تُستخدم التنسيقات الثنائية الخاصة بـ WMO مثل BUFR وGRIB على نطاق واسع في مجتمع الأرصاد الجوية لتبادل بيانات الرصد والنماذج، وعادةً ما تتطلب أدوات متخصصة لفك تشفير البيانات وعرضها.

بعد تنزيل البيانات من WIS2، ستحتاج غالبًا إلى فك تشفير البيانات لاستخدامها بشكل أكبر.

تتوفر مكتبات برمجية متنوعة لكتابة السكربتات أو البرامج لفك تشفير التنسيقات الثنائية الخاصة بـ WMO. كما توجد أدوات توفر واجهة مستخدم لفك تشفير البيانات وعرضها دون الحاجة إلى كتابة برنامج برمجي.

في هذه الجلسة العملية، سنوضح كيفية فك تشفير ثلاثة أنواع مختلفة من البيانات باستخدام دفتر Jupyter:

- GRIB2 يحتوي على بيانات لتنبؤ عالمي جماعي كما يتم إنتاجه بواسطة نظام CMA Global Regional Assimilation PrEdiction System (GRAPES)
...
!!! note "حول صورة demo-decode-eccodes-jupyter"
    ...
    تم تطوير صورة `demo-decode-eccodes-jupyter` لهذا التدريب، وهي تستخدم صورة أساسية تتضمن مكتبة ecCodes وتضيف خادم دفتر Jupyter، بالإضافة إلى حزم Python لتحليل البيانات وعرضها.
...
بمجرد بدء تشغيل الحاوية، يمكنك الوصول إلى خادم دفتر Jupyter على جهازك الظاهري للطلاب عن طريق الانتقال إلى `http://YOUR-HOST:8888` في متصفح الويب الخاص بك.
...
في النهاية، يجب أن ترى مخططًا لاحتمالية الضربة لمسارات الأعاصير المدارية:
...
??? success "انقر لعرض الإجابة"
    ...
    لتحديث دفتر الملاحظات لعرض الاحتمالية المتوقعة لمسار العاصفة المدارية ضمن مسافة مختلفة، يمكنك تحديث المتغير `distance_threshold` في كتلة الكود التي تحسب احتمالية الضربة.
...
!!! note "فك تشفير بيانات BUFR"
    ...
    التمرين الذي قمت به للتو قدم مثالًا محددًا لكيفية فك تشفير بيانات BUFR باستخدام مكتبة ecCodes. قد تتطلب أنواع البيانات المختلفة خطوات فك تشفير مختلفة، وقد تحتاج إلى الرجوع إلى الوثائق الخاصة بنوع البيانات التي تعمل عليها.
