...
!!! abstract "نتائج التعلم!"
    ...
    - تشغيل حاوية Docker باستخدام الصورة "demo-decode-eccodes-jupyter"
...
تتوفر مكتبات برمجية متنوعة لكتابة نصوص أو برامج لفك تشفير التنسيقات الثنائية الخاصة بـ WMO. كما توجد أدوات توفر واجهة مستخدم لفك تشفير البيانات وعرضها دون الحاجة إلى كتابة برنامج برمجي.

في هذه الجلسة العملية، سنوضح كيفية فك تشفير ثلاثة أنواع مختلفة من البيانات باستخدام دفتر Jupyter:

- GRIB2 يحتوي على بيانات لتنبؤ عالمي جماعي كما يتم إنتاجه بواسطة CMA Global Regional Assimilation PrEdiction System (GRAPES)
- BUFR يحتوي على بيانات مسارات الأعاصير المدارية من نظام التنبؤ الجماعي ECMWF
- NetCDF يحتوي على بيانات الشذوذ الحراري الشهري
...
- `-v ~/wis2-downloads:/root/downloads` يقوم بربط دليل `~/wis2-downloads` على جهازك الظاهري بـ `/root/downloads` داخل الحاوية. هنا يتم تخزين البيانات التي قمت بتنزيلها من WIS2 بعد اتباع التعليمات في الجلسة العملية السابقة حول WIS2 Downloader.
...
- [Panoply](https://www.giss.nasa.gov/tools/panoply/) - تطبيق متعدد المنصات يقوم برسم المصفوفات الجغرافية وغيرها من NetCDF وHDF وGRIB وغيرها
