...
يجب أن تتوافق البيانات المنشورة على WIS2 مع المتطلبات والمعايير التي تحددها مجتمعات الخبراء في مجالات نظم الأرض المختلفة. لتسهيل عملية نشر البيانات للملاحظات السطحية البرية، يوفر wis2box أدوات لتحويل البيانات إلى تنسيق BUFR. تتوفر هذه الأدوات عبر حاوية wis2box-api ويمكن استخدامها من خلال سطر الأوامر لاختبار عملية تحويل البيانات.

التحويلات الرئيسية التي يدعمها wis2box حاليًا هي تحويل تقارير FM-12 SYNOP إلى BUFR وتحويل بيانات CSV إلى BUFR. يتم دعم بيانات FM-12 لأنها لا تزال تُستخدم على نطاق واسع ويتم تبادلها داخل مجتمع WMO، بينما يتم دعم بيانات CSV للسماح بربط البيانات الناتجة عن محطات الطقس الآلية بتنسيق BUFR.

### حول FM-12 SYNOP

تم الإبلاغ عن تقارير الطقس السطحية من محطات السطح البرية تاريخيًا كل ساعة أو في الساعات الرئيسية (00، 06، 12، و18 بالتوقيت العالمي) والساعات الوسيطة (03، 09، 15، 21 بالتوقيت العالمي). قبل الانتقال إلى BUFR، تم ترميز هذه التقارير في شكل نصي عادي باستخدام كود FM-12 SYNOP. على الرغم من أن الانتقال إلى BUFR كان من المقرر أن يكتمل بحلول عام 2012، إلا أن عددًا كبيرًا من التقارير لا يزال يتم تبادلها بتنسيق FM-12 SYNOP القديم. يمكن العثور على مزيد من المعلومات حول تنسيق FM-12 SYNOP في دليل WMO للكودات، المجلد I.1 (WMO-No. 306، Volume I.1).
...
- **csv2bufr**: لتمكين تحويل بيانات CSV الناتجة عن محطات الطقس الآلية إلى تنسيق BUFR. تُستخدم وحدة csv2bufr لتحويل بيانات CSV إلى تنسيق BUFR باستخدام قالب تعيين يحدد كيفية ربط بيانات CSV بتنسيق BUFR. راجع [مستودع csv2bufr على GitHub](https://github.com/World-Meteorological-Organization/csv2bufr)
...
لاستخدام أدوات BUFR عبر سطر الأوامر، ستحتاج إلى تسجيل الدخول إلى حاوية wis2box-api. ما لم يُذكر خلاف ذلك، يجب تشغيل جميع الأوامر على هذه الحاوية. ستحتاج أيضًا إلى فتح MQTT Explorer والاتصال بوسيطك.
...
توضح التمارين التالية كيفية استخدام `bufr_ls` و`bufr_dump` لفحص محتويات ملف BUFR.

### bufr_ls

في هذا التمرين الأول، ستستخدم أمر `bufr_ls` لفحص رؤوس ملف BUFR وتحديد نوع المحتويات الموجودة في الملف.
...
!!! question
    ...
    قم بتنفيذ الأمر السابق وقم بتفسير الإخراج باستخدام [جدول الكود المشترك C-13](https://github.com/wmo-im/CCT/blob/master/C13.csv) لتحديد فئة البيانات والفئة الفرعية.
...
- الفئة الفرعية الدولية هي 4، مما يشير إلى بيانات **"تقارير درجة الحرارة/الرطوبة/الرياح العلوية من محطات أرضية ثابتة (TEMP)"**.
...
ينتج عن ذلك ملف JSON قد يكون من الصعب تحليله. حاول استخدام الخيار `-p` لإخراج البيانات بتنسيق نص عادي (key=value):

```{.copy}
bufr_dump -p bufr-cli-ex2.bufr4
```

سترى عددًا كبيرًا من المفاتيح في الإخراج، العديد منها مفقود. هذا أمر شائع مع البيانات الواقعية حيث لا يتم ملء جميع مفاتيح eccodes بالبيانات المبلغ عنها.
...
### مراجعة رسالة SYNOP المثال

افحص ملف رسالة SYNOP المثال لهذا التمرين `synop_message.txt`:
...
??? success "اضغط للكشف عن الإجابة"
    ...
    إذا قمت بفحص محتويات الدليل باستخدام الأمر `ls -lh`، يجب أن ترى أنه تم إنشاء ملفي BUFR جديدين: `WIGOS_0-20000-0-15015_20240921T120000.bufr4` و`WIGOS_0-20000-0-15020_20240921T120000.bufr4`.
...
يُستخدم أيضًا وحدة `synop2bufr` في تطبيق `wis2box-webapp` لتحويل بيانات FM-12 SYNOP إلى تنسيق BUFR باستخدام نموذج إدخال عبر الويب.  
...
??? success "اضغط للكشف عن الإجابة"
    ...
    إذا قدمت رمزًا مميزًا غير صالح، ستظهر الرسالة التالية:
    
    - النتيجة: غير مصرح بها، يرجى تقديم رمز "processes/wis2box" صالح

    إذا قدمت رمزًا مميزًا صالحًا، ستظهر الرسالة "WARNINGS: 3". انقر على "WARNINGS" لفتح القائمة المنسدلة التي ستعرض:
...
!!! question
    كم عدد صفوف البيانات الموجودة في ملف CSV؟ ما هو معرف محطة WIGOS للمحطات التي تبلغ في ملف CSV؟
...
??? question "اضغط للكشف عن الإجابة"
    ...
    بعد صف الرأس، هناك 3 صفوف من البيانات، تمثل 3 ملاحظات جوية من نفس المحطة بمعرف محطة WIGOS `0-20000-0-60355` عند ثلاث طوابع زمنية مختلفة: `2024-03-31 01:00:00`، `2024-03-31 02:00:00`، و `2024-03-31 03:00:00`.

### مراجعة aws-template

يتضمن `wis2box-api` مجموعة من قوالب تعيين BUFR المثبتة مسبقًا في الدليل `/opt/csv2bufr/templates`.
...
سيعيد هذا ملف JSON كبيرًا، يوفر التعيين لـ 43 عمودًا في CSV.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    لن تحصل على أي نتيجة، مما يشير إلى أن القيمة للمفتاح `airTemperature` مفقودة في ملف BUFR `WIGOS_0-20000-0-60355_20240331T020000.bufr4`. رفض `csv2bufr` ترميز القيمة `25.0` من بيانات CSV لأنها تقع خارج النطاق الصالح بين `193.15` و`333.15` كما هو محدد في قالب التعيين.

لاحظ أن تحويل CSV إلى BUFR باستخدام أحد قوالب التعيين المعرّفة مسبقًا له قيود:
