...
يجب أن تفي البيانات المنشورة على WIS2 بالمتطلبات والمعايير التي تحددها مجتمعات الخبراء في مجالات نظم الأرض المختلفة. لتسهيل عملية نشر البيانات للملاحظات السطحية البرية، يوفر wis2box أدوات لتحويل البيانات إلى تنسيق BUFR. هذه الأدوات متاحة عبر حاوية wis2box-api ويمكن استخدامها من سطر الأوامر لاختبار عملية تحويل البيانات.
...
تم الإبلاغ عن تقارير الطقس السطحي من المحطات البرية تاريخيًا على أساس ساعي أو في الساعات الرئيسية (00، 06، 12، و18 بالتوقيت العالمي) والساعات الوسيطة (03، 09، 15، 21 بالتوقيت العالمي). قبل الانتقال إلى BUFR، كانت هذه التقارير تُرمز في شكل كود نصي عادي FM-12 SYNOP. على الرغم من أن الانتقال إلى BUFR كان من المقرر أن يكتمل بحلول عام 2012، إلا أن عددًا كبيرًا من التقارير لا يزال يتم تبادلها بتنسيق FM-12 SYNOP القديم. يمكن العثور على مزيد من المعلومات حول تنسيق FM-12 SYNOP في دليل WMO على الأكواد، المجلد I.1 (WMO-No. 306، المجلد I.1).

### حول ecCodes

مكتبة ecCodes هي مجموعة من المكتبات البرمجية والأدوات المصممة لفك وترميز البيانات الأرصادية في تنسيقات GRIB وBUFR. يتم تطويرها بواسطة المركز الأوروبي للتنبؤات الجوية متوسطة المدى (ECMWF)، راجع [وثائق ecCodes](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation) لمزيد من المعلومات.
...
!!! warning "المتطلبات المسبقة"
    ...
    - الاتصال بـ MQTT broker الخاص بمثيل wis2box الخاص بك باستخدام MQTT Explorer
    - فتح تطبيق wis2box على الويب (`http://YOUR-HOST/wis2box-webapp`) والتأكد من تسجيل الدخول
    - فتح لوحة معلومات Grafana لمثيلك عن طريق الذهاب إلى `http://YOUR-HOST:3000`
...
بمفرده، هذا الإخراج ليس مفيدًا جدًا، حيث يوفر معلومات محدودة فقط عن محتويات الملف.
...
- الفئة الفرعية الدولية هي 4، مما يشير إلى بيانات **"تقارير درجة الحرارة/الرطوبة/الرياح على المستويات العليا من محطات ثابتة على اليابسة (TEMP)"**.
...
سترى عددًا كبيرًا من المفاتيح في الإخراج، العديد منها مفقود. هذا أمر شائع مع بيانات العالم الحقيقي حيث لا يتم تعبئة جميع مفاتيح eccodes بالبيانات المبلغ عنها.
...
يُستخدم الوسيط `--metadata` لتحديد ملف بيانات تعريف المحطة، الذي يوفر معلومات إضافية ليتم ترميزها في ملف BUFR. يُستخدم الوسيط `--output-dir` لتحديد الدليل الذي سيتم كتابة ملفات BUFR المحولة فيه. تُستخدم الوسيطات `--year` و`--month` لتحديد سنة وشهر الملاحظة.

تُستخدم وحدة `synop2bufr` أيضًا في wis2box-webapp لتحويل بيانات FM-12 SYNOP إلى تنسيق BUFR باستخدام نموذج إدخال عبر الويب.
...
??? success "انقر للكشف عن الإجابة"
    ...
    يتوافق هذا مع بيانات تعريف المحطات لمحطتين: معرفات محطات WIGOS هي `0-20000-0-15015` و`0-20000-0-15020`.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    - النتيجة: غير مصرح، يرجى تقديم رمز "processes/wis2box" صالح.
...
!!! note
    تأكد من أنك ما زلت مسجلاً الدخول في حاوية `wis2box-api` وفي الدليل `/root/data-conversion-exercises`. إذا خرجت من الحاوية في التمرين السابق، يمكنك تسجيل الدخول مرة أخرى كما يلي:
...
??? success "اضغط للكشف عن الإجابة"
    ...
    تشير المخرجات إلى أنه تم إنشاء 3 ملفات BUFR: `WIGOS_0-20000-0-60355_20240331T010000.bufr4`، `WIGOS_0-20000-0-60355_20240331T020000.bufr4`، و`WIGOS_0-20000-0-60355_20240331T030000.bufr4`.
