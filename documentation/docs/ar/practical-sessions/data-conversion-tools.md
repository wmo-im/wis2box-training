...
يجب أن تفي البيانات المنشورة على WIS2 بالمتطلبات والمعايير التي تحددها مجتمعات الخبراء في مجالات نظم الأرض المختلفة. لتسهيل عملية نشر البيانات للملاحظات السطحية الأرضية، يوفر `wis2box` أدوات لتحويل البيانات إلى صيغة BUFR. هذه الأدوات متاحة عبر حاوية `wis2box-api` ويمكن استخدامها من سطر الأوامر لاختبار عملية تحويل البيانات.

التحويلات الرئيسية المدعومة حاليًا بواسطة `wis2box` هي تقارير FM-12 SYNOP إلى BUFR وبيانات CSV إلى BUFR. يتم دعم بيانات FM-12 لأنها لا تزال مستخدمة على نطاق واسع ويتم تبادلها في مجتمع WMO، بينما يتم دعم بيانات CSV للسماح بتخطيط البيانات التي تنتجها محطات الطقس الآلية إلى صيغة BUFR.

### حول FM-12 SYNOP

تم الإبلاغ عن تقارير الطقس السطحي من المحطات السطحية الأرضية تاريخيًا كل ساعة أو في الساعات الرئيسية (00، 06، 12، و18 بالتوقيت العالمي) والساعات الوسيطة (03، 09، 15، 21 بالتوقيت العالمي). قبل الانتقال إلى BUFR، تم ترميز هذه التقارير في صيغة النص العادي FM-12 SYNOP. على الرغم من أن الانتقال إلى BUFR كان من المقرر أن يكتمل بحلول عام 2012، إلا أن عددًا كبيرًا من التقارير لا يزال يتم تبادلها بصيغة FM-12 SYNOP القديمة. يمكن العثور على مزيد من المعلومات حول صيغة FM-12 SYNOP في دليل WMO على الرموز، المجلد I.1 (WMO-No. 306، المجلد I.1).

### حول ecCodes

مكتبة ecCodes هي مجموعة من المكتبات البرمجية والأدوات المصممة لفك وترميز البيانات الأرصادية بصيغتي GRIB وBUFR. تم تطويرها بواسطة المركز الأوروبي للتنبؤات الجوية متوسطة المدى (ECMWF)، راجع [وثائق ecCodes](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation) لمزيد من المعلومات.

يتضمن برنامج `wis2box` مكتبة ecCodes في الصورة الأساسية لحاوية `wis2box-api`. يتيح ذلك للمستخدمين الوصول إلى أدوات سطر الأوامر والمكتبات من داخل الحاوية. يتم استخدام مكتبة ecCodes داخل `wis2box-stack` لفك وترميز رسائل BUFR.
...
- **synop2bufr**: لدعم صيغة FM-12 SYNOP القديمة التي كانت تُستخدم تقليديًا من قبل المراقبين اليدويين. تعتمد وحدة synop2bufr على بيانات وصفية إضافية للمحطات لترميز معلمات إضافية في ملف BUFR. راجع [مستودع synop2bufr على GitHub](https://github.com/World-Meteorological-Organization/synop2bufr)
- **csv2bufr**: لتمكين تحويل بيانات CSV التي تنتجها محطات الطقس الآلية إلى صيغة BUFR. تُستخدم وحدة csv2bufr لتحويل بيانات CSV إلى صيغة BUFR باستخدام قالب تخطيط يحدد كيفية تخطيط بيانات CSV إلى صيغة BUFR. راجع [مستودع csv2bufr على GitHub](https://github.com/World-Meteorological-Organization/csv2bufr)
...
!!! warning "المتطلبات المسبقة"
    ...
    - تأكد من إعداد مجموعة بيانات واحدة على الأقل وتكوين محطة واحدة على الأقل في `wis2box` الخاص بك
    ...
    - افتح لوحة معلومات Grafana لمثيلك بالانتقال إلى `http://YOUR-HOST:3000`
...
??? success "اضغط للكشف عن الإجابة"
    يمكنك تغيير تنسيق الإخراج إلى JSON باستخدام العلامة `-j`، أي:
    ```bash
...
يمثل الإخراج القيم لبعض مفاتيح الرؤوس في ملف BUFR.

بمفرده، لا يكون هذا الإخراج مفيدًا جدًا، حيث يقدم معلومات محدودة فقط عن محتويات الملف.

عند فحص ملف BUFR، غالبًا ما نرغب في تحديد نوع البيانات الموجودة في الملف والتاريخ/الوقت النموذجي للبيانات في الملف. يمكن سرد هذه المعلومات باستخدام العلامة `-p` لتحديد الرؤوس المطلوب عرضها. يمكن تضمين رؤوس متعددة باستخدام قائمة مفصولة بفواصل.
...
!!! question
    ...
    قم بتنفيذ الأمر السابق وفسر الإخراج باستخدام [جدول الرموز المشترك C-13](https://github.com/wmo-im/CCT/blob/master/C13.csv) لتحديد فئة البيانات والفئة الفرعية.
...
حاول تشغيل أمر `bufr_dump` على الملف الثاني `bufr-cli-ex2.bufr4`:
...
!!! question
    ...
    ما هو الضغط المعدل إلى مستوى سطح البحر المتوسط والمبلغ عنه في ملف BUFR `bufr-cli-ex2.bufr4`؟
...
??? success "اضغط لعرض الإجابة"
    ...
    ```
    يشير هذا إلى أن الضغط المعدل إلى مستوى سطح البحر المتوسط هو 105590 باسكال (1055.90 هكتوباسكال).
...
يُستخدم الوسيط `--metadata` لتحديد ملف بيانات المحطة الوصفية الذي يوفر معلومات إضافية ليتم ترميزها في ملف BUFR. يُستخدم الوسيط `--output-dir` لتحديد الدليل الذي سيتم كتابة ملفات BUFR المحولة فيه. تُستخدم الوسيطات `--year` و`--month` لتحديد سنة وشهر الملاحظة.
...
افحص ملف رسالة SYNOP النموذجي لهذا التمرين `synop_message.txt`:
...
??? success "اضغط لعرض الإجابة"
    ...
    إذا قمت بفحص محتوى الدليل الخاص بك باستخدام `ls -lh`، يجب أن ترى ملفين جديدين من نوع BUFR تم إنشاؤهما: `WIGOS_0-20000-0-15015_20240921T120000.bufr4` و`WIGOS_0-20000-0-15020_20240921T120000.bufr4`.

    يشير تحذير WARNING إلى أن المحطة ذات المعرف التقليدي `15090` لم يتم العثور عليها في ملف قائمة المحطات `station_list.csv`. هذا يعني أن تقرير SYNOP لهذه المحطة لم يتم تحويله إلى تنسيق BUFR.
...
??? success "اضغط لعرض الإجابة"
    ...
    لاحظ أن هذا يتضمن البيانات المقدمة بواسطة ملف `station_list.csv`.
...
اختر `SYNOP Form` من القائمة على اليسار، ثم قم بنسخ ولصق محتويات ملف `synop_message.txt`:
...
??? success "اضغط للكشف عن الإجابة"
    ...
    إذا قدمت رمزًا مميزًا صالحًا، سترى "WARNINGS: 3". اضغط على "WARNINGS" لفتح القائمة المنسدلة التي ستعرض:
...
الآن دعونا نلقي نظرة على كيفية تحويل بيانات CSV إلى صيغة BUFR باستخدام الوحدة `csv2bufr`. الوحدة مثبتة في حاوية `wis2box-api` ويمكن استخدامها من سطر الأوامر كما يلي:
...
!!! question
    ما هي قيمة مفتاح eccodes `airTemperature` في ملف BUFR `WIGOS_0-20000-0-60355_20240331T010000.bufr4`؟ ماذا عن ملف BUFR `WIGOS_0-20000-0-60355_20240331T020000.bufr4`؟
...
??? success "اضغط لعرض الإجابة"
    ...
    لن تحصل على نتيجة، مما يشير إلى أن القيمة للمفتاح `airTemperature` مفقودة في ملف BUFR `WIGOS_0-20000-0-60355_20240331T020000.bufr4`. رفض `csv2bufr` ترميز القيمة `25.0` من بيانات CSV لأنها تقع خارج النطاق المسموح به بين `193.15` و `333.15` كما هو محدد في قالب التعيين.
...
- تكون عمليات التحقق من الجودة محدودة بالتحقق المحدد في قالب التعيين.
