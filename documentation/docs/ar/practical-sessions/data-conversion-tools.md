...
يجب أن تلتزم البيانات المنشورة على WIS2 بالمتطلبات والمعايير التي تحددها مجتمعات الخبراء في مجالات نظم الأرض المختلفة. لتسهيل نشر البيانات للمراقبات السطحية البرية، يوفر `wis2box` أدوات لتحويل البيانات إلى تنسيق BUFR. هذه الأدوات متوفرة عبر حاوية `wis2box-api` ويمكن استخدامها من سطر الأوامر لاختبار عملية تحويل البيانات.
...
تقارير الطقس السطحي من المحطات البرية كانت تُبلغ تاريخيًا كل ساعة أو في الساعات الرئيسية (00، 06، 12، و18 بالتوقيت العالمي) والساعات الوسيطة (03، 09، 15، 21 بالتوقيت العالمي). قبل الانتقال إلى BUFR، كانت هذه التقارير تُرمز بتنسيق النص العادي FM-12 SYNOP. بينما كان من المقرر أن يكتمل الانتقال إلى BUFR بحلول عام 2012، لا يزال يتم تبادل عدد كبير من التقارير بتنسيق FM-12 SYNOP القديم. يمكن العثور على مزيد من المعلومات حول تنسيق FM-12 SYNOP في دليل WMO على الرموز، المجلد I.1 (WMO-No. 306، المجلد I.1).

### حول ecCodes

مكتبة ecCodes هي مجموعة من المكتبات البرمجية والأدوات المصممة لفك وترميز البيانات الأرصادية بتنسيقات GRIB وBUFR. تم تطويرها بواسطة المركز الأوروبي للتنبؤات الجوية متوسطة المدى (ECMWF)، راجع [وثائق ecCodes](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation) لمزيد من المعلومات.
...
- **csv2bufr**: لتمكين تحويل ملفات CSV الناتجة عن محطات الطقس الآلية إلى تنسيق BUFR. تُستخدم وحدة csv2bufr لتحويل بيانات CSV إلى تنسيق BUFR باستخدام قالب تعيين يحدد كيفية تعيين بيانات CSV إلى تنسيق BUFR. راجع [مستودع csv2bufr على GitHub](https://github.com/World-Meteorological-Organization/csv2bufr)
...
!!! warning "المتطلبات المسبقة"
    ...
    - افتح لوحة تحكم Grafana الخاصة بمثيلك عن طريق الذهاب إلى `http://YOUR-HOST:3000`
...
أولاً، قم بالاتصال بـ VM الخاص بك عبر عميل SSH الخاص بك وانسخ مواد التمرين إلى دليل "/wis2box-api" داخل حاوية `wis2box-api`:
...
!!! question
    ...
    ما هو الأمر الذي يمكن استخدامه لإظهار الإخراج السابق بتنسيق JSON؟
...
!!! question
    ...
    ما هو الضغط المعدل إلى مستوى سطح البحر المتوسط الذي تم الإبلاغ عنه في ملف BUFR `bufr-cli-ex2.bufr4`؟
...
ستوضح التمارين القليلة التالية كيفية عمل وحدة `synop2bufr` وكيفية استخدامها لتحويل بيانات FM-12 SYNOP إلى تنسيق BUFR.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    يشير رسالة التحذير إلى أن المحطة ذات المعرف التقليدي `15090` لم يتم العثور عليها في ملف قائمة المحطات `station_list.csv`. هذا يعني أن تقرير SYNOP لهذه المحطة لم يتم تحويله إلى تنسيق BUFR.
...
!!! note
    ...
    python3 wis2box-ctl.py login wis2box-api
    ```
    وقم بتغيير الدليل إلى دليل التمرين:
...
يعيد هذا ملف JSON كبير، يوفر التعيين لـ 43 عمودًا في CSV.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    لن تحصل على أي نتيجة، مما يشير إلى أن قيمة المفتاح `airTemperature` مفقودة في ملف BUFR `WIGOS_0-20000-0-60355_20240331T020000.bufr4`. رفض `csv2bufr` ترميز القيمة `25.0` من بيانات CSV لأنها تقع خارج النطاق المسموح به بين `193.15` و `333.15` كما هو محدد في قالب التعيين.
