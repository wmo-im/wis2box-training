...
!!! abstract "الأهداف التعليمية"
    ...
    - تشغيل تحويل `synop2bufr` من خلال `wis2box-webapp`
    - استخدام أداة `csv2bufr` لتحويل بيانات CSV إلى BUFR من سطر الأوامر

## المقدمة

يجب أن تتوافق البيانات المنشورة على WIS2 مع المتطلبات والمعايير التي تحددها مجتمعات الخبراء في مجالات نظم الأرض المختلفة. لتسهيل عملية نشر البيانات للملاحظات السطحية الأرضية، يوفر `wis2box` أدوات لتحويل البيانات إلى صيغة BUFR. تتوفر هذه الأدوات عبر حاوية `wis2box-api` ويمكن استخدامها من سطر الأوامر لاختبار عملية تحويل البيانات.

التحويلات الرئيسية التي يدعمها `wis2box` حاليًا هي تقارير FM-12 SYNOP إلى BUFR وبيانات CSV إلى BUFR. يتم دعم بيانات FM-12 لأنها لا تزال تُستخدم على نطاق واسع ويتم تبادلها في مجتمع WMO، بينما يتم دعم بيانات CSV للسماح بتخطيط البيانات التي تنتجها محطات الطقس الآلية إلى صيغة BUFR.

### حول FM-12 SYNOP

تقارير الطقس السطحي من المحطات السطحية الأرضية كانت تُبلغ تاريخيًا بشكل ساعي أو في الساعات الرئيسية (00، 06، 12، و18 بالتوقيت العالمي) والساعات الوسيطة (03، 09، 15، 21 بالتوقيت العالمي). قبل الانتقال إلى BUFR، كانت هذه التقارير تُرمز بصيغة النص العادي FM-12 SYNOP. بينما كان من المقرر أن يكتمل الانتقال إلى BUFR بحلول عام 2012، لا يزال يتم تبادل عدد كبير من التقارير بصيغة FM-12 SYNOP القديمة. يمكن العثور على مزيد من المعلومات حول صيغة FM-12 SYNOP في دليل WMO للأكواد، المجلد I.1 (WMO-No. 306، المجلد I.1).
...
- **synop2bufr**: لدعم صيغة FM-12 SYNOP القديمة التي تُستخدم تقليديًا من قبل المراقبين اليدويين. تعتمد وحدة `synop2bufr` على بيانات وصفية إضافية للمحطات لترميز معلمات إضافية في ملف BUFR. راجع [مستودع synop2bufr على GitHub](https://github.com/World-Meteorological-Organization/synop2bufr)
- **csv2bufr**: لتمكين تحويل ملفات CSV التي تنتجها محطات الطقس الآلية إلى صيغة BUFR. تُستخدم وحدة `csv2bufr` لتحويل بيانات CSV إلى صيغة BUFR باستخدام قالب تخطيط يحدد كيفية تخطيط بيانات CSV إلى صيغة BUFR. راجع [مستودع csv2bufr على GitHub](https://github.com/World-Meteorological-Organization/csv2bufr)
...
تتضمن مكتبة ecCodes المدمجة في حاوية `wis2box-api` عددًا من أدوات سطر الأوامر للعمل مع ملفات BUFR. 
...
بمفرده، هذه المعلومات ليست مفيدة جدًا، حيث توفر معلومات محدودة فقط عن محتوى الملف.

عند فحص ملف BUFR، غالبًا ما نرغب في تحديد نوع البيانات الموجودة في الملف والتاريخ/الوقت النموذجي للبيانات في الملف. يمكن سرد هذه المعلومات باستخدام الخيار `-p` لتحديد الرؤوس المطلوب طباعتها. يمكن تضمين رؤوس متعددة باستخدام قائمة مفصولة بفواصل.
...
يُستخدم الوسيط `--output-dir` لتحديد الدليل حيث سيتم كتابة ملفات BUFR المحولة. تُستخدم الوسيطتان `--year` و`--month` لتحديد سنة وشهر الملاحظة.
...
??? success "انقر للكشف عن الإجابة"
    ...
    لتحويل هذه البيانات إلى تنسيق BUFR، ستحتاج إلى تكوين المحطات المقابلة في `wis2box` والتأكد من أن المحطات مرتبطة بالموضوع الخاص بمجموعة بياناتك.
...
??? question "انقر للكشف عن الإجابة"
    ...
    يحتوي الصف الأول من ملف CSV على رؤوس الأعمدة، والتي تُستخدم لتحديد البيانات في كل عمود.
