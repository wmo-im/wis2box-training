...
يجب أن تلتزم البيانات المنشورة على WIS2 بالمتطلبات والمعايير التي تحددها مجتمعات الخبراء في مجالات نظم الأرض المختلفة. لتسهيل عملية نشر البيانات للملاحظات السطحية الأرضية، يوفر `wis2box` أدوات لتحويل البيانات إلى تنسيق BUFR. تتوفر هذه الأدوات عبر حاوية `wis2box-api` ويمكن استخدامها من سطر الأوامر لاختبار عملية تحويل البيانات.

التحويلات الرئيسية التي يدعمها `wis2box` حاليًا هي تقارير FM-12 SYNOP إلى BUFR وبيانات CSV إلى BUFR. يتم دعم بيانات FM-12 لأنها لا تزال تُستخدم على نطاق واسع ويتم تبادلها في مجتمع WMO، بينما يتم دعم بيانات CSV للسماح برسم خرائط البيانات التي تنتجها محطات الطقس الآلية إلى تنسيق BUFR.

### حول FM-12 SYNOP

تاريخيًا، كانت تقارير الطقس السطحية من المحطات الأرضية تُرسل كل ساعة أو في الساعات الرئيسية (00، 06، 12، و18 بالتوقيت العالمي) والساعات الوسيطة (03، 09، 15، 21 بالتوقيت العالمي). قبل الانتقال إلى BUFR، كانت هذه التقارير تُشفَّر باستخدام صيغة النص العادي FM-12 SYNOP. على الرغم من أن الانتقال إلى BUFR كان من المقرر أن يكتمل بحلول عام 2012، إلا أن عددًا كبيرًا من التقارير لا يزال يتم تبادلها بتنسيق FM-12 SYNOP القديم. يمكن العثور على مزيد من المعلومات حول تنسيق FM-12 SYNOP في دليل WMO للأكواد، المجلد I.1 (WMO-No. 306, Volume I.1).

### حول ecCodes

مكتبة ecCodes هي مجموعة من المكتبات البرمجية والأدوات المصممة لفك تشفير وترميز البيانات الجوية بتنسيقات GRIB وBUFR. تم تطويرها من قبل المركز الأوروبي للتنبؤات الجوية متوسطة المدى (ECMWF). لمزيد من المعلومات، راجع [وثائق ecCodes](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation).

يتضمن برنامج `wis2box` مكتبة ecCodes في الصورة الأساسية لحاوية `wis2box-api`. يتيح ذلك للمستخدمين الوصول إلى أدوات سطر الأوامر والمكتبات من داخل الحاوية. تُستخدم مكتبة ecCodes داخل `wis2box-stack` لفك تشفير وترميز رسائل BUFR.
...
- **csv2bufr**: لتمكين تحويل بيانات CSV المستخرجة من محطات الطقس الآلية إلى تنسيق BUFR. تُستخدم وحدة csv2bufr لتحويل بيانات CSV إلى تنسيق BUFR باستخدام قالب رسم خرائط يحدد كيفية تحويل بيانات CSV إلى تنسيق BUFR. راجع [مستودع csv2bufr على GitHub](https://github.com/World-Meteorological-Organization/csv2bufr)
...
!!! warning "المتطلبات المسبقة"
    ...
    - تأكد من إعداد مجموعة بيانات وتكوين محطة واحدة على الأقل في `wis2box`
    - الاتصال بوسيط MQTT الخاص بمثيل `wis2box` باستخدام MQTT Explorer
    - فتح تطبيق الويب الخاص بـ `wis2box` (`http://YOUR-HOST/wis2box-webapp`) والتأكد من تسجيل الدخول
    - فتح لوحة معلومات Grafana لمثيلك عن طريق الانتقال إلى `http://YOUR-HOST:3000`
...
أولاً، قم بالاتصال بجهاز الطالب الافتراضي الخاص بك عبر عميل SSH وانسخ مواد التمرين إلى الدليل "/wis2box-api" داخل حاوية `wis2box-api`:
...
عند فحص ملف BUFR، غالبًا ما نرغب في تحديد نوع البيانات الموجودة في الملف والتاريخ/الوقت النموذجي للبيانات في الملف. يمكن إدراج هذه المعلومات باستخدام العلمة `-p` لتحديد الرؤوس المطلوب طباعتها. يمكن تضمين رؤوس متعددة باستخدام قائمة مفصولة بفواصل.
...
- فئة البيانات هي 2، مما يشير إلى بيانات **"الرصدات الرأسية (غير الأقمار الصناعية)"**.
...
تُستخدم الوحدة `synop2bufr` أيضًا في wis2box-webapp لتحويل بيانات FM-12 SYNOP إلى تنسيق BUFR باستخدام نموذج إدخال قائم على الويب.
...
??? success "اضغط لعرض الإجابة"
    ...
    يتوافق هذا مع بيانات المحطات الوصفية لمحطتين: لمعرفات محطات WIGOS `0-20000-0-15015` و`0-20000-0-15020`.
...
??? success "اضغط لعرض الإجابة"
    ...
    إذا قمت بفحص محتويات الدليل الخاص بك باستخدام الأمر `ls -lh`، يجب أن ترى أنه تم إنشاء ملفي BUFR جديدين: `WIGOS_0-20000-0-15015_20240921T120000.bufr4` و`WIGOS_0-20000-0-15020_20240921T120000.bufr4`.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    لتحويل هذه البيانات إلى صيغة BUFR، ستحتاج إلى تكوين المحطات المقابلة في `wis2box` والتأكد من أن المحطات مرتبطة بالموضوع لمجموعة البيانات الخاصة بك.
...
??? success "انقر لعرض الإجابة"
    ...
    لن تحصل على نتيجة، مما يشير إلى أن القيمة للمفتاح `airTemperature` مفقودة في ملف BUFR `WIGOS_0-20000-0-60355_20240331T020000.bufr4`. حيث رفض `csv2bufr` ترميز القيمة `25.0` من بيانات CSV لأنها تقع خارج النطاق المسموح به بين `193.15` و `333.15` كما هو محدد في قالب التعيين.
