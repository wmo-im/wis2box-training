...
!!! abstract "نتائج التعلم!"
    ...
    - تغيير الإعداد الافتراضي لـ WIS2 Downloader لحالات الاستخدام الأكثر تقدمًا

## المقدمة

في WIS2، تحتوي جميع مجموعات البيانات على ملف بيانات وصفية يمكن العثور عليه في **Global Discovery Catalogues**. لذلك، يُقصد دائمًا أن يقوم المستخدمون بالرجوع إلى هذه الخدمات للعثور على البيانات التي يتم مشاركتها على WIS2.

يستخدم WIS2 Downloader هذا المبدأ من خلال العثور على جميع السجلات المتوفرة في هذه الكتالوجات ودمجها داخليًا لتمكين المستخدم من التنقل عبر البيانات المتاحة على WIS2. نظرًا للعدد الكبير من السجلات التي يجب عرضها، من الضروري توفير طريقة للمستخدم لتصفية هذه السجلات والعثور على السجل الصحيح. حتى بعد العثور على السجل الصحيح والاشتراك فيه، قد تكون هناك مجموعات بيانات تحتوي على عدد من الملفات يتجاوز احتياجات المستخدم الحالية. لهذا السبب، هناك حاجة إلى مستوى ثانٍ من التصفية — يعمل عند اتخاذ قرار بشأن ما إذا كان يجب تنزيل ملف معين.
...
!!! note "إعادة تحميل الكتالوج"
    ...
    يتم تحميل الكتالوج من GDCs عند بدء تشغيل WIS2 Downloader. إذا كنت تعتقد أن القائمة قديمة، يمكنك فرض إعادة تحميلها من قسم **الإعدادات** في الشريط الجانبي الأيسر.

### تمرين: العثور على مجموعة بيانات والاشتراك فيها

!!! question "العثور على مجموعة بيانات لمراقبة سطحية"

    استخدم عوامل التصفية في عرض الكتالوج للعثور على مجموعة بيانات **core** لمراقبة سطحية تتعلق بدرجة الحرارة والهطول.
...
**عرض الشجرة** يعرض تسلسل مواضيع WIS2 كهيكل شجري قابل للطي، مما يتيح لك استكشاف المواضيع المتاحة مستوى بمستوى — مشابه لتصفح المواضيع في MQTT Explorer. تم تصميمه لاستكشاف البيانات المتاحة على WIS2 من الأعلى إلى الأسفل، بدءًا من جذر التسلسل الهرمي والتعمق فيه. يختلف هذا عن عرض الكتالوج، الذي يأخذك مباشرة إلى سجلات مجموعات البيانات الفردية وهو أكثر ملاءمة عندما تعرف بالفعل ما تبحث عنه.

انتقل إلى **عرض الشجرة** من الشريط الجانبي الأيسر.

![WIS2 Downloader Tree View](../assets/img/wis2-downloader-tree-view.png)

يتم تنظيم الشجرة وفقًا لتسلسل مواضيع WIS2. قم بتوسيع كل مستوى بالنقر على عقدة للكشف عن العناصر الفرعية. في أي مستوى، يمكنك الاشتراك عن طريق تحديد عقدة والنقر على **Subscribe** — باستخدام رمز البدل (`#`) لالتقاط جميع المواضيع تحت تلك العقدة.

!!! note "الاشتراك في مستويات مختلفة"

    الاشتراك في مستوى أعلى في الشجرة (مثل مستوى **Centre ID**) سيشمل جميع مجموعات البيانات المنشورة بواسطة ذلك المركز. الاشتراك في مستوى أدنى يمنحك تحكمًا أكثر دقة. يتم تلقائيًا إضافة لاحقة رمز البدل `#` بواسطة WIS2 Downloader عند الاشتراك من عرض الشجرة.

### تمرين: العثور على مجموعة بيانات والاشتراك باستخدام عرض الشجرة

!!! question "الاشتراك في مجموعة بيانات عبر عرض الشجرة"

    استخدم عرض الشجرة للعثور على بيانات مراقبة سطحية من مركز معين والاشتراك فيها.
...
**عرض الاشتراك اليدوي** يتيح لك إنشاء اشتراك عن طريق إدخال موضوع مباشرة، دون الاعتماد على Global Discovery Catalogues. على عكس عرض الكتالوج وعرض الشجرة — اللذين يستمدان مواضيعهما من GDCs — فإن الاشتراك اليدوي مفيد عندما تعرف بالفعل الموضوع المحدد الذي تريد الاشتراك فيه وترغب في إعداده دون تصفح الكتالوج مع مزيد من الحرية في اختيار WTH.
...
!!! question "إعداد والاشتراك في مجموعة بيانات ذات وصول مقيد"
    ...
    **الخطوة 1 — إنشاء مجموعة بيانات ذات وصول مقيد على wis2box**
    ...
    على مثيل wis2box الخاص بك، قم بإنشاء مجموعة بيانات مع تمكين التحكم في الوصول ولاحظ الموضوع ورمز Bearer الذي تم إنشاؤه لها. إذا لم تقم بذلك بالفعل، راجع جلسة [Datasets with access control](datasets-with-access-control.md) العملية للحصول على خطوات الإعداد الكاملة.
...
إذا فشلت المصادقة، فلن يتم تنزيل الملفات حتى إذا ظهر الاشتراك نشطًا في عرض **Manage Subscriptions**. تحقق مرة أخرى من أن رمز المصادقة يتطابق مع الرمز الذي تم تكوينه على مجموعة البيانات في wis2box.
...
تتيح لك الفلاتر التحكم في الملفات التي يتم تنزيلها من الاشتراك على مستوى الإشعار — وهذا هو المستوى الثاني من التصفية المذكور في المقدمة. بدلاً من تنزيل كل ملف يتم نشره على موضوع معين، يمكنك تعريف فلتر بحيث يتم تشغيل التنزيل فقط للإشعارات التي تطابق معايير محددة.

بعد تحديد مجموعة بيانات في **Catalogue View** أو **Tree View**، تظهر لوحة الفلتر على الجانب الأيمن من الشاشة قبل الاشتراك. هنا يمكنك ملء قيم الفلتر التي تريد تطبيقها. يقوم WIS2 Downloader بإنشاء كائن الفلتر تلقائيًا بناءً على مدخلاتك.
...
- **مجموعة البيانات** — تقييد التنزيلات على مجموعة بيانات معينة باستخدام معرف البيانات الوصفية الخاص بها.  
- **الصندوق المحيط** — تقييد التنزيلات على الإشعارات التي تقع بياناتها ضمن منطقة مكانية، يتم تعريفها بواسطة قيم `north`، `south`، `east`، و`west`.  
- **نطاق التاريخ والوقت** — تقييد التنزيلات على الإشعارات المنشورة ضمن نطاق زمني محدد.  
- **الفلاتر المخصصة** — التصفية بناءً على أي خاصية إشعار أخرى كما هو محدد في سجل البيانات الوصفية عن طريق تحديد قيمة الخاصية (مثل التصفية باستخدام `wigos_station_identifier` لتنزيل البيانات فقط من محطة معينة).  
...
سيتم قبول وتنزيل الإشعارات التي تطابق جميع الشروط التي حددتها فقط. سيتم رفض جميع الإشعارات الأخرى بواسطة قاعدة الالتقاط الافتراضية.
