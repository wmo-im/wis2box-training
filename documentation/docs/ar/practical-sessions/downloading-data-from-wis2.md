...
قبل البدء، يرجى تسجيل الدخول إلى جهاز الطالب الافتراضي الخاص بك والتأكد من أن مثيل wis2box يعمل.

## أساسيات wis2downloader

يتم تضمين wis2downloader كحاوية منفصلة في wis2box كما هو محدد في ملفات Docker Compose. تم تكوين حاوية Prometheus في wis2box لجمع المقاييس من حاوية wis2downloader، ويمكن عرض هذه المقاييس من خلال لوحة تحكم في Grafana.
...
تُستخدم متغيرات البيئة التالية بواسطة wis2downloader:
    ...
    - DOWNLOAD_BROKER_PORT: المنفذ الخاص بوسيط MQTT للاتصال به. القيمة الافتراضية هي 443 (HTTPS للويب سوكيت)
    ...
    - DOWNLOAD_BROKER_TRANSPORT: websockets أو tcp، آلية النقل المستخدمة للاتصال بوسيط MQTT. القيمة الافتراضية هي websockets
...
إذا احتفظت بالتكوين الافتراضي لـ wis2downloader، فهو متصل حاليًا بوسيط WIS2 العالمي المستضاف بواسطة Météo-France.
...
تحقق من أن دليل التنزيلات على جهازك المضيف فارغ مرة أخرى:
...
إذا قمت في التمرين السابق باستبدال `de-dwd-gts-to-wis2` بـ `jp-jma-gts-to-wis2`، ستتلقى الإشعارات والبيانات المنشورة من بوابة JMA GTS-to-WIS2.

!!! note "المواضيع الأصلية مقابل مواضيع التخزين المؤقت"

عند الاشتراك في موضوع يبدأ بـ `origin/`، ستتلقى إشعارات تحتوي على عنوان URL قانوني يشير إلى خادم بيانات مقدم من مركز WIS الذي ينشر البيانات.
