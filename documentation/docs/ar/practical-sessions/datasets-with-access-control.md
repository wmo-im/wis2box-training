...
- **core**: بيانات تُقدم بشكل مجاني وغير مقيد، بدون رسوم وبدون شروط على الاستخدام.
- **recommended**: بيانات قد تُقدم بشروط على الاستخدام و/أو تخضع لترخيص.

البيانات التي تُشارك كـ "موصى بها":

- قد تخضع لشروط على الاستخدام وإعادة الاستخدام؛
- قد تُطبق عليها ضوابط وصول؛
- لا يتم تخزينها مؤقتًا داخل WIS2 بواسطة Global Caches؛
- يجب أن تحتوي على رابط إلى ترخيص يحدد شروط استخدام البيانات المضمنة في بيانات الاكتشاف الوصفية.

يتطلب محرر مجموعات البيانات في wis2box-webapp منك تقديم رابط ترخيص عند اختيار سياسة البيانات 'موصى بها'. ويمكنك اختياريًا إضافة رمز وصول إلى مجموعة البيانات لتقييد الوصول إلى البيانات.

في هذه الجلسة العملية، ستقوم بإنشاء مجموعة بيانات جديدة بسياسة بيانات 'موصى بها' وستتعلم كيفية إضافة التحكم في الوصول.
كما ستوجهك الجلسة خلال خطوات إضافة ملف ترخيص مخصص إلى مثيل wis2box الخاص بك.
...
انتقل إلى صفحة 'محرر مجموعات البيانات' في wis2box-webapp وأنشئ مجموعة بيانات جديدة. اختر نوع البيانات = 'weather/surface-weather-observations/synop'.
...
لديك خيار تقديم رابط إلى ترخيص يصف شروط استخدام مجموعة البيانات. على سبيل المثال، يمكنك استخدام
`https://creativecommons.org/licenses/by/4.0/`
للإشارة إلى ترخيص Creative Commons Attribution 4.0 International (CC BY 4.0).

أو يمكنك استخدام `WIS2BOX_URL/data/license.txt` للإشارة إلى ملف ترخيص مخصص استضافته على خادم الويب الخاص بك، حيث `WIS2BOX_URL` هو الرابط الذي قمت بتعريفه في ملف wis2box.env:

<img alt="create-dataset-license-url" src="/../assets/img/create-dataset-license-custom.png" width="800">

استمر في ملء الحقول المطلوبة لخصائص المكان ومعلومات الاتصال، ثم انقر على 'Validate form' للتحقق من وجود أي أخطاء.
...
تحقق من MQTT Explorer للتأكد من أنك تتلقى رسالة إشعار WIS2 تعلن عن سجل بيانات الاكتشاف الوصفية الجديد على الموضوع `origin/a/wis2/<your-centre-id>/metadata`.

## مراجعة مجموعة البيانات الجديدة في wis2box-api

عرض قائمة مجموعات البيانات في wis2box-api عن طريق فتح الرابط `WIS2BOX_URL/oapi/collections/discovery-metadata/items` في متصفح الويب الخاص بك، مع استبدال `WIS2BOX_URL` برابط مثيل wis2box الخاص بك.
...
إذا كنت قد استخدمت `http://YOUR-HOST/data/license.txt` كرابط الترخيص، فلن يعمل الرابط حاليًا، لأننا لم نضف ملف الترخيص بعد إلى مثيل wis2box.
...
من سطر أوامر wis2box، يمكننا استيعاب ملف البيانات التجريبي `aws-example.csv` في مجموعة بيانات محددة كما يلي:
...
تحقق من الرابط القانوني في رسالة إشعار WIS2 وقم بنسخ/لصق الرابط إلى المتصفح لمحاولة تنزيل البيانات.
...
هذه الخطوة مطلوبة فقط إذا كنت تريد تقديم ترخيص مخصص مستضاف بواسطة مثيل wis2box الخاص بك، بدلاً من استخدام رابط ترخيص خارجي.

قم بإنشاء ملف نصي على جهازك المحلي باستخدام محرر النصوص المفضل لديك وأضف بعض معلومات الترخيص إلى الملف، مثل:

*هذا ملف ترخيص مخصص لمجموعة البيانات الموصى بها مع التحكم في الوصول. يمكنك استخدام هذه البيانات بحرية، ولكن يرجى الإشارة إلى مزود البيانات.*

لتحميل ملف محلي تم إنشاؤه باسم license.txt، استخدم MinIO Console المتاح على المنفذ 9001 لمثيل wis2box، عن طريق الذهاب إلى متصفح الويب وزيارة `http://YOUR-HOST:9001`
...
بعد تحميل ملف الترخيص، تحقق مما إذا كان الملف متاحًا عن طريق زيارة `WIS2BOX_URL/data/license.txt` في متصفح الويب الخاص بك، مع استبدال `WIS2BOX_URL` برابط مثيل wis2box الخاص بك.

!!! note

    يقوم الوكيل العكسي في wis2box بتمرير جميع الملفات المخزنة في دلو "wis2box-public" تحت المسار `WIS2BOX_URL/data/`

يجب أن يعمل الرابط الخاص بـ "License for this dataset" المضمن في بيانات الاكتشاف الوصفية لمجموعة البيانات الموصى بها الآن.
