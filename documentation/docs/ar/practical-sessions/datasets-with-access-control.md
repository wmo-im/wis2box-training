...
**أساسية**: بيانات يتم توفيرها بشكل مجاني وغير مقيد، بدون رسوم وبدون شروط على الاستخدام.
...
في هذه الجلسة العملية، ستقوم بإنشاء مجموعة بيانات جديدة بسياسة بيانات 'موصى بها' باستخدام محرر مجموعة البيانات في wis2box-webapp. ستتعلم أيضًا كيفية توفير ترخيص مستضاف ذاتيًا وكيفية إضافة التحكم في الوصول بشكل اختياري.

!!! note "بيانات الطيران في WIS2"
    
    في هذا التمرين، يُطلب منك إنشاء مجموعة بيانات لمشاركة بيانات METAR، وهو تنسيق قياسي للإبلاغ عن ملاحظات الطقس للطيران. نظرًا لأن بيانات الأرصاد الجوية للطيران تخضع لقيود الاستخدام، فإن سياسة البيانات **الموصى بها** تنطبق، وفقًا لسياسة البيانات الموحدة للمنظمة العالمية للأرصاد الجوية (WMO Unified Data Policy)، القرار 1 (Cg-Ext(2021)).

    يرجى الرجوع إلى !["WIS2 Cookbook"](https://wmo-im.github.io/wis2-cookbook/cookbook/latest/wis2-cookbook-STABLE.html#_publishing_aeronautical_meteorology_data_on_wis2) للحصول على "الوصفة" المعتمدة لنشر بيانات الأرصاد الجوية للطيران على WIS2.
...
تأكد من أن لديك متصفح ويب مفتوح مع wis2box-webapp الخاص بمثيلك عن طريق الانتقال إلى `http://YOUR-HOST/wis2box-webapp`.
...
- اختر سياسة بيانات المنظمة العالمية للأرصاد الجوية = 'موصى بها'

<img alt="create-dataset-recommended" src="/../assets/img/create-dataset-aviation-metar-example.png" width="800">

لاحظ أنه عند اختيار سياسة بيانات المنظمة العالمية للأرصاد الجوية = 'موصى بها'، أضاف محرر مجموعة البيانات تلقائيًا حقلًا لـ 'License URL' وهو إلزامي لمجموعات البيانات الموصى بها.
...
!!! note "حول عنوان URL للترخيص"
    
    يُعلم عنوان URL للترخيص المرتبط بمجموعة بيانات موصى بها مستهلكي البيانات لهذه المجموعة بشروط استخدام البيانات.
...
تحقق من MQTT Explorer للتأكد من أنك تتلقى رسالة إشعار WIS2 تعلن عن سجل بيانات وصفية جديد على الموضوع `origin/a/wis2/<your-centre-id>/metadata`.
...
لتحميل هذا الملف، استخدم وحدة التحكم MinIO المتوفرة على المنفذ 9001 لمثيل wis2box، عن طريق الانتقال إلى متصفح ويب وزيارة `http://YOUR-HOST:9001`
...
!!! note "استكشاف الأخطاء وإصلاحها"
    ...
    إذا لم تتلق إشعارًا بعد رفع البيانات، يمكنك التحقق من السجلات الحديثة لحاوية `wis2box-management` لاستكشاف المشكلة:
...
إذا قمت بإضافة رمز الوصول بشكل صحيح إلى مجموعة البيانات، يجب أن **لا** تكون قادرًا على الوصول إلى البيانات في متصفح الويب الخاص بك، وبدلاً من ذلك سترى خطأ *401 Authorization Required*.
...
!!! note "التحكم في الوصول اختياري، فقط للمجموعات الموصى بها"
    ...
    يحدد WIS2 أن المجموعات الموصى بها *قد* يتم تطبيق التحكم في الوصول على البيانات. ليس من الإلزامي إضافة التحكم في الوصول إلى مجموعة بيانات موصى بها ويمكنك الاعتماد فقط على رابط الترخيص لإبلاغ مستهلكي البيانات بشروط استخدام البيانات. إذا قمت بتطبيق التحكم في الوصول، فأنت مسؤول عن مشاركة رمز الوصول مع أي مستهلكي بيانات يحتاجون إلى الوصول إلى هذه البيانات.
