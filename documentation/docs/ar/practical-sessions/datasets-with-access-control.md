...
!!! note "تنزيل البيانات الموصى بها"
    ...
    يجب على مستهلك البيانات تنزيل البيانات من خادم البيانات المستضاف بواسطة مزود البيانات، باستخدام الرابط الكنسي (Canonical URL) المقدم في الإشعارات على الموضوع `origin/a/wis2/<centre-id>/data/recommended/...`

في هذه الجلسة العملية، ستقوم بإنشاء مجموعة بيانات جديدة بسياسة بيانات 'موصى بها' باستخدام محرر مجموعات البيانات في wis2box-webapp. ستتعلم أيضًا كيفية توفير ترخيص مستضاف ذاتيًا وكيفية إضافة التحكم في الوصول بشكل اختياري.
...
تأكد من أن لديك متصفح ويب مفتوح مع wis2box-webapp الخاص بمثيلك عن طريق الذهاب إلى `http://YOUR-HOST/wis2box-webapp`.
...
اضغط على 'CONTINUE TO FORM' للمتابعة.
...
نظرًا لأنك اخترت القالب = 'other'، لم يتم ملء أي كلمات مفتاحية مسبقًا لمجموعة البيانات. أضف 3 كلمات مفتاحية على الأقل ذات صلة بمجموعة البيانات:

![create-dataset-metar-keywords](../assets/img/create-dataset-metar-keywords.png)

تابع لملء الحقول المطلوبة لخصائص المكان ومعلومات الاتصال. اضغط على 'Validate form' للتحقق من أن جميع الحقول الإلزامية قد تم ملؤها.
...
افتح رابط مجموعة البيانات التي تم إنشاؤها للتو وانتقل لأسفل إلى قسم 'links' في استجابة JSON:
...
إذا قمت بالنقر على الرابط، ستحصل على خطأ لأن ملف الترخيص لم يتم إضافته بعد إلى مثيل wis2box الخاص بك.
...
لتحميل هذا الملف، استخدم MinIO Console المتاح على المنفذ 9001 لمثيل wis2box، عن طريق الذهاب إلى متصفح الويب وزيارة `http://YOUR-HOST:9001`.
...
بمجرد تسجيل الدخول إلى MinIO Console، قم بتحميل ملف الترخيص إلى المسار الأساسي لحاوية **wis2box-public** باستخدام زر "Upload":
...
!!! note
    ...
    يقوم الوكيل (web-proxy) في wis2box بتمرير جميع الملفات المخزنة في حاوية "wis2box-public" تحت المسار `WIS2BOX_URL/data/`.
...
انسخ والصق الرابط القانوني (canonical-link) من رسالة إشعار WIS2 في متصفح الويب الخاص بك. يجب أن يبدو الرابط مشابهًا لهذا:
...
واستخدم الأمر `wis2box auth remove-token` لإزالة رمز الوصول من مجموعة البيانات، باستخدام العلم `--metadata-id` لتحديد المعرف الوصفي لمجموعة البيانات ورمز الوصول كمعامل:
...
!!! note "التحكم في الوصول اختياري، فقط للمجموعات الموصى بها"
    ...
    يحدد WIS2 أن المجموعات الموصى بها *قد* يتم تطبيق ضوابط الوصول على بياناتها. ليس من الإلزامي إضافة التحكم في الوصول إلى مجموعة بيانات موصى بها، ويمكنك الاعتماد فقط على رابط الترخيص لإبلاغ مستهلكي البيانات بشروط استخدام البيانات. إذا قمت بتطبيق التحكم في الوصول، فأنت مسؤول عن مشاركة رمز الوصول مع أي مستهلكي بيانات يحتاجون إلى الوصول إلى هذه البيانات.

    إذا قمت بتطبيق `wis2box auth add-token` على مجموعة بيانات بسياسة بيانات WMO 'core'، فستقوم Global Caches بالإبلاغ عن خطأ إلى Global Monitoring لأنها لن تتمكن من تنزيل البيانات وستُعتبر مجموعة بياناتك *غير متوافقة* مع اللوائح الفنية لـ WIS2.
