...
**core**: البيانات التي يتم توفيرها مجانًا وبدون قيود، دون أي تكلفة أو شروط على الاستخدام.

**recommended**: البيانات التي قد يتم توفيرها بشروط على الاستخدام و/أو تخضع لترخيص.

البيانات التي تتم مشاركتها كـ "موصى بها":
...
!!! note "تنزيل البيانات الموصى بها"
    ...
    يجب على مستهلك البيانات تنزيل البيانات من خادم البيانات المستضاف من قبل مزود البيانات، باستخدام الرابط الأساسي المقدم في الإشعارات على الموضوع `origin/a/wis2/<centre-id>/data/recommended/...`

في هذه الجلسة العملية، ستقوم بإنشاء مجموعة بيانات جديدة بسياسة بيانات "موصى بها" باستخدام محرر مجموعات البيانات في wis2box-webapp. ستتعلم أيضًا كيفية توفير ترخيص مستضاف ذاتيًا وكيفية إضافة ضوابط الوصول بشكل اختياري.
...
!!! note "حول رابط الترخيص"
    ...
    يشير رابط الترخيص المرتبط بمجموعة بيانات موصى بها إلى مستهلكي البيانات حول شروط استخدام البيانات.
...
تحقق من MQTT Explorer للتأكد من أنك تلقيت رسالة إشعار WIS2 تعلن عن سجل البيانات الوصفية الجديد على الموضوع `origin/a/wis2/<your-centre-id>/metadata`.
...
إذا قمت بالنقر على الرابط، فستحصل على خطأ لأن ملف الترخيص لم يتم إضافته بعد إلى مثيل wis2box الخاص بك.
...
!!! note
    ...
    يقوم الوكيل العكسي في wis2box بتوجيه جميع الملفات المخزنة في دلو "wis2box-public" تحت المسار `WIS2BOX_URL/data/`.

يجب أن يعمل الرابط لـ "License for this dataset" المضمن في البيانات الوصفية لمجموعة البيانات الموصى بها الآن كما هو متوقع.
