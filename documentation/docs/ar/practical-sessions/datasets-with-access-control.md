...
**core**: بيانات يتم توفيرها مجانًا وبدون قيود، بدون تكلفة وبدون شروط على الاستخدام.

**recommended**: بيانات قد يتم توفيرها بشروط على الاستخدام و/أو تخضع لترخيص.
...
!!! note "تنزيل البيانات الموصى بها"
    ...
    يجب على مستهلك البيانات تنزيل البيانات من خادم البيانات المستضاف من قبل مزود البيانات، باستخدام الرابط الأساسي (canonical URL) المقدم في الإشعارات على الموضوع `origin/a/wis2/<centre-id>/data/recommended/...`.

في هذه الجلسة العملية، ستقوم بإنشاء مجموعة بيانات جديدة بسياسة بيانات 'موصى بها' باستخدام محرر مجموعة البيانات في wis2box-webapp. ستتعلم أيضًا كيفية توفير ترخيص مستضاف ذاتيًا وكيفية إضافة ضوابط الوصول اختياريًا.
...
تأكد من أن لديك متصفح ويب مفتوحًا مع wis2box-webapp لمثيلك عن طريق الانتقال إلى `http://YOUR-HOST/wis2box-webapp`.
...
أرسل مجموعة البيانات، باستخدام رمز المصادقة الذي تم إنشاؤه مسبقًا، وتحقق من إنشاء مجموعة البيانات الجديدة في wis2box-webapp.

تحقق من MQTT Explorer للتأكد من أنك تلقيت رسالة إشعار WIS2 تعلن عن سجل بيانات وصفية جديد على الموضوع `origin/a/wis2/<your-centre-id>/metadata`.
...
يجب أن ترى رابطًا لـ "License for this dataset" يشير إلى الرابط الذي تم توفيره في محرر مجموعة البيانات.

إذا نقرت على الرابط، ستظهر لك رسالة خطأ لأن ملف الترخيص لم يُضف بعد إلى مثيل wis2box الخاص بك.
...
لتحميل هذا الملف، استخدم MinIO Console المتاح على المنفذ 9001 لمثيل wis2box، عن طريق فتح متصفح ويب وزيارة `http://YOUR-HOST:9001`.
...
وتحقق من أنه قد تم إزالة رمز الوصول عن طريق محاولة تنزيل البيانات مرة أخرى باستخدام `wget` مع رمز الوصول في ترويسات HTTP، أو عن طريق محاولة الوصول إلى البيانات في متصفح الويب الخاص بك.

!!! note "التحكم في الوصول اختياري، فقط للمجموعات الموصى بها"

    يحدد WIS2 أن المجموعات الموصى بها *قد* يتم تطبيق ضوابط الوصول عليها. ليس من الإلزامي إضافة التحكم في الوصول إلى مجموعة بيانات موصى بها، ويمكنك الاعتماد فقط على رابط الترخيص لإبلاغ مستهلكي البيانات بشروط استخدام البيانات. إذا قمت بتطبيق التحكم في الوصول، فأنت مسؤول عن مشاركة رمز الوصول مع أي مستهلكي بيانات يحتاجون للوصول إلى هذه البيانات.
