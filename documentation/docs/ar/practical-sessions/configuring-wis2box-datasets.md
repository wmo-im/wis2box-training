...
تُستخدم **بيانات الاكتشاف الوصفية** لإنشاء سجل WMO Core Metadata Profile (WCMP2) يتم مشاركته باستخدام إشعار WIS2 يتم نشره على الوسيط الخاص بـ wis2box.
...
!!! note "إعداد مجموعات البيانات بدون استخدام wis2box-webapp"
    ...
    يدعم wis2box أيضًا إعداد مجموعات البيانات باستخدام [ملف التحكم بالبيانات الوصفية (MCF)](https://geopython.github.io/pygeometa/reference/mcf) الذي يحدده أداة [pygeometa](https://geopython.github.io/pygeometa).
    
    يتيح استخدام MCF مرونة وتحكمًا أكبر ولكنه يتطلب كتابة دقيقة لضمان تنسيق MCF بشكل صحيح واتباع المخطط المطلوب.
...
افتح متصفحًا وانتقل إلى `http://YOUR-HOST/wis2box-webapp`. تأكد من تسجيل الدخول والوصول إلى صفحة 'محرر مجموعات البيانات'.
...
- **معرف المركز (Centre ID)**: هذا هو اختصار الوكالة (بالأحرف الصغيرة وبدون مسافات)، كما هو محدد من قبل عضو WMO، الذي يحدد مركز البيانات المسؤول عن نشر البيانات.
- **القالب (Template)**: نوع البيانات التي تقوم بإنشاء بيانات وصفية لها. يمكنك الاختيار بين استخدام قالب محدد مسبقًا أو اختيار *other*.

<img alt="نافذة منبثقة لإنشاء مجموعة بيانات جديدة" src="/../assets/img/wis2box-create-new-dataset-pop-up.png" width="600">

!!! Note "معرف المركز (Centre ID)"

    يجب أن يبدأ معرف المركز الخاص بك بـ TLD الخاص ببلدك، متبوعًا بشرطة (`-`) واسم مختصر لمؤسستك (على سبيل المثال `fr-meteofrance`). يجب أن يكون معرف المركز بأحرف صغيرة فقط ويستخدم الأحرف الأبجدية الرقمية فقط. تعرض القائمة المنسدلة جميع معرفات المراكز المسجلة حاليًا على WIS2 بالإضافة إلى أي معرف مركز قمت بإنشائه بالفعل في wis2box. يرجى اختيار معرف مركز مناسب لمؤسستك.

!!! Note "القالب (Template)"

    يتيح لك حقل *القالب* اختيار قالب من قائمة القوالب المتاحة في محرر مجموعات البيانات في wis2box-webapp. سيقوم القالب بملء النموذج مسبقًا بقيم افتراضية مقترحة مناسبة لنوع البيانات. يتضمن ذلك عنوانًا مقترحًا وكلمات مفتاحية للبيانات الوصفية ومكونات بيانات مهيأة مسبقًا.
...
- مجموعة بيانات باستخدام القالب=*weather/surface-based-observations/synop*، والتي تتضمن مكونات بيانات تقوم بتحويل البيانات إلى تنسيق BUFR قبل النشر.
- مجموعة بيانات باستخدام القالب=*other*، حيث تكون مسؤولاً عن تعريف موضوع WIS2 وستستخدم المكون "Universal" لنشر البيانات دون تحويل.

## القالب=weather/surface-based-observations/synop

بالنسبة إلى **القالب**، اختر **weather/surface-based-observations/synop**:
...
انقر على *continue to form* للمتابعة. ستظهر الآن **نموذج محرر مجموعة البيانات**.
...
نظرًا لأنك استخدمت قالبًا لإنشاء مجموعة البيانات الخاصة بك، فقد تم تعبئة تعيينات البيانات مسبقًا بالمكونات الإضافية الافتراضية لقالب 'weather/surface-based-observations/synop'. تُستخدم مكونات البيانات الإضافية في 'wis2box' لتحويل البيانات قبل نشرها باستخدام إشعار WIS2.
...
!!! note "حول مربع الاختيار لتخزين البيانات الأساسية مؤقتًا"
    ...
    إذا قمت بإلغاء تحديد هذا المربع، سيضيف 'wis2box' الخاصية `properties.cache=False` مما يشير إلى أنك لا ترغب في أن يقوم 'Global Cache' بتنزيل نسخة من بياناتك وتخزينها. بشكل افتراضي، يتم تحديد هذا المربع، مما يعني أن 'Global Cache' سيقوم بتخزين بياناتك الأساسية مؤقتًا، وهو الإعداد الموصى به لمعظم مجموعات البيانات.

### نشر مجموعة البيانات الخاصة بك

أخيرًا، يمكنك النقر على 'submit' لنشر مجموعة البيانات الخاصة بك.

ستحتاج إلى توفير رمز التفويض الخاص بـ 'processes/wis2box' الذي أنشأته سابقًا. إذا لم تقم بذلك، يمكنك إنشاء رمز جديد باتباع التعليمات في قسم التحضير.
...
بعد النقر على 'OK'، سيتم إعادة توجيهك إلى الصفحة الرئيسية لمحرر مجموعة البيانات. الآن إذا نقرت على علامة التبويب 'Dataset'، يجب أن ترى مجموعة البيانات الجديدة الخاصة بك مدرجة:
...
!!! question
    ...
    حاول العثور على العنوان والوصف والكلمات المفتاحية التي قدمتها في بيانات الاكتشاف الوصفية داخل إشعار WIS2. هل يمكنك العثور عليها؟

??? success "انقر لعرض الإجابة"

    **العنوان والوصف والكلمات المفتاحية التي قدمتها في بيانات الاكتشاف الوصفية غير موجودة في حمولة إشعار WIS2!** 
    
    بدلاً من ذلك، حاول البحث عن الرابط الأساسي في قسم "links" داخل إشعار WIS2:
    ...
    **يتضمن إشعار WIS2 رابطًا أساسيًا إلى سجل WCMP2 الذي تم نشره.** 
    ...
    انسخ هذا الرابط الأساسي والصقه في متصفحك للوصول إلى سجل WCMP2. بناءً على إعدادات المتصفح الخاص بك، قد يُطلب منك تنزيل الملف أو قد يتم عرضه مباشرة في المتصفح.
...
نظرًا لأنك اخترت Template=*other*، لم يتم تحديد أي كلمات مفتاحية مسبقًا. تأكد من إضافة 3 كلمات مفتاحية على الأقل من اختيارك:
...
عند استخدام Template=other، لا يتم توفير أي تعيينات بيانات افتراضية. نتيجة لذلك، سيكون محرر تعيينات البيانات فارغًا ويجب على المستخدمين تكوين التعيينات وفقًا لمتطلباتهم الخاصة.
...
عند إضافة هذا المكون، ستحتاج إلى تحديد **امتداد الملف** ونمط **الملف** (يُحدد بواسطة تعبير منتظم) الذي يتطابق مع نمط تسمية ملفات البيانات الخاصة بك. في حالة مكون "Universal"، يُستخدم نمط الملف أيضًا لتحديد خاصية "datetime" للبيانات.

!!! Note "تحليل التاريخ والوقت من اسم الملف"

    يفترض مكون "Universal" أن المجموعة الأولى في التعبير المنتظم تتوافق مع التاريخ والوقت للبيانات.
    ...
    عند إدخال البيانات باستخدام مكون "Universal"، إما أن تعيد تسمية ملفاتك لتتطابق مع الافتراضي أو تُحدث نمط الملف مع التأكد من أن المجموعة الأولى في التعبير المنتظم تتوافق مع التاريخ والوقت.
...
### الإرسال ومراجعة النتيجة

أخيرًا، قم بتوفير رمز التفويض الخاص بـ 'processes/wis2box' الذي أنشأته سابقًا وانقر على 'submit' لنشر مجموعة البيانات الخاصة بك.
...
من خلال فتح واجهة المستخدم الخاصة بـ wis2box على `http://YOUR-HOST`، يجب أن ترى مجموعتي بيانات مدرجتين مع تسلسل موضوعات WIS2 الخاص بهما. لرؤية وصف كل مجموعة بيانات، يمكنك النقر على "metadata"، والذي سيعيد توجيهك إلى عنصر 'discovery-metadata' المقابل كما يتم تقديمه بواسطة wis2box-api.
...
في الرسالة، ستتغير قيمة *"rel": "canonical"* إلى *"rel": "update"*، مما يشير إلى أن البيانات المنشورة سابقًا قد تم تعديلها. لعرض الوصف المحدث، قم بنسخ الرابط ولصقه في متصفحك، وسترى الوصف المحدث.

!!! question

حاول تحديث تسلسل الموضوعات الخاص بآخر مجموعة بيانات قمت بإنشائها عن طريق تغيير الاختيار في "Sub-Discipline Topics". هل ترى إشعار WIS2 جديدًا منشورًا على الموضوع `origin/a/wis2/<your-centre-id>/metadata`؟

??? success "اضغط للكشف عن الإجابة"

لا يمكنك **تحديث** تسلسل الموضوعات الخاص بمجموعة بيانات موجودة. حقل تسلسل الموضوعات معطل في نموذج تحرير مجموعة البيانات بعد إنشاء المجموعة. إذا كنت ترغب في استخدام تسلسل موضوعات مختلف، قم أولاً بحذف مجموعة البيانات الموجودة ثم قم بإنشاء مجموعة بيانات جديدة باستخدام تسلسل الموضوعات المطلوب.
