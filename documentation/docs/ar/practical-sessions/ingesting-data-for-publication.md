...
!!! abstract "نتائج التعلم"
    ...
    - تشغيل سير عمل `wis2box` عن طريق تحميل البيانات إلى `MinIO` باستخدام واجهة الويب الخاصة بـ `MinIO`، أو بروتوكول `SFTP`، أو باستخدام سكربت مكتوب بلغة بايثون.
    - الوصول إلى لوحة معلومات `Grafana` لمراقبة حالة استيعاب البيانات وعرض سجلات مثيل `wis2box` الخاص بك.
...
لمراقبة حالة سير عمل بيانات `wis2box`، يمكنك استخدام **لوحة معلومات Grafana** و **MQTT Explorer**. تستخدم لوحة معلومات `Grafana` بيانات من `Prometheus` و`Loki` لعرض حالة `wis2box` الخاص بك، بينما يتيح لك `MQTT Explorer` رؤية إشعارات بيانات `WIS2` التي ينشرها مثيل `wis2box` الخاص بك.
...
احتفظ بلوحة معلومات `Grafana` مفتوحة في متصفحك حيث سنستخدمها لاحقًا لمراقبة حالة استيعاب البيانات.
...
- *wis2box-public*: هذه هي الحاوية التي ينشر فيها `wis2box` البيانات التي تم استيعابها ومعالجتها بنجاح.
...
??? success "انقر للكشف عن الإجابة"
    ...
    يمكنك التحقق من لوحة معلومات `Grafana` لمعرفة ما إذا تم استيعاب البيانات ونشرها بنجاح.

    انظر إلى اللوحة السفلية من لوحة معلومات `Grafana` وسترى **خطأ في التحقق من صحة المسار** يشير إلى أن المسار لا يتطابق مع أي مجموعة بيانات مكونة:
...
أدخل الدليل الذي تم إنشاؤه حديثًا، وانقر على `Upload` وقم بتحميل الملف الذي استخدمته سابقًا، *mydata.nc*، إلى الدليل الجديد. تحقق من لوحة معلومات `Grafana` لمعرفة ما إذا تم استيعاب البيانات ونشرها بنجاح.
...
قم بتنزيل هذا الملف [GEPS_18August2025.grib2](../sample-data/GEPS_18August2025.grib2) إلى جهاز الكمبيوتر المحلي الخاص بك وقم بتحميله إلى الدليل الذي أنشأته لمجموعة بيانات التوقعات. تحقق من لوحة معلومات `Grafana` و`MQTT Explorer` لمعرفة ما إذا تم استيعاب البيانات ونشرها بنجاح.
...
??? success "انقر للكشف عن الإجابة"
    ...
    اسم الملف لا يتطابق مع التعبير النمطي الذي قمت بتعريفه في تكوين مجموعة البيانات. يجب أن يتطابق اسم الملف مع النمط `^.*?_(\d{8}).*?\..*$`، والذي يتطلب وجود تاريخ مكون من 8 أرقام (YYYYMMDD) في اسم الملف.
...
بعد إصلاح المشكلة المتعلقة باسم الملف، تحقق من لوحة معلومات `Grafana` و`MQTT Explorer` لمعرفة ما إذا تم استيعاب البيانات ونشرها بنجاح.
...
!!! note "حول المكون الإضافي Universal"
    ...
    يتيح المكون الإضافي "Universal" نشر البيانات دون أي تحويل. إنه مكون إضافي *يمرر البيانات كما هي*، حيث يستوعب ملف البيانات وينشره كما هو. لإضافة خاصية "datetime" إلى إشعار بيانات `WIS2`، يعتمد المكون الإضافي على المجموعة الأولى في نمط الملف لمطابقة التاريخ للبيانات التي تقوم بنشرها.
...
??? success "انقر للكشف عن الإجابة"
    ...
    لا. 
    في لوحة معلومات `Grafana` سترى خطأ يشير إلى أن البيانات تم نشرها بالفعل:
...
```bash
    ...
    إذا كنت ترغب في إعادة إرسال الإشعار لنفس البيانات، قم بحذف البيانات من الحاوية 'wis2box-public' قبل إعادة إدخال البيانات.
...
لاحظ أن أحد المكونات الإضافية هو **بيانات FM-12 المحولة إلى BUFR** (synop2bufr) والذي تم تكوينه للعمل على الملفات ذات الامتداد **txt**.
...
!!! note "حول المكون الإضافي FM-12 المحول إلى BUFR"
    ...
    كجزء من عملية التحويل، يقوم المكون الإضافي بإضافة البيانات الوصفية المفقودة إلى البيانات الناتجة، مثل معرف محطة WIGOS، وموقع المحطة، وارتفاع البارومتر. لإضافة هذه البيانات الوصفية، يبحث المكون الإضافي عن هذه المعلومات في قائمة المحطات في مثيل 'wis2box' الخاص بك باستخدام المعرف التقليدي (المكون من 5 أرقام) (64400 في هذه الحالة).
...
قم بإضافة المحطة بمعرف WIGOS `0-20000-0-64400` إلى مثيل 'wis2box' الخاص بك باستخدام محرر المحطات في 'wis2box-webapp'، كما تعلمت في الجلسة العملية [تكوين بيانات المحطة الوصفية](./configuring-station-metadata.md).
...
تحقق من لوحة تحكم Grafana ومستكشف MQTT للتأكد من أن البيانات قد تم نشرها بنجاح. إذا رأيت الإشعار أدناه، فهذا يعني أنك نشرت بيانات العينة 'synop' بنجاح:
...
على جهاز الطالب الخاص بك، سيتم تثبيت حزمة 'minio' لـ Python مسبقًا.
...
!!! note
    ...
    ستواجه خطأ لأن السكربت غير مهيأ للوصول إلى نقطة النهاية MinIO على مثيل 'wis2box' الخاص بك.

يحتاج السكربت إلى معرفة نقطة النهاية الصحيحة للوصول إلى MinIO على مثيل 'wis2box'. إذا كان 'wis2box' يعمل على مضيفك، فإن نقطة النهاية MinIO متاحة على `http://YOUR-HOST:9000`. كما يحتاج السكربت إلى تحديث كلمة مرور التخزين الخاصة بك والمسار في حاوية MinIO لتخزين البيانات.
...
بمجرد أن تتمكن من تشغيل السكربت بنجاح، سترى رسالة تشير إلى أن الملف تم نسخه إلى MinIO، ويجب أن ترى إشعارات البيانات المنشورة بواسطة مثيل 'wis2box' الخاص بك في مستكشف MQTT.
...
يمكن أيضًا الوصول إلى خدمة MinIO في `wis2box` عبر SFTP. إذا كان لديك نظام موجود يمكن تكوينه لإرسال البيانات عبر SFTP، يمكنك استخدام هذه الطريقة كبديل لأتمتة عملية إدخال البيانات.
...
يمكنك التنقل إلى الحاوية `wis2box-incoming` ثم إلى المجلد الخاص بمجموعة البيانات الخاصة بك. سترى الملفات التي قمت بتحميلها في التمارين السابقة:
