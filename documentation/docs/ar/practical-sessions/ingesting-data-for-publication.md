...
لاحظ أن `wis2box` سيحول البيانات النموذجية إلى تنسيق BUFR قبل نشرها إلى وسيط MQTT، وفقًا لتعيينات البيانات التي تم تكوينها مسبقًا في مجموعة البيانات الخاصة بك. في هذا التمرين، سنركز على الطرق المختلفة لتحميل البيانات إلى مثيل `wis2box` الخاص بك والتحقق من نجاح الإدخال والنشر. سيتم تغطية تحويل البيانات لاحقًا في الجلسة العملية [أدوات تحويل البيانات](./data-conversion-tools.md).
...
كما يتطلب معرفة بتكوين المحطات في **wis2box-webapp**، كما هو موضح في الجلسة العملية [تكوين بيانات تعريف المحطة](./configuring-station-metadata.md).
...
تأكد من أن لديك متصفح ويب مفتوح مع لوحة تحكم Grafana لمثيلك عن طريق الانتقال إلى `http://YOUR-HOST:3000`.
...
### إدخال ونشر باستخدام Universal plugin
...
حدد bucket `wis2box-incoming` وانقر على "Create new path". يجب أن يتطابق اسم الدليل مع معرف البيانات الوصفية لمجموعة بيانات "other" التي قمت بإنشائها مسبقًا في الجلسة العملية [تكوين مجموعات البيانات في wis2box](./configuring-wis2box-datasets.md). لذا في هذه الحالة، يرجى إنشاء الدليل:

```bash
urn:wmo:md:nl-knmi-test:customized-geps-dataset-wis2-training
```

ادخل إلى الدليل الذي تم إنشاؤه حديثًا، انقر على "Upload"، وحدد [sample-data-for-universal-plugin](../sample-data/Z_NAFP_C_BABJ_20250818000000_P_CMA-GEPS-GLB-024.grib2) الذي قمت بتنزيله إلى جهازك المحلي مسبقًا.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    لا، لأنه عند تغيير اسم البيانات إلى "sample-geps-data.grib2"، فلن يتبع قاعدة regex.
    ...
    عند تحميل البيانات، يجب أن تتوافق أسماء الملفات مع اتفاقية التسمية المطلوبة التي تم تعريفها بواسطة تعبير منتظم:
    ...
    شرطة سفلية (_) متبوعة مباشرة بسلسلة تاريخ مكونة من 8 أرقام بتنسيق YYYYMMDD (على سبيل المثال، 20250904).
...
!!! question "تغيير امتداد الملف من .grib2 إلى .bufr4 (دون تغيير محتوى الملف الداخلي)"
...
??? success "اضغط للكشف عن الإجابة"
    ...
    عند تحميل البيانات باستخدام Universal plugin، يجب أن يحتوي الملف على امتداد الملف الصحيح كما هو محدد في تكوين مجموعة البيانات. يضمن هذا الشرط أن عملية الإدخال يمكنها التعرف على تنسيق الملف ومعالجته بشكل صحيح. على سبيل المثال، إذا تم تكوين مجموعة البيانات لملفات grib2، فسيتم قبول الملفات التي تنتهي بـ .grib2 فقط. استخدام امتداد غير صحيح (مثل .txt أو .bin) سيؤدي إلى رفض الملف وعدم نشره.

!!! question "إعادة تحميل البيانات باستخدام واجهة MinIO على الويب"

    انتقل إلى واجهة MinIO على الويب في متصفحك وتصفح إلى bucket `wis2box-incoming`. سترى الملف `Z_NAFP_C_BABJ_20250818000000_P_CMA-GEPS-GLB-024.grib2` الذي قمت بتحميله في التمارين السابقة.
...
### إدخال ونشر باستخدام synop2bufr-plugin
...
اختر الحاوية `wis2box-incoming` وانقر على "Create new path". يجب أن يتطابق اسم الدليل مع معرف البيانات الوصفية لمجموعة بيانات "surface-based-observations/synop" التي قمت بإنشائها مسبقًا في جلسة العمل العملية [Configuring Datasets in wis2box](./configuring-wis2box-datasets.md). لذا، في هذه الحالة، يرجى إنشاء الدليل:
...
??? success "انقر للكشف عن الإجابة"
    ...
    لا. في لوحة تحكم Grafana سترى خطأ يشير إلى فشل الإدخال:

    <img alt="grafana_data_ingest" src="/../assets/img/grafana_data-ingest-error.png" width="800"> 

    عند استخدام قالب مجموعة بيانات `synop` مع الإضافات الافتراضية لـ `synop` (لبيانات CSV وTXT وBUFR SYNOP)، يجب أن تتضمن كل سجل معرف محطة صالح. يفشل الإدخال إذا لم تكن المحطة معروفة في مثيل `wis2box` الخاص بك. لذلك، يجب عليك إضافة المحطة أولاً قبل نشر بيانات SYNOP.
    ...
    أضف المحطة إلى مجموعات البيانات التي أنشأتها للنشر في "../surface-based-observations/synop" واحفظ التغييرات باستخدام رمز المصادقة الخاص بك:
...
بعد الانتهاء من تكوين بيانات المحطة الوصفية، تحقق باستخدام MQTT Explorer للتأكد من أن البيانات قد تم نشرها بنجاح. إذا رأيت الإشعار أدناه، فهذا يعني أنك قمت بنشر بيانات العينة `synop` بنجاح.
...
في جهاز الطالب الافتراضي الخاص بك، ستكون حزمة 'minio' لـ Python مثبتة بالفعل.

في دليل `exercise-materials/data-ingest-exercises`، ستجد سكربت مثال يسمى `copy_file_to_incoming.py` يمكن استخدامه لنسخ الملفات إلى MinIO.
...
!!! note
    ...
    ستظهر لك رسالة خطأ لأن السكربت غير مهيأ للوصول إلى نقطة النهاية الخاصة بـ MinIO في `wis2box` الخاص بك حتى الآن.

يحتاج السكربت إلى معرفة نقطة النهاية الصحيحة للوصول إلى MinIO في `wis2box` الخاص بك. إذا كان `wis2box` يعمل على مضيفك، فإن نقطة النهاية الخاصة بـ MinIO متاحة على `http://YOUR-HOST:9000`. كما يحتاج السكربت إلى تحديث كلمة مرور التخزين الخاصة بك والمسار في حاوية MinIO لتخزين البيانات.
...
!!! question "تحديث السكربت وإدخال بيانات CSV"
    ...
    - باستخدام WinSCP: ابدأ اتصالاً جديدًا باستخدام بروتوكول الملفات `SCP` ونفس بيانات الاعتماد الخاصة بعميل SSH الخاص بك. انتقل إلى الدليل `wis2box-data/data-ingest-exercises` وقم بتحرير `copy_file_to_incoming.py` باستخدام محرر النصوص المدمج.

    تأكد من أنك:

    - تحدد نقطة النهاية الصحيحة لـ MinIO لمضيفك.
    - تقدم كلمة مرور التخزين الصحيحة لمثيل MinIO الخاص بك.
    - تقدم المسار الصحيح في حاوية MinIO لتخزين البيانات.
...
بمجرد أن تتمكن من تشغيل السكربت بنجاح، سترى رسالة تشير إلى أن الملف قد تم نسخه إلى MinIO، ويجب أن ترى إشعارات البيانات التي تم نشرها بواسطة مثيل `wis2box` الخاص بك في MQTT Explorer.
...
??? success "انقر للكشف عن الإجابة"
    ...
    إذا تم تعريف جميع المحطات المستخدمة في ملف BUFR في مثيل `wis2box` الخاص بك، يجب أن ترى 10 رسائل تم نشرها إلى وسيط MQTT. كل إشعار يتوافق مع بيانات لمحطة واحدة ولطابع زمني واحد للملاحظة.
...
!!! question "تحميل البيانات باستخدام SFTP"
    ...
    [synop_202503030900.txt](./../sample-data/synop_202503030900.txt) (انقر بزر الماوس الأيمن وحدد "حفظ باسم" لتنزيل الملف).
...
??? success "انقر للكشف عن الإجابة"
    ...
    يجب أن ترى إشعار بيانات WIS2 جديد تم نشره لمحطة الاختبار `0-20000-0-64400`، مما يشير إلى أن البيانات قد تم إدخالها ونشرها بنجاح.
...
!!! success "تهانينا!"
    ...
    - مراقبة إشعارات بيانات WIS2 التي تم نشرها بواسطة `wis2box` الخاص بك في لوحة تحكم Grafana وMQTT Explorer.