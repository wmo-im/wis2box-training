...
!!! abstract "الأهداف التعليمية"
    ...
    - تشغيل سير عمل `wis2box` عن طريق تحميل البيانات إلى MinIO باستخدام واجهة MinIO على الويب، أو SFTP، أو باستخدام سكربت بايثون.
...
خلال التمرين، سنراقب حالة إدخال البيانات باستخدام **لوحة تحكم Grafana** و**MQTT Explorer**. تستخدم لوحة تحكم Grafana بيانات من Prometheus وLoki لعرض حالة `wis2box` الخاص بك، بينما يتيح لك MQTT Explorer رؤية إشعارات بيانات WIS2 التي ينشرها مثيل `wis2box` الخاص بك.
...
كما يتطلب معرفة تكوين المحطات في **wis2box-webapp**، كما هو موضح في الجلسة العملية [تكوين بيانات المحطة الوصفية](./configuring-station-metadata.md).
...
<img alt="واجهة MinIO: واجهة MinIO" src="/../assets/img/minio-ui.png" width="400">
...
<img alt="واجهة MinIO: إنشاء مسار جديد" src="/../assets/img/minio-create-new-path.png" width="800">
...
<img alt="واجهة MinIO: إنشاء مسار جديد فارغ" src="/../assets/img/minio-ui-create-path-empty.png" width="700">

لذا في هذه الحالة، يرجى إنشاء الدليل:

```bash
urn:wmo:md:my-centre-id:my-other-dataset
```

ادخل إلى الدليل الذي تم إنشاؤه حديثًا، انقر على `Upload`، وابحث عن الملف [geps_202508180000.grib2](../sample-data/geps_202508180000.grib2) الذي قمت بتنزيله إلى جهازك المحلي من قبل وقم بتحميل هذا الملف إلى bucket `wis2box-incoming`.

<img alt="واجهة MinIO: تحميل ملفك" src="/../assets/img/minio-other-dataset-upload.png" width="650">

بمجرد الانتهاء من التحميل، ستظهر هذا الملف في bucket `wis2box-incoming` في MinIO:

<img alt="واجهة MinIO: تحميل ملفك" src="/../assets/img/minio-geps-file-upload.png" width="650">

بعد التحميل، تحقق باستخدام MQTT Explorer للتأكد من أن البيانات تم نشرها بنجاح.

<img alt="محرر البيانات الوصفية: العنوان، الوصف، الكلمات المفتاحية" src="/../assets/img/mqtt-explorer-wis2-notification-geps-sample.png" width="800">
...
??? success "اضغط للكشف عن الإجابة"
    ...
    نعم.
    <img alt="واجهة MinIO: ملف geps بامتداد nc" src="/../assets/img/minio-upload-geps-with-nc-extension.png" width="800">
...
??? success "اضغط للكشف عن الإجابة"
    ...
    يوضح هذا أن سير العمل الخاص بالبيانات تم تشغيله، ولكن لم يتم إعادة نشر البيانات. لن يقوم `wis2box` بنشر البيانات إذا لم يتمكن من مطابقة امتداد الملف `grib2`.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    نعم.
    <img alt="واجهة MinIO: ملف geps المعاد تسميته" src="/../assets/img/minio-upload-renamed-geps.png" width="800">
...
??? success "اضغط للكشف عن الإجابة"
    ...
    لا.
    سترى رسالة تشير إلى وجود خطأ يتعلق بعدم تطابق البيانات مع النمط regex.

    ```bash
    ERROR - ERROR - geps_renamed_sample_data.grib2 did not match ^.*?_(\d{8}).*?\..*$
    ``` 
    
    يوضح هذا أن سير العمل الخاص بالبيانات تم تشغيله، ولكن لم يتم إعادة نشر البيانات. لن يقوم `wis2box` بنشر البيانات إذا لم يتمكن من مطابقة نمط الملف `^.*?_(\d{8}).*?\..*$`.
...
تحميل ملف إلى MinIO ينجح دائمًا طالما أن المستخدم لديه صلاحية الوصول. ومع ذلك، فإن نشر إشعار بيانات WIS2 يتطلب تحققًا أكثر صرامة. الملفات التي لا تستوفي قواعد الامتداد أو اسم الملف سيتم تخزينها في حاوية `wis2box-incoming`، ولكن لن يقوم `Universal plugin` بنشر إشعار لها. هذا يفسر لماذا يتم قبول الملفات ذات الامتداد غير المدعوم (مثل `geps_202508180000.nc`) أو ذات اسم الملف غير الصحيح (مثل `geps_renamed_sample_data.grib2`) من قبل MinIO ولكنها لا تظهر في WIS2.
...
### الاستيعاب والنشر باستخدام synop2bufr-plugin
...
ادخل إلى الدليل الذي تم إنشاؤه حديثًا، انقر على `Upload`، وحدد [synop_202502040900.txt](../sample-data/synop_202502040900.txt) الذي قمت بتنزيله إلى جهازك المحلي مسبقًا ثم قم بتحميله.
...
??? success "انقر للكشف عن الإجابة"
    ...
    يوضح هذا أن سير العمل الخاص بالبيانات قد تم تشغيله، ولكن هناك حاجة إلى بيانات وصفية لمحطة معينة.
...
أثناء هذه العملية، يحتاج المكون الإضافي إلى تحليل ورسم خريطة لمعلومات المحطة الواردة في البيانات.
...
بعد الانتهاء من تكوين بيانات المحطة الوصفية، تحقق باستخدام MQTT Explorer للتأكد من أن البيانات قد نُشرت بنجاح. إذا رأيت الإشعار أدناه، فهذا يعني أنك نشرت بيانات synop التجريبية بنجاح.
...
على جهاز VM الخاص بك، سيتم تثبيت حزمة 'minio' لـ Python مسبقًا.
...
حاول تشغيل البرنامج النصي لنسخ ملف البيانات التجريبي `synop_202501030900.txt` إلى حاوية `wis2box-incoming` في MinIO كما يلي:
...
!!! note
    ...
    ستظهر لك رسالة خطأ لأن البرنامج النصي غير مهيأ للوصول إلى نقطة النهاية MinIO على wis2box الخاص بك بعد.

يحتاج البرنامج النصي إلى معرفة نقطة النهاية الصحيحة للوصول إلى MinIO على wis2box الخاص بك. إذا كان wis2box يعمل على مضيفك، فإن نقطة النهاية MinIO متاحة على `http://YOUR-HOST:9000`. كما يحتاج البرنامج النصي إلى التحديث باستخدام كلمة مرور التخزين الخاصة بك والمسار في حاوية MinIO لتخزين البيانات.
...
!!! question "تحديث البرنامج النصي واستيعاب بيانات CSV"
    ...
    أعد تشغيل البرنامج النصي لاستيعاب ملف البيانات التجريبي `synop_202501030900.txt` في MinIO:
...
!!! question "استيعاب البيانات الثنائية بتنسيق BUFR"
    ...
    قم بتشغيل الأمر التالي لنسخ ملف البيانات الثنائي `bufr-example.bin` إلى حاوية `wis2box-incoming` في MinIO:
...
يمكن أيضًا الوصول إلى خدمة MinIO في wis2box عبر SFTP. يتم ربط خادم SFTP الخاص بـ MinIO بالمنفذ 8022 على المضيف (المنفذ 22 مستخدم لـ SSH).
...
??? success "انقر لعرض الإجابة"
    ...
    يجب أن ترى إشعار بيانات WIS2 جديد منشورًا لمحطة الاختبار `0-20000-0-64400`، مما يشير إلى أن البيانات تم استيعابها ونشرها بنجاح.
