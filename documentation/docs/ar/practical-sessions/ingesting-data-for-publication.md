...
لمراقبة حالة سير عمل بيانات `wis2box`، يمكنك استخدام **لوحة تحكم Grafana** و**MQTT Explorer**. تستخدم لوحة تحكم Grafana بيانات من Prometheus وLoki لعرض حالة `wis2box` الخاص بك، بينما يسمح لك MQTT Explorer بمشاهدة إشعارات بيانات WIS2 التي ينشرها مثيل `wis2box` الخاص بك.
...
## الإدخال والنشر باستخدام المكون الإضافي Universal

الآن بعد أن عرفت كيفية تحميل البيانات إلى MinIO، دعنا نحاول تحميل البيانات لمجموعة البيانات التي أنشأتها في الجلسة العملية السابقة باستخدام المكون الإضافي `Universal`.
...
أدخل الدليل الذي تم إنشاؤه حديثًا، وانقر على `Upload` وحمّل الملف الذي استخدمته سابقًا، *mydata.nc*، إلى الدليل الجديد. تحقق من لوحة تحكم Grafana لمعرفة ما إذا تم إدخال البيانات ونشرها بنجاح.
...
## إدخال ونشر البيانات باستخدام synop2bufr-plugin

بعد ذلك، ستعمل على مجموعة البيانات التي أنشأتها في الجلسة العملية السابقة باستخدام **القالب 'weather/surface-based-observations/synop'**. يقوم القالب بتكوين الإضافات التالية للبيانات مسبقًا:

<img alt="synop-dataset-plugins" src="/../assets/img/wis2box-data-mappings.png" width="1000">

لاحظ أن إحدى الإضافات هي **بيانات FM-12 المحولة إلى BUFR** (synop2bufr)، والتي تم تكوينها للعمل على الملفات ذات الامتداد **txt**.
...
??? success "اضغط لعرض الإجابة"
    ...
    تم تشغيل سير العمل الخاص بالبيانات، ولكن الإضافة لم تتمكن من معالجة البيانات بسبب نقص بيانات المحطة الوصفية.

!!! note "حول الإضافة FM-12 المحولة إلى BUFR"

    تحاول هذه الإضافة تحويل بيانات FM-12 المدخلة إلى تنسيق BUFR.

    كجزء من عملية التحويل، تضيف الإضافة البيانات الوصفية المفقودة إلى البيانات الناتجة، مثل معرف محطة WIGOS، الموقع، وارتفاع البارومتر للمحطة. للقيام بذلك، تبحث الإضافة عن هذه المعلومات في قائمة المحطات في مثيل 'wis2box' الخاص بك باستخدام المعرف التقليدي (5 أرقام) (64400 في هذه الحالة).

    إذا لم يتم العثور على المحطة في قائمة المحطات، فلن تتمكن الإضافة من إضافة البيانات الوصفية المفقودة ولن تنشر أي بيانات.
...
تحقق من لوحة تحكم Grafana وMQTT Explorer للتأكد من أن البيانات قد تم نشرها بنجاح. إذا رأيت الإشعار أدناه، فهذا يعني أنك قمت بنشر بيانات العينة synop بنجاح:
...
??? success "اضغط لعرض الإجابة"
    ...
    امتداد الملف هو `.bufr4`، مما يشير إلى أن البيانات قد تم تحويلها بنجاح من تنسيق FM-12 إلى تنسيق BUFR بواسطة الإضافة.
...
يحتاج السكربت إلى معرفة نقطة النهاية الصحيحة للوصول إلى MinIO على 'wis2box' الخاص بك. إذا كان 'wis2box' يعمل على المضيف الخاص بك، فإن نقطة النهاية MinIO متوفرة على `http://YOUR-HOST:9000`. كما يحتاج السكربت إلى تحديث كلمة مرور التخزين الخاصة بك ومسار الحاوية في MinIO لتخزين البيانات.
...
??? success "اضغط لعرض الإجابة"
    ...
    سترى أخطاء تم الإبلاغ عنها في Grafana لأن المحطات الموجودة في ملف BUFR غير معرفة في قائمة المحطات في مثيل 'wis2box' الخاص بك.
    ...
    يقوم الإضافة `wis2box.data.bufr4.ObservationDataBUFR` بتقسيم ملف BUFR إلى رسائل BUFR فردية وينشر رسالة واحدة لكل محطة وتوقيت ملاحظة.
...
يمكنك إعداد اتصال جديد باستخدام WinSCP كما هو موضح في لقطة الشاشة التالية:

<img alt="winscp-sftp-connection" src="/../assets/img/winscp-sftp-login.png" width="400">

بيانات الاعتماد لاتصال SFTP يتم تعريفها بواسطة `WIS2BOX_STORAGE_USERNAME` و `WIS2BOX_STORAGE_PASSWORD` في ملف `wis2box.env` وهي نفس بيانات الاعتماد التي استخدمتها للاتصال بواجهة MinIO.
...
!!! question "تحميل البيانات باستخدام SFTP"
    ...
    تحقق من لوحة معلومات Grafana و MQTT Explorer لمعرفة ما إذا تم استيعاب البيانات ونشرها بنجاح.

??? success "انقر للكشف عن الإجابة"

    يجب أن ترى إشعار بيانات WIS2 جديد تم نشره للمحطة التجريبية `0-20000-0-64400`، مما يشير إلى أن البيانات تم استيعابها ونشرها بنجاح.
...
!!! success "تهانينا!"
    ...
    - مراقبة إشعارات بيانات WIS2 التي ينشرها wis2box الخاص بك في لوحة معلومات Grafana و MQTT Explorer.