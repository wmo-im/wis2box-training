...
!!! abstract "الأهداف التعليمية"
    ...
    - تشغيل سير عمل **wis2box** عن طريق تحميل البيانات إلى **MinIO** باستخدام واجهة الويب الخاصة بـ MinIO، أو SFTP، أو سكربت Python.
...
لتشغيل سير العمل الخاص بالبيانات في **WIS2 Node** باستخدام برنامج **wis2box**، يجب تحميل البيانات إلى حاوية **wis2box-incoming** في **MinIO**، مما يؤدي إلى بدء سير عمل **wis2box**. ينتج عن هذه العملية نشر البيانات عبر إشعار بيانات **WIS2**. بناءً على خرائط البيانات التي تم تكوينها في مثيل **wis2box** الخاص بك، قد يتم تحويل البيانات إلى تنسيق **BUFR** قبل نشرها.
...
أثناء التمرين، سنراقب حالة إدخال البيانات باستخدام **لوحة تحكم Grafana** و**MQTT Explorer**. تستخدم لوحة تحكم Grafana بيانات من **Prometheus** و**Loki** لعرض حالة **wis2box** الخاص بك، بينما يتيح لك **MQTT Explorer** رؤية إشعارات بيانات **WIS2** التي ينشرها مثيل **wis2box** الخاص بك.
...
أولاً، سنستخدم واجهة الويب الخاصة بـ **MinIO**، والتي تتيح لك تنزيل وتحميل البيانات إلى **MinIO** باستخدام متصفح الويب.

### الوصول إلى واجهة MinIO

افتح واجهة الويب الخاصة بـ **MinIO**، والمتوفرة عادةً على `http://YOUR-HOST:9001`.
...
استخدم قيم **WIS2BOX_STORAGE_USERNAME** و**WIS2BOX_STORAGE_PASSWORD** كاسم المستخدم وكلمة المرور عند تسجيل الدخول إلى **MinIO**.

### إدخال ونشر باستخدام المكون الإضافي Universal

قم بتنزيل بيانات العينة **geps** [geps_202508180000.grib2](../sample-data/geps_202508180000.grib2) إلى بيئتك المحلية:
...
ادخل إلى الدليل الذي تم إنشاؤه حديثًا، وانقر على `Upload`، وابحث عن الملف [geps_202508180000.grib2](../sample-data/geps_202508180000.grib2) الذي قمت بتنزيله إلى جهازك المحلي مسبقًا وقم بتحميل هذا الملف إلى حاوية **wis2box-incoming**.

<img alt="minio ui: upload your file" src="/../assets/img/minio-other-dataset-upload.png" width="650">

بمجرد الانتهاء من التحميل، ستظهر لك هذا الملف في حاوية **wis2box-incoming** في **MinIO**:

<img alt="minio ui: upload your file" src="/../assets/img/minio-geps-file-upload.png" width="650">

بعد التحميل، تحقق باستخدام **MQTT Explorer** للتأكد من أن البيانات قد تم نشرها بنجاح.
...
بعد ذلك، قم بتنزيل بيانات العينة **geps** بامتداد ملف مختلف [geps_202508180000.nc](../sample-data/geps_202508180000.nc) إلى بيئتك المحلية. قم بتحميل هذا الملف إلى نفس الدليل كما فعلت في التمرين السابق.
...
!!! question "سؤال"
    ...
    هل يمكنك نشر رسائل إشعارات البيانات بنجاح من خلال **MinIO**؟ 
    تحقق من لوحة تحكم **Grafana** و**MQTT Explorer** لمعرفة ما إذا تم إدخال البيانات ونشرها بنجاح.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    يوضح هذا أن سير العمل الخاص بالبيانات قد تم تشغيله، ولكن لم يتم إعادة نشر البيانات. لن يقوم **wis2box** بنشر البيانات إذا لم يتمكن من مطابقة امتداد الملف **grib2**.

بعد ذلك، قم بتنزيل بيانات العينة **geps** المعاد تسميتها [geps_renamed_sample_data.grib2](../sample-data/geps_renamed_sample_data.grib2) إلى بيئتك المحلية. قم بتحميل هذا الملف إلى نفس الدليل كما فعلت في التمرينين السابقين.
...
??? success "اضغط للكشف عن الإجابة"
    ...
    لا.
    سترى رسالة تشير إلى وجود خطأ يتعلق بعدم تطابق البيانات مع النمط المطلوب.

    ```bash
    ERROR - ERROR - geps_renamed_sample_data.grib2 did not match ^.*?_(\d{8}).*?\..*$
    ``` 
    
    يوضح هذا أن سير العمل الخاص بالبيانات قد تم تشغيله، ولكن لم يتم إعادة نشر البيانات. لن يقوم **wis2box** بنشر البيانات إذا لم يتمكن من مطابقة نمط الملف **^.*?_(\d{8}).*?\..*$**.
...
`نمط اسم الملف` – يجب أن يتطابق اسم الملف مع التعبير النمطي المحدد في مجموعة البيانات.

إذا تم استيفاء كلا الشرطين، يتم إدخال الملف ويتم نشر الإشعار.

تحميل ملف إلى MinIO ينجح دائمًا طالما أن المستخدم لديه صلاحية الوصول. ومع ذلك، يتطلب نشر إشعار بيانات WIS2 تحققًا أكثر صرامة. الملفات التي لا تستوفي قواعد الامتداد أو اسم الملف سيتم تخزينها في حاوية `wis2box-incoming`، ولكن لن يقوم `Universal plugin` بنشر إشعار لها. يوضح هذا السبب وراء قبول MinIO للملفات ذات الامتداد غير المدعوم (مثل `geps_202508180000.nc`) أو اسم الملف غير الصحيح (مثل `geps_renamed_sample_data.grib2`) ولكنها لا تظهر في WIS2.

بعد ذلك، انتقل إلى واجهة MinIO على المتصفح وتصفح إلى الحاوية `wis2box-incoming`. سترى الملف `geps_202508180000.grib2` الذي قمت بتحميله في التمارين السابقة.
...
!!! question "سؤال"
    ...
    تحقق من لوحة تحكم Grafana وMQTT Explorer لمعرفة ما إذا تم استيعاب البيانات ونشرها بنجاح.
...
كما في التمارين السابقة، قم بإنشاء دليل داخل حاوية `wis2box-incoming` يتطابق مع معرف البيانات الوصفية لمجموعة بيانات surface-based-observations/synop الخاصة بك.

ادخل إلى الدليل الذي تم إنشاؤه حديثًا، انقر على `Upload`، وحدد [synop_202502040900.txt](../sample-data/synop_202502040900.txt) الذي قمت بتنزيله إلى جهازك المحلي ثم قم بتحميله.
...
??? success "انقر لعرض الإجابة"
    ...
    لا.
    في لوحة تحكم Grafana سترى تحذيرًا يشير إلى فقدان سجل المحطة 64400:
...
الغرض من هذا المكون الإضافي هو معالجة بيانات FM-12 المقدمة بتنسيق نص عادي وتحويلها إلى BUFR ثنائي.
أثناء هذه العملية، يحتاج المكون الإضافي إلى تحليل ورسم خريطة لمعلومات المحطة الموجودة في البيانات.
...
بعد الانتهاء من تكوين بيانات المحطة الوصفية، تحقق باستخدام MQTT Explorer للتأكد من أن البيانات تم نشرها بنجاح. إذا رأيت الإشعار أدناه، فهذا يعني أنك قمت بنشر بيانات synop التجريبية بنجاح.
...
على جهاز الطالب الافتراضي الخاص بك، سيتم تثبيت حزمة 'minio' الخاصة بـ Python مسبقًا.
...
في دليل `exercise-materials/data-ingest-exercises`، ستجد نصًا تجريبيًا `copy_file_to_incoming.py` يمكن استخدامه لنسخ الملفات إلى MinIO.

حاول تشغيل النص لنسخ ملف البيانات التجريبي `synop_202501030900.txt` إلى حاوية `wis2box-incoming` في MinIO كما يلي:
...
!!! note
    ...
    ستتلقى خطأ لأن النص غير مهيأ للوصول إلى نقطة النهاية الخاصة بـ MinIO على wis2box الخاص بك.

يحتاج النص إلى معرفة نقطة النهاية الصحيحة للوصول إلى MinIO على wis2box الخاص بك. إذا كان wis2box يعمل على المضيف الخاص بك، فإن نقطة النهاية الخاصة بـ MinIO متوفرة على `http://YOUR-HOST:9000`. كما يحتاج النص إلى التحديث باستخدام كلمة مرور التخزين الخاصة بك والمسار في حاوية MinIO لتخزين البيانات.

!!! question "تحديث النص واستيعاب بيانات CSV"
    
    قم بتحرير النص `copy_file_to_incoming.py` لمعالجة الأخطاء، باستخدام إحدى الطرق التالية:
    - من سطر الأوامر: استخدم محرر النصوص `nano` أو `vim` لتحرير النص.
    ...
    - تحدد نقطة النهاية الصحيحة لـ MinIO على المضيف الخاص بك.
    - توفر كلمة مرور التخزين الصحيحة لمثيل MinIO الخاص بك.
    - توفر المسار الصحيح في حاوية MinIO لتخزين البيانات.

    أعد تشغيل النص لاستيعاب ملف البيانات التجريبي `synop_202501030900.txt` في MinIO:
...
بمجرد أن تتمكن من تشغيل النص بنجاح، سترى رسالة تشير إلى أن الملف تم نسخه إلى MinIO، ويجب أن ترى إشعارات البيانات التي تم نشرها بواسطة مثيل wis2box الخاص بك في MQTT Explorer.

يمكنك أيضًا التحقق من لوحة تحكم Grafana لمعرفة ما إذا تم استيعاب البيانات ونشرها بنجاح.

الآن بعد أن أصبح النص يعمل، يمكنك محاولة نسخ ملفات أخرى إلى MinIO باستخدام نفس النص.
...
تحقق من لوحة تحكم Grafana وMQTT Explorer لمعرفة ما إذا تم استيعاب بيانات الاختبار ونشرها بنجاح. إذا رأيت أي أخطاء، حاول حلها.
...
??? success "انقر لعرض الإجابة"
    ...
    إذا تم تعريف جميع المحطات المستخدمة في ملف BUFR في مثيل wis2box الخاص بك، فيجب أن ترى 10 رسائل منشورة إلى وسيط MQTT. كل إشعار يتوافق مع بيانات لمحطة واحدة ووقت ملاحظة واحد.
...
يمكن أيضًا الوصول إلى خدمة MinIO في wis2box عبر SFTP. يتم ربط خادم SFTP الخاص بـ MinIO بالمنفذ 8022 على المضيف (المنفذ 22 مخصص لـ SSH).
...
يمكنك التنقل إلى الحاوية `wis2box-incoming` ثم إلى المجلد الخاص بمجموعة البيانات الخاصة بك. ستظهر الملفات التي قمت بتحميلها في التمارين السابقة:
...
!!! question "تحميل البيانات باستخدام SFTP"
    ...
    قم بتنزيل هذا الملف النموذجي إلى جهاز الكمبيوتر المحلي الخاص بك:
    ...
    تحقق من لوحة معلومات Grafana وMQTT Explorer للتأكد من أن البيانات تم استيعابها ونشرها بنجاح.
