...
!!! abstract "الأهداف التعليمية"
    ...
    - الوصول إلى لوحة التحكم في Grafana لمراقبة حالة إدخال البيانات وعرض سجلات مثيل `wis2box` الخاص بك.
...
لتشغيل سير عمل البيانات في WIS2 Node باستخدام برنامج `wis2box`، يجب تحميل البيانات إلى الحاوية **wis2box-incoming** في **MinIO**، مما يؤدي إلى بدء سير عمل بيانات `wis2box` لمعالجة البيانات ونشرها.
...
لاختبار عملية إدخال البيانات يدويًا، سنستخدم واجهة MinIO على الويب، والتي تتيح لك تنزيل وتحميل البيانات إلى MinIO باستخدام متصفح الويب.

في بيئة الإنتاج، يتم عادةً إدخال البيانات باستخدام عمليات مؤتمتة، مثل السكربتات أو التطبيقات التي تنقل البيانات إلى MinIO عبر S3 أو SFTP.

## التحضير

يفترض هذا القسم أنك أكملت بنجاح الجلسة العملية [تكوين مجموعات البيانات في wis2box](./configuring-wis2box-datasets.md). إذا اتبعت التعليمات في تلك الجلسة، يجب أن يكون لديك مجموعة بيانات واحدة تستخدم المكون الإضافي `Universal`، وأخرى تستخدم المكون الإضافي `FM-12 data converted to BUFR`.

تأكد من أنه يمكنك تسجيل الدخول إلى جهاز الطالب الافتراضي الخاص بك باستخدام عميل SSH الخاص بك (مثل PuTTY).
...
لتسجيل الدخول، تحتاج إلى استخدام بيانات الاعتماد المعرفة بواسطة WIS2BOX_STORAGE_USERNAME وWIS2BOX_STORAGE_PASSWORD في ملف `wis2box.env`. يمكنك التحقق من قيم هذه المتغيرات عن طريق تشغيل الأوامر التالية على جهاز الطالب الافتراضي الخاص بك:
...
بعد تسجيل الدخول، ستكون في عرض مستعرض الكائنات الخاص بـ MinIO. هنا يمكنك رؤية الحاويات التي يستخدمها `wis2box`:
...
أدخل مسار المجلد الجديد = *new-directory* وقم بتحميل ملف المثال هذا [mydata.nc](./../sample-data/mydata.nc) (انقر بزر الماوس الأيمن واختر "حفظ باسم" لتنزيل الملف). يمكنك استخدام زر "Upload" في MinIO لتحميل الملف إلى الدليل الجديد:
...
ارجع إلى واجهة MinIO على الويب في متصفحك، وحدد الحاوية `wis2box-incoming`، وانقر على `Create new path`.
...
أدخل الدليل الذي تم إنشاؤه حديثًا، انقر على `Upload` وقم بتحميل الملف الذي استخدمته سابقًا، *mydata.nc*، إلى الدليل الجديد. تحقق من لوحة تحكم Grafana لمعرفة ما إذا تم إدخال البيانات ونشرها بنجاح.
...
قم بتنزيل هذا الملف [GEPS_18August2025.grib2](../sample-data/GEPS_18August2025.grib2) إلى جهاز الكمبيوتر الخاص بك وقم بتحميله إلى الدليل الذي أنشأته لمجموعة بيانات التوقعات. تحقق من لوحة تحكم Grafana وMQTT Explorer لمعرفة ما إذا تم إدخال البيانات ونشرها بنجاح.
...
??? success "انقر للكشف عن الإجابة"
    ...
    اسم الملف لا يتطابق مع التعبير العادي الذي قمت بتعريفه في تكوين مجموعة البيانات. يجب أن يتطابق اسم الملف مع النمط `^.*?_(\d{8}).*?\..*$`، والذي يتطلب وجود تاريخ مكون من 8 أرقام (YYYYMMDD) في اسم الملف.
...
إذا كنت تريد إعادة إرسال الإشعار لنفس البيانات، قم بحذف البيانات من الحاوية 'wis2box-public' قبل إعادة إدخال البيانات.

## إدخال ونشر البيانات: المكون الإضافي "synop2bufr"

بعد ذلك، ستقوم بإدخال مجموعة البيانات التي أنشأتها في الجلسة العملية السابقة باستخدام **القالب='weather/surface-based-observations/synop'**. القالب مُعد مسبقًا لتكوين المكونات الإضافية التالية:

<img alt="synop-dataset-plugins" src="/../assets/img/wis2box-data-mappings.png" width="1000">

لاحظ أن أحد المكونات الإضافية هو **بيانات FM-12 المحولة إلى BUFR** (synop2bufr) والمُعد للعمل مع الملفات ذات الامتداد **txt**.

قم بتنزيل بيانات العينة [synop_202502040900.txt](../sample-data/synop_202502040900.txt) (انقر بزر الماوس الأيمن واختر "حفظ باسم" لتنزيل الملف) إلى جهاز الكمبيوتر الخاص بك. قم بإنشاء مسار جديد في MinIO يتطابق مع معرف البيانات الوصفية لمجموعة بيانات 'synop'، وقم بتحميل بيانات العينة في هذا المسار.
...
!!! note "حول المكون الإضافي FM-12 المحول إلى BUFR"
    ...
    كجزء من عملية التحويل، يضيف المكون الإضافي البيانات الوصفية المفقودة إلى البيانات الناتجة، مثل معرف محطة WIGOS، الموقع، وارتفاع البارومتر للمحطة. للقيام بذلك، يبحث المكون الإضافي عن هذه المعلومات في قائمة المحطات في مثيل 'wis2box' الخاص بك باستخدام المعرف التقليدي (5 أرقام) (في هذه الحالة 64400).
...
بعد إضافة المحطة، أعد تشغيل سير العمل الخاص بـ 'wis2box' عن طريق تحميل ملف بيانات العينة *synop_202502040900.txt* مرة أخرى في نفس المسار في MinIO.

تحقق من لوحة معلومات Grafana وMQTT Explorer للتأكد من أن البيانات تم نشرها بنجاح. إذا رأيت الإشعار أدناه، فهذا يعني أنك قمت بنشر بيانات العينة 'synop' بنجاح:
...
??? success "اضغط لعرض الإجابة"
    ...
    امتداد الملف هو `.bufr4`، مما يشير إلى أن البيانات تم تحويلها بنجاح من تنسيق FM-12 إلى تنسيق BUFR بواسطة المكون الإضافي.
...
على جهاز VM الخاص بك، ستكون حزمة 'minio' الخاصة بـ Python مثبتة مسبقًا.
...
!!! note
    ...
    ستظهر لك رسالة خطأ لأن السكربت غير مهيأ للوصول إلى نقطة النهاية MinIO على مثيل 'wis2box' الخاص بك حتى الآن.

يحتاج السكربت إلى معرفة نقطة النهاية الصحيحة للوصول إلى MinIO على مثيل 'wis2box' الخاص بك. إذا كان 'wis2box' يعمل على مضيفك، فإن نقطة النهاية MinIO متاحة على `http://YOUR-HOST:9000`. كما يحتاج السكربت إلى تحديث كلمة مرور التخزين الخاصة بك والمسار في حاوية MinIO لتخزين البيانات.
...
??? success "اضغط لعرض الإجابة"
    ...
    يقوم المكون الإضافي `wis2box.data.bufr4.ObservationDataBUFR` بتقسيم ملف BUFR إلى رسائل BUFR فردية وينشر رسالة واحدة لكل محطة وطابع زمني للملاحظة.

## إدخال البيانات عبر SFTP
```

يمكن أيضًا الوصول إلى خدمة MinIO في `wis2box` عبر SFTP. إذا كان لديك نظام حالي يمكن تكوينه لإعادة توجيه البيانات عبر SFTP، يمكنك استخدام هذه الطريقة كبديل لأتمتة عملية إدخال البيانات.
...
??? success "انقر للكشف عن الإجابة"
    ...
    يجب أن ترى إشعار بيانات جديد لـ WIS2 تم نشره للمحطة التجريبية `0-20000-0-64400`، مما يشير إلى أن البيانات تم إدخالها ونشرها بنجاح.
...
!!! success "تهانينا!"
    ...
    - مراقبة إشعارات بيانات WIS2 التي تم نشرها بواسطة `wis2box` الخاص بك في لوحة معلومات Grafana ومستكشف MQTT.