...
لتشغيل سير العمل الخاص بالبيانات في عقدة WIS2 باستخدام برنامج `wis2box`، يجب تحميل البيانات إلى الحاوية **wis2box-incoming** في **MinIO**، مما يؤدي إلى بدء سير عمل بيانات `wis2box` لمعالجة البيانات ونشرها.

لمراقبة حالة سير عمل بيانات `wis2box`، يمكنك استخدام **لوحة تحكم Grafana** و**MQTT Explorer**. تستخدم لوحة تحكم Grafana بيانات من Prometheus وLoki لعرض حالة `wis2box` الخاص بك، بينما يتيح لك MQTT Explorer رؤية إشعارات بيانات WIS2 التي ينشرها مثيل `wis2box` الخاص بك.
...
يفترض هذا القسم أنك أكملت بنجاح الجلسة العملية [تكوين مجموعات البيانات في wis2box](./configuring-wis2box-datasets.md). إذا اتبعت التعليمات في تلك الجلسة، يجب أن يكون لديك مجموعة بيانات واحدة تستخدم الإضافة `Universal`، وأخرى تستخدم الإضافة `FM-12 data converted to BUFR`.
...
ابقِ لوحة تحكم Grafana مفتوحة في متصفحك حيث سنستخدمها لاحقًا لمراقبة حالة إدخال البيانات.
...
انقر على الحاوية *wis2box-incoming*. جرّب خيار تحديد مسار جديد في هذه الحاوية بالنقر على `Create new path`:

<img alt="minio ui: minio ui after login" src="/../assets/img/minio-incoming-create-new-path.png" width="800">

أدخل مسار المجلد الجديد = *new-directory* وقم بتحميل ملف المثال هذا [mydata.nc](./../sample-data/mydata.nc) (انقر بزر الماوس الأيمن وحدد "حفظ باسم" لتنزيل الملف). يمكنك استخدام زر "Upload" في MinIO لتحميل الملف إلى الدليل الجديد:

<img alt="minio ui: create new path" src="/../assets/img/minio-initial-example-upload.png" width="800">

!!! question "سؤال"

    بعد تحميل الملف، كيف يمكنك معرفة ما إذا تم تشغيل سير العمل الخاص بالبيانات في `wis2box` بنجاح؟

??? success "انقر للكشف عن الإجابة"

    يمكنك التحقق من لوحة تحكم Grafana لمعرفة ما إذا تم إدخال البيانات ونشرها بنجاح.

    انظر إلى اللوحة السفلية في لوحة تحكم Grafana وسترى **خطأ في التحقق من المسار** يشير إلى أن المسار لا يتطابق مع أي مجموعة بيانات مكونة:
...
## الإدخال والنشر: إضافة "Universal"

الآن بعد أن عرفت كيفية تحميل البيانات إلى MinIO، دعنا نحاول تحميل البيانات لمجموعة بيانات التوقعات التي أنشأتها في الجلسة العملية السابقة والتي تستخدم إضافة "Universal".
...
??? success "انقر للكشف عن الإجابة"
    ...
    تم تكوين مجموعة البيانات لمعالجة الملفات ذات الامتداد `.grib2` فقط. يعد تكوين امتداد الملف جزءًا من تعيينات البيانات التي قمت بتحديدها في الجلسة العملية السابقة.

قم بتنزيل هذا الملف [GEPS_18August2025.grib2](../sample-data/GEPS_18August2025.grib2) إلى جهاز الكمبيوتر المحلي الخاص بك وقم بتحميله إلى الدليل الذي أنشأته لمجموعة بيانات التوقعات. تحقق من لوحة تحكم Grafana وMQTT Explorer لمعرفة ما إذا تم إدخال البيانات ونشرها بنجاح.
...
??? success "انقر للكشف عن الإجابة"
    ...
    اسم الملف لا يتطابق مع التعبير النمطي الذي قمت بتحديده في تكوين مجموعة البيانات. يجب أن يتطابق اسم الملف مع النمط `^.*?_(\d{8}).*?\..*$`، والذي يتطلب وجود تاريخ مكون من 8 أرقام (YYYYMMDD) في اسم الملف.

    أعد تسمية الملف إلى *GEPS_202508180000.grib2* وقم بتحميله مرة أخرى إلى نفس المسار في MinIO لإعادة تشغيل سير عمل `wis2box`. (أو قم بتنزيل الملف الذي تم إعادة تسميته من هنا: [GEPS_202508180000.grib2](../sample-data/GEPS_202508180000.grib2)).
...
!!! note "حول إضافة Universal"

    تتيح إضافة "Universal" نشر البيانات دون أي تحويل. إنها إضافة تمريرية *pass-through* تقوم بإدخال ملف البيانات ونشره كما هو. لإضافة خاصية "datetime" إلى إشعار بيانات WIS2، تعتمد الإضافة على المجموعة الأولى في نمط الملف لاستخراج التاريخ للبيانات التي تقوم بنشرها.
...
الآن ستقوم باستخدام مجموعة البيانات التي أنشأتها في الجلسة العملية السابقة باستخدام **القالب='weather/surface-based-observations/synop'**. القالب قام بإعداد المكونات الإضافية التالية مسبقًا:
...
!!! note "حول المكون الإضافي FM-12 المحول إلى BUFR"
    ...
    كجزء من عملية التحويل، يضيف المكون بيانات وصفية مفقودة إلى البيانات الناتجة، مثل معرف محطة WIGOS، الموقع، وارتفاع البارومتر الخاص بالمحطة. لإضافة هذه البيانات الوصفية، يبحث المكون عن هذه المعلومات في قائمة المحطات الخاصة بـ 'wis2box' باستخدام المعرف التقليدي (5 أرقام) (في هذه الحالة 64400).

    إذا لم يتم العثور على المحطة في قائمة المحطات، فلن يتمكن المكون من إضافة البيانات الوصفية المفقودة ولن ينشر أي بيانات.
...
تحقق من لوحة تحكم Grafana وMQTT Explorer للتأكد من أن البيانات تم نشرها بنجاح. إذا رأيت الإشعار أدناه، فهذا يعني أنك قمت بنشر بيانات العينة synop بنجاح:
...
??? success "انقر لعرض الإجابة"
    ...
    تحقق من قسم الروابط في إشعار بيانات WIS2 في MQTT Explorer وسترى الرابط الأساسي:
...
استخدام واجهة MinIO على الويب هو وسيلة مريحة لتحميل البيانات يدويًا إلى MinIO لأغراض الاختبار. ومع ذلك، في بيئة الإنتاج، ستستخدم عادةً عمليات مؤتمتة لتحميل البيانات إلى MinIO، على سبيل المثال باستخدام سكربتات أو تطبيقات تستخدم واجهة برمجة التطبيقات المتوافقة مع S3 الخاصة بـ MinIO.
...
يحتاج السكربت إلى معرفة نقطة النهاية الصحيحة للوصول إلى MinIO على مثيل 'wis2box'. إذا كان 'wis2box' يعمل على المضيف الخاص بك، فإن نقطة النهاية الخاصة بـ MinIO متوفرة على `http://YOUR-HOST:9000`. كما يحتاج السكربت إلى تحديث كلمة مرور التخزين الخاصة بك والمسار في حاوية MinIO لتخزين البيانات.
...
!!! question "تحديث السكربت وإدخال بيانات CSV"
    ...
    - باستخدام WinSCP: ابدأ اتصالًا جديدًا باستخدام بروتوكول الملفات `SCP` وبنفس بيانات الاعتماد الخاصة بعميل SSH الخاص بك. انتقل إلى الدليل `wis2box-data/data-ingest-exercises` وقم بتحرير `copy_file_to_incoming.py` باستخدام محرر النصوص المدمج.
...
تحقق من لوحة تحكم Grafana وMQTT Explorer لمعرفة ما إذا تم إدخال بيانات الاختبار ونشرها بنجاح. إذا رأيت أي أخطاء، حاول حلها.
...
??? success "انقر لعرض الإجابة"
    ...
    سترى أخطاء تم الإبلاغ عنها في Grafana لأن المحطات الموجودة في ملف BUFR غير معرفة في قائمة المحطات الخاصة بمثيل 'wis2box' الخاص بك.
...
!!! success "تهانينا!"
    ...
    - تصحيح الأخطاء الشائعة في عملية إدخال البيانات باستخدام لوحة تحكم Grafana وسجلات مثيل `wis2box` الخاص بك.
    - مراقبة إشعارات بيانات WIS2 المنشورة بواسطة `wis2box` الخاص بك في لوحة تحكم Grafana ومستكشف MQTT.