...
!!! abstract "نتائج التعلم"
    ...
    - مراجعة هيكل الموضوع الخاص بـ WIS2
    - مراجعة هيكل رسائل الإشعارات الخاصة بـ WIS2

## المقدمة

يستخدم WIS2 بروتوكول MQTT للإعلان عن توفر بيانات الطقس/المناخ/المياه. يقوم WIS2 Global Broker بالاشتراك في جميع WIS2 Nodes في الشبكة ويعيد نشر الرسائل التي يتلقاها. يقوم Global Cache بالاشتراك في Global Broker، بتنزيل البيانات الموجودة في الرسالة، ثم يعيد نشر الرسالة على موضوع `cache` مع عنوان URL جديد. ينشر Global Discovery Catalogue بيانات الاكتشاف الوصفية من Broker ويوفر واجهة برمجة تطبيقات للبحث.

هذا مثال على هيكل رسالة إشعار WIS2 لرسالة تم تلقيها على الموضوع `origin/a/wis2/br-inmet/data/core/weather/surface-based-observations/synop`:
...
MQTT Explorer هي أداة مفيدة لتصفح ومراجعة هيكل الموضوع الخاص بـ MQTT broker معين لمراجعة البيانات المنشورة.

!!! note "حول MQTT"
    يوفر MQTT Explorer واجهة سهلة الاستخدام للاتصال بـ MQTT broker واستكشاف المواضيع وهيكل الرسائل المستخدمة بواسطة WIS2.
    
    في الممارسة العملية، يتم استخدام MQTT للتواصل بين الأجهزة، حيث يشترك تطبيق أو خدمة في المواضيع ويعالج الرسائل برمجيًا في الوقت الفعلي.
...
انقر على زر 'ADVANCED'، وقم بإزالة المواضيع المسبقة الإعداد وأضف المواضيع التالية للاشتراك فيها:
...
!!! note
    ...
    - **مستوى واحد (+)**: يستبدل رمز المستوى الواحد مستوى واحدًا من الموضوع
    - **مستويات متعددة (#)**: يستبدل رمز المستويات المتعددة مستويات متعددة من الموضوع
...
## التمرين 1: مراجعة هيكل الموضوع الخاص بـ WIS2

استخدم MQTT لتصفح هيكل الموضوع تحت المواضيع `origin`.
...
??? success "انقر للكشف عن الإجابة"
    ...
    يخبرنا أن البيانات تم نشرها من مركز WIS برمز المركز `br-inmet`، وهو رمز المركز الخاص بـ Instituto Nacional de Meteorologia - INMET، البرازيل.
...
??? success "انقر للكشف عن الإجابة"
    ...
    يمكننا التمييز بين الرسائل القادمة من بوابة GTS-to-WIS2 من خلال النظر إلى رمز المركز في هيكل الموضوع. على سبيل المثال، الموضوع التالي:

    `origin/a/wis2/de-dwd-gts-to-wis2/data/core/I/S/A/I/01/sbbr`

    يخبرنا أن البيانات تم نشرها بواسطة بوابة GTS-to-WIS2 المستضافة بواسطة Deutscher Wetterdienst (DWD)، ألمانيا. بوابة GTS-to-WIS2 هي نوع خاص من ناشري البيانات الذين ينشرون البيانات من نظام الاتصالات العالمي (GTS) إلى WIS2. يتكون هيكل الموضوع من رؤوس TTAAii CCCC الخاصة برسائل GTS.

## التمرين 2: مراجعة هيكل الرسائل الخاصة بـ WIS2

افصل الاتصال بـ MQTT Explorer وقم بتحديث قسم 'Advanced' لتغيير الاشتراك إلى المواضيع التالية:
...
يمكنك عرض محتوى رسالة WIS2 في قسم "Value" على الجانب الأيمن. حاول توسيع هيكل الموضوع لرؤية المستويات المختلفة للرسالة حتى تصل إلى المستوى الأخير ومراجعة محتوى إحدى الرسائل.
...
??? success "انقر للكشف عن الإجابة"
    ...
    الطابع الزمني الذي تم نشر البيانات فيه موجود في قسم `properties` من الرسالة مع المفتاح `pubtime`.

    الطابع الزمني الذي تم جمع البيانات فيه موجود في قسم `properties` من الرسالة مع المفتاح `datetime`.
...
حاول تحديد رسالة لنفس رمز المركز تم نشرها على كل من مواضيع `origin` و `cache`.
...
!!! success "تهانينا!"
    ...
    - هيكل الموضوع الخاص بـ WIS2
    - هيكل رسائل الإشعارات الخاصة بـ WIS2
    - الفرق بين البيانات الأساسية والموصى بها
    - هيكل الموضوع المستخدم بواسطة بوابة GTS-to-WIS2
    - الفرق بين الرسائل الخاصة بـ Global Broker المنشورة على مواضيع `origin` و `cache`