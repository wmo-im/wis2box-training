...
فيما يلي مثال على بنية رسالة إشعار WIS2 لرسالة تم تلقيها على الموضوع `origin/a/wis2/br-inmet/data/core/weather/surface-based-observations/synop`:
...
في هذه الجلسة العملية، ستتعلم كيفية استخدام أداة MQTT Explorer لإعداد اتصال عميل MQTT بـ WIS2 Global Broker والقدرة على عرض رسائل إشعارات WIS2.

MQTT Explorer هي أداة مفيدة لتصفح ومراجعة بنية المواضيع لأي MQTT broker لمراجعة البيانات المنشورة.

!!! note "حول MQTT"
    يوفر MQTT Explorer واجهة سهلة الاستخدام للاتصال بـ MQTT broker واستكشاف المواضيع وبنية الرسائل المستخدمة في WIS2.
    
    في الممارسة العملية، يهدف MQTT إلى استخدامه في الاتصال بين الآلات، حيث يشترك تطبيق أو خدمة في المواضيع ويعالج الرسائل برمجيًا في الوقت الفعلي.
...
!!! note
    ...
    - **مستوى واحد (+)**: يستبدل البدل ذو المستوى الواحد مستوى واحدًا من الموضوع
    - **مستويات متعددة (#)**: يستبدل البدل ذو المستويات المتعددة مستويات متعددة من الموضوع
...
??? success "انقر لعرض الإجابة"
    ...
    يخبرنا أن البيانات تم نشرها من مركز WIS بمُعرف المركز `br-inmet`، وهو مُعرف المركز الخاص بـ Instituto Nacional de Meteorologia - INMET، البرازيل.
...
??? success "انقر لعرض الإجابة"
    ...
    يمكننا التمييز بين الرسائل القادمة من بوابة GTS-to-WIS2 من خلال النظر إلى مُعرف المركز في بنية المواضيع. على سبيل المثال، الموضوع التالي:

    `origin/a/wis2/de-dwd-gts-to-wis2/data/core/I/S/A/I/01/sbbr`

    يخبرنا أن البيانات تم نشرها بواسطة بوابة GTS-to-WIS2 المستضافة من قبل Deutscher Wetterdienst (DWD)، ألمانيا. بوابة GTS-to-WIS2 هي نوع خاص من ناشري البيانات الذين ينشرون البيانات من نظام الاتصالات العالمي (GTS) إلى WIS2. تتكون بنية المواضيع من رؤوس TTAAii CCCC لرسائل GTS.
...
يمكنك عرض محتوى رسالة WIS2 في قسم "Value" على الجانب الأيمن. حاول توسيع بنية المواضيع لرؤية المستويات المختلفة للرسالة حتى تصل إلى المستوى الأخير وقم بمراجعة محتوى إحدى الرسائل.
...
??? success "انقر لعرض الإجابة"
    ...
    الرسائل المنشورة على مواضيع `cache` هي الرسائل التي تم تنزيل بياناتها بواسطة Global Cache. إذا قمت بفحص محتوى الرسالة من الموضوع الذي يبدأ بـ `cache`، ستلاحظ أن الرابط 'canonical' قد تم تحديثه إلى عنوان URL جديد.
