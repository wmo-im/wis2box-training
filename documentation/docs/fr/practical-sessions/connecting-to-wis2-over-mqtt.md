...
!!! note "À propos de MQTT"
    ...
    En pratique, MQTT est conçu pour être utilisé dans des communications machine-à-machine, où une application ou un service s'abonne à des topics et traite les messages de manière programmatique en temps réel.
...
Vous pouvez visualiser le contenu des messages WIS2 dans la section "Value" sur la droite. Essayez de développer la structure des topics pour voir les différents niveaux du message jusqu'à atteindre le dernier niveau et examinez le contenu d'un des messages.
...
??? success "Cliquez pour révéler la réponse"
    ...
    L'horodatage auquel les données ont été publiées se trouve dans la section `properties` du message avec la clé `pubtime`.

    L'horodatage auquel les données ont été collectées se trouve dans la section `properties` du message avec la clé `datetime`.
...
??? success "Cliquez pour révéler la réponse"
    ...
    L'URL se trouve dans la section `links` avec `rel="canonical"` et est définie par la clé `href`.
