...
Les fichiers de données au format CSV (valeurs séparées par des virgules) sont souvent utilisés pour enregistrer des données d'observation et autres dans un format tabulaire. La plupart des enregistreurs de données utilisés pour collecter les sorties des capteurs peuvent exporter les observations dans des fichiers délimités, y compris au format CSV. De même, lorsqu'on ingère des données dans une base de données, il est facile d'exporter les données nécessaires dans des fichiers au format CSV.
...
Dans cette session, vous apprendrez à créer votre propre modèle de correspondance pour convertir des données CSV en BUFR. Vous apprendrez également à utiliser les modèles intégrés AWS et DAYCLI pour cette conversion.
...
Assurez-vous que MQTT Explorer est ouvert et connecté à votre courtier en utilisant les identifiants `everyone/everyone`.

## Création d'un modèle de correspondance

Le module `csv2bufr` est livré avec un outil en ligne de commande pour créer votre propre modèle de correspondance en utilisant un ensemble de séquences BUFR et/ou d'éléments BUFR comme entrée.
...
!!! Note "csv2bufr mapping list"
    ...
    La commande `csv2bufr mapping list` affichera les modèles de correspondance disponibles dans le système. Les modèles par défaut sont stockés dans le répertoire `/opt/wis2box/csv2bufr/templates` dans le conteneur.
...
Notez que les en-têtes de ce fichier CSV sont les mêmes que les en-têtes du CSV dans le modèle de mappage que vous avez créé.
...
Le modèle 'AWS' fournit un modèle de mappage pour convertir les données CSV en séquences BUFR 301150, 307096, en support des exigences minimales de GBON.
...
Créez un nouveau dossier dans le bucket MinIO qui correspond à l'identifiant du jeu de données que vous avez créé avec le template='weather/surface-weather-observations/synop' :
...
Après avoir téléchargé les données, vérifiez qu'il n'y a pas d'AVERTISSEMENTS (WARNINGS) ou d'ERREURS (ERRORS) dans le tableau de bord Grafana et vérifiez dans MQTT Explorer si vous recevez des notifications de données WIS2.
