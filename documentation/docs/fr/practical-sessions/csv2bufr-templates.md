...
Dans cette session, vous apprendrez à créer votre propre modèle de mappage pour convertir des données CSV en BUFR. Vous apprendrez également à utiliser les modèles intégrés AWS et DAYCLI pour effectuer cette conversion.
...
Le module `csv2bufr` est livré avec un outil en ligne de commande pour créer votre propre modèle de mappage en utilisant un ensemble de séquences BUFR et/ou d'éléments BUFR comme entrée.
...
Le modèle de mappage que vous avez créé manque de métadonnées importantes concernant l'observation effectuée, la date et l'heure de l'observation, ainsi que la latitude et la longitude de la station.
...
??? success "Cliquez pour révéler la réponse"
    ...
    Le modèle de mappage que vous avez créé mappe désormais **18** colonnes CSV aux éléments BUFR :
...
Vérifiez le contenu du fichier `custom_template_data.csv` dans le répertoire `/wis2box-api/data-conversion-exercises` :
...
Pour tester la conversion des données, nous pouvons utiliser l'outil en ligne de commande `csv2bufr` pour convertir le fichier CSV en BUFR en utilisant le fichier de mappage que nous avons créé :
...
??? success "Cliquez pour révéler la réponse"
    ...
    Dans la sortie, vous verrez les valeurs des éléments BUFR que vous avez mappés dans le fichier de mappage. Par exemple, la "airTemperature" affichera :
...
### Utiliser le fichier de mappage dans le wis2box

Pour que le nouveau fichier de mappage soit reconnu par le conteneur wis2box-api, vous devez redémarrer le conteneur :

```bash
docker restart wis2box-api
```

Vous pouvez maintenant configurer votre jeu de données dans le wis2box-webapp pour utiliser le fichier de mappage personnalisé pour le plugin de conversion CSV en BUFR.

Le wis2box-webapp détectera automatiquement le fichier de mappage que vous avez créé et le rendra disponible dans la liste des fichiers de mappage pour le plugin de conversion CSV en BUFR.
...
Vous devriez voir le nouveau fichier de mappage que vous avez créé dans la liste des fichiers disponibles :

<img alt="Image montrant les fichiers csv2bufr-templates dans le wis2box-webapp" src="/../assets/img/wis2box-webapp-csv2bufr-templates.png"/>

!!! hint

    Notez que si vous ne voyez pas le nouveau fichier de mappage que vous avez créé, essayez de rafraîchir la page ou de l'ouvrir dans une nouvelle fenêtre de navigation privée.

Pour l'instant, conservez la sélection par défaut du fichier de mappage AWS (cliquez en haut à droite pour fermer la configuration du plugin).

## Utiliser le fichier de mappage 'AWS'

Le fichier de mappage 'AWS' fournit un modèle pour convertir les données CSV en séquences BUFR 301150, 307096, afin de répondre aux exigences minimales de GBON.

La description du fichier de mappage AWS se trouve ici [aws-template](./../csv2bufr-templates/aws-template.md).
...
Ouvrez le fichier que vous avez téléchargé dans un éditeur et inspectez son contenu :
...
??? success "Cliquez pour révéler la réponse"
    Les données manquantes dans le fichier sont représentées par des cellules vides. Dans un fichier CSV, cela serait encodé par ``,,``. Notez qu'il s'agit d'une cellule vide et non d'une chaîne de longueur zéro, par exemple ``,"",``.
...
Mettez à jour le fichier d'exemple que vous avez téléchargé pour utiliser la date et l'heure d'aujourd'hui et modifiez les identifiants des stations WIGOS pour utiliser des stations que vous avez enregistrées dans le wis2box-webapp.

### Télécharger les données dans MinIO et vérifier le résultat
...
<img width="450" alt="Image montrant MQTT Explorer après le téléchargement d'AWS" src="/../assets/img/mqtt-explorer-aws-upload.png"/>

## Utiliser le fichier de mappage 'DayCLI'

Le fichier de mappage **DayCLI** fournit un modèle pour convertir les données climatiques quotidiennes CSV en séquence BUFR 307075, afin de rapporter des données climatiques quotidiennes.

La description du fichier de mappage DAYCLI se trouve ici [daycli-template](./../csv2bufr-templates/daycli-template.md).

Pour partager ces données sur WIS2, vous devrez créer un nouveau jeu de données dans le wis2box-webapp qui a la hiérarchie de sujet WIS2 correcte et qui utilise le fichier de mappage DAYCLI pour convertir les données CSV en BUFR.

### Créer un jeu de données wis2box pour publier des messages DAYCLI
...
Examinez les plugins de données pour les jeux de données. Cliquez sur "UPDATE" à côté du plugin nommé "CSV data converted to BUFR" et vous verrez que le fichier de mappage est défini sur **DayCLI** :

<img alt="Mettre à jour le plugin de données pour le jeu de données afin d'utiliser le fichier de mappage DAYCLI" src="/../assets/img/wis2box-webapp-update-data-plugin-daycli.png"/>
...
Vous devriez maintenant avoir un deuxième jeu de données dans le wis2box-webapp configuré pour utiliser le fichier de mappage DAYCLI pour convertir les données CSV en BUFR.
...
!!! question
    Quelles variables supplémentaires sont incluses dans le fichier de mappage daycli ?

??? success "Cliquez pour révéler la réponse"
    Le fichier de mappage daycli inclut des métadonnées importantes sur l'emplacement des instruments et les classifications de qualité des mesures pour la température et l'humidité, des indicateurs de contrôle qualité et des informations sur la manière dont la température moyenne quotidienne a été calculée.

### Mettre à jour le fichier d'exemple

Le fichier d'exemple contient une ligne de données pour chaque jour d'un mois et rapporte des données pour une station. Mettez à jour le fichier d'exemple que vous avez téléchargé pour utiliser la date et l'heure d'aujourd'hui et modifiez les identifiants des stations WIGOS pour utiliser une station que vous avez enregistrée dans le wis2box-webapp.
