...
!!! question "Trouver un ensemble de données d'observation de surface"
    ...
    3. Définissez les mots-clés pour inclure `temperature, precipitation` et observez comment les résultats changent.
...
- **Bounding box** — restreindre les téléchargements aux notifications dont les données se situent dans une zone spatiale définie par les valeurs `north`, `south`, `east` et `west`.  
...
3. Dans les entrées de filtre, définissez une **bounding box** pour une région de votre choix.  
