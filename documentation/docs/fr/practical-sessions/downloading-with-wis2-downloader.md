...
WIS2 Downloader utilise ce principe en recherchant tous les enregistrements disponibles dans ces GDCs et en les combinant en interne pour permettre à l'utilisateur de naviguer parmi les données disponibles sur WIS2. Étant donné le grand nombre d'enregistrements à afficher, il est essentiel de fournir un moyen permettant à l'utilisateur de filtrer ces enregistrements et de trouver le bon. Même après avoir trouvé et s'être abonné au bon enregistrement, il peut y avoir des ensembles de données où le nombre de fichiers dépasse les besoins actuels de l'utilisateur. Pour cette raison, un second niveau de filtrage est nécessaire — celui qui opère au moment de décider si un fichier doit être téléchargé.

## Utilisation de la vue Catalogue
...
Vous pouvez également filtrer spatialement en définissant une **boîte englobante** à l'aide de quatre coordonnées — **Nord**, **Ouest**, **Sud** et **Est** — exprimées en valeurs décimales de latitude et de longitude. Lorsque vous définissez une boîte englobante, vous pouvez choisir entre deux modes de correspondance :
...
??? success "Cliquez pour révéler la réponse"
    ...
    Tout enregistrement dont le sujet contient `surface-based-observations` et dont la politique de données est `core` est un choix valide. L'application du filtre de mots-clés `temperature, precipitation` réduira davantage les résultats aux ensembles de données pertinents pour ces variables.
...
La **vue Arborescence** présente la hiérarchie des sujets WIS2 sous forme d'arborescence repliable, permettant de parcourir les sujets disponibles niveau par niveau — similaire à la navigation des sujets dans MQTT Explorer. Elle est conçue pour une exploration de haut niveau, en partant de la racine de la hiérarchie et en descendant. Cela contraste avec la vue Catalogue, qui vous mène directement aux enregistrements individuels des ensembles de données et est mieux adaptée lorsque vous savez déjà ce que vous recherchez.
...
??? success "Cliquez pour révéler la réponse"
    ...
    Tout chemin de sujet suivant le modèle `cache/a/wis2/<centre-id>/data/core/weather/surface-based-observations/#` est un choix valide. Le segment Centre ID variera en fonction du centre sélectionné dans l'arborescence.
...
- **Authentication** — les identifiants requis pour les ensembles de données protégés par un contrôle d'accès.
...
- **Jeton Bearer** — fournissez un jeton délivré par l'éditeur de données à la place d'un nom d'utilisateur et d'un mot de passe.
...
Par défaut, WIS2 Downloader écoute le Global Broker. Pour recevoir directement les notifications de votre instance de wis2box, vous devez ajouter un abonné dans le fichier compose de WIS2 Downloader qui pointe vers le broker MQTT interne de wis2box.
...
Sur votre instance de wis2box, publiez un fichier dans le dataset à accès contrôlé. Consultez la session pratique [Ingesting data for publication](ingesting-data-for-publication.md) pour les étapes d'ingestion des données.
...
Si l'authentification échoue, les fichiers ne seront pas téléchargés même si l'abonnement apparaît actif dans la vue **Manage Subscriptions**. Vérifiez que le token bearer correspond à celui configuré sur le dataset dans wis2box.
...
Les filtres permettent de contrôler quels fichiers sont téléchargés à partir d'un abonnement au niveau des notifications — il s'agit du deuxième niveau de filtrage mentionné dans l'introduction. Plutôt que de télécharger chaque fichier publié sur un topic, vous pouvez définir un filtre afin que seules les notifications correspondant à des critères spécifiques déclenchent un téléchargement.
...
- **Boîte englobante** — restreindre les téléchargements aux notifications dont les données se situent dans une zone spatiale définie par les valeurs `north`, `south`, `east` et `west`.
