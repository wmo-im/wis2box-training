...
La sortie imprimée représente les valeurs de certains des clés d'en-tête dans le fichier BUFR.
...
Vous pouvez utiliser la commande `grep` pour filtrer la sortie et n'afficher que les clés qui ne sont pas manquantes. Par exemple, pour afficher toutes les clés non manquantes, vous pouvez utiliser la commande suivante :
...
??? success "Cliquez pour révéler la réponse"
    ...
    Cela correspond aux métadonnées de station pour 2 stations : avec les identifiants de station WIGOS `0-20000-0-15015` et `0-20000-0-15020`.
...
Voyons maintenant comment convertir des données CSV au format BUFR en utilisant le module `csv2bufr`. Le module est installé dans le conteneur wis2box-api et peut être utilisé depuis la ligne de commande comme suit :
...
**Il n'existe aucune colonne CSV mappée à la clé eccodes `internationalDataSubCategory`**, à la place, la valeur constante 2 est utilisée et sera encodée dans tous les fichiers BUFR produits avec ce modèle de mappage.
...
Notez que la conversion d'un CSV en BUFR à l'aide d'un des modèles de mappage BUFR prédéfinis présente des limitations :
