...
La bibliothèque ecCodes est un ensemble de bibliothèques logicielles et d'utilitaires conçus pour décoder et encoder des données météorologiques aux formats GRIB et BUFR. Elle est développée par le Centre Européen pour les Prévisions Météorologiques à Moyen Terme (CEPMMT). Consultez la [documentation ecCodes](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation) pour plus d'informations.
...
Pour utiliser les outils en ligne de commande BUFR, vous devrez être connecté au conteneur wis2box-api. Sauf indication contraire, toutes les commandes doivent être exécutées sur ce conteneur. Vous devrez également avoir MQTT Explorer ouvert et connecté à votre broker.
...
Vous verrez un grand nombre de clés en sortie, dont beaucoup sont manquantes. Cela est typique des données du monde réel, car toutes les clés `eccodes` ne sont pas renseignées avec des données rapportées.
...
!!! question
    ...
    Quel est l'identifiant de station WIGOS de la station ayant rapporté les données dans le fichier BUFR `bufr-cli-ex2.bufr4` ?
...
Ensuite, examinons comment convertir des données FM-12 SYNOP au format BUFR en utilisant le module `synop2bufr`. Le module `synop2bufr` est utilisé pour convertir des données FM-12 SYNOP au format BUFR. Ce module est installé dans le conteneur wis2box-api et peut être utilisé depuis la ligne de commande comme suit :
...
Le module `synop2bufr` est également utilisé dans l'application web `wis2box-webapp` pour convertir des données FM-12 SYNOP au format BUFR via un formulaire d'entrée basé sur le web.
...
Sélectionnez le formulaire `SYNOP Form` dans le menu de gauche et copiez-collez le contenu du fichier `synop_message.txt` :
...
Notez que la conversion de CSV en BUFR en utilisant l'un des modèles de mappage BUFR prédéfinis a des limitations :
