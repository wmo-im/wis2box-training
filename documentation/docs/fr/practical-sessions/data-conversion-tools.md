...
Tout d'abord, connectez-vous à votre VM étudiant via votre client SSH et copiez les matériaux d'exercice dans le répertoire "/wis2box-api" à l'intérieur du conteneur wis2box-api :
...
Vous verrez un grand nombre de clés en sortie, dont beaucoup sont manquantes. Cela est typique avec des données du monde réel, car toutes les clés `eccodes` ne sont pas toujours renseignées avec des données signalées.
...
Pour tester cela, allez sur `http://YOUR-HOST/wis2box-webapp` et connectez-vous.
