...
Pour démontrer comment décoder les données téléchargées, nous allons démarrer un nouveau conteneur en utilisant l'image 'decode-bufr-jupyter'.
...
Une fois le conteneur démarré, vous pouvez accéder au serveur de notebook Jupyter sur votre VM étudiant en naviguant à l'adresse `http://YOUR-HOST:8888` dans votre navigateur web.
...
![Température de la prévision globale d'ensemble](../assets/img/grib2-global-ensemble-prediction-map.png)
...
??? success "Cliquez pour révéler la réponse"
    ...
    Pour mettre à jour le notebook afin d'afficher la probabilité prévue de trajectoire de tempête tropicale dans une distance différente, vous pouvez mettre à jour la variable `distance_threshold` dans le bloc de code qui calcule la probabilité d'impact.
