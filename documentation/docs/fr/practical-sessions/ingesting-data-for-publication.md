...
Dans WIS2, les données sont partagées en temps réel via des notifications de données WIS2 qui contiennent un lien "canonique" à partir duquel les données peuvent être téléchargées.

Pour déclencher le flux de travail des données dans un WIS2 Node en utilisant le logiciel wis2box, les données doivent être téléchargées dans le seau **wis2box-incoming** sur **MinIO**, ce qui initie le flux de travail wis2box. Ce processus aboutit à la publication des données via une notification de données WIS2. Selon les mappages de données configurés dans votre instance wis2box, les données peuvent être transformées en format BUFR avant d'être publiées.
...
Pendant l'exercice, nous surveillerons le statut de l'ingestion des données en utilisant le **tableau de bord Grafana** et **MQTT Explorer**. Le tableau de bord Grafana utilise les données de Prometheus et Loki pour afficher le statut de votre wis2box, tandis que MQTT Explorer vous permet de voir les notifications de données WIS2 publiées par votre instance wis2box.

Notez que wis2box transformera les données exemples en format BUFR avant de les publier sur le courtier MQTT, conformément aux mappages de données préconfigurés dans votre ensemble de données. Pour cet exercice, nous nous concentrerons sur les différentes méthodes pour télécharger des données sur votre instance wis2box et vérifier l'ingestion et la publication réussies. La transformation des données sera abordée ultérieurement dans la session pratique [Outils de conversion de données](../data-conversion-tools).

## Préparation

Cette section utilise l'ensemble de données pour "surface-based-observations/synop" précédemment créé dans la session pratique [Configuration des ensembles de données dans wis2box](/practical-sessions/configuring-wis2box-datasets). Elle nécessite également des connaissances sur la configuration des stations dans le **wis2box-webapp**, comme décrit dans la session pratique [Configuration des métadonnées des stations](/practical-sessions/configuring-station-metadata).
...
Vérifiez que les données exemples suivantes sont disponibles dans le répertoire `/data/wis2box/` à l'intérieur du conteneur **wis2box-management** :
...
!!! question "Ingestion de données en utilisant `wis2box data ingest`"
    ...
    Exécutez la commande suivante pour ingérer le fichier de données exemple dans votre instance wis2box :
...
Allez à la console MinIO dans votre navigateur et vérifiez si le fichier `synop_202412030900.txt` a été téléchargé dans le seau `wis2box-incoming`. Vous devriez voir un nouveau répertoire avec le nom de l'ensemble de données que vous avez fourni dans l'option `--metadata-id`, et à l'intérieur de ce répertoire, vous trouverez le fichier `synop_202412030900.txt` :

<img alt="minio-wis2box-incoming-dataset-folder" src="../../assets/img/minio-data-ingest-test-data.png" width="800">

!!! note
    La commande `wis2box data ingest` a téléchargé le fichier dans le seau `wis2box-incoming` sur MinIO dans un répertoire nommé d'après l'identifiant de métadonnées que vous avez fourni.
...
??? success "Cliquez pour révéler la réponse"
    ...
    Le contenu du message comporte une section "liens" avec une clé "rel" de "canonique" et une clé "href" avec l'URL pour télécharger les données. L'URL sera au format `http://YOUR-HOST/data/...`. 
...
## Téléchargement de données en utilisant l'interface web MinIO
...
Ensuite, nous utiliserons l'interface web de MinIO, qui vous permet de télécharger et de téléverser des données vers MinIO à l'aide d'un navigateur web.

!!! question "Retéléverser des données en utilisant l'interface web de MinIO"

    Accédez à l'interface web de MinIO dans votre navigateur et naviguez jusqu'au seau `wis2box-incoming`. Vous verrez le fichier `synop_202412030900.txt` que vous avez téléversé lors des exercices précédents.
    ...
    Vous pouvez télécharger ce fichier et le retéléverser au même chemin dans MinIO pour redéclencher le flux de travail de wis2box.
...
??? success "Cliquez pour révéler la réponse"
    ...
    Cela démontre que le flux de données a été déclenché, mais les données n'ont pas été republiées. Le wis2box ne publiera pas les mêmes données deux fois. 
...
??? success "Cliquez pour révéler la réponse"
    ...
    Allez sur le tableau de bord Grafana et vérifiez si les données ont été correctement ingérées et publiées.

    Si vous utilisez le mauvais chemin, vous verrez un message d'erreur dans les journaux.

    Si vous utilisez le bon chemin, vous verrez une notification de données WIS2 supplémentaire publiée pour la station de test `0-20000-0-64400`, indiquant que les données ont été correctement ingérées et publiées.

    <img alt="grafana_data_ingest" src="../../assets/img/grafana_data-ingest-test2.png" width="400"> 

## Téléverser des données en utilisant SFTP
...
Lorsque vous vous connectez, vous verrez les seaux utilisés par wis2box dans MinIO :

<img alt="winscp-sftp-bucket" src="../../assets/img/winscp-buckets.png" width="600">

Vous pouvez naviguer jusqu'au seau `wis2box-incoming` puis jusqu'au dossier de votre ensemble de données. Vous verrez les fichiers que vous avez téléversés lors des exercices précédents :

<img alt="winscp-sftp-incoming-path" src="../../assets/img/winscp-incoming-data-path.png" width="600">

!!! question "Téléverser des données en utilisant SFTP"
...
## Téléverser des données en utilisant un script Python
...
Sur votre machine virtuelle étudiante, le paquet 'minio' pour Python sera déjà installé.
...
Essayez d'exécuter le script pour copier le fichier de données exemple `synop_202501030900.txt` dans le seau `wis2box-incoming` dans MinIO comme suit :
...
!!! question "Mettre à jour le script et ingérer les données CSV"
    ...
    - En utilisant WinSCP : démarrez une nouvelle connexion en utilisant le protocole de fichier `SCP` et les mêmes identifiants que votre client SSH. Naviguez dans le répertoire `wis2box-data/data-ingest-exercises` et éditez `copy_file_to_incoming.py` en utilisant l'éditeur de texte intégré.
...
Une fois que vous réussissez à exécuter le script, vous verrez un message indiquant que le fichier a été copié dans MinIO, et vous devriez voir des notifications de données publiées par votre instance wis2box dans MQTT Explorer.

Vous pouvez également vérifier le tableau de bord Grafana pour voir si les données ont été correctement ingérées et publiées.
...
Vérifiez le tableau de bord Grafana et MQTT Explorer pour voir si les données de test ont été correctement ingérées et publiées. Si vous voyez des erreurs, essayez de les résoudre.
...
!!! success "Félicitations !"
    ...
    - Déclencher le flux de travail de wis2box en téléversant des données vers MinIO en utilisant diverses méthodes.
