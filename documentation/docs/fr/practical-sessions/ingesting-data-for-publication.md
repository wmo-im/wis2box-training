...
Assurez-vous que wis2box est démarré et en cours d'exécution :
...
- *wis2box-incoming* : C'est le bucket où vous téléversez les données pour déclencher le workflow de wis2box.
...
## Ingestion & Publication : Plugin "Universal"
...
Cela démontre que le workflow des données a été déclenché, mais que les données n'ont pas été re-publiées. Le wis2box ne publiera pas les mêmes données deux fois.
...
Ensuite, vous allez utiliser l'ensemble de données que vous avez créé lors de la session pratique précédente en utilisant **Template='weather/surface-based-observations/synop'**. Ce modèle a préconfiguré les plugins de données suivants pour vous :
...
??? success "Cliquez pour révéler la réponse"
    ...
    Le workflow des données a été déclenché, mais le plugin de données n'a pas pu traiter les données en raison de métadonnées de station manquantes.

!!! note "À propos du plugin FM-12 data converted to BUFR"

    Ce plugin tente de transformer les données d'entrée FM-12 en format BUFR.
...
Après avoir ajouté la station, relancez le workflow wis2box en téléchargeant à nouveau le fichier de données d'exemple *synop_202502040900.txt* dans le même chemin dans MinIO.
...
Le service MinIO dans wis2box peut également être accessible via SFTP. Si vous disposez d’un système existant pouvant être configuré pour transférer des données via SFTP, vous pouvez l’utiliser comme méthode alternative pour automatiser l’ingestion de vos données.
