...
**Метаданные обнаружения** используются для создания записи WCMP2 (WMO Core Metadata Profile 2), которая распространяется через уведомление WIS2, опубликованное в вашем wis2box-broker.

**Сопоставления данных** используются для связывания плагинов данных с вашими входными данными, что позволяет преобразовывать данные перед их публикацией в WIS2.
...
!!! note "Настройка наборов данных без использования wis2box-webapp"
    ...
    wis2box также поддерживает настройку наборов данных с использованием формата [файла управления метаданными (MCF)](https://geopython.github.io/pygeometa/reference/mcf).
    
    Использование MCF предоставляет больше гибкости и контроля, но может быть более подвержено ошибкам, так как необходимо убедиться, что MCF правильно отформатирован и соответствует требуемой схеме.
...
!!! Note
    ...
    Никогда не нужно делиться учетными данными вашего внутреннего брокера с внешними пользователями. Пользователь 'everyone' является публичным пользователем, чтобы обеспечить обмен уведомлениями WIS2.
...
См. раздел [Инициализация wis2box](./initializing-wis2box.md), если вы забыли, как подключиться к брокеру или получить доступ к wis2box-webapp.
...
После получения токена выйдите из контейнера wis2box-management:
...
Форма редактора набора данных позволяет вам указать метаданные обнаружения для вашего набора данных, которые контейнер wis2box-management будет использовать для публикации записи WCMP2.
...
Убедитесь, что после отправки вашего набора данных вы получили следующее сообщение, подтверждающее успешную публикацию набора данных:
...
Изучите содержимое уведомления WIS2, которое вы опубликовали. Вы должны увидеть JSON-структуру, соответствующую формату WIS Notification Message (WNM).
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    **Уведомление WIS2 содержит каноническую ссылку на опубликованную запись WCMP2.** 
    ...
    Скопируйте и вставьте эту каноническую ссылку в ваш браузер, чтобы получить доступ к записи WCMP2. В зависимости от настроек вашего браузера, файл может быть загружен или отображен непосредственно в браузере.

    Вы найдете заголовок, описание и ключевые слова, которые вы указали, внутри записи WCMP2.

wis2box предоставляет лишь ограниченное количество предопределенных шаблонов. Эти шаблоны предназначены для распространенных типов наборов данных, но они могут не подходить для специализированных данных. Для всех других типов наборов данных вы можете создать свой набор данных, выбрав Template=*other*.
...
Когда используется Template=other, предопределенные сопоставления данных не предоставляются. В результате редактор сопоставлений данных будет пустым, и пользователям нужно будет настроить сопоставления в соответствии с их конкретными требованиями.
...
!!! Note "Извлечение даты и времени из имени файла"
    ...
    Шаблон по умолчанию — это `^.*?_(\d{8}).*?\..*$`, который соответствует 8 цифрам, предшествующим символу подчеркивания, и за которыми следуют любые символы и точка перед расширением файла. Например:

    - `mydata_20250101.txt` соответствует и извлекает 25 января 2025 года как свойство datetime для данных
    - `mydata_2025010112.txt` не соответствует, так как в имени 10 цифр вместо 8
...
Нажмите "SAVE", чтобы сохранить настройки плагина, и убедитесь, что теперь вы видите плагин в списке редактора сопоставлений данных:

<img alt="Data Mappings: update plugin" src="/../assets/img/wis2box-dat

Попробуйте обновить иерархию тем для последнего набора данных, который вы создали, изменив выбор в разделе "Темы поддисциплины". Видите ли вы новое уведомление WIS2, опубликованное по теме `origin/a/wis2/<your-centre-id>/metadata`?
...
- использовать редактор наборов данных в wis2box-webapp
- создавать новые наборы данных, используя шаблоны *weather/surface-based-observations/synop* и *other*
- определять метаданные для обнаружения
