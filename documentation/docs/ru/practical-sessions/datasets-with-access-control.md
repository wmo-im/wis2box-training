...
**recommended** : данные, которые могут предоставляться с условиями использования и/или при наличии лицензии
...
!!! note "Авиационные данные в WIS2"
    ...
    Для этого упражнения вам предлагается создать набор данных для обмена данными METAR — стандартным форматом для отчётов о метеорологических наблюдениях в авиации. 
...
!!! note "О примерном файле лицензии для авиации"
    ...
    Это пример файла лицензии для авиационных данных. Возможно, вы захотите отредактировать файл, чтобы включить информацию, относящуюся к вашей организации.
...
Учётные данные для доступа к MinIO Console определены в файле `wis2box.env` переменными окружения `WIS2BOX_STORAGE_USERNAME` и `WIS2BOX_STORAGE_PASSWORD`.
...
После входа в MinIO Console загрузите файл лицензии в базовый путь корзины **wis2box-public** с помощью кнопки "Upload":
...
Из командной строки внутри контейнера вы можете защитить набор данных, используя команду `wis2box auth add-token`, указав флаг `--metadata-id` для идентификатора метаданных набора данных и токен доступа в качестве аргумента.
...
Затем добавьте этот файл в ваш набор данных, используя MinIO Console. Для доступа к MinIO Console откройте веб-браузер и перейдите по адресу `http://YOUR-HOST:9001`, затем войдите в систему, используя учетные данные, указанные в файле `wis2box.env` в переменных окружения `WIS2BOX_STORAGE_USERNAME` и `WIS2BOX_STORAGE_PASSWORD`.
