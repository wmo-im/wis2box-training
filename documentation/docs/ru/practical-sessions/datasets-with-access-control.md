...
Данные распространяются в WIS2 в соответствии с Единой политикой данных ВМО, которая определяет две категории данных.

**core** : данные, предоставляемые на бесплатной и неограниченной основе, без оплаты и без условий использования

**recommended** : данные, которые могут предоставляться с условиями использования и/или на основании лицензии

Данные, распространяемые как recommended:

- могут быть предоставлены с условиями использования и повторного использования
- могут иметь контроль доступа, применяемый к данным
- не кэшируются глобальными кэшами WIS2
- должны иметь метаданные, включающие URL лицензии

!!! note "Загрузка рекомендованных данных"
    
    Поскольку глобальные кэши WIS не кэшируют рекомендованные данные, вы **не** увидите уведомления по теме `cache/a/wis2/<centre-id>/data/recommended/..` 

    Потребитель данных должен загрузить данные с сервера данных, размещенного поставщиком данных, используя канонический URL, указанный в уведомлениях по теме `origin/a/wis2/<centre-id>/data/recommended/...`

В этой практической сессии вы создадите новый набор данных с политикой данных 'recommended', используя редактор наборов данных в wis2box-webapp. Вы также узнаете, как предоставить самостоятелно размещенную лицензию и, при необходимости, добавить контроль доступа.

!!! note "Авиационные данные в WIS2"
    
    Для этого упражнения вам предлагается создать набор данных для обмена данными METAR, стандартным форматом для отчетов о метеорологических наблюдениях для авиации. Поскольку данные аэрометеорологии подлежат ограничениям на использование, применяется политика данных **Recommended** в соответствии с Единой политикой данных ВМО, Резолюция 1 (Cg-Ext(2021)).

    Пожалуйста, обратитесь к !["WIS2 Cookbook"](https://wmo-im.github.io/wis2-cookbook/cookbook/latest/wis2-cookbook-STABLE.html#_publishing_aeronautical_meteorology_data_on_wis2) для утвержденного "рецепта" публикации данных аэрометеорологии в WIS2.
...
Убедитесь, что вы подключены к брокеру MQTT вашего экземпляра wis2box, используя MQTT Explorer. Вы можете использовать публичные учетные данные `everyone/everyone` для подключения к брокеру.

Убедитесь, что у вас открыт веб-браузер с доступом к wis2box-webapp для вашего экземпляра, перейдя по адресу `http://YOUR-HOST/wis2box-webapp`.
...
Для "Centre ID" используйте тот же, что и в предыдущих практических сессиях.
...
- укажите название и описание набора данных
...
- Используйте `WIS2BOX_URL/data/aviation-license.html`, чтобы указать на пользовательский файл лицензии, размещенный на вашем экземпляре, заменив `WIS2BOX_URL` на URL вашего экземпляра wis2box.
...
!!! note "О URL лицензии"
    ...
    Для этого упражнения мы будем использовать самостоятелно размещенный файл лицензии. Вы добавите файл 'aviation-license.html' в ваш экземпляр wis2box позже в этой практической сессии, чтобы убедиться, что URL лицензии действителен.
...
Добавьте плагин для 'Universal data without conversion' и убедитесь, что расширение файла установлено в `.txt`, чтобы соответствовать файлам данных METAR, которые вы опубликуете в этом наборе данных позже в этой практической сессии:
...
Проверьте MQTT Explorer, чтобы убедиться, что вы получили сообщение уведомления WIS2, объявляющее новую запись метаданных обнаружения по теме `origin/a/wis2/<your-centre-id>/metadata`.	

## Проверьте ваш новый набор данных в wis2box-api
...
## Добавьте файл лицензии в ваш экземпляр wis2box
...
Учетные данные для доступа к консоли MinIO определены переменными окружения `WIS2BOX_STORAGE_USERNAME` и `WIS2BOX_STORAGE_PASSWORD` в файле wis2box.env.

Вы можете найти их в файле `wis2box.env`, выполнив следующую команду:
...
После входа в консоль MinIO загрузите файл лицензии в базовый путь корзины **wis2box-public** с помощью кнопки “Upload”:

<img alt="minio-upload-license" src="/../assets/img/minio-upload-license.png" width="800">

После загрузки файла лицензии проверьте, доступен ли файл, перейдя по адресу `WIS2BOX_URL/data/aviation-license.html` в вашем веб-браузере, заменив `WIS2BOX_URL` на URL вашего экземпляра wis2box. 
...
## Добавьте токен доступа к набору данных
...
Из командной строки внутри контейнера вы можете защитить набор данных, используя команду `wis2box auth add-token`, указав флаг `--metadata-id` для задания идентификатора метаданных набора данных и токен доступа в качестве аргумента.
...
Затем загрузите этот файл в ваш набор данных с использованием MinIO Console. Чтобы получить доступ к MinIO Console, откройте веб-браузер и перейдите по адресу `http://YOUR-HOST:9001`, затем войдите в систему, используя учетные данные, указанные в файле `wis2box.env` в переменных окружения `WIS2BOX_STORAGE_USERNAME` и `WIS2BOX_STORAGE_PASSWORD`.

Чтобы загрузить файл в ваш набор данных, перейдите в корзину **wis2box-incoming**, создайте новую папку с именем, соответствующим идентификатору метаданных вашего набора данных, и загрузите пример файла данных METAR в эту папку, используя кнопку «Upload»:
...
!!! note "Устранение неполадок"
    ...
    Если вы не получили уведомление после загрузки данных, вы можете проверить последние логи контейнера `wis2box-management` для устранения проблемы:
...
Из командной строки в вашей виртуальной машине выполните команду `wget` с канонической ссылкой, которую вы скопировали из уведомления WIS2.
...
И используйте команду `wis2box auth remove-token`, чтобы удалить токен доступа из набора данных, указав флаг `--metadata-id` для идентификатора метаданных набора данных и сам токен в качестве аргумента:
...
И проверьте, что токен доступа был удален, попробовав снова загрузить данные с помощью `wget` с токеном доступа в HTTP-заголовках или попытавшись получить доступ к данным через веб-браузер.

!!! note "Контроль доступа является необязательным и применяется только к рекомендованным наборам данных"

    WIS2 указывает, что к рекомендованным наборам данных *может* применяться контроль доступа. Добавление контроля доступа к рекомендованному набору данных не является обязательным, и вы можете полагаться только на URL лицензии, чтобы информировать потребителей данных об условиях использования данных. Если вы применяете контроль доступа, вы несете ответственность за предоставление токена доступа всем потребителям данных, которым требуется доступ к этим данным.

    Если вы примените `wis2box auth add-token` к набору данных с политикой данных WMO 'core', Global Caches сообщат об ошибке в Global Monitoring, так как они не смогут загрузить данные, и ваш набор данных будет считаться *несоответствующим* техническим регламентам WIS2.
