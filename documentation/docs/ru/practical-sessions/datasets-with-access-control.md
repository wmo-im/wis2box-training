...
**recommended** : данные, которые могут предоставляться с условиями использования и/или подлежат лицензированию
...
В этой практической сессии вы создадите новый набор данных с политикой данных 'recommended' с использованием редактора наборов данных в wis2box-webapp. Вы также узнаете, как предоставить самозахостенный лицензионный файл и, при необходимости, добавить контроль доступа.
...
Убедитесь, что вы подключены к брокеру MQTT вашего экземпляра wis2box с использованием MQTT Explorer. Вы можете использовать публичные учётные данные `everyone/everyone` для подключения к брокеру.
...
!!! note "О URL лицензии"
    ...
    Вы можете использовать URL, указывающий на лицензионный файл, размещённый на вашем экземпляре wis2box, или URL, указывающий на лицензионный файл, размещённый на внешнем веб-сайте.

    Для этого упражнения мы будем использовать самозахостенный лицензионный файл. Вы добавите файл 'aviation-license.html' в ваш экземпляр wis2box позже в этой практической сессии, чтобы убедиться, что URL лицензии действителен.
...
Добавьте плагин для 'Universal data without conversion' и убедитесь, что расширение файла установлено на `.txt`, чтобы соответствовать файлам данных METAR, которые вы будете публиковать в этот набор данных позже в этой практической сессии:
...
## Добавьте лицензионный файл в ваш экземпляр wis2box

Убедитесь, что ссылка "License for this dataset" в метаданных вашего рекомендованного набора данных работает как ожидается.

Скачайте этот пример лицензионного файла для авиации: [aviation-license.html](./../../sample-data/aviation-license.html). 

!!! note "О примерном лицензионном файле для авиации"
...
После входа в консоль MinIO загрузите лицензионный файл в базовый путь корзины **wis2box-public** с помощью кнопки “Upload”:
...
## Опубликуйте данные в набор данных
...
Чтобы добавить файл в ваш набор данных, перейдите в корзину **wis2box-incoming**, создайте новую папку с именем, соответствующим идентификатору метаданных вашего набора данных, и загрузите пример файла METAR в эту папку, используя кнопку «Upload»:
...
Скопируйте URL для канонической ссылки из сообщения об уведомлении WIS2 и вставьте его в ваш веб-браузер. URL должен выглядеть примерно так:
...
Если вы правильно добавили токен доступа к набору данных, вы **не** сможете получить доступ к данным через веб-браузер, вместо этого вы увидите ошибку *401 Authorization Required*.
...
Из командной строки в вашей виртуальной машине выполните команду `wget` с канонической ссылкой, которую вы скопировали из сообщения об уведомлении WIS2.
...
И проверьте, что токен доступа был удален, попробовав снова загрузить данные с использованием `wget` с токеном доступа в HTTP-заголовках или попытавшись получить доступ к данным через веб-браузер.
...
!!! note "Контроль доступа является необязательным и применяется только к рекомендованным наборам данных"
    ...
    Если вы примените `wis2box auth add-token` к набору данных с политикой данных WMO 'core', то Global Caches сообщат об ошибке в Global Monitoring, так как они не смогут загрузить данные, и ваш набор данных будет считаться *несоответствующим* техническим регламентам WIS2.
