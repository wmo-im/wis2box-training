...
- могут иметь примененные механизмы контроля доступа;
...
Убедитесь, что у вас есть доступ по SSH к вашей виртуальной машине (VM) для студентов, и ваш экземпляр wis2box запущен и работает.

Убедитесь, что вы подключены к MQTT брокеру вашего экземпляра wis2box, используя MQTT Explorer. Вы можете использовать публичные учетные данные `everyone/everyone` для подключения к брокеру.
...
Чтобы загрузить этот файл, используйте MinIO Console, доступную на порту 9001 вашего экземпляра wis2box, открыв веб-браузер и перейдя по адресу `http://YOUR-HOST:9001`.

Учетные данные для доступа к MinIO Console определены в файле `wis2box.env` переменными окружения `WIS2BOX_STORAGE_USERNAME` и `WIS2BOX_STORAGE_PASSWORD`.
...
После загрузки файла лицензии проверьте его доступность, перейдя по адресу `WIS2BOX_URL/data/aviation-license.html` в вашем веб-браузере, заменив `WIS2BOX_URL` на URL вашего экземпляра wis2box.
...
Если вы добавили токен доступа правильно, вы **не** сможете получить доступ к данным через веб-браузер. Вместо этого вы увидите ошибку *401 Authorization Required*.

## Добавление токена доступа в HTTP-заголовки для доступа к набору данных

Чтобы продемонстрировать, что токен доступа необходим для доступа к набору данных, мы воспроизведем ошибку, которую вы видели в браузере, используя команду `wget`.

Из командной строки вашей виртуальной машины (VM) выполните команду `wget` с канонической ссылкой, которую вы скопировали из уведомления WIS2:
...
И используйте команду `wis2box auth remove-token`, чтобы удалить токен доступа из набора данных, указав флаг `--metadata-id` для определения идентификатора метаданных набора данных и сам токен в качестве аргумента:
