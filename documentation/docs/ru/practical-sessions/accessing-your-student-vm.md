...
В рамках локальных семинаров по обучению WIS2 вы можете получить доступ к вашей персональной учебной виртуальной машине в локальной учебной сети с названием "WIS2-training".
...
!!! note
    ...
    Если вы хотите пройти это обучение вне локальной учебной сессии, вы можете использовать собственный экземпляр виртуальной машины через любого облачного провайдера, например:

    - GCP (Google Cloud Platform) экземпляр VM `e2-medium`
    - AWS (Amazon Web Services) экземпляр ec2 `t3a.medium`
    - Azure (Microsoft) виртуальная машина Azure `standard_b2s`
    ...
    Архив релиза для wis2box, используемый в этом обучении, можно скачать следующим образом:
    ...
    Вы всегда можете найти последнюю версию архива 'wis2box-setup' по адресу [https://github.com/World-Meteorological-Organization/wis2box/releases](https://github.com/World-Meteorological-Organization/wis2box-release/releases).
...
Подключите ваш ПК к локальной сети Wi-Fi, раздаваемой в помещении во время обучения WIS2, согласно инструкциям, предоставленным тренером.
...
Если на вашем локальном ПК установлен WinSCP, вы можете использовать его для подключения к вашей учебной виртуальной машине, чтобы просмотреть содержимое домашнего каталога и загружать или выгружать файлы между вашей виртуальной машиной и локальным ПК. 
