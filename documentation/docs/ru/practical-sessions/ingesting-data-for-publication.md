...
!!! abstract "Результаты обучения"
    ...
    - получать доступ к панели мониторинга Grafana для отслеживания статуса загрузки данных и просмотра журналов вашей инстанции wis2box.
    - просматривать уведомления данных WIS2, опубликованные вашей инстанцией wis2box, с помощью MQTT Explorer.

## Введение

В WIS2 данные передаются в реальном времени с использованием уведомлений данных WIS2, которые содержат "каноническую" ссылку, по которой можно загрузить данные.
...
Для мониторинга статуса рабочего процесса данных wis2box вы можете использовать **панель мониторинга Grafana** и **MQTT Explorer**. Панель Grafana использует данные из Prometheus и Loki для отображения статуса вашей инстанции wis2box, в то время как MQTT Explorer позволяет видеть уведомления данных WIS2, опубликованные вашей инстанцией wis2box.

В этом разделе мы сосредоточимся на том, как загрузить данные в вашу инстанцию wis2box и проверить успешную загрузку и публикацию. Преобразование данных будет рассмотрено позже в практической сессии [Инструменты преобразования данных](./data-conversion-tools.md).
...
Держите панель Grafana открытой в вашем браузере, так как мы будем использовать её позже для мониторинга статуса загрузки данных.
...
Для входа вам понадобятся учетные данные, определенные переменными WIS2BOX_STORAGE_USERNAME и WIS2BOX_STORAGE_PASSWORD в файле wis2box.env. Вы можете проверить значения этих переменных, выполнив следующие команды на вашей виртуальной машине студента:
...
!!! question "Вопрос"
    ...
    После загрузки файла, как вы можете проверить, что рабочий процесс данных в wis2box был успешно запущен?
...
!!! note "О плагине Universal"
    ...
    Плагин "Universal" позволяет публиковать данные без какого-либо преобразования. Это *проходной* плагин, который загружает файл данных и публикует его как есть. Чтобы добавить свойство "datetime" в уведомление данных WIS2, плагин использует первую группу в шаблоне имени файла для извлечения даты публикуемых данных.

!!! question "Бонусный вопрос"

    Попробуйте загрузить тот же файл снова в тот же путь в MinIO. Получите ли вы новое уведомление в MQTT Explorer?
...
## Импорт и публикация: плагин "synop2bufr"
...
Проверьте панель мониторинга Grafana и MQTT Explorer, чтобы убедиться, что данные были успешно импортированы и опубликованы.
...
!!! note "О плагине FM-12 data converted to BUFR"
    ...
    В рамках преобразования плагин добавляет недостающие метаданные в выходные данные, такие как идентификатор станции WIGOS, местоположение и высота барометра станции. Чтобы добавить эти метаданные, плагин ищет эту информацию в списке станций вашей системы 'wis2box', используя традиционный (5-значный) идентификатор (в данном случае 64400).

    Если станция не найдена в списке станций, плагин не может добавить недостающие метаданные и не публикует данные.

Добавьте станцию с идентификатором WIGOS `0-20000-0-64400` в вашу систему 'wis2box' с помощью редактора станций в 'wis2box-webapp', как вы узнали в практической сессии [Configuring Station Metadata](./configuring-station-metadata.md).
...
После добавления станции повторно запустите рабочий процесс 'wis2box', загрузив пример файла данных *synop_202502040900.txt* снова в тот же путь в MinIO.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Проверьте раздел Links в уведомлении WIS2 в MQTT Explorer, и вы увидите каноническую ссылку:
...
Использование веб-интерфейса MinIO — это удобный способ вручную загружать данные в MinIO для тестирования. Однако в производственной среде обычно используются автоматизированные процессы для загрузки данных в MinIO, например, с использованием скриптов или приложений, которые используют API MinIO, совместимый с S3.
...
На вашей виртуальной машине 'student VM' пакет 'minio' для Python уже установлен.
...
!!! note
    ...
    Вы получите ошибку, так как скрипт не настроен для доступа к конечной точке MinIO в вашей системе 'wis2box'.

Скрипту необходимо знать правильную конечную точку для доступа к MinIO в вашей системе 'wis2box'. Если 'wis2box' работает на вашем хосте, конечная точка MinIO доступна по адресу `http://YOUR-HOST:9000`. Скрипт также должен быть обновлен с вашим паролем для хранилища и путем в корзине MinIO для сохранения данных.

!!! question "Обновите скрипт и импортируйте данные CSV"
    ...
    - С помощью WinSCP: установите новое соединение, используя File Protocol `SCP` и те же учетные данные, что и ваш SSH-клиент. Перейдите в каталог `wis2box-data/data-ingest-exercises` и отредактируйте `copy_file_to_incoming.py` с помощью встроенного текстового редактора.
    ...
    Повторно запустите скрипт, чтобы импортировать пример файла данных `synop_202501030900.txt` в MinIO:
...
После успешного выполнения скрипта вы увидите сообщение о том, что файл был скопирован в MinIO, и должны увидеть уведомления о данных, опубликованных вашей системой 'wis2box', в MQTT Explorer.

Вы также можете проверить панель мониторинга Grafana, чтобы убедиться, что данные были успешно импортированы и опубликованы.

Теперь, когда скрипт работает, вы можете попробовать копировать другие файлы в MinIO, используя тот же скрипт.

!!! question "Импорт бинарных данных в формате BUFR"
...
Проверьте панель мониторинга Grafana и MQTT Explorer, чтобы убедиться, что тестовые данные были успешно импортированы и опубликованы. Если вы видите ошибки, попробуйте их устранить.

!!! question "Проверка импорта данных"

    Сколько сообщений было опубликовано в брокере MQTT для этого примера данных?

??? success "Нажмите, чтобы увидеть ответ"

    Вы увидите ошибки, указанные в Grafana, так как станции в файле BUFR не определены в списке станций вашей системы 'wis2box'.

    Если все станции, используемые в файле BUFR, определены в вашей системе 'wis2box', вы должны увидеть 10 сообщений, опубликованных в брокере MQTT. Каждое уведомление соответствует данным для одной станции за один временной интервал наблюдения.

    Плагин `wis2box.data.bufr4.ObservationDataBUFR` разделяет файл BUFR на отдельные сообщения BUFR и публикует одно сообщение для каждой станции и временного интервала наблюдения.

## Импорт данных через SFTP
```
...
!!! success "Поздравляем!"
    ...
    - устранять распространенные ошибки в процессе загрузки данных с помощью панели мониторинга Grafana и логов вашего экземпляра `wis2box`.
    - отслеживать уведомления WIS2 о данных, опубликованных вашим `wis2box`, на панели мониторинга Grafana и в MQTT Explorer.