...
Для запуска рабочего процесса данных в WIS2 Node с использованием программного обеспечения wis2box данные должны быть загружены в **корзину wis2box-incoming** в **MinIO**, что инициирует рабочий процесс wis2box для обработки и публикации данных.
...
В этом разделе мы сосредоточимся на том, как загружать данные в ваш экземпляр wis2box и проверять успешность их загрузки и публикации. Преобразование данных будет рассмотрено позже в практической сессии [Инструменты преобразования данных](./data-conversion-tools.md).
...
Убедитесь, что вы можете войти в свою виртуальную машину (VM) студента, используя SSH-клиент (например, PuTTY).
...
Держите панель мониторинга Grafana открытой в вашем браузере, так как мы будем использовать её позже для мониторинга статуса загрузки данных.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Посмотрите на нижнюю панель панели мониторинга Grafana, и вы увидите **ошибку проверки пути**, указывающую, что путь не соответствует ни одному настроенному набору данных:
...
На этот раз убедитесь, что **создаете директорию, соответствующую идентификатору метаданных для набора прогнозов**, который вы создали в предыдущей практической сессии:
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Или, если у вас не было станций, связанных с этой темой, вы увидите:
...
!!! note "О плагине FM-12, преобразованном в BUFR"
    ...
    В рамках преобразования плагин добавляет недостающие метаданные в выходные данные, такие как идентификатор станции WIGOS, местоположение и высота барометра станции. Чтобы добавить эти метаданные, плагин ищет эту информацию в списке станций вашей установки 'wis2box', используя традиционный (5-значный) идентификатор (в данном случае 64400).

    Если станция не найдена в списке станций, плагин не может добавить недостающие метаданные и не публикует данные.

Добавьте станцию с идентификатором WIGOS `0-20000-0-64400` в вашу установку 'wis2box', используя редактор станций в 'wis2box-webapp', как вы узнали на практическом занятии [Configuring Station Metadata](./configuring-station-metadata.md).
...
Использование веб-интерфейса MinIO — это удобный способ вручную загружать данные в MinIO для тестирования. Однако в производственной среде обычно используются автоматизированные процессы для загрузки данных в MinIO, например, с помощью скриптов или приложений, использующих API MinIO, совместимый с S3.
...
Скрипту необходимо знать правильную конечную точку для доступа к MinIO на вашем 'wis2box'. Если 'wis2box' работает на вашем хосте, конечная точка MinIO доступна по адресу `http://YOUR-HOST:9000`. Скрипт также должен быть обновлен с вашим паролем для хранилища и путем в корзине MinIO для сохранения данных.
...
После успешного выполнения скрипта вы увидите сообщение, указывающее, что файл был скопирован в MinIO, и должны увидеть уведомления о данных, опубликованные вашей установкой 'wis2box' в MQTT Explorer.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Вы увидите ошибки, указанные в Grafana, так как станции в файле BUFR не определены в списке станций вашей установки 'wis2box'.

    Если все станции, используемые в файле BUFR, определены в вашей установке 'wis2box', вы должны увидеть 10 сообщений, опубликованных в брокере MQTT. Каждое уведомление соответствует данным для одной станции за один временной штамп наблюдения.
...
!!! success "Поздравляем!"
    ...
    - устранять распространенные ошибки в процессе загрузки данных с использованием панели Grafana и логов вашей инстанции `wis2box`.
    - отслеживать уведомления WIS2 о данных, опубликованных вашим `wis2box`, в панели Grafana и MQTT Explorer.