...
!!! abstract "Результаты обучения"
    ...
    - Запускать рабочий процесс wis2box, загружая данные в MinIO с использованием веб-интерфейса MinIO, SFTP или Python-скрипта.
...
Для запуска рабочего процесса данных в узле WIS2 с использованием программного обеспечения wis2box данные должны быть загружены в **корзину wis2box-incoming** в **MinIO**, что инициирует рабочий процесс wis2box. Этот процесс приводит к публикации данных через уведомление данных WIS2. В зависимости от настроек отображения данных в вашей инстанции wis2box данные могут быть преобразованы в формат BUFR перед публикацией.

В этом упражнении мы будем использовать примерные файлы данных для запуска рабочего процесса wis2box и **публикации уведомлений данных WIS2** для набора данных, который вы настроили в предыдущей практической сессии.

Во время упражнения мы будем отслеживать статус загрузки данных с использованием **панели мониторинга Grafana** и **MQTT Explorer**. Панель Grafana использует данные из Prometheus и Loki для отображения статуса вашей инстанции wis2box, а MQTT Explorer позволяет видеть уведомления данных WIS2, опубликованные вашей инстанцией wis2box.

В этом упражнении мы сосредоточимся на различных методах загрузки данных в вашу инстанцию wis2box и проверке успешной загрузки и публикации. Преобразование данных будет рассмотрено позже в практической сессии [Инструменты преобразования данных](./data-conversion-tools.md).
...
Также требуется знание настройки станций в **wis2box-webapp**, как описано в практической сессии [Настройка метаданных станций](./configuring-station-metadata.md).

Убедитесь, что вы можете войти в свою виртуальную машину для студентов с использованием SSH-клиента (например, PuTTY).
...
Убедитесь, что MQTT Explorer запущен и подключен к вашей инстанции с использованием публичных учетных данных `everyone/everyone` и подпиской на тему `origin/a/wis2/#`.
...
Сначала мы будем использовать веб-интерфейс MinIO, который позволяет загружать и скачивать данные в MinIO с использованием веб-браузера.
...
Выберите корзину wis2box-incoming и нажмите `Create new path`.
...
Таким образом, создайте директорию:

```bash
urn:wmo:md:my-centre-id:my-other-dataset
```

Перейдите в только что созданную директорию, нажмите `Upload`, найдите файл [geps_202508180000.grib2](../sample-data/geps_202508180000.grib2), который вы скачали на свой локальный компьютер, и загрузите этот файл в корзину wis2box-incoming.

<img alt="minio ui: upload your file" src="/../assets/img/minio-other-dataset-upload.png" width="650">

После завершения загрузки вы увидите этот файл в корзине wis2box-incoming MinIO:
...
!!! question "Вопрос"
    ...
    Удалось ли вам успешно загрузить файл в корзину wis2box-incoming?
...
!!! hint
    ...
    Есть ли у плагина требования к формату файла, и где они указаны?
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Это демонстрирует, что рабочий процесс данных был запущен, но данные не были повторно опубликованы. wis2box не публикует данные, если не может сопоставить расширение файла с grib2.

Затем скачайте переименованные примерные данные geps [geps_renamed_sample_data.grib2](../sample-data/geps_renamed_sample_data.grib2) в вашу локальную среду. Загрузите этот файл в ту же директорию, что и в предыдущих двух упражнениях.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Это демонстрирует, что рабочий процесс данных был запущен, но данные не были повторно опубликованы. wis2box не публикует данные, если имя файла не соответствует шаблону ^.*?_(\d{8}).*?\..*$.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Вы увидите сообщение, указывающее, что данные уже были опубликованы wis2box:
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Нет. 
    В панели мониторинга Grafana вы увидите предупреждение, указывающее на отсутствие записи для станции 64400:

    ```bash
    WARNING - Station 64400 not found in station file
    ``` 
    
    Это демонстрирует, что рабочий процесс данных был запущен, но требуется определённая метаинформация о станции.
...
Во время этого процесса плагин должен анализировать и сопоставлять информацию о станции, содержащуюся в данных.

Если отсутствует необходимая метаинформация о станции, плагин не сможет корректно обработать файл, и преобразование завершится с ошибкой.

Поэтому необходимо убедиться, что соответствующая метаинформация о станции добавлена в wis2box перед публикацией данных SYNOP.
...
!!! note
    `WIS2BOX_HOST_DATADIR` монтируется как `/data/wis2box/` внутри контейнера wis2box-management с помощью файла `docker-compose.yml`, включённого в каталог `wis2box`.
    
    Это позволяет обмениваться данными между хостом и контейнером.

В каталоге `exercise-materials/data-ingest-exercises` вы найдёте пример скрипта `copy_file_to_incoming.py`, который можно использовать для копирования файлов в MinIO.
...
Скрипту необходимо знать правильную конечную точку для доступа к MinIO на вашем wis2box. Если wis2box работает на вашем хосте, конечная точка MinIO доступна по адресу `http://YOUR-HOST:9000`. Также скрипт должен быть обновлён с вашим паролем для хранилища и путём в корзине MinIO для сохранения данных.
...
!!! question "Обновите скрипт и загрузите данные CSV"
    ...
    - Используя WinSCP: создайте новое соединение, используя протокол SCP и те же учетные данные, что и ваш SSH-клиент. Перейдите в каталог `wis2box-data/data-ingest-exercises` и отредактируйте `copy_file_to_incoming.py`, используя встроенный текстовый редактор.
...
После успешного выполнения скрипта вы увидите сообщение, указывающее, что файл был скопирован в MinIO, и вы должны увидеть уведомления о данных, опубликованные вашим экземпляром wis2box в MQTT Explorer.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Вы увидите ошибки, сообщённые в Grafana, так как станции в файле BUFR не определены в списке станций вашего экземпляра wis2box. 
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Если вы используете неправильный путь, в логах появится сообщение об ошибке.
...
!!! success "Поздравляем!"
    ...
    - Отлаживать распространенные ошибки в процессе обработки данных с использованием панели мониторинга Grafana и логов вашего экземпляра wis2box.
    - Мониторить уведомления WIS2 о данных, опубликованные вашим wis2box, через панель мониторинга Grafana и MQTT Explorer.