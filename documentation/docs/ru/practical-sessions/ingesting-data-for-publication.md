...
Скачайте пример данных geps [geps_202508180000.grib2](../sample-data/geps_202508180000.grib2) в вашу локальную среду:
...
После завершения загрузки вы увидите этот файл в корзине wis2box-incoming в MinIO:
...
Затем скачайте пример данных geps в другом формате файла [geps_202508180000.nc](../sample-data/geps_202508180000.nc) в вашу локальную среду. Загрузите этот файл в ту же директорию, что и в предыдущем упражнении.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Нет.
    Вы увидите сообщение, указывающее на ошибку неизвестного типа файла.
...
Затем скачайте переименованный пример данных geps [geps_renamed_sample_data.grib2](../sample-data/geps_renamed_sample_data.grib2) в вашу локальную среду. Загрузите этот файл в ту же директорию, что и в предыдущих двух упражнениях.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Нет.
    Вы увидите сообщение, указывающее на ошибку, что данные не соответствуют регулярному выражению.

    ```bash
    ERROR - ERROR - geps_renamed_sample_data.grib2 did not match ^.*?_(\d{8}).*?\..*$
    ``` 
    
    Это демонстрирует, что рабочий процесс данных был запущен, но данные не были повторно опубликованы. wis2box не публикует данные, если они не соответствуют шаблону имени файла ^.*?_(\d{8}).*?\..*$.

Универсальный плагин предоставляет общий механизм для загрузки и публикации файлов без применения доменно-специфического декодирования. Вместо этого он выполняет набор базовых проверок перед публикацией уведомления WIS2:

`Расширение файла` – файл должен использовать расширение, разрешенное конфигурацией набора данных.
...
Цель этого плагина — обрабатывать данные FM-12, предоставленные в текстовом формате, и преобразовывать их в бинарный формат BUFR. 
...
Поэтому необходимо убедиться, что соответствующие метаданные станции добавлены в wis2box перед публикацией данных SYNOP.
...
Обратите внимание, что вы можете удалить эту станцию из вашего набора данных после практической сессии.

После завершения настройки метаданных станции проверьте с помощью MQTT Explorer, чтобы убедиться, что данные были успешно опубликованы. Если вы увидите уведомление ниже, значит, вы успешно опубликовали пример данных synop.
...
Скрипт должен знать правильную конечную точку для доступа к MinIO на вашем wis2box. Если wis2box работает на вашем хосте, конечная точка MinIO доступна по адресу `http://YOUR-HOST:9000`. Скрипт также должен быть обновлен с вашим паролем для хранилища и путем в корзине MinIO для сохранения данных.
...
!!! question "Обновите скрипт и обработайте данные CSV"
    ...
    - С помощью WinSCP: создайте новое соединение, используя протокол File Protocol `SCP` и те же учетные данные, что и ваш SSH-клиент. Перейдите в каталог `wis2box-data/data-ingest-exercises` и отредактируйте `copy_file_to_incoming.py`, используя встроенный текстовый редактор.
...
!!! question "Загрузка данных с использованием SFTP"
    ...
    Проверьте панель мониторинга Grafana и MQTT Explorer, чтобы убедиться, что данные успешно обработаны и опубликованы.
...
??? success "Нажмите, чтобы раскрыть ответ"
    ...
    Если вы используете неверный путь, вы увидите сообщение об ошибке в логах.
...
!!! success "Поздравляем!"
    ...
    - Отслеживать уведомления WIS2 о данных, опубликованные вашим wis2box, через панель мониторинга Grafana и MQTT Explorer.