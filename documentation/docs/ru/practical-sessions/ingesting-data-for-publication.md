...
В этом разделе мы сосредоточимся на том, как загружать данные в вашу инстанцию wis2box и проверять успешность их загрузки и публикации. Преобразование данных будет рассмотрено позже в практической сессии [Инструменты преобразования данных](./data-conversion-tools.md).
...
Введите новый путь к папке = *new-directory* и загрузите этот пример файла [mydata.nc](./../sample-data/mydata.nc) (щелкните правой кнопкой мыши и выберите "сохранить как", чтобы скачать файл). Вы можете использовать кнопку "Upload" в MinIO для загрузки файла в новую директорию:
...
Скачайте этот файл [GEPS_18August2025.grib2](../sample-data/GEPS_18August2025.grib2) на ваш локальный компьютер и загрузите его в директорию, которую вы создали для набора прогнозов. Проверьте панель Grafana и MQTT Explorer, чтобы увидеть, были ли данные успешно загружены и опубликованы.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Нет. 
    На панели Grafana вы увидите ошибку, указывающую, что данные уже были опубликованы:
...
Добавьте станцию с идентификатором WIGOS `0-20000-0-64400` в ваш экземпляр `wis2box`, используя редактор станций в `wis2box-webapp`, как вы узнали в практической сессии [Configuring Station Metadata](./configuring-station-metadata.md).
...
!!! question "Обновите скрипт и загрузите данные CSV"
    ...
    - С помощью WinSCP: установите новое соединение, используя протокол File Protocol `SCP` и те же учетные данные, что и ваш SSH-клиент. Перейдите в каталог `wis2box-data/data-ingest-exercises` и отредактируйте `copy_file_to_incoming.py` с помощью встроенного текстового редактора.
