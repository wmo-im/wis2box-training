...
Данные, публикуемые в WIS2, должны соответствовать требованиям и стандартам, определенным различными экспертными сообществами в области наук о Земле. Чтобы снизить порог для публикации данных наземных наблюдений, wis2box предоставляет инструменты для преобразования данных в формат BUFR. Эти инструменты доступны через контейнер wis2box-api и могут использоваться из командной строки для тестирования процесса преобразования данных.

Основные преобразования, поддерживаемые wis2box в настоящее время, включают преобразование отчетов FM-12 SYNOP в BUFR и данных CSV в BUFR. Поддержка данных FM-12 обусловлена их широким использованием и обменом в сообществе ВМО, а поддержка данных CSV позволяет преобразовывать данные, создаваемые автоматическими метеостанциями, в формат BUFR.

### О формате FM-12 SYNOP

Отчеты о погоде с наземных станций традиционно передавались ежечасно или в основные (00, 06, 12 и 18 UTC) и промежуточные (03, 09, 15, 21 UTC) синоптические часы. До перехода на BUFR эти отчеты кодировались в текстовом формате FM-12 SYNOP. Хотя переход на BUFR планировалось завершить к 2012 году, значительное количество отчетов все еще передается в устаревшем формате FM-12 SYNOP. Дополнительную информацию о формате FM-12 SYNOP можно найти в Руководстве ВМО по кодам, Том I.1 (WMO-No. 306, Том I.1).

### О библиотеке ecCodes

Библиотека ecCodes — это набор программных библиотек и утилит, предназначенных для декодирования и кодирования метеорологических данных в форматах GRIB и BUFR. Она разработана Европейским центром среднесрочных прогнозов погоды (ECMWF). Подробнее см. [документацию ecCodes](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation).
...
- **csv2bufr**: для преобразования данных CSV, создаваемых автоматическими метеостанциями, в формат BUFR. Модуль csv2bufr используется для преобразования данных CSV в BUFR с использованием шаблона отображения, который определяет, как данные CSV должны быть сопоставлены с форматом BUFR. См. [репозиторий csv2bufr на GitHub](https://github.com/World-Meteorological-Organization/csv2bufr)

Эти модули могут использоваться как автономно, так и в составе стека wis2box.
...
Сначала подключитесь к вашей виртуальной машине через SSH-клиент и скопируйте материалы упражнения в каталог "/wis2box-api" внутри контейнера wis2box-api:
...
Убедитесь, что вы находитесь в каталоге `/wis2box-api/data-conversion-exercises`:
...
Различные параметры могут быть переданы в `bufr_ls` для изменения как формата, так и отображаемых полей заголовка.
...
При анализе файла BUFR мы часто хотим определить тип данных, содержащихся в файле, и типичную дату/время данных в файле. Эта информация может быть выведена с использованием флага `-p` для выбора заголовков для вывода. Несколько заголовков могут быть включены с использованием списка, разделенного запятыми.
...
!!! question
    ...
    Выполните предыдущую команду и интерпретируйте вывод, используя [Таблицу общих кодов C-13](https://github.com/wmo-im/CCT/blob/master/C13.csv), чтобы определить категорию данных и подкатегорию.

    Какой тип данных (категория данных и подкатегория) содержится в файле? Каковы типичная дата и время данных?
...
- Международная подкатегория — 4, что указывает на данные **"Отчёты о температуре/влажности/ветре на верхних уровнях с фиксированных наземных станций (TEMP)"**.
...
Попробуйте выполнить команду `bufr_dump` для второго примерного файла `bufr-cli-ex2.bufr4`:

```{.copy}
bufr_dump bufr-cli-ex2.bufr4
```

Это приведёт к созданию JSON, который может быть сложным для анализа. Попробуйте использовать флаг `-p`, чтобы вывести данные в простом текстовом формате (ключ=значение):
...
Вы можете использовать команду `grep`, чтобы отфильтровать вывод и показать только ключи, которые не отсутствуют. Например, чтобы показать все ключи, которые не отсутствуют, можно использовать следующую команду:
...
!!! question
    ...
    Каков идентификатор станции WIGOS для станции, которая сообщила данные в BUFR-файле `bufr-cli-ex2.bufr4`?
...
Аргумент `--metadata` используется для указания файла метаданных станции, который предоставляет дополнительную информацию для кодирования в BUFR-файле. Аргумент `--output-dir` используется для указания каталога, в который будут записаны преобразованные BUFR-файлы. Аргументы `--year` и `--month` используются для указания года и месяца наблюдения.
...
Следующие упражнения продемонстрируют, как работает модуль `synop2bufr` и как использовать его для преобразования данных FM-12 SYNOP в формат BUFR.

### Просмотр примерного сообщения SYNOP

Изучите примерный файл сообщения SYNOP для этого упражнения `synop_message.txt`:
...
Используйте следующую команду, чтобы изучить содержимое файла `station_list.csv`:
...
??? success "Нажмите, чтобы увидеть ответ"
    Вы можете использовать следующую команду для проверки содержимого BUFR-файла:
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Вам нужно выбрать набор данных и указать токен для "processes/wis2box", который вы создали в предыдущем упражнении, чтобы отправить форму.

    Если вы укажете недействительный токен, вы увидите:

    - Результат: Unauthorized, please provide a valid 'processes/wis2box' token

    Если вы укажете действительный токен, вы увидите "WARNINGS: 3". Нажмите на "WARNINGS", чтобы открыть выпадающий список, который покажет:

    - Станция 15015 не найдена в файле станций
    - Станция 15020 не найдена в файле станций
    - Станция 15090 не найдена в файле станций
...
!!! note
    ...
    В упражнении [ingesting-data-for-publication](./ingesting-data-for-publication.md) вы загрузили файл "synop_202412030900.txt", и он был преобразован в формат BUFR модулем synop2bufr.
...
!!! note
    Убедитесь, что вы все еще вошли в контейнер `wis2box-api` и находитесь в директории `/wis2box-api/data-conversion-exercises`. Если вы вышли из контейнера в предыдущем упражнении, вы можете снова войти следующим образом:

    ```bash
    cd ~/wis2box
    python3 wis2box-ctl.py login wis2box-api
    ```
    И перейдите в директорию с упражнениями:
...
Аргумент `--bufr-template` используется для указания файла шаблона сопоставления BUFR, который задает соответствие между входными данными CSV и выходными данными BUFR в формате JSON. Шаблоны сопоставления по умолчанию установлены в директории `/opt/csv2bufr/templates` в контейнере `wis2box-api`.

### Просмотр примера CSV-файла

Просмотрите содержимое примерного CSV-файла `aws-example.csv`:
...
!!! question
    Сколько строк данных содержится в CSV-файле? Каков идентификатор станции WIGOS, сообщающей данные в CSV-файле?
...
??? question "Нажмите, чтобы увидеть ответ"
    ...
    После строки заголовков есть 3 строки данных, представляющие 3 метеорологических наблюдения с одной и той же станции с идентификатором WIGOS `0-20000-0-60355` в три разных временных метки: `2024-03-31 01:00:00`, `2024-03-31 02:00:00` и `2024-03-31 03:00:00`.

### Просмотр aws-template

`wis2box-api` включает набор предопределенных шаблонов сопоставления BUFR, которые установлены в директории `/opt/csv2bufr/templates`.
...
**CSV-столбец не сопоставлен с ключом eccodes `internationalDataSubCategory`**, вместо этого используется постоянное значение 2, которое будет закодировано во всех BUFR-файлах, созданных с использованием этого шаблона сопоставления.
...
- CSV-файл должен соответствовать формату, определенному в шаблоне сопоставления, то есть имена столбцов CSV должны совпадать с именами, определенными в шаблоне сопоставления
...
Для получения информации о том, как создавать и использовать пользовательские шаблоны сопоставления BUFR, см. следующую практическую задачу [csv2bufr-templates](./csv2bufr-templates.md).
