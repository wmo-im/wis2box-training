---
title: Инструменты для преобразования данных
---
...
- **csv2bufr**: позволяет преобразовывать данные CSV, полученные с автоматических метеостанций, в формат BUFR. Модуль csv2bufr используется для преобразования данных CSV в BUFR с использованием шаблона отображения, который определяет, как данные CSV должны быть сопоставлены с форматом BUFR. Подробнее см. [репозиторий csv2bufr на GitHub](https://github.com/World-Meteorological-Organization/csv2bufr)
...
Сначала подключитесь к вашей виртуальной машине студента через SSH-клиент и скопируйте материалы упражнения в директорию "/wis2box-api" внутри контейнера wis2box-api:
...
Сам по себе этот вывод не очень информативен, так как содержит только ограниченную информацию о содержимом файла.
...
Это приведёт к созданию JSON, который может быть трудно анализировать. Попробуйте использовать флаг `-p`, чтобы вывести данные в текстовом формате (ключ=значение):
...
Вы можете использовать команду `grep`, чтобы отфильтровать вывод и показать только ключи, которые не отсутствуют. Например, чтобы показать все ключи, которые не отсутствуют, вы можете использовать следующую команду:
...
Далее рассмотрим, как конвертировать данные FM-12 SYNOP в формат BUFR с использованием модуля `synop2bufr`. Модуль `synop2bufr` используется для преобразования данных FM-12 SYNOP в формат BUFR. Модуль установлен в контейнере wis2box-api и может быть использован из командной строки следующим образом:
...
Аргумент `--metadata` используется для указания файла метаданных станции, который предоставляет дополнительную информацию для кодирования в BUFR-файле. Аргумент `--output-dir` используется для указания директории, в которой будут записаны преобразованные BUFR-файлы. Аргументы `--year` и `--month` используются для указания года и месяца наблюдения.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Обратите внимание, что это включает данные, предоставленные в файле `station_list.csv`.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Вам необходимо выбрать набор данных и предоставить токен для "processes/wis2box", который вы создали в предыдущем упражнении, чтобы отправить форму.

    Если вы предоставите неверный токен, вы увидите:
    ...
    Чтобы преобразовать эти данные в формат BUFR, вам нужно настроить соответствующие станции в вашем wis2box и убедиться, что станции связаны с темой для вашего набора данных.
...
!!! note
    Убедитесь, что вы все еще вошли в контейнер `wis2box-api` и находитесь в директории `/wis2box-api/data-conversion-exercises`. Если вы вышли из контейнера в предыдущем упражнении, вы можете войти снова следующим образом:
...
Аргумент `--bufr-template` используется для указания файла шаблона сопоставления BUFR, который предоставляет сопоставление между входными данными CSV и выходными данными BUFR, указанными в JSON-файле. Шаблоны сопоставления по умолчанию установлены в директории `/opt/csv2bufr/templates` в контейнере `wis2box-api`.

### Просмотр примера CSV-файла

Просмотрите содержимое примера CSV-файла `aws-example.csv`:
...
!!! question
    Сколько строк данных содержится в CSV-файле? Каков идентификатор станции WIGOS для станций, представленных в CSV-файле?
...
```{.copy}
    ...
    **Для ключа eccodes `internationalDataSubCategory` не сопоставлен столбец CSV**, вместо этого используется постоянное значение 2, которое будет закодировано во всех BUFR-файлах, созданных с использованием этого шаблона сопоставления.
...
- вы можете кодировать только те ключи, которые определены в шаблоне сопоставления
