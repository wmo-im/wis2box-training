...
!!! abstract "Результаты обучения"
    ...
    - Применять инструмент synop2bufr для преобразования отчетов FM-12 SYNOP в BUFR из командной строки
...
Сначала подключитесь к вашей виртуальной машине через SSH-клиент и скопируйте материалы упражнения в контейнер wis2box-api:
...
Различные параметры могут быть переданы в `bufr_ls` для изменения формата и полей заголовка, которые выводятся.
...
При изучении файла BUFR мы часто хотим определить тип данных, содержащихся в файле, и типичную дату/время данных в файле. Эта информация может быть выведена с использованием флага `-p` для выбора заголовков для вывода. Несколько заголовков можно включить, используя список, разделенный запятыми.
...
!!! question
    ...
    Выполните предыдущую команду и интерпретируйте вывод, используя [Общую кодовую таблицу C-13](https://github.com/wmo-im/CCT/blob/master/C13.csv), чтобы определить категорию данных и подкатегорию.
...
- Категория данных — 2, что указывает на данные типа **"Вертикальные зондирования (кроме спутников)"**.
...
Это приведёт к созданию JSON, который может быть трудным для анализа. Попробуйте использовать флаг `-p`, чтобы вывести данные в формате plain text (ключ=значение):
...
!!! question
    ...
    Какой идентификатор станции WIGOS у станции, которая сообщила данные в BUFR-файле `bufr-cli-ex2.bufr4`?
...
Далее рассмотрим, как конвертировать данные FM-12 SYNOP в формат BUFR с использованием модуля `synop2bufr`. Модуль `synop2bufr` используется для преобразования данных FM-12 SYNOP в формат BUFR. Этот модуль установлен в контейнере wis2box-api и может быть использован из командной строки следующим образом:
...
Следующие упражнения демонстрируют, как работает модуль `synop2bufr` и как использовать его для преобразования данных FM-12 SYNOP в формат BUFR.
...
!!! note
    Убедитесь, что вы все еще находитесь в контейнере `wis2box-api` и в директории `/root/data-conversion-exercises`. Если вы вышли из контейнера в предыдущем упражнении, вы можете войти снова следующим образом:
...
Аргумент `--bufr-template` используется для указания файла шаблона сопоставления BUFR, который предоставляет соответствие между входными данными CSV и выходными данными BUFR, указанными в JSON-файле. Шаблоны сопоставления по умолчанию установлены в директории `/opt/csv2bufr/templates` в контейнере `wis2box-api`.
...
Контейнер `wis2box-api` включает набор предопределенных шаблонов сопоставления BUFR, которые установлены в директории `/opt/csv2bufr/templates`.
