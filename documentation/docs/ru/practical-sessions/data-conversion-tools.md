...
Отчеты о погоде с наземных станций традиционно передавались ежечасно или в основные (00, 06, 12 и 18 UTC) и промежуточные (03, 09, 15, 21 UTC) синоптические часы. До перехода на BUFR эти отчеты кодировались в текстовом формате FM-12 SYNOP. Хотя переход на BUFR был запланирован на завершение к 2012 году, значительное количество отчетов все еще передается в устаревшем формате FM-12 SYNOP. Дополнительную информацию о формате FM-12 SYNOP можно найти в Руководстве ВМО по кодам, Том I.1 (WMO-No. 306, Том I.1).
...
!!! warning "Предварительные требования"
    ...
    - Подключитесь к MQTT брокеру вашего экземпляра wis2box с помощью MQTT Explorer
...
Для использования инструментов командной строки BUFR вам нужно войти в контейнер wis2box-api. Если не указано иное, все команды должны выполняться в этом контейнере. Вам также потребуется открыть MQTT Explorer и подключиться к вашему брокеру.

Сначала подключитесь к вашей виртуальной машине (VM) через SSH-клиент и скопируйте материалы упражнения в директорию "/wis2box-api" внутри контейнера wis2box-api:

```bash
docker cp ~/exercise-materials/data-conversion-exercises wis2box-api:/wis2box-api
```

Затем войдите в контейнер wis2box-api и перейдите в директорию, где находятся материалы упражнения:
...
Убедитесь, что вы находитесь в директории `/wis2box-api/data-conversion-exercises`:
...
Различные параметры могут быть переданы в `bufr_ls` для изменения как формата, так и выводимых полей заголовков.
...
Сам по себе эта информация не очень информативна, предоставляя лишь ограниченные сведения о содержимом файла.

При изучении файла BUFR мы часто хотим определить тип данных, содержащихся в файле, и типичную дату/время данных в файле. Эта информация может быть выведена с использованием флага `-p` для выбора заголовков для вывода. Несколько заголовков могут быть включены с использованием списка, разделенного запятыми.
...
!!! question
    ...
    Какой тип данных (категория и подкатегория) содержится в файле? Какова типичная дата и время для данных?
...
Это создаст JSON, который может быть трудно анализировать. Попробуйте использовать флаг `-p`, чтобы вывести данные в текстовом формате (ключ=значение):
...
Далее рассмотрим, как преобразовать данные FM-12 SYNOP в формат BUFR с использованием модуля `synop2bufr`. Модуль `synop2bufr` используется для преобразования данных FM-12 SYNOP в формат BUFR. Модуль установлен в контейнере `wis2box-api` и может быть использован из командной строки следующим образом:
...
Аргумент `--output-dir` используется для указания каталога, в котором будут записаны преобразованные BUFR-файлы. Аргументы `--year` и `--month` используются для указания года и месяца наблюдения.
...
!!! note
    Убедитесь, что вы все еще вошли в контейнер `wis2box-api` и находитесь в каталоге `/wis2box-api/data-conversion-exercises`. Если вы вышли из контейнера в предыдущем упражнении, вы можете снова войти следующим образом:

    ```bash
    cd ~/wis2box
    python3 wis2box-ctl.py login wis2box-api
    ```
    И перейдите в каталог упражнения:
...
Аргумент `--bufr-template` используется для указания файла шаблона сопоставления BUFR, который предоставляет сопоставление между входными данными CSV и выходными данными BUFR, указанными в JSON-файле. Шаблоны сопоставления по умолчанию установлены в каталоге `/opt/csv2bufr/templates` в контейнере `wis2box-api`.
...
!!! question
    Сколько строк данных содержится в файле CSV? Каков идентификатор станции WIGOS, передающей данные в файле CSV?
...
**Нет CSV-столбца, сопоставленного с ключом eccodes `internationalDataSubCategory`,** вместо этого используется постоянное значение 2, которое будет закодировано во всех BUFR-файлах, созданных с использованием этого шаблона сопоставления.
...
Чтобы проверить содержимое BUFR-файлов, игнорируя отсутствующие значения, можно использовать следующую команду:
...
??? success "Нажмите, чтобы увидеть ответ"
    Чтобы отфильтровать вывод, можно использовать следующую команду:
...
- CSV-файл должен соответствовать формату, определенному в шаблоне сопоставления, т.е. названия столбцов CSV должны совпадать с названиями, определенными в шаблоне сопоставления
- можно кодировать только ключи, определенные в шаблоне сопоставления
