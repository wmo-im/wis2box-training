...
Основные преобразования, поддерживаемые wis2box, включают преобразование отчетов FM-12 SYNOP в BUFR и данных CSV в BUFR. Поддержка данных FM-12 обусловлена их широким использованием и обменом в сообществе ВМО, а поддержка данных CSV позволяет преобразовывать данные, производимые автоматическими метеостанциями, в формат BUFR.
...
Сначала подключитесь к вашей виртуальной машине (VM) через SSH-клиент и скопируйте материалы для упражнений в каталог "/wis2box-api" внутри контейнера wis2box-api:

```bash
docker cp ~/exercise-materials/data-conversion-exercises wis2box-api:/wis2box-api
```

Затем войдите в контейнер wis2box-api и перейдите в каталог, где находятся материалы для упражнений:
...
Различные параметры могут быть переданы в `bufr_ls`, чтобы изменить как формат, так и поля заголовка, которые выводятся.
...
Сам по себе эта информация не очень информативна, так как предоставляет только ограниченные данные о содержимом файла.
...
!!! question
    ...
    Выполните предыдущую команду и интерпретируйте вывод, используя [Таблицу общих кодов C-13](https://github.com/wmo-im/CCT/blob/master/C13.csv), чтобы определить категорию и подкатегорию данных.
...
Это создаст JSON, который может быть сложным для анализа. Попробуйте использовать флаг `-p`, чтобы вывести данные в текстовом формате (ключ=значение):
...
Вы можете использовать команду `grep`, чтобы отфильтровать вывод и показать только те ключи, которые не отсутствуют. Например, чтобы показать все ключи, которые не отсутствуют, вы можете использовать следующую команду:
...
Аргумент `--metadata` используется для указания файла с метаданными станции, который предоставляет дополнительную информацию для кодирования в BUFR-файле.  
Аргумент `--output-dir` используется для указания директории, в которую будут записаны преобразованные BUFR-файлы. Аргументы `--year` и `--month` используются для указания года и месяца наблюдения.
...
!!! note
    ...
    В автоматизированном рабочем процессе в wis2box год и месяц автоматически извлекаются из имени файла и используются для заполнения аргументов `--year` и `--month`, необходимых для synop2bufr, а метаданные станции автоматически извлекаются из конфигурации станции в wis2box.

## Преобразование csv2bufr

!!! note
    Убедитесь, что вы все еще вошли в контейнер `wis2box-api` и находитесь в каталоге `/wis2box-api/data-conversion-exercises`. Если вы вышли из контейнера в предыдущем упражнении, вы можете войти снова следующим образом:

    ```bash
    cd ~/wis2box
    python3 wis2box-ctl.py login wis2box-api
    ```
    И перейдите в каталог с упражнениями:
...
Аргумент `--bufr-template` используется для указания файла шаблона сопоставления BUFR, который предоставляет сопоставление между входными данными CSV и выходными данными BUFR в формате JSON. Шаблоны сопоставления по умолчанию установлены в каталоге `/opt/csv2bufr/templates` в контейнере `wis2box-api`.
...
!!! question
    Какое значение ключа eccodes `airTemperature` в BUFR-файле `WIGOS_0-20000-0-60355_20240331T010000.bufr4`? А в BUFR-файле `WIGOS_0-20000-0-60355_20240331T020000.bufr4`?
