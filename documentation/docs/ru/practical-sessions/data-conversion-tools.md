...
Сначала подключитесь к виртуальной машине студента через SSH-клиент и скопируйте материалы упражнения в директорию "/wis2box-api" внутри контейнера wis2box-api:
...
!!! question
    ...
    Вы можете выполнить команду `bufr_ls` с флагом `-h`, чтобы увидеть доступные опции.
...
- Категория данных — 2, что указывает на данные **"Вертикальные зондирования (не спутниковые)"**.
...
Это приведёт к созданию JSON, который может быть сложно анализировать. Попробуйте использовать флаг `-p`, чтобы вывести данные в текстовом формате (ключ=значение):
...
!!! note
    ...
    python3 wis2box-ctl.py login wis2box-api
    ```
    И перейдите в директорию упражнения:
...
- файл CSV должен быть в формате, определенном в шаблоне сопоставления, то есть имена столбцов CSV должны совпадать с именами, определенными в шаблоне сопоставления
