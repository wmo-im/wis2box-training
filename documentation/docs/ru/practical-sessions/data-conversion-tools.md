...
Сначала подключитесь к виртуальной машине студента через SSH-клиент и скопируйте материалы упражнения в контейнер wis2box-api:
...
??? success "Нажмите, чтобы увидеть ответ"
    Вы можете изменить формат вывода на JSON, используя флаг `-j`, то есть:
    ```bash
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Это соответствует метаданным для 2 станций: для идентификаторов станций WIGOS `0-20000-0-15015` и `0-20000-0-15020`.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Вам необходимо выбрать набор данных и указать токен для "processes/wis2box", который вы создали в предыдущем упражнении, чтобы отправить форму.

    Если вы укажете недействительный токен, вы увидите:

    - Результат: Unauthorized, please provide a valid 'processes/wis2box' token

    Если вы укажете действительный токен, вы увидите "WARNINGS: 3". Нажмите на "WARNINGS", чтобы открыть выпадающий список, в котором будет указано:
...
!!! note
    Убедитесь, что вы все еще вошли в контейнер `wis2box-api` и находитесь в каталоге `/root/data-conversion-exercises`. Если вы вышли из контейнера в предыдущем упражнении, вы можете войти снова следующим образом:
...
Теперь давайте рассмотрим, как преобразовать данные CSV в формат BUFR с использованием модуля `csv2bufr`. Этот модуль установлен в контейнере `wis2box-api` и может быть использован из командной строки следующим образом:
...
Аргумент `--bufr-template` используется для указания файла шаблона BUFR, который предоставляет сопоставление между входными данными CSV и выходными данными BUFR, указанными в файле JSON. Шаблоны сопоставления по умолчанию установлены в каталоге `/opt/csv2bufr/templates` в контейнере `wis2box-api`.
...
!!! question
    Сколько строк данных содержится в файле CSV? Каков идентификатор станции WIGOS для станций, представленных в файле CSV?
...
- файл CSV должен соответствовать формату, определенному в шаблоне сопоставления, т.е. названия столбцов CSV должны совпадать с названиями, определенными в шаблоне сопоставления
...
Для получения информации о том, как создавать и использовать пользовательские шаблоны сопоставления BUFR, смотрите следующую практическую задачу [csv2bufr-templates](./csv2bufr-templates.md).
