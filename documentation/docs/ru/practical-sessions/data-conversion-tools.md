...
Результатом будет JSON, который может быть сложным для анализа. Попробуйте использовать флаг `-p`, чтобы вывести данные в текстовом формате (key=value):
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Если вы предоставите действительный токен, вы увидите "WARNINGS: 3". Нажмите на "WARNINGS", чтобы открыть выпадающий список, где будет указано:
...
!!! question
    Какое значение имеет ключ eccodes `airTemperature` в BUFR-файле `WIGOS_0-20000-0-60355_20240331T010000.bufr4`? А в BUFR-файле `WIGOS_0-20000-0-60355_20240331T020000.bufr4`?
