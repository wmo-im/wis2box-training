...
Программное обеспечение wis2box включает библиотеку ecCodes в базовый образ контейнера wis2box-api. Это позволяет пользователям использовать инструменты командной строки и библиотеки внутри контейнера. Библиотека ecCodes используется в стеке wis2box для декодирования и кодирования сообщений BUFR.
...
Сначала подключитесь к вашей виртуальной машине (VM) через SSH-клиент и скопируйте материалы упражнения в контейнер wis2box-api:
...
При проверке файла BUFR мы часто хотим определить тип данных, содержащихся в файле, и типичную дату/время данных в файле. Эта информация может быть выведена с использованием флага `-p` для выбора заголовков для вывода. Несколько заголовков могут быть включены с использованием списка, разделенного запятыми.
...
Результатом будет JSON, который может быть сложным для анализа. Попробуйте использовать флаг `-p`, чтобы вывести данные в простом текстовом формате (ключ=значение):
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    В файле содержится 3 отчёта SYNOP, соответствующих 3 различным станциям (идентифицированным по 5-значным традиционным идентификаторам станций: 15015, 15020 и 15090). Обратите внимание, что конец каждого отчёта отмечен символом `=`.
...
!!! note
    Убедитесь, что вы все еще находитесь в контейнере `wis2box-api` и в директории `/root/data-conversion-exercises`. Если вы вышли из контейнера в предыдущем упражнении, вы можете снова войти следующим образом:
...
Теперь давайте посмотрим, как преобразовать данные CSV в формат BUFR с использованием модуля `csv2bufr`. Модуль установлен в контейнере `wis2box-api` и может быть использован из командной строки следующим образом:
...
Для получения информации о создании и использовании пользовательских шаблонов сопоставления BUFR см. следующую практическую задачу [csv2bufr-templates](./csv2bufr-templates.md).
