...
Вы можете использовать опцию генерации случайных паролей при запросе `WIS2BOX_WEBAPP_PASSWORD`, `WIS2BOX_STORAGE_PASSWORD`, `WIS2BOX_BROKER_PASSWORD` или задать свои собственные.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Контейнер wis2box-management выполняет cron-задачу ежедневно для удаления данных, старше количества дней, определенного переменной WIS2BOX_STORAGE_DATA_RETENTION_DAYS, из бакета `wis2box-public` и API-бэкенда:
...
!!! note
    ...
    Не редактируйте файл `wis2box.env`, если вы не уверены в вносимых изменениях. Неправильные изменения могут привести к остановке работы вашего wis2box.
...
Выберите ``y``, и скрипт создаст файл ``docker-compose.images-1.2.0.yml``, загрузит необходимые Docker-образы и запустит сервисы.
...
!!! note "wis2box и Docker"
    wis2box работает как набор Docker-контейнеров, управляемых с помощью docker-compose.
    ...
    Вам не нужно знать детали Docker-контейнеров для работы с программным стеком wis2box, но вы можете изучить файлы `docker-compose*.yml`, чтобы увидеть, как определены сервисы. Если вы хотите узнать больше о Docker, вы можете найти дополнительную информацию в [документации Docker](https://docs.docker.com/).
...
Запустите следующую команду, чтобы увидеть запущенные Docker-контейнеры на вашей хост-машине:
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Чтобы просмотреть коллекции, доступные в данный момент через API, нажмите `View the collections in this service`:
...
Откройте MQTT Explorer на вашем компьютере и подготовьте новое подключение для подключения к вашему брокеру (работающему через контейнер **wis2box-broker**).
...
После подключения убедитесь, что внутренние статистические данные mosquitto публикуются вашим брокером в теме `$SYS`:
