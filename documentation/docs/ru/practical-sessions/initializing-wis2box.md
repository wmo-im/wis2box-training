...
!!! note
    ...
    См. [accessing-your-student-vm](./accessing-your-student-vm.md) для инструкций по загрузке и установке программного стека wis2box, если вы выполняете это обучение вне локальной учебной сессии.
...
Скрипт запросит у вас директорию, которая будет использоваться для переменной окружения `WIS2BOX_HOST_DATADIR`.
...
Вы можете использовать опцию генерации случайных паролей, когда будет предложено ввести `WIS2BOX_WEBAPP_PASSWORD`, `WIS2BOX_STORAGE_PASSWORD`, `WIS2BOX_BROKER_PASSWORD`, или задать свои собственные.
...
wis2box содержит API (интерфейс прикладного программирования), который предоставляет доступ к данным и процессы для интерактивной визуализации, преобразования данных и публикации.
...
!!! question
     ...
     Какие коллекции доступны в настоящее время?

??? success "Нажмите, чтобы увидеть ответ"
    
    Чтобы просмотреть коллекции, доступные в настоящее время через API, нажмите `View the collections in this service`:
...
Обязательно нажмите "SAVE", чтобы сохранить данные подключения.
...
!!! success "Поздравляем!"
    ...
    - запустить скрипт `wis2box-create-config.py` для создания начальной конфигурации
    - запустить wis2box и проверить состояние его компонентов
