...
Скачайте последний релиз tarball с GitHub и распакуйте его на своей виртуальной машине студента:
...
!!! note "Проверка запущенных контейнеров"
    Вы можете убедиться, что все контейнеры успешно запущены, используя:
    ```bash
    docker compose ps
    ```
...
Вы окажетесь на главной странице, которая по умолчанию настроена на отображение представления `Dashboard` с панелью Grafana.
...
- **Manual Subscribe** — создание подписки путем ввода деталей темы напрямую, без использования Global Discovery Catalogues. Полезно для подписки на темы с использованием произвольного количества подстановочных знаков и доступа к темам, которые не найдены в GDC, например, шлюзы GTS и темы, опубликованные на частных брокерах при использовании нестандартных конфигураций.
...
- **Settings** — в настоящее время позволяет перезагрузить каталог наборов данных из Global Discovery Catalogues. Этот раздел будет расширен в будущих версиях для охвата общей конфигурации и управления WIS2 Downloader.
...
На последнем этапе мы можем удалить подписку, перейдя в представление **Manage Subscriptions** и нажав кнопку **Unsubscribe**.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    API менеджера подписок слушает запросы на порту `5002`, как определено в `WIS2DOWNLOADER_SUBSCRIPTION_MANAGER_URL`.
...
Как и ранее, загруженные файлы можно просмотреть, проверив папку `surface-obs` в директории загрузки:
...
!!! success "Поздравляем!"
    ...
    - работать с UI для создания и удаления подписок
