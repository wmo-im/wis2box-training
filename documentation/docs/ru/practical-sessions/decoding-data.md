...
- GRIB2, содержащий данные для глобального ансамблевого прогнозирования, созданного системой CMA Global Regional Assimilation PrEdiction System (GRAPES)
...
- `-v ~/wis2-downloads:/root/downloads` монтирует каталог `~/wis2-downloads` на вашей виртуальной машине в `/root/downloads` внутри контейнера. Здесь будут храниться данные, которые вы загрузили из WIS2 после выполнения инструкций из предыдущего практического занятия по загрузчику WIS2.
...
- `-e JUPYTER_TOKEN=dataismagic!` задает токен, необходимый для доступа к серверу Jupyter ноутбуков. Вам нужно будет указать этот токен при доступе к серверу через веб-браузер
...
!!! note "О демо-образе decode-eccodes-jupyter"
    ...
    Образ `demo-decode-eccodes-jupyter` разработан для этого обучения и использует базовый образ, включающий библиотеку ecCodes, а также добавляет сервер Jupyter ноутбуков и Python пакеты для анализа данных и визуализации.

    Исходный код для этого образа, включая пример ноутбуков, можно найти на [wmo-im/demo-decode-eccodes-jupyter](https://github.com/wmo-im/demo-decode-eccodes-jupyter).
...
![Главная страница Jupyter ноутбуков](../assets/img/jupyter-files-screen1.png)
...
![Пример Jupyter ноутбуков](../assets/img/jupyter-files-screen2.png)
...
![Jupyter ноутбук GRIB2 глобального ансамблевого прогнозирования](../assets/img/jupyter-grib2-global-ensemble-prediction.png)

Прочитайте инструкции в ноутбуке и выполните ячейки для декодирования загруженных данных для глобального ансамблевого прогнозирования. Выполняйте каждую ячейку, щелкнув на ней и затем нажав кнопку выполнения на панели инструментов или нажав `Shift+Enter`.

После выполнения всех ячеек вы должны увидеть визуализацию "Вероятности аномалии температуры на уровне 850hPa ниже -1.5 стандартных отклонений":
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    ```python
    # показать визуализацию для сообщения номер 8 (Вероятность аномалии температуры на уровне 850hPa ниже -1.5 стандартных отклонений)
...
![Jupyter ноутбук треков тропических циклонов](../assets/img/jupyter-tropical-cyclone-track.png)

Прочитайте инструкции в ноутбуке и выполните ячейки для декодирования загруженных данных для треков тропических циклонов. Выполняйте каждую ячейку, щелкнув на ней и затем нажав кнопку выполнения на панели инструментов или нажав `Shift+Enter`.
...
!!! question 
    ...
    Результат отображает предсказанную вероятность трека тропического шторма в пределах 200 км. Как можно обновить ноутбук, чтобы отобразить предсказанную вероятность трека тропического шторма в пределах 300 км?

??? success "Нажмите, чтобы увидеть ответ"

    Чтобы обновить ноутбук для отображения предсказанной вероятности трека тропического шторма в пределах другого расстояния, вы можете обновить переменную `distance_threshold` в блоке кода, который рассчитывает вероятность удара.
...
![Jupyter ноутбук ежемесячных температурных аномалий](../assets/img/jupyter-netcdf4-monthly-temperature-anomalies.png)

Прочитайте инструкции в ноутбуке и выполните ячейки для декодирования загруженных данных для ежемесячных температурных аномалий. Выполняйте каждую ячейку, щелкнув на ней и затем нажав кнопку выполнения на панели инструментов или нажав `Shift+Enter`.
