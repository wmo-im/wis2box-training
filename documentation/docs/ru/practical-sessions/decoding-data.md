...
Существуют различные библиотеки кода для написания скриптов или программ для декодирования бинарных форматов ВМО. Также доступны инструменты с пользовательским интерфейсом, которые позволяют декодировать и визуализировать данные без необходимости писать код.

В этом практическом занятии мы продемонстрируем, как декодировать три разных типа данных с использованием Jupyter ноутбука:
...
Этот контейнер запустит сервер Jupyter ноутбуков на вашем экземпляре, который включает библиотеку "ecCodes", используемую для декодирования данных BUFR.
...
!!! note "Об образе demo-decode-eccodes-jupyter"
    ...
    Образ `demo-decode-eccodes-jupyter` разработан для этого обучения и использует базовый образ с включенной библиотекой ecCodes, а также сервер Jupyter ноутбуков и набор библиотек Python для анализа и визуализации данных.
...
В конце вы должны увидеть карту, отображающую давление, приведенное к среднему уровню моря:
...
!!! note "Декодирование данных NetCDF"
    ...
    NetCDF — это гибкий формат, который в данном примере содержит значения переменной 'anomaly', представленные вдоль измерений 'lat', 'lon'. Разные наборы данных NetCDF могут использовать разные названия переменных и измерений.
...
Вы также можете использовать другие инструменты для декодирования и визуализации бинарных форматов ВМО без необходимости писать код, такие как:
