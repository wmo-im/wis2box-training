...
Нажмите на пункт "dashboards" в левом меню:
...
Эта панель мониторинга основана на метриках, публикуемых сервисом wis2downloader, и показывает статус текущих загрузок.

В левом верхнем углу вы можете увидеть активные подписки.

Держите эту панель мониторинга открытой, так как вы будете использовать ее для мониторинга прогресса загрузки в следующем упражнении.
...
Следующие переменные окружения используются сервисом wis2downloader:
    ...
    - DOWNLOAD_MIN_FREE_SPACE_GB: Минимальный свободный объем пространства в ГБ, который нужно оставить на томе, где хранятся загрузки. По умолчанию: 1.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Период хранения данных по умолчанию составляет 24 часа.

!!! note "Обновление конфигурации wis2downloader"

    Чтобы обновить конфигурацию wis2downloader, вы можете отредактировать файл wis2box.env. Чтобы применить изменения, вы можете повторно запустить команду для запуска стека wis2box:
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    В зависимости от времени начала подписки, вы можете не увидеть загруженных файлов в этой директории. Если файлов пока нет, подождите еще несколько минут и проверьте снова.

Проверьте панель мониторинга wis2downloader в Grafana, чтобы увидеть прогресс загрузки. Вы увидите добавленную подписку в левом верхнем углу панели мониторинга и увеличение количества загрузок по мере загрузки данных:
...
Убедитесь, что директория загрузок на вашем хосте снова пуста:
...
Если в предыдущем упражнении вы замените `de-dwd-gts-to-wis2` на `jp-jma-gts-to-wis2`, вы будете получать уведомления и данные, публикуемые шлюзом JMA GTS-to-WIS2.
...
При подписке на тему, начинающуюся с `cache/`, вы будете получать несколько уведомлений для одних и тех же данных, по одному для каждого Global Cache. Каждое уведомление будет содержать канонический URL, указывающий на сервер данных соответствующего Global Cache. Программа wis2downloader загрузит данные с первого канонического URL, к которому она сможет подключиться.
...
Проверьте логи wis2downloader, чтобы убедиться, что подключение к новому брокеру было успешным:
...
Теперь мы настроим новую подписку на тему для загрузки данных о траектории циклонов с WIS2 Training Broker.
