...
!!! abstract "Результаты обучения"
    ...
    - создать новый шаблон преобразования BUFR для ваших данных в формате CSV
    - редактировать и отлаживать ваш собственный шаблон преобразования BUFR из командной строки
    - настроить плагин данных CSV-to-BUFR для использования пользовательского шаблона преобразования BUFR
...
Модуль `csv2bufr` включает инструмент командной строки для создания собственного шаблона преобразования с использованием набора последовательностей BUFR и/или элементов BUFR в качестве входных данных.
...
- `csv2bufr mappings list` : Показать доступные в системе шаблоны преобразования

!!! Note "csv2bufr mapping list"

    Команда `csv2bufr mapping list` покажет вам доступные в системе шаблоны преобразования. 
    Шаблоны по умолчанию хранятся в каталоге `/opt/wis2box/csv2bufr/templates` в контейнере.

    Чтобы поделиться пользовательскими шаблонами преобразования с системой, вы можете сохранить их в каталоге, определенном переменной `$CSV2BUFR_TEMPLATES`, которая по умолчанию установлена в `/data/wis2box/mappings` в контейнере. Поскольку каталог `/data/wis2box/mappings` в контейнере смонтирован в каталог `$WIS2BOX_HOST_DATADIR/mappings` на хосте, вы найдете свои пользовательские шаблоны преобразования в каталоге `$WIS2BOX_HOST_DATADIR/mappings` на хосте.

Попробуем создать новый пользовательский шаблон преобразования с помощью команды `csv2bufr mapping create`, используя в качестве входных данных последовательность BUFR 301150 и элемент BUFR 012101.
...
!!! question "Проверка шаблона преобразования"

    Сколько столбцов CSV сопоставляются с элементами BUFR? Какой заголовок CSV соответствует каждому элементу BUFR?

??? success "Нажмите, чтобы увидеть ответ"
    
    Созданный вами шаблон преобразования сопоставляет **5** столбцов CSV с элементами BUFR, а именно 4 элемента BUFR из последовательности 301150 и элемент BUFR 012101. 

    Следующие столбцы CSV сопоставляются с элементами BUFR:
...
Созданный вами шаблон преобразования не содержит важные метаданные об измерении, такие как дата и время наблюдения, а также широта и долгота станции.
...
!!! question "Проверка обновленного шаблона преобразования"

    Сколько столбцов CSV теперь сопоставляются с элементами BUFR? Какой заголовок CSV соответствует каждому элементу BUFR?

??? success "Нажмите, чтобы увидеть ответ"
    
    Обновленный шаблон преобразования теперь сопоставляет **18** столбцов CSV с элементами BUFR:
...
wis2box-webapp автоматически обнаружит созданный вами шаблон сопоставления и сделает его доступным в списке шаблонов для плагина преобразования CSV в BUFR.
...
??? success "Нажмите, чтобы увидеть ответ"
    Каждый столбец содержит одну часть информации. Например, дата разделена на год, месяц и день, что отражает способ хранения данных в BUFR. Сегодняшняя дата будет разделена на столбцы "year", "month" и "day". Аналогично, время нужно разделить на "hour" и "minute", а идентификатор станции WIGOS — на соответствующие компоненты.
...
!!! hint "Отсутствующие данные"
    Признаётся, что данные могут отсутствовать по разным причинам, будь то отказ датчика или отсутствие наблюдаемого параметра. В таких случаях отсутствующие данные могут быть закодированы, как указано выше, остальные данные в отчёте остаются действительными.
...
Если данные были успешно загружены, вы должны увидеть 3 уведомления в MQTT Explorer по теме `origin/a/wis2/<centre-id>/data/weather/surface-weather-observations/synop` для 3 станций, для которых вы сообщили данные:
...
Нажмите "CONTINUE TO FORM" и добавьте описание для вашего набора данных, установите ограничивающий прямоугольник и предоставьте контактную информацию для набора данных. После заполнения всех разделов нажмите 'VALIDATE FORM' и проверьте форму.
...
После загрузки данных убедитесь, что на панели мониторинга Grafana нет ПРЕДУПРЕЖДЕНИЙ или ОШИБОК, и проверьте MQTT Explorer, чтобы убедиться, что вы получаете WIS2 уведомления о данных.
