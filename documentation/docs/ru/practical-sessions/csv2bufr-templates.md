...
Убедитесь, что у вас открыт MQTT Explorer и он подключен к вашему брокеру с использованием учетных данных `everyone/everyone`.
...
Чтобы найти конкретные последовательности и элементы BUFR, вы можете обратиться к таблицам BUFR по ссылке [https://confluence.ecmwf.int/display/ECC/BUFR+tables](https://confluence.ecmwf.int/display/ECC/BUFR+tables).
...
Чтобы вывести на экран справочную страницу для команды `csv2bufr mapping`:
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Созданный вами шаблон сопоставления связывает **5** столбцов CSV с элементами BUFR, а именно 4 элемента BUFR из последовательности 301150 и элемент BUFR 012101. 
...
- **010004** для давления
- **007031** для высоты барометра над средним уровнем моря
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Обновленный шаблон сопоставления связывает **18** столбцов CSV с элементами BUFR:
...
Чтобы протестировать преобразование данных, мы можем использовать инструмент командной строки `csv2bufr` для преобразования CSV файла в BUFR с использованием созданного нами шаблона сопоставления:
...
!!! question "Проверьте содержимое BUFR файла"
    ...
    Как можно проверить содержимое только что созданного BUFR файла и убедиться, что данные закодированы правильно?

??? success "Нажмите, чтобы увидеть ответ"

    Вы можете использовать команду `bufr_dump -p`, чтобы проверить содержимое только что созданного BUFR файла. 
    Команда покажет содержимое BUFR файла в читаемом для человека формате.
...
Теперь вы можете настроить ваш набор данных в wis2box-webapp для использования пользовательского шаблона сопоставления с плагином преобразования CSV в BUFR.
...
Откройте загруженный файл в редакторе и изучите его содержимое:
...
!!! hint "Отсутствующие данные"
    Признаётся, что данные могут отсутствовать по разным причинам, будь то сбой датчика или отсутствие наблюдаемого параметра. В таких случаях отсутствующие данные могут быть закодированы, как указано выше, при этом остальные данные в отчёте остаются действительными.

### Обновите пример файла

Обновите загруженный вами пример файла, чтобы использовать сегодняшнюю дату и время, а также измените идентификаторы станций WIGOS на станции, которые вы зарегистрировали в wis2box-webapp.
...
Создайте новую папку в корзине MinIO, которая соответствует dataset-id для набора данных, который вы создали с template='weather/surface-weather-observations/synop':
...
Проверьте панель Grafana по адресу `http://YOUR-HOST:3000`, чтобы увидеть, есть ли какие-либо предупреждения (WARNINGS) или ошибки (ERRORS). Если вы их видите, попробуйте устранить их и повторите упражнение.
...
Нажмите "CONTINUE TO FORM" и добавьте описание для вашего набора данных, задайте ограничивающий прямоугольник и укажите контактную информацию для набора данных. После того как вы заполните все разделы, нажмите 'VALIDATE FORM' и проверьте форму.

Просмотрите плагины данных для набора данных. Нажмите "UPDATE" рядом с плагином с именем "CSV data converted to BUFR", и вы увидите, что шаблон установлен на **DayCLI**:
...
Пример файла содержит одну строку данных для каждого дня месяца и сообщает данные для одной станции. Обновите загруженный вами пример файла, чтобы использовать сегодняшнюю дату и время, а также измените идентификаторы станций WIGOS на станцию, которую вы зарегистрировали в wis2box-webapp.

### Загрузите данные в MinIO и проверьте результат

Как и ранее, вам нужно загрузить данные в корзину 'wis2box-incoming' в MinIO для обработки с помощью конвертера csv2bufr. На этот раз вам нужно будет создать новую папку в корзине MinIO, которая соответствует dataset-id для набора данных, который вы создали с шаблоном `climate/surface-based-observations/daily`. Этот dataset-id будет отличаться от того, который вы использовали в предыдущем упражнении:

<img alt="Image showing MinIO UI with DAYCLI-example uploaded" src="/../assets/img/minio-upload-daycli-example.png"/></center>

После загрузки данных проверьте, что на панели мониторинга Grafana нет ПРЕДУПРЕЖДЕНИЙ (WARNINGS) или ОШИБОК (ERRORS), и проверьте MQTT Explorer, чтобы убедиться, что вы получаете WIS2 уведомления о данных (data-notifications).

Если данные были успешно обработаны, вы должны увидеть 30 уведомлений в MQTT Explorer по теме `origin/a/wis2/<centre-id>/data/climate/surface-based-observations/daily` за 30 дней месяца, за который вы отправили данные:
