...
Чтобы найти конкретные последовательности и элементы BUFR, вы можете обратиться к таблицам BUFR по адресу [https://confluence.ecmwf.int/display/ECC/BUFR+tables](https://confluence.ecmwf.int/display/ECC/BUFR+tables).
...
Попробуем создать новый пользовательский шаблон сопоставления с помощью команды `csv2bufr mapping create`, используя в качестве входных данных последовательность BUFR 301150 и элемент BUFR 012101.
...
!!! question "Проверка шаблона сопоставления"
    ...
    Сколько столбцов CSV сопоставлено элементам BUFR? Какой заголовок CSV соответствует каждому элементу BUFR?

??? success "Нажмите, чтобы увидеть ответ"
    
    Созданный вами шаблон сопоставления сопоставляет **5** столбцов CSV элементам BUFR, а именно 4 элемента BUFR из последовательности 301150 и элемент BUFR 012101. 

    Следующие столбцы CSV сопоставлены элементам BUFR:

    - **wigosIdentifierSeries** сопоставлен `"eccodes_key": "#1#wigosIdentifierSeries"` (элемент BUFR 001125)
    - **wigosIssuerOfIdentifier** сопоставлен `"eccodes_key": "#1#wigosIssuerOfIdentifier` (элемент BUFR 001126)
    - **wigosIssueNumber** сопоставлен `"eccodes_key": "#1#wigosIssueNumber"` (элемент BUFR 001127)
    - **wigosLocalIdentifierCharacter** сопоставлен `"eccodes_key": "#1#wigosLocalIdentifierCharacter"` (элемент BUFR 001128)
    - **airTemperature** сопоставлен `"eccodes_key": "#1#airTemperature"` (элемент BUFR 012101)

Созданный вами шаблон сопоставления не включает важные метаданные об измерении, такие как дата и время наблюдения, а также широта и долгота станции.
...
Выполните следующую команду, чтобы обновить шаблон сопоставления:
...
!!! question "Проверка обновленного шаблона сопоставления"
    ...
    Сколько столбцов CSV теперь сопоставлено элементам BUFR? Какой заголовок CSV соответствует каждому элементу BUFR?

??? success "Нажмите, чтобы увидеть ответ"
    
    Обновленный шаблон сопоставления теперь сопоставляет **18** столбцов CSV элементам BUFR:
    ...
    - **wigosIssueNumber** сопоставлен `"eccodes_key": "#1#wigosIssueNumber"` (элемент BUFR 001127)
    - **wigosLocalIdentifierCharacter** сопоставлен `"eccodes_key": "#1#wigosLocalIdentifierCharacter"` (элемент BUFR 001128)
    - **year** сопоставлен `"eccodes_key": "#1#year"` (элемент BUFR 004001)
    - **month** сопоставлен `"eccodes_key": "#1#month"` (элемент BUFR 004002)
    - **day** сопоставлен `"eccodes_key": "#1#day"` (элемент BUFR 004003)
    - **hour** сопоставлен `"eccodes_key": "#1#hour"` (элемент BUFR 004004)
    - **minute** сопоставлен `"eccodes_key": "#1#minute"` (элемент BUFR 004005)
    - **latitude** сопоставлен `"eccodes_key": "#1#latitude"` (элемент BUFR 005002)
    - **longitude** сопоставлен `"eccodes_key": "#1#longitude"` (элемент BUFR 006002)
    - **heightOfBarometerAboveMeanSeaLevel"** сопоставлен `"eccodes_key": "#1#heightOfBarometerAboveMeanSeaLevel"` (элемент BUFR 007031)
    - **airTemperature** сопоставлен `"eccodes_key": "#1#airTemperature"` (элемент BUFR 012101)
    - **nonCoordinatePressure** сопоставлен `"eccodes_key": "#1#nonCoordinatePressure"` (элемент BUFR 010004)
...
!!! question "Проверка содержимого BUFR файла"
...
Теперь вы можете настроить свой набор данных в wis2box-webapp для использования пользовательского шаблона сопоставления с плагином преобразования CSV в BUFR.
...
Нажмите на набор данных, который вы создали в предыдущей практической сессии, и нажмите "UPDATE" рядом с плагином с именем "CSV data converted to BUFR":
...
!!! hint
    ...
    Обратите внимание, что если вы не видите созданный вами новый шаблон, попробуйте обновить страницу или открыть её в новом режиме инкогнито.
...
Шаблон 'AWS' предоставляет шаблон сопоставления для преобразования данных CSV в последовательность BUFR 301150, 307096, в поддержку минимальных требований GBON.

Описание шаблона AWS можно найти здесь [aws-template](./../csv2bufr-templates/aws-template.md).

### Обзор входных данных aws-example
...
??? success "Нажмите, чтобы увидеть ответ"
    Каждый столбец содержит отдельный элемент информации. Например, дата разделена на год, месяц и день, что отражает способ хранения данных в BUFR. Сегодняшняя дата будет разделена на столбцы "year", "month" и "day". Аналогично, время нужно разделить на "hour" и "minute", а идентификатор станции WIGOS — на соответствующие компоненты.

!!! question
    Как в файле данных закодированы отсутствующие данные?
...
!!! hint "Отсутствующие данные"
    Признаётся, что данные могут отсутствовать по разным причинам, будь то сбой датчика или отсутствие наблюдаемого параметра. В таких случаях отсутствующие данные могут быть закодированы, как указано выше, остальные данные в отчёте остаются действительными.
...
Создайте новую папку в бакете MinIO, которая соответствует dataset-id для набора данных, который вы создали с template='weather/surface-weather-observations/synop':
...
Проверьте панель Grafana по адресу `http://YOUR-HOST:3000`, чтобы увидеть, есть ли какие-либо WARNINGS или ERRORS. Если вы их видите, попробуйте исправить их и повторить упражнение.

Проверьте MQTT Explorer, чтобы убедиться, что вы получаете уведомления о данных WIS2.

Если данные успешно загружены, вы должны увидеть 3 уведомления в MQTT Explorer на теме `origin/a/wis2/<centre-id>/data/weather/surface-weather-observations/synop` для 3 станций, по которым вы отправили данные:
...
Шаблон **DayCLI** предоставляет шаблон сопоставления для преобразования ежедневных климатических данных CSV в последовательность BUFR 307075, в поддержку отчётности о ежедневных климатических данных.

Описание шаблона DAYCLI можно найти здесь [daycli-template](./../csv2bufr-templates/daycli-template.md).

Чтобы поделиться этими данными в WIS2, вам нужно будет создать новый набор данных в wis2box-webapp, который имеет правильную иерархию WIS2 Topic Hierarchy и использует шаблон DAYCLI для преобразования данных CSV в BUFR.
...
Нажмите "CONTINUE TO FORM" и добавьте описание для вашего набора данных, задайте границы области и укажите контактную информацию для набора данных. После заполнения всех разделов нажмите 'VALIDATE FORM' и проверьте форму.
...
### Обзор входных данных daycli-example
...
Примерный файл содержит одну строку данных для каждого дня месяца и отчёт по одной станции. Обновите скачанный примерный файл, чтобы использовать сегодняшнюю дату и время, а также измените идентификаторы станций WIGOS на станцию, которую вы зарегистрировали в wis2box-webapp.

### Загрузка данных в MinIO и проверка результата
```

Как и раньше, вам нужно загрузить данные в корзину 'wis2box-incoming' в MinIO, чтобы они были обработаны конвертером csv2bufr. На этот раз вам нужно создать новую папку в корзине MinIO, которая соответствует dataset-id для набора данных, который вы создали с использованием шаблона `climate/surface-based-observations/daily`. Этот dataset-id будет отличаться от того, который вы использовали в предыдущем упражнении:

<img alt="Image showing MinIO UI with DAYCLI-example uploaded" src="/../assets/img/minio-upload-daycli-example.png"/></center>

После загрузки данных проверьте, что на панели мониторинга Grafana нет ПРЕДУПРЕЖДЕНИЙ или ОШИБОК, и проверьте MQTT Explorer, чтобы убедиться, что вы получаете уведомления о данных WIS2.

Если вы успешно загрузили данные, вы должны увидеть 30 уведомлений в MQTT Explorer по теме `origin/a/wis2/<centre-id>/data/climate/surface-based-observations/daily` за 30 дней месяца, за который вы сообщили данные:
...
!!! success "Поздравляем"
    ...
    - как создать пользовательский шаблон сопоставления для преобразования данных CSV в BUFR
