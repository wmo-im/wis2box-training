...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Команда отобразит содержимое BUFR файла в читаемом для человека формате.
...
Теперь вы можете настроить ваш набор данных в wis2box-webapp, чтобы использовать пользовательский шаблон сопоставления для плагина преобразования CSV в BUFR.
...
Нажмите на набор данных, созданный вами в предыдущей практической сессии, и нажмите "UPDATE" рядом с плагином с именем "CSV data converted to BUFR":
...
Шаблон 'AWS' предоставляет шаблон сопоставления для преобразования данных CSV в последовательности BUFR 301150, 307096, поддерживающие минимальные требования GBON.
...
??? success "Нажмите, чтобы увидеть ответ"
    Каждая колонка содержит отдельный элемент информации. Например, дата разделена на год, месяц и день, что соответствует тому, как данные хранятся в BUFR. Сегодняшняя дата будет разделена на колонки "year", "month" и "day". Аналогично, время нужно разделить на "hour" и "minute", а идентификатор станции WIGOS — на соответствующие компоненты.
...
Обновите скачанный примерный файл, чтобы использовать сегодняшнюю дату и время, а также измените идентификаторы станций WIGOS на те, которые вы зарегистрировали в wis2box-webapp.
...
Создайте новую папку в бакете MinIO, которая соответствует идентификатору набора данных (dataset-id), созданного вами с шаблоном='weather/surface-weather-observations/synop':
...
Проверьте панель Grafana по адресу `http://YOUR-HOST:3000`, чтобы увидеть, есть ли какие-либо ПРЕДУПРЕЖДЕНИЯ или ОШИБКИ. Если вы их видите, попробуйте устранить их и повторите упражнение.

Проверьте MQTT Explorer, чтобы увидеть, получаете ли вы уведомления о данных WIS2.

Если данные были успешно загружены, вы должны увидеть 3 уведомления в MQTT Explorer на теме `origin/a/wis2/<centre-id>/data/weather/surface-weather-observations/synop` для 3 станций, для которых вы сообщили данные:
...
Шаблон **DayCLI** предоставляет шаблон сопоставления для преобразования ежедневных климатических данных CSV в последовательность BUFR 307075, поддерживающую отчёты о ежедневных климатических данных.
...
Перейдите в редактор набора данных в wis2box-webapp и создайте новый набор данных. Используйте тот же идентификатор центра (centre-id), что и в предыдущих практических сессиях, и выберите **Data Type='climate/surface-based-observations/daily'**:

<img alt="Create a new dataset in the wis2box-webapp for DAYCLI" src="/../assets/img/wis2box-webapp-create-dataset-daycli.png"/>

Нажмите "CONTINUE TO FORM" и добавьте описание для вашего набора данных, задайте ограничивающий прямоугольник (bounding box) и укажите контактную информацию для набора данных. После того как вы заполните все разделы, нажмите 'VALIDATE FORM' и проверьте форму.
...
??? success "Нажмите, чтобы увидеть ответ"
    Шаблон daycli включает важные метаданные о размещении инструментов и классификациях качества измерений температуры и влажности, флаги контроля качества и информацию о том, как была рассчитана среднесуточная температура.

### Обновление примерного файла

Примерный файл содержит одну строку данных для каждого дня месяца и сообщает данные для одной станции. Обновите скачанный примерный файл, чтобы использовать сегодняшнюю дату и время, а также измените идентификаторы станций WIGOS на те, которые вы зарегистрировали в wis2box-webapp.

### Загрузка данных в MinIO и проверка результата
```

Как и ранее, вам нужно загрузить данные в корзину 'wis2box-incoming' в MinIO, чтобы они были обработаны конвертером csv2bufr. На этот раз вам нужно будет создать новую папку в корзине MinIO, которая соответствует dataset-id для набора данных, который вы создали с шаблоном `climate/surface-based-observations/daily`. Этот dataset-id будет отличаться от того, который вы использовали в предыдущем упражнении:

<img alt="Image showing MinIO UI with DAYCLI-example uploaded" src="/../assets/img/minio-upload-daycli-example.png"/></center>

После загрузки данных убедитесь, что на панели мониторинга Grafana нет ПРЕДУПРЕЖДЕНИЙ (WARNINGS) или ОШИБОК (ERRORS), и проверьте MQTT Explorer, чтобы убедиться, что вы получаете уведомления о данных WIS2.

Если данные были успешно обработаны, вы должны увидеть 30 уведомлений в MQTT Explorer по теме `origin/a/wis2/<centre-id>/data/climate/surface-based-observations/daily` за 30 дней месяца, за который вы сообщили данные:
