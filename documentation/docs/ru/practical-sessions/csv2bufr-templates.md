...
Большинство регистраторов данных, используемых для записи выходных данных датчиков, могут экспортировать наблюдения в файлы с разделителями, включая CSV.
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Созданный вами шаблон сопоставляет **5** столбцов CSV с элементами BUFR, а именно 4 элемента BUFR из последовательности 301150 и элемент BUFR 012101. 
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    Обновленный шаблон сопоставляет **18** столбцов CSV с элементами BUFR:
...
Проверьте содержимое файла `custom_template_data.csv` в каталоге `/tmp/data-conversion-exercises`:
...
Для тестирования преобразования данных можно использовать инструмент командной строки `csv2bufr`, чтобы преобразовать CSV файл в BUFR с использованием созданного вами шаблона сопоставления:
...
??? success "Нажмите, чтобы увидеть ответ"
    ...
    В выводе вы увидите значения для элементов BUFR, которые вы сопоставили в шаблоне, например, "airTemperature" будет отображаться как:
...
!!! question
    Изучая поля даты, времени и идентификаторы (WIGOS и традиционные идентификаторы), что вы замечаете? Как будет представлена сегодняшняя дата?

??? success "Нажмите, чтобы увидеть ответ"
    Каждая колонка содержит отдельный элемент информации. Например, дата разделена на год, месяц и день, что отражает способ хранения данных в BUFR. Сегодняшняя дата будет разделена на колонки "year", "month" и "day". Аналогично, время нужно разделить на "hour" и "minute", а идентификатор станции WIGOS — на соответствующие компоненты.
...
!!! hint "Отсутствующие данные"
    Признаётся, что данные могут отсутствовать по разным причинам, например, из-за отказа датчика или отсутствия наблюдаемого параметра. В таких случаях отсутствующие данные могут быть закодированы, как указано выше, при этом остальные данные в отчёте остаются действительными.

### Обновление примерного файла

Обновите примерный файл, который вы скачали, чтобы использовать сегодняшнюю дату и время, а также измените идентификаторы станций WIGOS на станции, зарегистрированные вами в wis2box-webapp.

### Загрузка данных в MinIO и проверка результата

Перейдите в интерфейс MinIO и войдите, используя учетные данные из файла `wis2box.env`.
...
Создайте новую папку в бакете MinIO, которая соответствует dataset-id для набора данных, созданного вами с template='weather/surface-weather-observations/synop':
...
Загрузите примерный файл, который вы скачали, в созданную вами папку в бакете MinIO:

<img alt="Image showing MinIO UI with aws-example uploaded" src="/../assets/img/minio-upload-aws-example.png"/></center>

Проверьте панель мониторинга Grafana по адресу `http://YOUR-HOST:3000`, чтобы увидеть, есть ли какие-либо WARNINGS или ERRORS. Если они есть, попробуйте исправить их и повторить упражнение.

Проверьте MQTT Explorer, чтобы убедиться, что вы получаете уведомления о данных WIS2.

Если данные успешно загружены, вы должны увидеть 3 уведомления в MQTT Explorer на теме `origin/a/wis2/<centre-id>/data/weather/surface-weather-observations/synop` для 3 станций, по которым вы сообщили данные:
...
Чтобы поделиться этими данными в WIS2, вам нужно будет создать новый набор данных в wis2box-webapp с правильной иерархией тем WIS2 и использовать шаблон DAYCLI для преобразования данных CSV в BUFR.
...
Нажмите "CONTINUE TO FORM" и добавьте описание для вашего набора данных, установите границы области и предоставьте контактную информацию для набора данных. После того как вы заполните все разделы, нажмите 'VALIDATE FORM' и проверьте форму.
...
Закройте конфигурацию плагина и отправьте форму, используя токен аутентификации, созданный вами в предыдущей практической сессии.
...
??? success "Нажмите, чтобы увидеть ответ"
    Шаблон daycli включает важные метаданные о размещении инструментов и классификации качества измерений температуры и влажности, флаги контроля качества и информацию о том, как была рассчитана среднесуточная температура.

### Обновление примерного файла

Примерный файл содержит одну строку данных для каждого дня месяца и сообщает данные для одной станции. Обновите примерный файл, который вы скачали, чтобы использовать сегодняшнюю дату и время, а также измените идентификаторы станций WIGOS на станцию, зарегистрированную вами в wis2box-webapp.

### Загрузка данных в MinIO и проверка результата
```

Как и ранее, вам нужно загрузить данные в корзину 'wis2box-incoming' в MinIO, чтобы они были обработаны конвертером csv2bufr. На этот раз вам нужно создать новую папку в корзине MinIO, которая соответствует dataset-id для набора данных, который вы создали с шаблоном `climate/surface-based-observations/daily`. Этот dataset-id будет отличаться от того, который вы использовали в предыдущем упражнении:

<img alt="Image showing MinIO UI with DAYCLI-example uploaded" src="/../assets/img/minio-upload-daycli-example.png"/></center>

После загрузки данных убедитесь, что на панели мониторинга Grafana нет предупреждений (WARNINGS) или ошибок (ERRORS), и проверьте MQTT Explorer, чтобы убедиться, что вы получаете WIS2 data-notifications.
