...
The **wis2box-management** container listens to events from the MinIO storage service to trigger data ingestion based on the data-plugins configured for your dataset. This allows you to upload data into MinIO and trigger the wis2box workflow to publish data on the WIS2 broker.

The same steps can be done programmatically by using any MinIO or S3 client software, allowing you to automate your data ingestion as part of your operational workflows. If you are unable to use the MinIO client, you can also use the **wis2box-ftp** service to forward data to the MinIO storage service.
