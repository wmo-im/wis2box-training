...
!!! abstract "Resultados de aprendizaje"
    ...
    - Ver las notificaciones de datos de WIS2 publicadas por tu wis2box utilizando MQTT Explorer.
...
En un entorno de producción, los datos normalmente se ingerirían utilizando procesos automatizados, como scripts o aplicaciones que envían datos a MinIO a través de S3 o SFTP.

## Preparación

Esta sección asume que has completado con éxito la sesión práctica [Configuración de conjuntos de datos en wis2box](./configuring-wis2box-datasets.md). Si seguiste las instrucciones en esa sesión, deberías tener un conjunto de datos que utiliza el plugin `Universal` y otro que utiliza el plugin `FM-12 data converted to BUFR`.

Asegúrate de poder iniciar sesión en tu máquina virtual de estudiante (VM) utilizando tu cliente SSH (por ejemplo, PuTTY).
...
Entra en el directorio recién creado, haz clic en `Upload` y sube el archivo que usaste anteriormente, *mydata.nc*, al nuevo directorio. Verifica el panel de Grafana para ver si los datos fueron ingeridos y publicados con éxito.
...
??? success "Haz clic para revelar la respuesta"
    ...
    El conjunto de datos se configuró para procesar únicamente archivos con la extensión `.grib2`. La configuración de la extensión de archivo es parte de los mapeos de datos que definiste en la sesión práctica anterior.

Descarga este archivo [GEPS_18August2025.grib2](../sample-data/GEPS_18August2025.grib2) en tu computadora local y súbelo al directorio que creaste para el conjunto de datos de pronóstico. Verifica el panel de Grafana y MQTT Explorer para ver si los datos fueron ingeridos y publicados con éxito.
...
Deberías ver una nueva notificación de datos de WIS2 en MQTT Explorer:
...
Agrega la estación con el identificador WIGOS `0-20000-0-64400` a tu instancia de wis2box utilizando el editor de estaciones en la aplicación wis2box-webapp, como aprendiste en la sesión práctica [Configuring Station Metadata](./configuring-station-metadata.md).
...
Revisa el panel de Grafana y MQTT Explorer para confirmar que los datos se publicaron correctamente. Si ves la notificación a continuación, entonces publicaste los datos de ejemplo synop con éxito:
...
??? success "Haz clic para revelar la respuesta"
    ...
    Revisa la sección de Enlaces en la notificación de datos de WIS2 en MQTT Explorer y verás el enlace canónico:
    ...
    La extensión del archivo es `.bufr4`, lo que indica que los datos se transformaron con éxito del formato FM-12 al formato BUFR por el plugin.
...
También puedes revisar el panel de Grafana para verificar si los datos se ingresaron y publicaron correctamente.
...
Revisa el panel de Grafana y MQTT Explorer para verificar si los datos de prueba se ingresaron y publicaron correctamente. Si ves algún error, intenta resolverlo.
...
??? success "Haz clic para revelar la respuesta"
    ...
    Si todas las estaciones utilizadas en el archivo BUFR están definidas en tu instancia de wis2box, deberías ver 10 mensajes publicados en el broker MQTT. Cada notificación corresponde a datos para una estación en un momento de observación.
...
El servicio MinIO en wis2box también se puede acceder a través de SFTP. Si tienes un sistema existente que puede configurarse para reenviar datos mediante SFTP, puedes usar esto como un método alternativo para automatizar la ingesta de datos.
...
Las credenciales para la conexión SFTP están definidas por `WIS2BOX_STORAGE_USERNAME` y `WIS2BOX_STORAGE_PASSWORD` en tu archivo `wis2box.env` y son las mismas credenciales que usaste para conectarte a la interfaz de usuario de MinIO.
...
!!! question "Cargar datos usando SFTP"
    ...
    Verifica el panel de Grafana y MQTT Explorer para comprobar si los datos se ingirieron y publicaron correctamente.
