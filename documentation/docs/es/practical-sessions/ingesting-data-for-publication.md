...
!!! abstract "Resultados de Aprendizaje"
    ...
    - Activar el flujo de trabajo de wis2box subiendo datos a MinIO utilizando la línea de comandos, la interfaz web de MinIO, SFTP o un script de Python.
...
Esta sección utiliza el conjunto de datos para "surface-based-observations/synop" creado previamente en la sesión práctica [Configurando Conjuntos de Datos en wis2box](/practical-sessions/configuring-wis2box-datasets). También requiere conocimiento sobre la configuración de estaciones en **wis2box-webapp**, como se describe en la sesión práctica [Configurando Metadatos de Estación](/practical-sessions/configuring-station-metadata).
...
Asegúrate de que MQTT Explorer esté funcionando y conectado a tu instancia usando las credenciales públicas `everyone/everyone` con una suscripción al tema `origin/a/wis2/#`.
...
!!! question "Ingestando Datos Usando `wis2box data ingest`"
    ...
    ¿Fue exitosa la ingestión de datos? Si no, ¿cuál fue el mensaje de error y cómo puedes solucionarlo?

??? success "Haz Clic para Revelar la Respuesta"

    La ingestión de datos **no** fue exitosa. Deberías ver lo siguiente:
...
!!! question "Verifica el Estado de la Ingestión de Datos en Grafana"
    ...
    ¿Cómo puedes saber si los datos fueron ingestados y publicados exitosamente?

??? success "Haz Clic para Revelar la Respuesta"
    
    Si la ingestión de datos fue exitosa, deberías ver lo siguiente:
...
## Subiendo Datos Usando la Interfaz Web de MinIO
...
A continuación, utilizaremos la interfaz web de MinIO, que permite descargar y subir datos a MinIO utilizando un navegador web.

!!! question "Volver a subir datos utilizando la interfaz web de MinIO"

    Accede a la interfaz web de MinIO en tu navegador y navega hasta el bucket `wis2box-incoming`. Verás el archivo `synop_202412030900.txt` que subiste en los ejercicios anteriores.
    ...
    Revisa el tablero de Grafana y MQTT Explorer para ver si los datos fueron ingestados y publicados con éxito.
...
??? success "Haz clic para revelar la respuesta"
    ...
    Esto demuestra que el flujo de trabajo de datos fue activado, pero los datos no fueron republicados. El wis2box no publicará los mismos datos dos veces. 
...
!!! question "Subir nuevos datos utilizando la interfaz web de MinIO"
    ...
    ¿Fueron los datos ingestados y publicados con éxito?

??? success "Haz clic para revelar la respuesta"

    Ve al tablero de Grafana y verifica si los datos fueron ingestados y publicados con éxito.

    Si usas un camino incorrecto, verás un mensaje de error en los registros.

    Si usas el camino correcto, verás una notificación más de datos WIS2 publicada para la estación de prueba `0-20000-0-64400`, indicando que los datos fueron ingestados y publicados con éxito.

    <img alt="grafana_data_ingest" src="../../assets/img/grafana_data-ingest-test2.png" width="400"> 

## Subiendo datos utilizando SFTP

El servicio de MinIO en wis2box también puede ser accedido a través de SFTP. El servidor SFTP para MinIO está vinculado al puerto 8022 en el host (el puerto 22 se utiliza para SSH).

En este ejercicio, demostraremos cómo usar WinSCP para subir datos a MinIO utilizando SFTP.

Puedes configurar una nueva conexión de WinSCP como se muestra en esta captura de pantalla:

<img alt="winscp-sftp-connection" src="../../assets/img/winscp-sftp-login.png" width="400">

Las credenciales para la conexión SFTP están definidas por `WIS2BOX_STORAGE_USERNAME` y `WIS2BOX_STORAGE_PASSWORD` en tu archivo `wis2box.env` y son las mismas que utilizaste para conectarte a la UI de MinIO.
...
!!! question "Subir datos utilizando SFTP"
...
??? success "Haz clic para revelar la respuesta"
    ...
    Deberías ver una nueva notificación de datos WIS2 publicada para la estación de prueba `0-20000-0-64400`, indicando que los datos fueron ingestados y publicados con éxito.

    <img alt="grafana_data_ingest" src="../../assets/img/grafana_data-ingest-test3.png" width="400"> 

    Si usas un camino incorrecto, verás un mensaje de error en los registros.

## Subiendo datos utilizando un script de Python

En este ejercicio, utilizaremos el cliente de Python de MinIO para copiar datos en MinIO.

MinIO proporciona un cliente de Python, que puede ser instalado de la siguiente manera:
...
En el directorio `exercise-materials/data-ingest-exercises`, encontrarás un script de ejemplo `copy_file_to_incoming.py` que puede ser utilizado para copiar archivos en MinIO.
...
El script necesita conocer el punto final correcto para acceder a MinIO en tu wis2box. Si wis2box está ejecutándose en tu host, el punto final de MinIO está disponible en `http://YOUR-HOST:9000`. El script también necesita ser actualizado con tu contraseña de almacenamiento y el camino en el bucket de MinIO para almacenar los datos.

!!! question "Actualizar el script e ingerir los datos CSV"
    
    Edita el script `copy_file_to_incoming.py` para abordar los errores, utilizando uno de los siguientes métodos:
    - Desde la línea de comandos: usa el editor de texto `nano` o `vim` para editar el script.
    - Utilizando WinSCP: inicia una nueva conexión utilizando el Protocolo de Archivos `SCP` y las mismas credenciales que tu cliente SSH. Navega al directorio `wis2box-data/data-ingest-exercises` y edita `copy_file_to_incoming.py` utilizando el editor de texto integrado.
    ...
    - Proporcionar el camino correcto en el bucket de MinIO para almacenar los datos.
    ...
    Asegúrate de que los errores estén resueltos.
...
También puedes revisar el tablero de Grafana para ver si los datos fueron ingestados y publicados con éxito.

Ahora que el script está funcionando, puedes intentar copiar otros archivos en MinIO utilizando el mismo script.

!!! question "Ingerir datos binarios en formato BUFR"
...
Revisa el tablero de Grafana y MQTT Explorer para ver si los datos de prueba fueron ingestados y publicados con éxito. Si ves algún error, intenta resolverlo.

!!! question "Verificar la ingestión de datos"
...
??? success "Haz clic para revelar la respuesta"
    ...
    Si todas las estaciones utilizadas en el archivo BUFR están definidas en tu instancia de wis2box, deberías ver 10 mensajes publicados al broker MQTT. Cada notificación corresponde a datos para una estación por cada marca de tiempo de observación.
...
!!! success "¡Felicidades!"
    ...
    - Depurar errores comunes en el proceso de ingestión de datos utilizando el tablero de Grafana y los registros de tu instancia de wis2box.
    - Monitorear las notificaciones de datos WIS2 publicadas por tu wis2box en el tablero de Grafana y MQTT Explorer.