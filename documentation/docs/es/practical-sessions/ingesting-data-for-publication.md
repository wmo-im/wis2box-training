...
En esta sección, nos centraremos en cómo subir datos a tu instancia de wis2box y verificar la ingestión y publicación exitosa. La transformación de datos se cubrirá más adelante en la sesión práctica [Herramientas de Conversión de Datos](./data-conversion-tools.md).
...
Esta sección asume que has completado con éxito la sesión práctica [Configurando Conjuntos de Datos en wis2box](./configuring-wis2box-datasets.md). Si seguiste las instrucciones de esa sesión, deberías tener un conjunto de datos que utiliza el plugin `Universal` y otro que utiliza el plugin `FM-12 data converted to BUFR`.
...
??? success "Haz clic para Revelar la Respuesta"
    ...
    Puedes verificar el panel de Grafana para ver si los datos se ingresaron y publicaron con éxito.
...
Ingresa al directorio recién creado, haz clic en `Upload` y sube el archivo que usaste previamente, *mydata.nc*, al nuevo directorio. Verifica el panel de Grafana para ver si los datos se ingresaron y publicaron con éxito.
...
Descarga este archivo [GEPS_18August2025.grib2](../sample-data/GEPS_18August2025.grib2) a tu computadora local y súbelo al directorio que creaste para el conjunto de datos de pronóstico. Verifica el panel de Grafana y MQTT Explorer para ver si los datos se ingresaron y publicaron con éxito.
...
Después de solucionar el problema con el nombre del archivo, verifica el panel de Grafana y MQTT Explorer para ver si los datos se ingresaron y publicaron con éxito.
...
Una vez que logres ejecutar el script con éxito, verás un mensaje indicando que el archivo se copió en MinIO, y deberías ver notificaciones de datos publicadas por tu instancia de wis2box en MQTT Explorer.

También puedes revisar el panel de Grafana para verificar si los datos se ingirieron y publicaron correctamente.
...
Revisa el panel de Grafana y MQTT Explorer para verificar si los datos de prueba se ingirieron y publicaron correctamente. Si ves algún error, intenta resolverlo.
...
El servicio MinIO en wis2box también se puede acceder a través de SFTP. Si tienes un sistema existente que puede configurarse para reenviar datos mediante SFTP, puedes usar esto como un método alternativo para automatizar la ingesta de tus datos.
