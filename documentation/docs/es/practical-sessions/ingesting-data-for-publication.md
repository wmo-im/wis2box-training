...
Esta sección asume que has completado exitosamente la sesión práctica [Configuración de Conjuntos de Datos en wis2box](./configuring-wis2box-datasets.md). Si seguiste las instrucciones en esa sesión, deberías tener un conjunto de datos que utiliza el plugin `Universal` y otro que utiliza el plugin `FM-12 data converted to BUFR`.
...
Para iniciar sesión, necesitas usar las credenciales definidas por WIS2BOX_STORAGE_USERNAME y WIS2BOX_STORAGE_PASSWORD en el archivo wis2box.env. Puedes verificar los valores de estas variables ejecutando los siguientes comandos en tu máquina virtual de estudiante:
...
- *wis2box-public*: Este es el bucket donde wis2box publica datos que han sido procesados e ingeridos exitosamente.
...
Entra al directorio recién creado, haz clic en `Upload` y sube el archivo que usaste previamente, *mydata.nc*, al nuevo directorio. Verifica el panel de Grafana para ver si los datos fueron ingeridos y publicados exitosamente.
...
??? success "Haz clic para revelar la respuesta"
    ...
    El conjunto de datos fue configurado para procesar únicamente archivos con la extensión `.grib2`. La configuración de Extensión de Archivo es parte de los mapeos de datos que definiste en la sesión práctica anterior.

Descarga este archivo [GEPS_18August2025.grib2](../sample-data/GEPS_18August2025.grib2) a tu computadora local y súbelo al directorio que creaste para el conjunto de datos de pronóstico. Verifica el panel de Grafana y MQTT Explorer para ver si los datos fueron ingeridos y publicados exitosamente.
...
!!! note
    ...
    Obtendrás un error porque el script no está configurado para acceder al endpoint de MinIO en tu instancia de wis2box todavía.
...
También puedes revisar el panel de Grafana para verificar si los datos fueron ingeridos y publicados exitosamente.

Ahora que el script funciona, puedes intentar copiar otros archivos a MinIO utilizando el mismo script.
...
Revisa el panel de Grafana y MQTT Explorer para verificar si los datos de prueba fueron ingeridos y publicados exitosamente. Si ves algún error, intenta resolverlo.
...
??? success "Haz clic para revelar la respuesta"
    ...
    Si todas las estaciones utilizadas en el archivo BUFR están definidas en tu instancia de wis2box, deberías ver 10 mensajes publicados al broker MQTT. Cada notificación corresponde a datos de una estación para un timestamp de observación.
...
!!! question "Cargar datos usando SFTP"
    ...
    Luego, súbelo a la ruta del conjunto de datos de entrada en MinIO utilizando tu sesión SFTP en WinSCP.
