...
Los datos se comparten en WIS2 de acuerdo con la Política de Datos Unificada de la OMM, que describe dos categorías de datos:
...
- no son almacenados en caché por los Servicios de Caché Global de WIS2
...
El editor de conjuntos de datos en el wis2box-webapp requerirá que proporciones una URL de licencia cuando selecciones la política de datos 'recommended'. Opcionalmente, puedes agregar un token de acceso al conjunto de datos para restringir el acceso a los datos.

En esta sesión práctica, crearás un nuevo conjunto de datos con la política de datos 'recommended' y aprenderás cómo agregar control de acceso.

Esta sesión también te guiará a través de los pasos para agregar un archivo de licencia personalizado a tu instancia de wis2box.
...
Ve a la página 'dataset editor' en el wis2box-webapp y crea un nuevo conjunto de datos. Selecciona el Data Type = 'weather/surface-weather-observations/synop'.
...
Cambia la política de datos de la OMM a 'recommended' y verás que el formulario agregó un nuevo campo de entrada para una URL que proporcione la información de la licencia para el conjunto de datos:

<img alt="create-dataset-license" src="/../assets/img/create-dataset-license.png" width="800">

Tienes la opción de proporcionar una URL a una licencia que describa los términos de uso del conjunto de datos. Por ejemplo, usa `https://creativecommons.org/licenses/by/4.0/` para apuntar a la licencia Creative Commons Attribution 4.0 International (CC BY 4.0).

O `WIS2BOX_URL/data/license.txt` para apuntar a un archivo de licencia personalizado que hayas alojado en tu propio servidor web, donde `WIS2BOX_URL` es la URL que definiste en el archivo `wis2box.env`:

<img alt="create-dataset-license-url" src="/../assets/img/create-dataset-license-custom.png" width="800">

Continúa completando los campos requeridos para las Propiedades Espaciales y la Información de Contacto. Haz clic en 'Validate form' para verificar si hay errores.
...
Si el tiempo lo permite, puedes agregar un archivo de licencia personalizado a tu instancia de wis2box al final de esta sesión práctica. Primero, continuaremos agregando un token de acceso al conjunto de datos.
...
A continuación, ve al editor de estaciones en el wis2box-webapp. Para cada estación que usaste en `aws-example.csv`, actualiza el campo 'topic' para que coincida con el 'topic' del conjunto de datos que creaste en el ejercicio anterior.

Esta estación ahora estará asociada a 2 topics, uno para el conjunto de datos 'core' y otro para el conjunto de datos 'recommended':
...
*Este es un archivo de licencia personalizado para el conjunto de datos recomendado con control de acceso. Puedes usar estos datos libremente, pero por favor reconoce al proveedor de los datos.*

Para subir un archivo local llamado `license.txt`, utiliza la Consola MinIO disponible en el puerto 9001 de la instancia de wis2box, accediendo a un navegador web y visitando `http://YOUR-HOST:9001`.
