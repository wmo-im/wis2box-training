...
En esta sesión práctica, crearás un nuevo conjunto de datos con la política de datos 'recommended' utilizando el editor de conjuntos de datos en wis2box-webapp. También aprenderás cómo proporcionar una licencia autohospedada y cómo agregar opcionalmente un control de acceso.
...
!!! note "Acerca de la URL de la licencia"
    ...
    Para este ejercicio, utilizaremos un archivo de licencia autohospedado. Agregarás el archivo 'aviation-license.html' a tu instancia de wis2box más adelante en esta sesión práctica para asegurarte de que la URL de la licencia sea válida.
...
Para ingerir el archivo en tu conjunto de datos, ve al bucket **wis2box-incoming** y crea una nueva carpeta con el nombre del identificador de metadatos de tu conjunto de datos. Sube el archivo de datos METAR de ejemplo a esta carpeta utilizando el botón “Upload”:
...
Copia y pega el enlace canónico desde el mensaje de notificación de WIS2 en tu navegador web. La URL debería verse similar a esta:
...
!!! note "El control de acceso es opcional, solo para conjuntos de datos recomendados"
    ...
    Si aplicas `wis2box auth add-token` a un conjunto de datos con la política de datos de la OMM 'core', las Global Caches informarán un error al Global Monitoring, ya que no pueden descargar los datos y tu conjunto de datos será considerado *no conforme* con las Regulaciones Técnicas de WIS2.
