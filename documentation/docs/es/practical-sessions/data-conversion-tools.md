...
Las principales conversiones actualmente soportadas por wis2box son los informes FM-12 SYNOP a BUFR y los datos CSV a BUFR. Los datos FM-12 son compatibles ya que todavía se utilizan ampliamente y se intercambian en la comunidad de la OMM, mientras que los datos CSV son compatibles para permitir el mapeo de datos producidos por estaciones meteorológicas automáticas al formato BUFR.

### Sobre FM-12 SYNOP

Los informes meteorológicos de estaciones superficiales terrestres históricamente se han reportado cada hora o en las horas principales (00, 06, 12 y 18 UTC) y las horas intermedias (03, 09, 15, 21 UTC) sinópticas. Antes de la migración a BUFR, estos informes se codificaban en el formato de texto plano FM-12 SYNOP. Aunque la migración a BUFR estaba programada para completarse en 2012, todavía se intercambia un gran número de informes en el formato heredado FM-12 SYNOP. Más información sobre el formato FM-12 SYNOP se puede encontrar en el Manual de Códigos de la OMM, Volumen I.1 (OMM-No. 306, Volumen I.1).
...
- **synop2bufr**: para soportar el formato heredado FM-12 SYNOP tradicionalmente utilizado por observadores manuales. El módulo synop2bufr depende de metadatos adicionales de estaciones para codificar parámetros adicionales en el archivo BUFR. Consulta el [repositorio synop2bufr en GitHub](https://github.com/World-Meteorological-Organization/synop2bufr)
...
Estos módulos pueden ser utilizados de forma independiente o como parte del stack wis2box.
...
Primero, conéctate a tu máquina virtual de estudiante a través de tu cliente SSH y copia los materiales del ejercicio a un directorio accesible por un usuario que no sea root dentro del contenedor wis2box-api (por ejemplo, /tmp):

```bash
docker cp ~/exercise-materials/data-conversion-exercises wis2box-api:/tmp
```

Los archivos copiados con docker cp pueden no ser modificables por el usuario wis2box-api.
Ejecuta el siguiente comando como root para actualizar la propiedad:
...
Ahora inicia sesión nuevamente como usuario regular y cambia al directorio del ejercicio:
...
En este punto, el directorio es modificable y está listo para los ejercicios.
...
Cuando examinamos un archivo BUFR, a menudo queremos determinar el tipo de datos contenidos en el archivo y la fecha/hora típica de los datos en el archivo. Esta información puede listarse usando el flag `-p` para seleccionar los encabezados a imprimir. Se pueden incluir múltiples encabezados utilizando una lista separada por comas.
...
!!! question
    ...
    Ejecuta el comando anterior e interpreta la salida usando [Common Code Table C-13](https://github.com/wmo-im/CCT/blob/master/C13.csv) para determinar la categoría y subcategoría de datos.
...
- La categoría de datos es 2, indicando datos de **"Sondeos verticales (distintos de satélites)"**.
- La subcategoría internacional es 4, indicando datos de **"Informes de temperatura/humedad/viento en niveles superiores desde estaciones terrestres fijas (TEMP)"**.
- La fecha y hora típicas son 2023-10-02 y 00:00:00z, respectivamente.

### bufr_dump

El comando `bufr_dump` puede ser utilizado para listar y examinar el contenido de un archivo BUFR, incluyendo los datos en sí.
...
Esto genera un JSON que puede ser difícil de analizar. Intenta usar la opción `-p` para mostrar los datos en texto plano (formato clave=valor):
...
A continuación, veamos cómo convertir datos FM-12 SYNOP al formato BUFR usando el módulo `synop2bufr`. El módulo `synop2bufr` se utiliza para convertir datos FM-12 SYNOP al formato BUFR. El módulo está instalado en el contenedor wis2box-api y puede ser utilizado desde la línea de comandos de la siguiente manera:
...
El argumento `--metadata` se utiliza para especificar el archivo de metadatos de la estación, que proporciona información adicional para ser codificada en el archivo BUFR. El argumento `--output-dir` se utiliza para especificar el directorio donde se escribirán los archivos BUFR convertidos. Los argumentos `--year` y `--month` se utilizan para especificar el año y el mes de la observación.

El módulo `synop2bufr` también se utiliza en la aplicación web wis2box-webapp para convertir datos FM-12 SYNOP al formato BUFR utilizando un formulario de entrada basado en la web.
...
El argumento `--metadata` requiere un archivo CSV usando un formato predefinido. Un ejemplo funcional se proporciona en el archivo `station_list.csv`:
...
!!! question
    ¿Cuántos archivos BUFR fueron creados? ¿Qué significa el mensaje de ADVERTENCIA en la salida?
...
??? success "Haz clic para revelar la respuesta"
    ...
    El mensaje de advertencia indica que la estación con el identificador tradicional de estación `15090` no fue encontrada en el archivo de lista de estaciones `station_list.csv`. Esto significa que el informe SYNOP para esta estación no fue convertido al formato BUFR.
...
El módulo `synop2bufr` también se utiliza en el `wis2box-webapp` para convertir datos FM-12 SYNOP al formato BUFR mediante un formulario de entrada basado en la web.  
Para probar esto, accede a `http://YOUR-HOST/wis2box-webapp` e inicia sesión.

Selecciona el `SYNOP Form` del menú a la izquierda y copia el contenido del archivo `synop_message.txt`:
...
!!! note
    ...
    En el ejercicio para [ingesting-data-for-publication](./ingesting-data-for-publication.md) ingresaste el archivo "synop_202412030900.txt" y fue convertido al formato BUFR por el módulo synop2bufr.

    En el flujo de trabajo automatizado en el wis2box, el año y el mes se extraen automáticamente del nombre del archivo y se utilizan para completar los argumentos `--year` y `--month` requeridos por synop2bufr, mientras que los metadatos de la estación se extraen automáticamente de la configuración de estaciones en el wis2box.
...
!!! note
    Asegúrate de que sigues conectado al contenedor `wis2box-api` y en el directorio `/tmp/data-conversion-exercises`. Si saliste del contenedor en el ejercicio anterior, puedes iniciar sesión nuevamente de la siguiente manera:
...
Ahora veamos cómo convertir datos CSV al formato BUFR utilizando el módulo `csv2bufr`. Este módulo está instalado en el contenedor `wis2box-api` y puede utilizarse desde la línea de comandos de la siguiente manera:
...
El argumento `--bufr-template` se utiliza para especificar el archivo de plantilla de mapeo BUFR, que proporciona el mapeo entre los datos de entrada en formato CSV y los datos de salida en formato BUFR, especificado en un archivo JSON. Las plantillas de mapeo predeterminadas están instaladas en el directorio `/opt/csv2bufr/templates` en el contenedor `wis2box-api`.
...
Los valores mínimo y máximo para esta clave son `193.15` y `333.15`, respectivamente.

!!! question

    ¿Qué columna CSV está asignada a la clave eccodes `internationalDataSubCategory`? ¿Cuál es el valor de esta clave?
...
??? success "Haz clic para revelar la respuesta"
    ...
    No obtienes ningún resultado, lo que indica que el valor para la clave `airTemperature` está ausente en el archivo BUFR `WIGOS_0-20000-0-60355_20240331T020000.bufr4`. El csv2bufr se negó a codificar el valor `25.0` de los datos CSV ya que está fuera del rango válido de `193.15` y `333.15` definido en la plantilla de mapeo.
...
Para obtener información sobre cómo crear y usar plantillas de mapeo BUFR personalizadas, consulta el siguiente ejercicio práctico [csv2bufr-templates](./csv2bufr-templates.md).
