...
!!! abstract "Resultados de aprendizaje"
    ...
    - ejecutar los notebooks de Jupyter de ejemplo para decodificar datos en los formatos GRIB2, NetCDF y BUFR
...
- GRIB2 que contiene datos para una Predicción Global por Conjunto realizada por el Sistema de Predicción Global Regional de Asimilación CMA (GRAPES)
- BUFR que contiene datos de seguimiento de ciclones tropicales del sistema de pronóstico por conjuntos del ECMWF
...
Este contenedor iniciará un servidor de notebooks de Jupyter en tu instancia, que incluye la biblioteca [ecCodes](https://sites.ecmwf.int/docs/eccodes) que puedes usar para decodificar datos BUFR.
...
- `-v ~/wis2-downloads:/root/downloads` monta el directorio `~/wis2-downloads` en tu VM en `/root/downloads` dentro del contenedor. Aquí es donde se almacenarán los datos que descargaste desde WIS2 después de seguir las instrucciones de la sesión práctica anterior sobre el Descargador de WIS2.
- `-p 8888:8888` asigna el puerto 8888 de tu VM al puerto 8888 del contenedor. Esto hace que el servidor de notebooks de Jupyter sea accesible desde tu navegador web en `http://YOUR-HOST:8888`
- `-e JUPYTER_TOKEN=dataismagic!` establece el token requerido para acceder al servidor de notebooks de Jupyter. Necesitarás proporcionar este token cuando accedas al servidor desde tu navegador web
...
!!! note "Acerca de la imagen demo-decode-eccodes-jupyter"
    ...
    La imagen `demo-decode-eccodes-jupyter` fue desarrollada para este entrenamiento y utiliza una imagen base que incluye la biblioteca ecCodes, además de agregar un servidor de notebooks de Jupyter y paquetes de Python para análisis y visualización de datos.

    El código fuente de esta imagen, incluyendo los notebooks de ejemplo, puede encontrarse en [wmo-im/demo-decode-eccodes-jupyter](https://github.com/wmo-im/demo-decode-eccodes-jupyter).
    
Una vez que el contenedor esté iniciado, puedes acceder al servidor de notebooks de Jupyter en tu VM de estudiante navegando a `http://YOUR-HOST:8888` en tu navegador web.

Verás una pantalla solicitando que ingreses una "Contraseña o token".

Proporciona el token `dataismagic!` para iniciar sesión en el servidor de notebooks de Jupyter (a menos que hayas utilizado un token diferente en el comando anterior).
...
Lee las instrucciones en el notebook y ejecuta las celdas para decodificar los datos descargados de la predicción global por conjuntos. Ejecuta cada celda haciendo clic en ella y luego haciendo clic en el botón de ejecución en la barra de herramientas o presionando `Shift+Enter`.
...
![Predicción global por conjuntos de anomalía de temperatura](../assets/img/grib2-global-ensemble-prediction-map.png)
...
??? success "Haz clic para revelar la respuesta"
    ...
    Puedes cambiar esta línea o agregar otra línea para visualizar uno de los otros mensajes en el archivo GRIB2 cambiando el número de mensaje:
...
!!! question 
    ...
    El resultado muestra la probabilidad predicha de la trayectoria de una tormenta tropical dentro de 200 km. ¿Cómo actualizarías el notebook para mostrar la probabilidad predicha de la trayectoria de una tormenta tropical dentro de 300 km?

??? success "Haz clic para revelar la respuesta"

    Para actualizar el notebook y mostrar la probabilidad predicha de la trayectoria de una tormenta tropical dentro de una distancia diferente, puedes actualizar la variable `distance_threshold` en el bloque de código que calcula la probabilidad de impacto.

    Para mostrar la probabilidad predicha de la trayectoria de una tormenta tropical dentro de 300 km:
...
!!! note "Decodificación de datos NetCDF"
    ...
    NetCDF es un formato flexible que, en este ejemplo, reportó los valores para la variable 'anomaly' reportados a lo largo de las dimensiones 'lat', 'lon'. Diferentes conjuntos de datos NetCDF pueden usar nombres de variables y dimensiones diferentes.

## Uso de otras herramientas para visualizar y decodificar formatos binarios de la OMM

Los notebooks de ejemplo demostraron cómo puedes decodificar formatos binarios de la OMM comúnmente utilizados utilizando Python.

También puedes usar otras herramientas para decodificar y visualizar formatos de código impulsados por tablas de la OMM sin necesidad de escribir software, como:

- [Panoply](https://www.giss.nasa.gov/tools/panoply/) - una aplicación multiplataforma que genera gráficos de arrays georreferenciados y otros arrays desde NetCDF, HDF, GRIB y otros conjuntos de datos
