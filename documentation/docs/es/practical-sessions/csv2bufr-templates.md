...
La mayoría de los registradores de datos utilizados para capturar la salida de sensores pueden exportar las observaciones en archivos delimitados, incluidos los CSV. 
...
Si no recuerdas tus credenciales de MinIO, puedes encontrarlas en el archivo `wis2box.env` en el directorio `wis2box` de tu VM de estudiante.
...
Intentemos crear una nueva plantilla de mapeo personalizada utilizando el comando `csv2bufr mapping create` con la secuencia BUFR 301150 y el elemento BUFR 012101 como entrada.
...
La plantilla de mapeo que creaste carece de metadatos importantes sobre la observación realizada, como la fecha y hora de la observación, y la latitud y longitud de la estación.
...
!!! question "Verifica el contenido del archivo BUFR"
    ...
    ¿Cómo puedes verificar el contenido del archivo BUFR que acabas de crear y comprobar que los datos se han codificado correctamente?
...
!!! hint "Datos faltantes"
    Se reconoce que los datos pueden faltar por una variedad de razones, ya sea por fallos en el sensor o porque el parámetro no fue observado. En estos casos, los datos faltantes pueden codificarse según la respuesta anterior, mientras que los otros datos en el informe permanecen válidos.
...
<img alt="Imagen que muestra la interfaz de usuario de MinIO con el botón para crear una nueva carpeta resaltado" src="/../assets/img/minio-create-new-path.png"/>
...
<img alt="Imagen que muestra la interfaz de usuario de MinIO con el botón para crear una nueva carpeta resaltado" src="/../assets/img/minio-create-new-path-metadata_id.png"/>
...
Revisa el panel de Grafana en `http://YOUR-HOST:3000` para ver si hay WARNINGS o ERRORES. Si ves alguno, intenta solucionarlo y repite el ejercicio.
...
??? success "Haz clic para revelar la respuesta"
    La plantilla daycli incluye metadatos importantes sobre la ubicación del instrumento y clasificaciones de calidad de medición para temperatura y humedad, indicadores de control de calidad e información sobre cómo se ha calculado la temperatura promedio diaria.
...
!!! success "¡Felicidades!"
    ...
    - cómo utilizar las plantillas integradas de AWS y DAYCLI para convertir datos CSV a BUFR