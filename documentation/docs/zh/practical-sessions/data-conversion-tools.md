...
发布在 WIS2 上的数据应符合由各地球系统学科/领域专家社区定义的要求和标准。为了降低地面观测数据发布的门槛，`wis2box` 提供了将数据转换为 BUFR 格式的工具。这些工具可以通过 `wis2box-api` 容器从命令行使用，以测试数据转换过程。

目前 `wis2box` 主要支持的转换是 FM-12 SYNOP 报告到 BUFR，以及 CSV 数据到 BUFR。支持 FM-12 数据是因为它在 WMO 社区中仍被广泛使用和交换，而支持 CSV 数据是为了将自动气象站生成的数据映射到 BUFR 格式。

### 关于 FM-12 SYNOP

地面气象站的地面天气报告历史上通常在整点或主要（00、06、12 和 18 UTC）及中间（03、09、15、21 UTC）时段进行报告。在迁移到 BUFR 之前，这些报告以纯文本 FM-12 SYNOP 编码格式进行编码。尽管迁移到 BUFR 的计划应在 2012 年完成，但仍有大量报告以传统的 FM-12 SYNOP 格式进行交换。有关 FM-12 SYNOP 格式的更多信息，请参阅 WMO 编码手册，第 I.1 卷（WMO-No. 306，第 I.1 卷）。
...
`wis2box` 软件在 `wis2box-api` 容器的基础镜像中包含了 `ecCodes` 库。这使用户可以从容器中访问命令行工具和库。在 `wis2box-stack` 中，`ecCodes` 库被用于解码和编码 BUFR 消息。
...
!!! warning "先决条件"
    ...
    - 确保您的 `wis2box` 已配置并启动
    - 确保您已设置一个数据集并在 `wis2box` 中至少配置了一个站点
...
要使用 BUFR 命令行工具，您需要登录到 `wis2box-api` 容器。除非另有说明，所有命令都应在该容器中运行。您还需要打开 `MQTT Explorer` 并连接到您的 broker。
...
在检查 BUFR 文件时，我们通常希望确定文件中包含的数据类型以及数据的典型日期/时间。可以使用 `-p` 标志选择要输出的头信息。多个头信息可以用逗号分隔的列表包含。
...
!!! question
    ...
    BUFR 文件 `bufr-cli-ex2.bufr4` 中报告的海平面气压值是多少？
...
??? success "点击查看答案"
    ...
    ```
    这表明海平面气压值为 105590 Pa（1055.90 hPa）。
...
`--output-dir` 参数用于指定转换后的 BUFR 文件的输出目录。  
...
??? success "点击查看答案"
    ...
    如果您使用 `ls -lh` 检查目录内容，您会看到创建了 2 个新的 BUFR 文件：`WIGOS_0-20000-0-15015_20240921T120000.bufr4` 和 `WIGOS_0-20000-0-15020_20240921T120000.bufr4`。
...
!!! question
    BUFR 文件 `WIGOS_0-20000-0-60355_20240331T010000.bufr4` 中 eccodes 键 `airTemperature` 的值是多少？`WIGOS_0-20000-0-60355_20240331T020000.bufr4` 中的值又是多少？
