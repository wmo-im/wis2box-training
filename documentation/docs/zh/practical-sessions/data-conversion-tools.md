...
!!! abstract "学习成果"
    ...
    - 使用命令行工具 synop2bufr 将 FM-12 SYNOP 报告转换为 BUFR
    - 通过 wis2box-webapp 触发 synop2bufr 转换
    - 使用命令行工具 csv2bufr 将 CSV 数据转换为 BUFR

## 引言

在 WIS2 上发布的数据应满足各个地球系统学科/领域专家社区定义的要求和标准。为了降低陆地表面观测数据发布的门槛，wis2box 提供了将数据转换为 BUFR 格式的工具。这些工具可通过 wis2box-api 容器使用，并可从命令行测试数据转换过程。

wis2box 当前主要支持的转换包括将 FM-12 SYNOP 报告转换为 BUFR 和将 CSV 数据转换为 BUFR。FM-12 数据得到支持，因为它在 WMO 社区中仍然被广泛使用和交换，而 CSV 数据的支持则是为了将自动气象站产生的数据映射到 BUFR 格式。

### 关于 FM-12 SYNOP

陆地表面站的地面天气报告历史上每小时或在主要（00、06、12 和 18 UTC）和中间（03、09、15、21 UTC）天气时报告。在迁移到 BUFR 之前，这些报告是以纯文本 FM-12 SYNOP 代码形式编码的。虽然计划在 2012 年前完成迁移到 BUFR，但大量报告仍以遗留的 FM-12 SYNOP 格式交换。有关 FM-12 SYNOP 格式的更多信息，请参阅 WMO 编码手册，第 I.1 卷（WMO-No. 306, Volume I.1）。

### 关于 ecCodes

ecCodes 库是一套软件库和工具，设计用于解码和编码气象数据，格式包括 GRIB 和 BUFR。它由欧洲中期天气预报中心（ECMWF）开发，有关更多信息，请参阅 [ecCodes 文档](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation)。

wis2box 软件在 wis2box-api 容器的基础镜像中包含了 ecCodes 库。这允许用户从容器内部访问命令行工具和库。ecCodes 库在 wis2box-stack 中用于解码和编码 BUFR 消息。

### 关于 csv2bufr 和 synop2bufr

除了 ecCodes，wis2box 还使用以下 Python 模块与 ecCodes 一起将数据转换为 BUFR 格式：
...
- **csv2bufr**：使自动气象站产生的 CSV 提取转换为 BUFR 格式成为可能。csv2bufr 模块用于将 CSV 数据转换为 BUFR 格式，使用映射模板定义如何将 CSV 数据映射到 BUFR 格式。请参阅 [GitHub 上的 csv2bufr 仓库](https://github.com/World-Meteorological-Organization/csv2bufr)
...
!!! warning "先决条件"
    ...
    - 确保您的 wis2box 已配置并启动
    - 确保您已设置数据集并至少配置了一个站点在您的 wis2box 中
    - 使用 MQTT Explorer 连接到您的 wis2box 实例的 MQTT 代理
    - 打开 wis2box 网页应用（`http://YOUR-HOST/wis2box-webapp`），确保您已登录
...
可以传递各种选项给 `bufr_ls`，以改变打印的格式和头字段。

!!! question
     
    如何使用命令列出上述输出的 JSON 格式？

    您可以运行带有 `-h` 标志的 `bufr_ls` 命令，查看可用选项。
...
打印的输出代表 BUFR 文件中一些头键的值。

单独来看，这些信息不是很有信息量，只提供了有限的文件内容信息。

在检查 BUFR 文件时，我们通常希望确定文件中包含的数据类型以及数据的典型日期/时间。可以使用 `-p` 标志选择要输出的头部，使用逗号分隔列表包含多个头部。
...
!!! question
    ...
    执行上述命令并使用 [公共代码表 C-13](https://github.com/wmo-im/CCT/blob/master/C13.csv) 解释输出，以确定数据类别和子类别。
...
- 数据类别为2，表示**“垂直探测（卫星以外）”**数据。
- 国际子类别为4，表示**“来自固定陆地站的高层温度/湿度/风报告（TEMP）”**数据。
- 典型日期和时间分别为2023-10-02和00:00:00z。

### bufr_dump

`bufr_dump`命令可用于列出和检查BUFR文件的内容，包括数据本身。
...
这将生成一个难以解析的JSON，尝试使用`-p`标志以纯文本（key=value格式）输出数据：

```{.copy}
bufr_dump -p bufr-cli-ex2.bufr4
```

您应该会看到大量的键作为输出，其中许多键缺失。这在真实世界数据中很典型，因为并非所有的eccodes键都填充了报告数据。

您可以使用`grep`命令过滤输出，仅显示未缺失的键。例如，要显示所有未缺失的键，您可以使用以下命令：
...
!!! question
    ...
    `bufr-cli-ex2.bufr4`文件中报告的海平面上的压力是多少？
...
??? success "点击查看答案"
    ...
    ```
    这表明海平面上的压力为105590 Pa（1055.90 hPa）。

!!! question

    报告`bufr-cli-ex2.bufr4`文件数据的站点的WIGOS站点标识符是什么？
...
接下来，让我们看看如何使用`synop2bufr`模块将FM-12 SYNOP数据转换为BUFR格式。`synop2bufr`模块用于将FM-12 SYNOP数据转换为BUFR格式。该模块安装在wis2box-api容器中，可以从命令行如下使用：
...
`--metadata`参数用于指定站点元数据文件，该文件提供要在BUFR文件中编码的附加信息。
`--output-dir`参数用于指定将写入转换后的BUFR文件的目录。`--year`和`--month`参数用于指定观测的年份和月份。

`synop2bufr`模块也用于wis2box-webapp中，通过基于Web的输入表单将FM-12 SYNOP数据转换为BUFR格式。

### 审查示例SYNOP消息

检查此练习的示例SYNOP消息文件`synop_message.txt`：
...
??? success "点击查看答案"
    ...
    文件中有3个SYNOP报告，对应于3个不同的站点（由5位传统站点标识符标识：15015, 15020, 和 15090）。
    注意，每个报告的结束由`=`字符标记。 

### 审查站点列表

`--metadata`参数需要一个使用预定义格式的CSV文件，一个工作示例在文件`station_list.csv`中提供：
...
??? success "点击查看答案"
    ...
    如果您检查目录的内容，使用`ls -lh`命令，您应该会看到创建了2个新的BUFR文件：`WIGOS_0-20000-0-15015_20240921T120000.bufr4`和`WIGOS_0-20000-0-15020_20240921T120000.bufr4`。

    警告消息表明传统站点标识符`15090`的站点在站点列表文件`station_list.csv`中未找到。这意味着这个站点的SYNOP报告未被转换为BUFR格式。

!!! question
    使用`bufr_dump`命令检查BUFR文件`WIGOS_0-20000-0-15015_20240921T120000.bufr4`的内容。

    您能验证`station_list.csv`文件中提供的信息是否存在于BUFR文件中吗？
...
请注意，这包括由 `station_list.csv` 文件提供的数据。
...
从左侧菜单选择 `SYNOP Form` 并复制粘贴 `synop_message.txt` 文件的内容：
...
??? success "点击查看答案"
    ...
    您需要选择一个数据集并提供之前练习中创建的 "processes/wis2box" 的令牌来提交表单。

    如果您提供了无效的令牌，您将看到：
    
    - 结果：未授权，请提供有效的 'processes/wis2box' 令牌

    如果您提供了有效的令牌，您将看到 "WARNINGS: 3"。点击 "WARNINGS" 打开下拉菜单，将显示：

    - 站点 15015 在站点文件中未找到
    - 站点 15020 在站点文件中未找到
    - 站点 15090 在站点文件中未找到

    要将这些数据转换为 BUFR 格式，您需要在您的 wis2box 中配置相应的站点，并确保这些站点与您的数据集的主题相关联。

!!! note

    在 [ingesting-data-for-publication](ingesting-data-for-publication.md) 的练习中，您摄取了文件 "synop_202412030900.txt"，并由 synop2bufr 模块转换为 BUFR 格式。

    在 wis2box 中的自动化工作流程中，年份和月份会自动从文件名中提取，并用来填充 synop2bufr 所需的 `--year` 和 `--month` 参数，同时站点元数据会自动从 wis2box 中的站点配置中提取。

## csv2bufr 转换

!!! note
    确保您仍然登录在 wis2box-api 容器中，并位于目录 `/root/data-conversion-exercises`，如果您在上一个练习中退出了容器，您可以按以下方式重新登录：
...
现在让我们看看如何使用 `csv2bufr` 模块将 CSV 数据转换为 BUFR 格式。该模块安装在 wis2box-api 容器中，可以通过命令行使用，如下所示：
...
`--bufr-template` 参数用于指定 BUFR 映射模板文件，该文件提供输入 CSV 数据与输出 BUFR 数据之间的映射，指定在 JSON 文件中。默认的映射模板安装在 wis2box-api 容器的目录 `/opt/csv2bufr/templates` 中。
...
!!! question
    CSV 文件中有多少行数据？报告 CSV 文件中的 WIGOS 站点标识符是什么？
...
这将返回一个大型 JSON 文件，提供了 43 个 CSV 列的映射。
...
??? success "点击查看答案"
    ...
    此键的最小值和最大值分别是 `193.15` 和 `333.15`。

!!! question
```

```markdown
CSV列中哪一列映射到了eccodes键`internationalDataSubCategory`？这个键的值是什么？
...
??? success "点击查看答案"
    ...
    **没有CSV列映射到eccodes键`internationalDataSubCategory`**，而是使用常数值2，并将在使用此映射模板生成的所有BUFR文件中编码。

### 将CSV转换为BUFR

让我们尝试使用`csv2bufr`命令将文件转换为BUFR格式：
...
??? success "点击查看答案"
    ...
    输出表明创建了3个BUFR文件：`WIGOS_0-20000-0-60355_20240331T010000.bufr4`、`WIGOS_0-20000-0-60355_20240331T020000.bufr4`和`WIGOS_0-20000-0-60355_20240331T030000.bufr4`。

要检查BUFR文件的内容，同时忽略缺失值，可以使用以下命令：
...
!!! question
    BUFR文件`WIGOS_0-20000-0-60355_20240331T010000.bufr4`中eccodes键`airTemperature`的值是多少？在BUFR文件`WIGOS_0-20000-0-60355_20240331T020000.bufr4`中呢？
...
??? success "点击查看答案"
    ...
    你得到的结果为空，表明在BUFR文件`WIGOS_0-20000-0-60355_20240331T020000.bufr4`中键`airTemperature`的值缺失。csv2bufr拒绝编码CSV数据中的值`25.0`，因为它超出了映射模板中定义的有效范围`193.15`和`333.15`。
...
!!! success "恭喜！"
    ...
    - 如何在wis2box-api容器中访问ecCodes命令行工具
