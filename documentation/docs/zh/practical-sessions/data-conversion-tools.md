...
发布在 WIS2 上的数据应符合由各地球系统学科/领域专家社区定义的要求和标准。为了降低发布陆地表面观测数据的门槛，`wis2box` 提供了将数据转换为 BUFR 格式的工具。这些工具可通过 `wis2box-api` 容器使用，并可从命令行测试数据转换过程。

目前 `wis2box` 支持的主要转换包括 FM-12 SYNOP 报告到 BUFR 和 CSV 数据到 BUFR。支持 FM-12 数据是因为它在 WMO 社区中仍被广泛使用和交换，而支持 CSV 数据是为了将自动气象站生成的数据映射到 BUFR 格式。

### 关于 FM-12 SYNOP

陆地表面站的地面天气报告历史上每小时或在主要（00、06、12 和 18 UTC）及中间（03、09、15 和 21 UTC）天气时段报告。在迁移到 BUFR 之前，这些报告以纯文本 FM-12 SYNOP 编码形式编码。尽管计划在 2012 年完成向 BUFR 的迁移，但仍有大量报告以传统的 FM-12 SYNOP 格式交换。有关 FM-12 SYNOP 格式的更多信息，请参阅 WMO 编码手册，第 I.1 卷（WMO-No. 306，第 I.1 卷）。

### 关于 ecCodes

`ecCodes` 库是一组软件库和工具，用于解码和编码 GRIB 和 BUFR 格式的气象数据。它由欧洲中期天气预报中心（ECMWF）开发，更多信息请参阅 [ecCodes 文档](https://confluence.ecmwf.int/display/ECC/ecCodes+documentation)。
...
在检查 BUFR 文件时，我们通常希望确定文件中包含的数据类型以及文件中数据的典型日期/时间。可以使用 `-p` 标志选择要输出的头信息。多个头信息可以通过逗号分隔的列表包含。
...
`synop2bufr` 模块也可以在 wis2box-webapp 中通过基于 Web 的输入表单将 FM-12 SYNOP 数据转换为 BUFR 格式。
...
??? success "点击查看答案"
    ...
    您需要选择一个数据集并提供在前一练习中创建的 "processes/wis2box" 的令牌以提交表单。
...
!!! note
    确保您仍然登录到 `wis2box-api` 容器中，并位于目录 `/wis2box-api/data-conversion-exercises` 中。如果您在前一练习中退出了容器，可以通过以下方式重新登录：
...
??? question "点击查看答案"
    ...
    在标题行之后，有 3 行数据，分别表示来自同一站点的 3 次气象观测，WIGOS 站点标识符为 `0-20000-0-60355`，时间戳分别为 `2024-03-31 01:00:00`、`2024-03-31 02:00:00` 和 `2024-03-31 03:00:00`。
...
??? success "点击查看答案"
    ...
    将被编码为 eccodes 键 `airTemperature` 的值来自于 CSV 列：**air_temperature**。
...
```{.copy}
    ...
    **没有任何 CSV 列映射到 eccodes 键 `internationalDataSubCategory`**，而是使用常量值 2，并将在使用此映射模板生成的所有 BUFR 文件中进行编码。
