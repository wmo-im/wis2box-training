...
大多数用于记录传感器输出的数据记录仪都能够以分隔文件（包括 CSV 格式）导出观测数据。  
同样，当数据被导入数据库时，也可以很容易地以 CSV 格式的文件导出所需的数据。
...
如果您不记得 MinIO 的凭据，可以在学生虚拟机的 `wis2box` 目录下的 `wis2box.env` 文件中找到它们。
...
让我们尝试使用 `csv2bufr mapping create` 命令创建一个新的自定义映射模板，输入 BUFR 序列为 301150 和 BUFR 元素为 012101：
...
您创建的映射模板缺少有关观测的关键元数据，例如观测的日期和时间，以及站点的纬度和经度。
...
??? success "点击查看答案"
    ...
    在输出中，您将看到映射模板中映射的 BUFR 元素的值，例如 "airTemperature" 将显示为：
...
您现在可以在 wis2box-webapp 中配置您的数据集，以便使用 CSV 到 BUFR 转换插件的自定义映射模板。

wis2box-webapp 将自动检测您创建的映射模板，并将其列入 CSV 到 BUFR 转换插件的模板列表中。

点击您在之前实践课程中创建的数据集，然后点击插件名称为 "CSV data converted to BUFR" 旁边的 "UPDATE"：
...
'AWS' 模板提供了将 CSV 数据转换为 BUFR 序列 301150 和 307096 的映射模板，以支持最低 GBON 要求。
...
??? success "点击查看答案"
    每一列包含一条单独的信息。例如，日期被分为年、月和日，这与数据在 BUFR 中的存储方式相对应。今天的日期将分布在 "year"、"month" 和 "day" 列中。同样，时间需要分为 "hour" 和 "minute"，WIGOS 站点标识符需要分为其各自的组成部分。
...
!!! hint "缺失数据"
    我们认识到，由于传感器故障或参数未被观测等多种原因，数据可能会缺失。在这些情况下，缺失数据可以按照上述答案进行编码，报告中的其他数据仍然有效。
...
在 MinIO 存储桶中创建一个新文件夹，该文件夹与您使用模板='weather/surface-weather-observations/synop' 创建的数据集的 dataset-id 相匹配：
...
检查 Grafana 仪表板 `http://YOUR-HOST:3000`，查看是否有任何 WARNINGS 或 ERRORS。如果有，请尝试修复它们并重复练习。
...
**DayCLI** 模板提供了将每日气候 CSV 数据转换为 BUFR 序列 307075 的映射模板，以支持每日气候数据的报告。

DAYCLI 模板的描述可以在此处找到 [daycli-template](./../csv2bufr-templates/daycli-template.md)。

为了在 WIS2 上共享此数据，您需要在 wis2box-webapp 中创建一个新的数据集，该数据集具有正确的 WIS2 主题层级，并使用 DAYCLI 模板将 CSV 数据转换为 BUFR。

### 为发布 DAYCLI 消息创建 wis2box 数据集

进入 wis2box-webapp 的数据集编辑器并创建一个新数据集。使用与之前实践课程相同的 centre-id，并选择 **Data Type='climate/surface-based-observations/daily'**：
...
??? success "点击查看答案"
    daycli 模板包含有关温度和湿度的仪器位置和测量质量分类的重要元数据、质量控制标志以及有关如何计算每日平均温度的信息。
