...
**发现元数据**用于创建一个 WMO Core Metadata Profile (WCMP2) 记录，该记录通过在您的 wis2box broker 上发布的 WIS2 通知进行共享。

**数据映射**用于将数据插件与您的输入数据关联，以便在数据发布到 WIS2 之前对其进行转换。

在本次实践课程中，您将学习如何使用 **wis2box-webapp 数据集编辑器** 创建和配置数据集。

!!! note "不使用 wis2box-webapp 配置数据集"

    wis2box 还支持使用 [metadata control file (MCF)](https://geopython.github.io/pygeometa/reference/mcf) 格式配置数据集，该格式由 [pygeometa](https://geopython.github.io/pygeometa) 工具定义。
...
!!! Note "Template"
    ...
    *Template* 字段允许您从 wis2box-webapp 数据集编辑器中提供的模板列表中进行选择。模板会根据数据类型预填充表单中的建议默认值。这包括元数据的建议标题和关键字以及预配置的数据插件。
    ...
    主题会自动设置为与所选模板关联的默认主题，除非您选择 *other*。如果您选择 *other*，则可以从基于 [WIS2 Topic Hierarchy](https://codes.wmo.int/wis/topic-hierarchy/_earth-system-discipline) 的下拉列表中定义主题。
