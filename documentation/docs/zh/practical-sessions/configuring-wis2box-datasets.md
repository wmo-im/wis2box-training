...
发现元数据用于创建一个 WCMP2 (WMO Core Metadata Profile 2) 记录，该记录通过在您的 wis2box-broker 上发布的 WIS2 通知进行共享。

数据映射用于将数据插件与您的输入数据关联，从而在通过 WIS2 通知发布之前对数据进行转换。

本课程将引导您通过默认模板和自定义模板创建新数据集，创建发现元数据，并配置数据映射。您将通过 wis2box-api 检查您的数据集，并审核发现元数据的 WIS2 通知。

## 准备工作

使用 MQTT Explorer 连接到您的 broker。

请使用公共凭据 `everyone/everyone`，而不是您的内部 broker 凭据：
...
要创建授权令牌，请通过 SSH 访问您的训练虚拟机，并使用以下命令：
...
创建令牌后，您可以退出 wis2box-management 容器：
...
在您的 wis2box 实例中，导航到 wis2box-webapp 的 'dataset editor' 页面，访问 `http://YOUR-HOST/wis2box-webapp` 并从左侧菜单中选择 'dataset editor'。

在 'dataset editor' 页面下的 'Datasets' 标签中，点击 "Create New ...":
...
!!! Note "Centre ID"
    ...
    您的 Centre ID 应以您国家的顶级域名 (TLD) 开头，后接一个短横线 (`-`) 和您组织的缩写名称（例如 `fr-meteofrance`）。Centre ID 必须为小写，仅使用字母数字字符。下拉列表显示了 WIS2 上当前注册的所有 Centre ID 以及您已在 wis2box 中创建的任何 Centre ID。请选择适合您组织的 Centre ID。

!!! Note "Template"

    *Template* 字段允许您从 wis2box-webapp 数据集编辑器中提供的模板列表中进行选择。模板将使用适合数据类型的建议默认值预填充表单。这包括元数据的建议标题和关键词以及预配置的数据插件。
    
    主题会自动设置为与所选模板关联的默认主题，除非您选择 *other*。如果选择 *other*，则可以从基于 [WIS2 Topic Hierarchy](https://codes.wmo.int/wis/topic-hierarchy/_earth-system-discipline) 的下拉列表中定义主题。

在本次培训中，您将创建两个数据集：
    
- 使用 Template=*weather/surface-based-observations/synop* 的数据集，其中包含在发布前将数据转换为 BUFR 格式的数据插件。
- 使用 Template=*other* 的数据集，您需要定义 WIS2 Topic，并使用 "Universal" 插件直接发布数据而不进行转换。
...
检查标题和关键词，根据需要更新，并为数据集提供描述。

注意，您可以选择将 'WMO Data Policy' 从 'core' 更改为 'recommended'，或者修改默认的 Metadata Identifier。请保持数据策略为 'core'，并使用默认的 Metadata Identifier。
...
如果有错误，请更正并再次点击 'VALIDATE FORM'。

确保没有错误，并且您看到一个弹出窗口，表明表单已验证成功：
...
由于您使用模板创建了数据集，因此数据集映射已使用 'weather/surface-based-observations/synop' 模板的默认插件预填充。数据插件用于在通过 WIS2 通知发布之前在 wis2box 中转换数据。

<img alt="Data Mappings: update plugin" src="/../assets/img/wis2box-data-mappings.png" width="800">

请注意，您可以点击 "Update" 按钮更改插件的设置，例如文件扩展名和文件模式。现在可以保留默认设置。

### 提交数据集

最后，您可以点击 'submit' 发布数据集。

您需要提供之前创建的 'processes/wis2box' 授权令牌。如果尚未创建，请按照准备部分的说明创建新令牌。

检查提交数据集后是否收到以下消息，表明数据集已成功提交：

<img alt="Submit Dataset Success" src="/../assets/img/wis2box-submit-dataset-success.png" width="400">

点击“OK”后，您将被重定向到数据集编辑器主页。现在，如果您点击“Dataset”标签，应该能看到新提交的数据集已列出：

<img alt="Dataset Editor: new dataset" src="/../assets/img/wis2box-dataset-editor-new-dataset.png" width="800">

### 审查用于发现元数据的 WIS2 通知

打开 MQTT Explorer。如果您已连接到 broker，应该会看到一个新的 WIS2 通知发布在主题 `origin/a/wis2/<your-centre-id>/metadata` 上：
...
??? success "点击查看答案"
    ...
    **您在发现元数据中提供的标题、描述和关键词并未出现在 WIS2 通知的有效载荷中！** 
    ...
    **WIS2 通知包含指向已发布 WCMP2 记录的规范链接。** 
    ...
    将此规范链接复制粘贴到浏览器中以访问 WCMP2 记录。根据您的浏览器设置，可能会提示您下载文件，或者直接在浏览器中显示。
...
再次点击“Create New ...”以创建新数据集。使用之前使用的相同 centre-id，它应该可以在下拉列表中找到。在 **Template** 中选择 **other**：
...
请注意，由于您选择了 Template=*other*，需要您通过下拉列表为 'Discipline' 和 'Sub-Discipline' 定义 WIS2 Topic Hierarchy。

在本练习中，请选择 Sub-Discipline Topic "prediction/analysis/medium-range/deterministic/global"：
...
使用自定义模板时，不会提供默认数据映射。因此，数据集映射编辑器将为空，用户必须根据具体需求配置映射。
...
添加此插件时，您需要指定 **File Extension** 和 **File Pattern**（由正则表达式定义），以匹配数据文件的命名模式。在 "Universal"-plugin 的情况下，File Pattern 还用于确定数据的 "datetime" 属性。

!!! Note "从文件名解析日期时间"

    "Universal"-plugin 假设正则表达式中的第一个分组对应于数据的日期时间。

    默认 File Pattern 是 `^.*?_(\d{8}).*?\..*$`，匹配下划线后面跟随的 8 位数字，以及点号前的任意字符。例如：
    ...
    使用 "Universal"-plugin 导入数据时，要么重命名文件以匹配默认模式，要么更新 File Pattern，确保正则表达式中的第一个分组对应于日期时间。

暂时保留 "File Name" 的默认值，因为它们与您将在下一次实践中导入的数据匹配：
...
最后，提供您之前为 'processes/wis2box' 创建的授权令牌，并点击 'submit' 发布数据集。

成功提交后，您的新数据集将显示在 Dataset 标签中：

<img alt="Dataset Editor: new dataset" src="/../assets/img/wis2box-dataset-editor-new-dataset-other.png" width="800">

打开 MQTT Explorer，如果您已连接到 broker，应该会看到另一个新的 WIS2 通知发布在主题 `origin/a/wis2/<your-centre-id>/metadata` 上。

!!! question
    
    访问 `http://YOUR-HOST` 上的 wis2box-UI。
    
    您看到列出了多少个数据集？如何查看每个数据集使用的 WIS2 Topic Hierarchy，以及如何查看每个数据集的描述？

??? success "点击查看答案"

    打开 `http://YOUR-HOST` 上的 wis2box UI，您应该会看到列出了 2 个数据集及其 WIS2 Topic Hierarchy。要查看每个数据集的描述，您可以点击 "metadata"，这将重定向到由 wis2box-api 提供的相应 'discovery-metadata' 项。

!!! question

    尝试更新您最后创建的数据集的描述。更新描述后，您是否看到一个新的 WIS2 通知发布在主题 `origin/a/wis2/<your-centre-id>/metadata` 上？新通知与之前的通知有何不同？
...
!!! question
    ...
    尝试通过更改 "Sub-Discipline Topics" 的选择来更新您最后创建的数据集的 Topic Hierarchy。您是否看到一个新的 WIS2 通知发布在主题 `origin/a/wis2/<your-centre-id>/metadata` 上？

??? success "点击查看答案"

    您 **无法** 更新现有数据集的 Topic Hierarchy。在数据集创建后，Dataset Editor Form 中的 Topic Hierarchy 字段将被禁用。如果您想使用不同的 Topic Hierarchy，请先删除现有数据集，然后创建一个具有所需 Topic Hierarchy 的新数据集。
