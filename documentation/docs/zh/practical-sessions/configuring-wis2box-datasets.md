...
**发现元数据** 用于创建 WMO 核心元数据规范 (WCMP2) 记录，该记录通过在您的 wis2box broker 上发布的 WIS2 通知共享。
...
!!! note "不使用 wis2box-webapp 配置数据集"
    ...
    使用 MCF 提供了更多的灵活性和控制，但需要精确编写，以确保 MCF 格式正确并符合所需的架构。
...
!!! 注意 "Template"
    ...
    *Template* 字段允许您从 wis2box-webapp 数据集编辑器中提供的模板列表中进行选择。模板将使用适合数据类型的建议默认值预填表单。这包括元数据的建议标题和关键字以及预配置的数据插件。
...
审核标题和关键字，根据需要更新，并为您的数据集提供描述。

请注意，可以将 'WMO Data Policy' 从 'core' 更改为 'recommended'，或修改默认的 Metadata Identifier。请将数据政策保持为 'core'，并使用默认的 Metadata Identifier。
...
确保没有错误，并且您收到一个弹出窗口，表明您的表单已通过验证：

<img alt="Metadata Editor: validation success" src="/../assets/img/wis2box-metadata-validation-success.png" width="800">

接下来，在提交数据集之前，请审核数据集的数据映射。

### 配置数据映射

由于您使用模板创建了数据集，因此数据集映射已预先填充了 'weather/surface-based-observations/synop' 模板的默认插件。数据插件在 wis2box 中用于在数据发布到 WIS2 通知之前对其进行转换。
...
您需要提供之前创建的 'processes/wis2box' 的授权令牌。如果尚未创建，可以按照准备部分的说明创建一个新令牌。
...
??? success "点击查看答案"
    ...
    **您在发现元数据中提供的标题、描述和关键字并未出现在 WIS2 通知的有效载荷中！** 
    ...
    **WIS2 通知包含指向已发布 WCMP2 记录的规范链接。** 
    ...
    将此规范链接复制粘贴到浏览器中以访问 WCMP2 记录。根据您的浏览器设置，可能会提示您下载文件，也可能直接在浏览器中显示。
...
请注意，由于您选择了 Template=*other*，因此需要通过下拉列表为 'Discipline' 和 'Sub-Discipline' 定义 WIS2 Topic Hierarchy。
...
由于您选择了 Template=*other*，没有预定义的关键字。请确保添加至少 3 个您自己选择的关键字：

<img alt="Metadata Editor: title, description, keywords" src="/../assets/img/wis2box-metadata-editor-part1-other-2.png" width="800">

填写完所需字段后，完成表单的其余部分，包括 'Temporal Properties'、'Spatial Properties' 和 'Contact Information of the Data Provider'，并确保验证表单。

### 配置数据映射

当使用 Template=other 时，没有默认的数据映射。因此，数据集映射编辑器将为空，用户必须根据其具体需求配置映射。
...
!!! Note "从文件名解析日期时间"
    ...
    默认的 File Pattern 是 `^.*?_(\d{8}).*?\..*$`，它匹配以下模式：一个下划线后跟 8 位数字，再后跟任意字符和一个点，最后是文件扩展名。例如：
...
点击 "SAVE" 保存插件设置，并验证您现在是否在数据集映射编辑器中看到插件已列出：

<img alt="Data Mappings: update plugin" src="/../assets/img/wis2box-data-mappings-other3.png" width="800">

请注意，当您导入数据时，文件扩展名和文件名的 File Pattern 必须与您在此处提供的设置匹配，否则数据将无法被处理，且 wis2box-management 容器将记录 ERROR 消息。
...
访问 wis2box-UI：`http://YOUR-HOST`。

你看到列出了多少个数据集？如何查看每个数据集使用的 WIS2 主题层次结构？如何查看每个数据集的描述？

??? success "点击查看答案"

通过打开位于 `http://YOUR-HOST` 的 wis2box UI，你应该会看到列出了 2 个数据集以及它们的 WIS2 主题层次结构。要查看每个数据集的描述，你可以点击“metadata”，这将重定向到由 wis2box-api 提供的相应 'discovery-metadata' 项目。

!!! question

尝试更新你创建的最后一个数据集的描述。在更新描述后，你是否看到在主题 `origin/a/wis2/<your-centre-id>/metadata` 上发布了新的 WIS2 通知？新通知与之前的通知有什么不同？

??? success "点击查看答案"

在你更新数据集后，你应该会看到在主题 `origin/a/wis2/<your-centre-id>/metadata` 上发送了一条新的数据通知消息。

在消息中，*"rel": "canonical"* 的值将更改为 *"rel": "update"*，表明之前发布的数据已被修改。要查看更新后的描述，可以将 URL 复制粘贴到浏览器中，你应该能看到更新后的描述。

!!! question

尝试通过更改“子学科主题”中的选择来更新你创建的最后一个数据集的主题层次结构。你是否看到在主题 `origin/a/wis2/<your-centre-id>/metadata` 上发布了新的 WIS2 通知？

??? success "点击查看答案"

你**无法**更新现有数据集的主题层次结构。在数据集创建后，数据集编辑表单中的主题层次结构字段将被禁用。如果你想使用不同的主题层次结构，首先需要删除现有数据集，然后创建一个具有所需主题层次结构的新数据集。
