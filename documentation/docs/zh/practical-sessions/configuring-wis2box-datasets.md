...
**发现元数据** 用于创建 WMO 核心元数据配置文件 (WCMP2) 记录，并通过在您的 wis2box broker 上发布的 WIS2 通知共享。

**数据映射** 用于将数据插件与您的输入数据关联，从而在数据发布到 WIS2 之前对其进行转换。
...
!!! note "不使用 wis2box-webapp 配置数据集"
    ...
    使用 MCF 提供了更多的灵活性和控制，但需要精确编写，确保 MCF 格式和缩进正确，并符合所需的架构。
...
!!! Note
    ...
    您永远不需要与外部用户共享您的内部 broker 凭据。'everyone' 用户是一个公共用户，用于共享 WIS2 通知。
...
您将需要一个针对 'processes/wis2box' 端点的授权令牌来发布您的数据集。

要创建授权令牌，请通过 SSH 访问您的培训虚拟机并使用以下命令：
...
在 'dataset editor' 页面下的 'Datasets' 选项卡中，点击 "Create New ...":
...
!!! Note "Template"
    ...
    *Template* 字段允许您从 wis2box-webapp 数据集编辑器中可用的模板列表中进行选择。模板将使用适合数据类型的建议默认值预填表单。这包括元数据的建议标题和关键字以及预配置的数据插件。
    
    主题会自动设置为与所选模板关联的默认主题，除非您选择 *other*。如果您选择 *other*，则可以从基于 [WIS2 主题层次结构](https://codes.wmo.int/wis/topic-hierarchy/_earth-system-discipline) 的下拉列表中定义主题。

在本次培训中，您将创建两个数据集：
    
- 使用模板=*weather/surface-based-observations/synop* 的数据集，其中包括在发布前将数据转换为 BUFR 格式的数据插件。
- 使用模板=*other* 的数据集，在该模板中，您需要定义 WIS2 主题，并使用 "Universal" 插件直接发布数据而不进行转换。
...
检查标题和关键字，根据需要更新它们，并为您的数据集提供描述。

请注意，可以将 'WMO Data Policy' 从 'core' 更改为 'recommended'，或者修改默认的 Metadata Identifier。请将数据政策保持为 'core'，并使用默认的 Metadata Identifier。
...
如果有任何错误，请更正它们并再次点击 'VALIDATE FORM'。

确保没有错误，并且您收到一个弹出窗口，指示您的表单已通过验证：
...
由于您使用模板创建了数据集，因此数据集映射已预先填充了适用于 'weather/surface-based-observations/synop' 模板的默认插件。数据插件在 wis2box 中用于在数据发布到 WIS2 通知之前对其进行转换。

<img alt="Data Mappings: update plugin" src="/../assets/img/wis2box-data-mappings.png" width="800">

请注意，您可以点击“Update”按钮更改插件的设置，例如文件扩展名和文件模式。目前您可以保留默认设置。
...
!!! note "关于核心数据缓存的复选框"
    ...
    如果取消选中此复选框，wis2box 将添加 `properties.cache=False`，表示您不希望 Global Cache 下载并存储您的数据副本。默认情况下，此复选框是选中的，这意味着 Global Cache 将缓存您的核心数据，这是大多数数据集推荐的设置。

### 提交您的数据集

最后，您可以点击“submit”来发布您的数据集。

您需要提供之前创建的 'processes/wis2box' 的授权令牌。如果尚未创建，您可以按照准备部分的说明创建一个新的令牌。
...
!!! note "触发重新发布发现元数据"
    WIS2 通知会在发现元数据成功提交后立即发布。如果您未看到通知，可能是因为发布时您未连接到代理。
    
    如果未看到 WIS2 通知，可以使用以下命令手动触发发现元数据的重新发布：
...
??? success "点击查看答案"
    ...
    **您在发现元数据中提供的标题、描述和关键词并未出现在 WIS2 通知的有效载荷中！** 
    ...
    相反，尝试在 WIS2 通知的“links”部分中查找规范链接：
    ...
    **WIS2 通知包含已发布的 WCMP2 记录的规范链接。** 
    ...
    将此规范链接复制粘贴到浏览器中访问 WCMP2 记录。根据您的浏览器设置，可能会提示您下载文件，或者直接在浏览器中显示。

    您会在 WCMP2 记录中找到您提供的标题、描述和关键词。

wis2box 提供的预定义模板数量有限。这些模板是为常见类型的数据集设计的，但可能并不总是适合专业化数据。对于所有其他类型的数据集，您可以通过选择 Template=*other* 来创建数据集。
...
在 'Title' 和 'Description' 字段中提供您自己的值，并确保将自动生成的 'Local ID' 替换为数据集的描述性名称：

<img alt="Metadata Editor: title, description, keywords" src="/../assets/img/wis2box-metadata-editor-part1-other.png" width="800">

请注意，由于您选择了 Template=*other*，需要您自行使用下拉列表定义 WIS2 主题层级中的 'Discipline' 和 'Sub-Discipline'。
...
由于您选择了 Template=*other*，未预定义任何关键词。请确保添加至少 3 个您自己选择的关键词：
...
当使用 Template=other 时，不会提供默认数据映射。因此，数据集映射编辑器将为空，用户必须根据其具体需求配置映射。

<img alt="Data Mappings: update plugin" src="/../assets/img/wis2box-data-mappings-other1.png" width="800">

点击 "ADD A PLUGIN +" 为您的数据集添加数据插件。

选择名称为 **"Universal data without conversion"** 的插件。此插件设计用于在不进行任何转换的情况下发布数据。
...
!!! Note "从文件名解析日期时间"
    ...
    默认的 File Pattern 是 `^.*?_(\d{8}).*?\..*$`，它匹配下划线后面的 8 位数字，后面跟随任意字符和文件扩展名前的点。例如：

    - `mydata_20250101.txt` 将匹配并提取 2025 年 1 月 25 日作为数据的 datetime 属性
    - `mydata_2025010112.txt` 将不匹配，因为有 10 位数字而不是 8 位
    - `mydata-20250101.txt` 将不匹配，因为日期前是连字符而不是下划线

    使用 "Universal"-插件导入数据时，可以重命名文件以匹配默认值，或者更新 File Pattern，确保正则表达式中的第一个分组对应于日期时间。

目前保留 "File Name" 的默认值，因为它们与您将在下一次实践中导入的数据匹配：
...
请注意，当您导入数据时，文件扩展名和文件名的 File Pattern 必须与您在此处提供的设置匹配，否则数据将无法处理，且 wis2box-management 容器将记录 ERROR 消息。

### 提交并查看结果

最后，提供之前创建的 'processes/wis2box' 的授权令牌，并点击“submit”发布您的数据集。
...
您看到列出了多少个数据集？如何查看每个数据集使用的 WIS2 主题层次结构？如何查看每个数据集的描述？

??? success "点击查看答案"

通过打开位于 `http://YOUR-HOST` 的 wis2box UI，您应该会看到列出了 2 个数据集及其对应的 WIS2 主题层次结构。要查看每个数据集的描述，您可以点击“metadata”，这将跳转到由 wis2box-api 提供的相应 'discovery-metadata' 项目。
...
在您更新数据集后，您应该会看到一个新的数据通知消息被发送到主题 `origin/a/wis2/<your-centre-id>/metadata`。

在消息中，*"rel": "canonical"* 的值将更改为 *"rel": "update"*，表明之前发布的数据已被修改。要查看更新后的描述，请将 URL 复制粘贴到浏览器中，您应该能够看到更新后的描述。
...
您**无法**更新现有数据集的主题层次结构。在数据集创建后，数据集编辑表单中的主题层次结构字段将被禁用。如果您想使用不同的主题层次结构，首先需要删除现有数据集，然后创建一个具有所需主题层次结构的新数据集。
