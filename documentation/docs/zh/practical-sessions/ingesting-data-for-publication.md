...
!!! abstract "学习目标"
    ...
    - 使用 MinIO Web 界面、SFTP 或 Python 脚本将数据上传到 MinIO，从而触发 wis2box 工作流。
...
- *wis2box-incoming*：这是触发 wis2box 工作流时上传数据的存储桶。
- *wis2box-public*：这是 wis2box 发布已成功摄取和处理数据的存储桶。

点击存储桶 *wis2box-incoming*。尝试通过点击 `Create new path` 在此存储桶中定义一个新路径：
...
现在您已经知道如何将数据上传到 MinIO，让我们尝试为您在上一节实践中创建的使用 "Universal"-plugin 的预测数据集上传数据。

返回浏览器中的 MinIO Web 界面，选择存储桶 `wis2box-incoming`，然后点击 `Create new path`。

这次请确保 **创建一个与您在上一节实践中为预测数据集创建的元数据标识符匹配的目录**：
...
??? success "点击查看答案"
    ...
    数据集被配置为仅处理扩展名为 `.grib2` 的文件。文件扩展名的配置是您在上一节实践中定义的数据映射的一部分。
...
!!! note "关于 Universal 插件"
    ...
    "Universal"-plugin 允许您在不进行任何转换的情况下发布数据。它是一个 *直通* 插件，摄取数据文件并按原样发布。为了将属性 "datetime" 添加到 WIS2 数据通知中，该插件依赖于文件模式中的第一个分组来匹配您发布数据的日期。
...
接下来，您将使用 **Template='weather/surface-based-observations/synop'** 模板处理在上一节实践中创建的数据集。该模板为您预先配置了以下数据插件：
...
使用 wis2box-webapp 中的站点编辑器，将 WIGOS 标识符为 `0-20000-0-64400` 的站点添加到您的 wis2box 实例中，如您在 [配置站点元数据](./configuring-station-metadata.md) 实践课程中所学的那样。
...
检查 Grafana 仪表板和 MQTT Explorer，确认数据是否成功发布。如果您看到以下通知，则表明您已成功发布 synop 示例数据：
...
??? success "点击查看答案"
    ...
    在 MQTT Explorer 的 WIS2 数据通知的链接部分中，您会看到规范链接：
...
!!! note
    ...
    您会收到一个错误提示，因为脚本尚未配置为访问您 wis2box 上的 MinIO 端点。

脚本需要知道访问您 wis2box 上 MinIO 的正确端点。如果 wis2box 在您的主机上运行，MinIO 端点可通过 `http://YOUR-HOST:9000` 访问。脚本还需要更新您的存储密码以及 MinIO 存储桶中存储数据的路径。
...
??? success "点击查看答案"
    ...
    如果 BUFR 文件中使用的所有站点均已在您的 wis2box 实例中定义，您应该会看到 MQTT broker 发布了 10 条消息。每条通知对应一个站点的一个观测时间戳的数据。
...
在 wis2box 中，MinIO 服务还可以通过 SFTP 访问。如果您有一个现有系统可以配置为通过 SFTP 转发数据，您可以使用这种方法作为自动化数据摄取的替代方式。
...
??? success "点击查看答案"
    ...
    您应该会看到一个新的 WIS2 数据通知发布到测试站点 `0-20000-0-64400`，表明数据已成功摄取并发布。
...
!!! success "恭喜！"
    ...
    - 使用 Grafana 仪表板和 wis2box 实例日志调试数据摄取过程中的常见错误。
