...
!!! abstract "学习目标"
    ...
    - 使用 MinIO 网页界面、SFTP 或 Python 脚本上传数据至 MinIO，从而触发 wis2box 工作流。
...
要使用 wis2box 软件在 WIS2 Node 中触发数据工作流，必须将数据上传到 **MinIO** 中的 **wis2box-incoming** 存储桶，从而启动 wis2box 工作流。此过程会通过 WIS2 数据通知发布数据。根据您在 wis2box 实例中配置的数据映射，数据可能会在发布之前转换为 BUFR 格式。

在本次练习中，我们将使用示例数据文件触发 wis2box 工作流，并为您在之前实践课程中配置的数据集**发布 WIS2 数据通知**。

在练习过程中，我们将使用 **Grafana 仪表板** 和 **MQTT Explorer** 监控数据摄取状态。Grafana 仪表板使用来自 Prometheus 和 Loki 的数据显示您的 wis2box 状态，而 MQTT Explorer 则允许您查看由您的 wis2box 实例发布的 WIS2 数据通知。
...
还需要了解如何在 **wis2box-webapp** 中配置站点元数据，如 [配置站点元数据](./configuring-station-metadata.md) 实践课程中所述。
...
首先，我们将使用 MinIO 网页界面，它允许您通过浏览器下载和上传数据到 MinIO。

### 访问 MinIO 界面

打开 MinIO 网页界面，通常可通过 `http://YOUR-HOST:9001` 访问。
...
接下来，下载另一个文件扩展名的 geps 示例数据 [geps_202508180000.nc](../sample-data/geps_202508180000.nc) 到您的本地环境。将此文件上传到与之前练习相同的目录中。
...
??? success "点击查看答案"
    ...
    这表明数据工作流已被触发，但数据未被重新发布。如果文件扩展名无法匹配 grib2，wis2box 将不会发布数据。
...
??? success "点击查看答案"
    ...
    这表明数据工作流已被触发，但数据未被重新发布。如果文件名无法匹配正则表达式 ^.*?_(\d{8}).*?\..*$，wis2box 将不会发布数据。
...
将文件上传到 MinIO 总是会成功，只要用户有访问权限即可。然而，发布 WIS2 数据通知需要更严格的验证。不符合扩展名或文件名规则的文件将存储在 `incoming` 存储桶中，但 `Universal plugin` 不会为它们发布通知。这就解释了为什么具有不支持的扩展名（例如 `geps_202508180000.nc`）或无效文件名（例如 `geps_renamed_sample_data.grib2`）的文件会被 MinIO 接受，但不会出现在 WIS2 中。

接下来，在浏览器中打开 MinIO 的 Web 界面，浏览到 `wis2box-incoming` 存储桶。你会看到在前面练习中上传的文件 `geps_202508180000.grib2`。
...
!!! question "问题"
    ...
    你能通过 MinIO 成功重新发布数据通知消息吗？  
    检查 Grafana 仪表板和 MQTT Explorer，查看数据是否成功被摄取和发布。

??? success "点击查看答案"

    你会看到一条消息，表明 wis2box 已经发布了此数据：
...
下载 synop 示例数据 [synop_202502040900.txt](../sample-data/synop_202502040900.txt) 到本地环境中：

与之前的练习一样，在 `wis2box-incoming` 存储桶下创建一个目录，该目录名称应与 surface-based-observations/synop 数据集的元数据标识符相匹配。

进入新创建的目录，点击 `Upload`，选择之前下载到本地的 [synop_202502040900.txt](../sample-data/synop_202502040900.txt)，然后上传。

!!! question "问题"

    你能通过 MinIO 成功发布数据通知消息吗？  
    检查 Grafana 仪表板和 MQTT Explorer，查看数据是否成功被摄取和发布。

??? success "点击查看答案"

    不。  
    在 Grafana 仪表板中，你会看到一条警告，表明缺少站点 64400 的记录：
...
在这种情况下，你正在使用 `FM-12 data converted to BUFR` 插件。

该插件的目的是处理以纯文本格式提供的 FM-12 数据，并将其转换为二进制 BUFR。在此过程中，插件需要解析并映射数据中包含的站点信息。

如果缺少必要的站点元数据，插件将无法正确解析文件，转换将失败。

因此，在发布 SYNOP 数据之前，必须确保相关的站点元数据已添加到 wis2box 中。

现在，为本次练习添加一个测试站点。

使用 wis2box-webapp 中的站点编辑器，将 WIGOS 标识符为 `0-20000-0-64400` 的站点添加到你的 wis2box 实例中。

从 OSCAR 检索站点信息：

<img alt="oscar-station" src="/../assets/img/webapp-test-station-oscar-search.png" width="600">

将站点添加到你为发布创建的 "../surface-based-observations/synop" 数据集中，并使用你的身份验证令牌保存更改：

<img alt="webapp-test-station" src="/../assets/img/webapp-test-station-save.png" width="800">

请注意，在实践课程结束后，你可以从数据集中移除此站点。

完成站点元数据配置后，使用 MQTT Explorer 检查数据是否成功发布。如果看到以下通知，则说明你成功发布了 synop 示例数据。
...
在你的学生虚拟机上，Python 的 `minio` 包已经安装。

将目录 `exercise-materials/data-ingest-exercises` 复制到 `wis2box.env` 文件中定义的 `WIS2BOX_HOST_DATADIR` 目录中：
...
在 `exercise-materials/data-ingest-exercises` 目录中，你会找到一个示例脚本 `copy_file_to_incoming.py`，可用于将文件复制到 MinIO。
...
!!! note
    ...
    你会收到一个错误，因为脚本尚未配置为访问 wis2box 上的 MinIO 端点。

脚本需要知道访问 wis2box 上 MinIO 的正确端点。如果 wis2box 运行在你的主机上，MinIO 端点可通过 `http://YOUR-HOST:9000` 访问。脚本还需要更新你的存储密码以及 MinIO 存储桶中存储数据的路径。

!!! question "更新脚本并摄取 CSV 数据"
    
    编辑脚本 `copy_file_to_incoming.py` 以解决错误，可以使用以下方法之一：
    - 从命令行：使用 `nano` 或 `vim` 文本编辑器编辑脚本。
    - 使用 WinSCP：启动一个新的连接，使用文件协议 `SCP` 和与你的 SSH 客户端相同的凭据。导航到目录 `wis2box-data/data-ingest-exercises` 并使用内置文本编辑器编辑 `copy_file_to_incoming.py`。
...
一旦你成功运行脚本，你会看到一条消息，表明文件已复制到 MinIO，并且你应该在 MQTT Explorer 中看到 wis2box 实例发布的数据通知。

你还可以检查 Grafana 仪表板，查看数据是否成功被摄取和发布。

现在脚本已正常工作，你可以尝试使用相同的脚本将其他文件复制到 MinIO。
...
检查 Grafana 仪表板和 MQTT Explorer，查看测试数据是否成功被摄取和发布。如果看到任何错误，请尝试解决。

!!! question "验证数据摄取"

    对于此数据样本，有多少条消息被发布到 MQTT broker？

??? success "点击查看答案"

    你会在 Grafana 中看到错误报告，因为 BUFR 文件中的站点未在你的 wis2box 实例的站点列表中定义。
    
    如果 BUFR 文件中使用的所有站点都已在你的 wis2box 实例中定义，你应该会看到 10 条消息发布到 MQTT broker。每条通知对应一个站点的一个观测时间戳的数据。
...
!!! question "使用 SFTP 上传数据"
    ...
    然后使用 WinSCP 中的 SFTP 会话，将其上传到 MinIO 的 incoming 数据集路径。

    在 Grafana 仪表板和 MQTT Explorer 中检查数据是否成功被摄取并发布。

??? success "点击查看答案"

    您应该会看到一个新的 WIS2 数据通知已为测试站点 `0-20000-0-64400` 发布，表明数据已成功被摄取并发布。

    <img alt="grafana_data_ingest" src="/../assets/img/grafana_data-ingest-test3.png" width="400"> 

    如果路径错误，您将在日志中看到一条错误消息。
