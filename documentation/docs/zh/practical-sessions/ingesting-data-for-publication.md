...
要使用 wis2box 软件在 WIS2 Node 中触发数据工作流，必须将数据上传到 **MinIO** 的 **wis2box-incoming** 存储桶中，这将启动 wis2box 数据工作流以处理并发布数据。
...
在本节中，我们将重点介绍如何将数据上传到您的 wis2box 实例并验证摄取和发布是否成功。数据转换将在 [数据转换工具](./data-conversion-tools.md) 实践课程中进一步讨论。
...
要登录，您需要使用在 wis2box.env 文件中定义的 WIS2BOX_STORAGE_USERNAME 和 WIS2BOX_STORAGE_PASSWORD 凭据。
...
## 摄取与发布："Universal"-plugin 

现在您已经知道如何将数据上传到 MinIO，让我们尝试为您在上一个实践课程中创建的使用 "Universal"-plugin 的预测数据集上传数据。

返回浏览器中的 MinIO 网页界面，选择存储桶 `wis2box-incoming`，然后点击 `Create new path`。

这次请确保 **创建一个与您在上一个实践课程中创建的预测数据集的元数据标识符匹配的目录**：
...
??? success "点击查看答案"
    ...
    数据集被配置为仅处理扩展名为 `.grib2` 的文件。文件扩展名配置是您在上一个实践课程中定义的数据映射的一部分。
...
!!! note "关于 Universal 插件"
    ...
    "Universal"-plugin 允许您在不进行任何转换的情况下发布数据。这是一个 *直通* 插件，它摄取数据文件并按原样发布。为了将属性 "datetime" 添加到 WIS2 数据通知中，该插件依赖于文件模式中的第一个分组来匹配您发布的数据的日期。
...
接下来，您将使用 **Template='weather/surface-based-observations/synop'** 模板处理上一节实践中创建的数据集。该模板为您预先配置了以下数据插件：

<img alt="synop-dataset-plugins" src="/../assets/img/wis2box-data-mappings.png" width="1000">

请注意，其中一个插件是 **FM-12 数据转换为 BUFR**（synop2bufr），它被配置为处理扩展名为 **txt** 的文件。
...
!!! note "关于 FM-12 数据转换为 BUFR 插件"
    ...
    在转换过程中，插件会向输出数据添加缺失的元数据，例如 WIGOS 站点标识符、站点位置和气压计高度。为了添加这些元数据，插件会使用传统的（5 位数）标识符（在本例中为 64400）在您的 wis2box 实例的站点列表中查找相关信息。

    如果在站点列表中找不到该站点，插件将无法添加缺失的元数据，并且不会发布任何数据。

使用 wis2box-webapp 中的站点编辑器，将 WIGOS 标识符 `0-20000-0-64400` 的站点添加到您的 wis2box 实例中，如您在 [配置站点元数据](./configuring-station-metadata.md) 实践课程中所学的那样。
...
添加站点后，通过再次将示例数据文件 *synop_202502040900.txt* 上传到 MinIO 中的相同路径来重新触发 wis2box 工作流。
...
使用 MinIO Web 界面是手动上传数据到 MinIO 进行测试的便捷方式。然而，在生产环境中，通常会使用自动化流程将数据上传到 MinIO，例如使用脚本或应用程序通过 MinIO 的 S3 兼容 API 上传数据。
...
!!! question "更新脚本并导入 CSV 数据"
    ...
    - 使用 WinSCP：启动一个新的连接，选择文件协议 `SCP`，并使用与您的 SSH 客户端相同的凭据。导航到目录 `wis2box-data/data-ingest-exercises` 并使用内置文本编辑器编辑 `copy_file_to_incoming.py`。
...
wis2box 中的 MinIO 服务还可以通过 SFTP 访问。如果您有一个现有系统可以配置为通过 SFTP 转发数据，则可以使用此方法作为自动化数据摄取的替代方式。
...
??? success "点击查看答案"
    ...
    您应该会看到一个新的 WIS2 数据通知发布，测试站点 `0-20000-0-64400` 的数据已成功摄取并发布。
