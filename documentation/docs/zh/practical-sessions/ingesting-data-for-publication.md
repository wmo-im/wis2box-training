...
!!! abstract "学习目标"
    ...
    - 通过使用 MinIO Web 界面、SFTP 或 Python 脚本上传数据来触发 wis2box 工作流。
    - 访问 Grafana 仪表板以监控数据摄取状态并查看您的 wis2box 实例的日志。
    - 使用 MQTT Explorer 查看由您的 wis2box 发布的 WIS2 数据通知。

## 简介

在 WIS2 中，数据通过实时共享的 WIS2 数据通知进行分发，这些通知包含一个“规范”链接，可用于下载数据。

要使用 wis2box 软件在 WIS2 节点中触发数据工作流，必须将数据上传到 **MinIO** 中的 **wis2box-incoming** 存储桶，这将启动 wis2box 数据工作流以处理和发布数据。
...
在本节中，我们将重点介绍如何将数据上传到您的 wis2box 实例并验证摄取和发布是否成功。数据转换将在 [数据转换工具](./data-conversion-tools.md) 实践课程中进行讲解。

为了手动测试数据摄取过程，我们将使用 MinIO Web 界面，该界面允许您通过 Web 浏览器下载和上传数据到 MinIO。

在生产环境中，数据通常通过自动化流程摄取，例如通过 S3 或 SFTP 将数据转发到 MinIO 的脚本或应用程序。

## 准备工作

本节假设您已成功完成 [配置 wis2box 数据集](./configuring-wis2box-datasets.md) 实践课程。如果您按照该课程中的说明操作，您应该已经有一个使用 `Universal` 插件的数据集，以及另一个使用 `FM-12 数据转换为 BUFR` 插件的数据集。
...
要登录，您需要使用在 wis2box.env 文件中由 WIS2BOX_STORAGE_USERNAME 和 WIS2BOX_STORAGE_PASSWORD 定义的凭据。
...
点击存储桶 *wis2box-incoming*。尝试通过点击 `Create new path` 选项在此存储桶中定义一个新路径：

<img alt="minio ui: minio ui after login" src="/../assets/img/minio-incoming-create-new-path.png" width="800">

输入新的文件夹路径 = *new-directory* 并上传此示例文件 [mydata.nc](./../sample-data/mydata.nc)（右键单击并选择“另存为”以下载文件）。您可以使用 MinIO 中的“上传”按钮将文件上传到新目录中：
...
现在您已经知道如何将数据上传到 MinIO，让我们尝试为您在上一实践课程中创建的使用 "Universal"-插件的预测数据集上传数据。

返回浏览器中的 MinIO Web 界面，选择存储桶 `wis2box-incoming`，然后点击 `Create new path`。

这次请确保**创建一个与您在上一实践课程中创建的预测数据集的元数据标识符匹配的目录**：

<img alt="minio-filepath-forecast-dataset" src="/../assets/img/minio-filepath-forecast-dataset.png" width="800">

进入新创建的目录，点击 `Upload` 并上传您之前使用的文件 *mydata.nc* 到新目录中。检查 Grafana 仪表板以查看数据是否成功摄取和发布。
...
??? success "点击查看答案"
    ...
    数据集被配置为仅处理扩展名为 `.grib2` 的文件。文件扩展名配置是您在上一实践课程中定义的数据映射的一部分。
...
??? success "点击查看答案"
    ...
    将文件重命名为 *GEPS_202508180000.grib2* 并再次上传到 MinIO 中的相同路径以重新触发 wis2box 工作流。（或者从此处下载重命名的文件：[GEPS_202508180000.grib2](../sample-data/GEPS_202508180000.grib2)）。
...
!!! note "关于 Universal 插件"
    ...
    "Universal"-插件允许您在不进行任何转换的情况下发布数据。它是一个*直通*插件，摄取数据文件并按原样发布。为了将 "datetime" 属性添加到 WIS2 数据通知中，该插件依赖于文件模式中的第一个分组来匹配您发布的数据的日期。
...
## 数据导入与发布："synop2bufr"-插件

接下来，您将使用 **Template='weather/surface-based-observations/synop'** 模板处理您在上一节实践中创建的数据集。该模板为您预先配置了以下数据插件：

<img alt="synop-dataset-plugins" src="/../assets/img/wis2box-data-mappings.png" width="1000">

请注意，其中一个插件是 **FM-12 数据转换为 BUFR** (`synop2bufr`)，它被配置为处理文件扩展名为 **txt** 的文件。
...
!!! note "关于插件 FM-12 数据转换为 BUFR"

    此插件尝试将 FM-12 输入数据转换为 BUFR 格式。

    在转换过程中，插件会向输出数据中添加缺失的元数据，例如 WIGOS 站点标识符、站点位置和气压计高度。为了添加这些元数据，插件会使用传统的（5 位数）标识符（在本例中为 64400）在您的 `wis2box` 实例的站点列表中查找这些信息。

    如果在站点列表中找不到该站点，插件将无法添加缺失的元数据，并且不会发布任何数据。

使用 `wis2box-webapp` 中的站点编辑器，将 WIGOS 标识符为 `0-20000-0-64400` 的站点添加到您的 `wis2box` 实例中，如您在 [配置站点元数据](./configuring-station-metadata.md) 实践课程中所学。
...
添加站点后，通过将示例数据文件 *synop_202502040900.txt* 再次上传到 MinIO 中的相同路径，重新触发 `wis2box` 工作流。
...
??? success "点击查看答案"
    ...
    在 MQTT Explorer 中检查 WIS2 数据通知的 Links 部分，您将看到规范链接：
    ...
    文件扩展名为 `.bufr4`，表明数据已由插件成功从 FM-12 格式转换为 BUFR 格式。

## 使用 Python 导入数据

使用 MinIO Web 界面是手动上传数据到 MinIO 进行测试的便捷方式。然而，在生产环境中，通常会使用自动化流程上传数据到 MinIO，例如使用脚本或应用程序通过 MinIO 的 S3 兼容 API 上传数据。
...
!!! note
    ...
    您将收到一个错误，因为脚本尚未配置为访问 `wis2box` 上的 MinIO 端点。
...
成功运行脚本后，您将看到一条消息，指示文件已复制到 MinIO，并且您应该在 MQTT Explorer 中看到您的 `wis2box` 实例发布的数据通知。
...
检查 Grafana 仪表板和 MQTT Explorer，确认测试数据是否成功导入并发布。如果您看到任何错误，请尝试解决。

!!! question "验证数据导入"

    对于此数据样本，向 MQTT broker 发布了多少条消息？

??? success "点击查看答案"

    您将在 Grafana 中看到错误，因为 BUFR 文件中的站点未在您的 `wis2box` 实例的站点列表中定义。
...
!!! question "使用 SFTP 上传数据"
    ...
    在 Grafana 仪表板和 MQTT Explorer 中检查数据是否成功摄取并发布。
...
!!! success "恭喜！"
    ...
    - 使用 Grafana 仪表板和您的 wis2box 实例日志调试数据摄取过程中的常见错误。
