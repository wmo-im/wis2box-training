...
在本次课程中，您将学习如何设置对 WIS2 Broker 的订阅，并使用 wis2box 中包含的 "wis2downloader" 服务将数据自动下载到本地系统。
...
wis2downloader 作为一个独立的容器包含在 wis2box 中，其定义在 Docker Compose 文件中。wis2box 中的 Prometheus 容器配置为从 wis2downloader 容器中抓取指标，这些指标可以通过 Grafana 仪表板进行可视化。
...
该仪表板基于 wis2downloader 服务发布的指标，显示当前正在进行的下载状态。

在左上角，您可以看到当前活动的订阅。

保持此仪表板打开，您将在下一步练习中使用它来监控下载进度。
...
以下是 wis2downloader 使用的环境变量：
    ...
    - DOWNLOAD_BROKER_TRANSPORT: websockets 或 tcp，连接 MQTT broker 使用的传输机制。默认值为 websockets
    - DOWNLOAD_RETENTION_PERIOD_HOURS: 下载数据的保留时间（小时）。默认值为 24
    - DOWNLOAD_WORKERS: 使用的下载工作线程数。默认值为 8，决定并行下载的数量。
    - DOWNLOAD_MIN_FREE_SPACE_GB: 保留在存储下载数据的卷上的最小空闲空间（GB）。默认值为 1。
...
!!! note "更新 wis2downloader 配置"
    ...
    要更新 wis2downloader 的配置，可以编辑 wis2box.env 文件。要应用更改，可以重新运行 wis2box-stack 的启动命令：

    ```bash
    python3 wis2box-ctl.py start
    ```

    然后您将看到 wis2downloader 服务以新配置重新启动。
...
在 Grafana 中检查 wis2downloader 仪表板以查看新添加的订阅。等待几分钟，您应该会看到首次下载开始。一旦确认下载开始，继续下一步练习。

wis2box 中的 wis2downloader 服务将数据下载到 `wis2box.env` 文件中定义的 `WIS2BOX_HOST_DATADIR` 目录下的 'downloads' 目录。要查看下载目录的内容，请使用以下命令：
...
??? success "点击查看答案"
    您应该会看到一个以 `cache/a/wis2/de-dwd-gts-to-wis2/` 开头的目录结构，在其下方会有更多以下载数据的 GTS 公报头命名的目录。

    根据您开始订阅的时间，您可能会或可能不会在此目录中看到任何下载的文件。如果尚未看到文件，请再等待几分钟后再次检查。

在 Grafana 的 wis2downloader 仪表板中查看下载进度，您将在仪表板左上角看到您添加的订阅，以及随着数据下载完成的下载数量增加：
...
访问 Grafana 仪表板以确认订阅已被删除并且下载已停止，您应该会看到订阅从仪表板左上角消失。
...
!!! note
    ...
    wis2downloader 容器中的目录 `app/data/downloads` 映射到 `wis2box.env` 文件中定义的 `WIS2BOX_HOST_DATADIR` 下的 `downloads` 目录。上述命令将删除所有下载的数据。
...
当订阅以 `cache/` 开头的主题时，您将为相同的数据收到多个通知，每个通知对应一个 Global Cache。每个通知将包含一个规范 URL，指向相应 Global Cache 的数据服务器。`wis2downloader` 将从其能够访问的第一个规范 URL 下载数据。

## 从 WIS2 培训代理下载示例数据

在本练习中，您将订阅发布用于培训目的的示例数据的 WIS2 培训代理。

### 修改 wis2downloader 配置

此步骤演示如何订阅非默认代理，并允许您下载由 WIS2 培训代理发布的一些数据。
...
检查 `wis2downloader` 的日志，查看是否成功连接到新代理：
...
现在我们将设置一个新的订阅主题，从 WIS2 培训代理下载气旋轨迹数据。
...
通过再次检查 `wis2downloader` 的日志，确认数据已下载：
...
您应该会看到一个新目录，名为 `origin/a/wis2/int-wis2-training/data/core/weather/prediction/forecast/medium-range/probabilistic/trajectory`，其中包含下载的数据。
...
接下来尝试添加另外两个订阅，从以下主题下载月度地表温度异常和全球集合预报数据：
...
!!! success "恭喜！"
    ...
    - 使用 `wis2downloader` 订阅 WIS2 代理并将数据下载到本地系统  
    - 在 Grafana 仪表板中查看下载状态  
