...
!!! note "关于 wis2downloader"
     ...
     wis2downloader 也可以作为独立服务运行，可以在与发布 WIS2 通知的系统不同的系统上运行。有关作为独立服务使用 wis2downloader 的更多信息，请参阅 [wis2downloader](https://pypi.org/project/wis2downloader/)。

     如果您希望开发自己的服务来订阅 WIS2 通知并下载数据，可以参考 [wis2downloader 源代码](https://github.com/World-Meteorological-Organization/wis2downloader)。

## 准备工作

在开始之前，请登录到您的学生虚拟机，并确保您的 wis2box 实例正在运行。

## wis2downloader 基础知识

wis2downloader 作为 wis2box 中的一个独立容器包含在 Docker Compose 文件中定义的服务中。wis2box 中的 Prometheus 容器被配置为从 wis2downloader 容器中抓取指标，这些指标可以通过 Grafana 仪表板进行可视化。

### 在 Grafana 中查看 wis2downloader 仪表板

打开一个网页浏览器，导航到您的 wis2box 实例的 Grafana 仪表板，地址为 `http://YOUR-HOST:3000`。

点击左侧菜单中的仪表板，然后选择 **wis2downloader dashboard**。
...
保持此仪表板打开，因为您将在下一次练习中使用它来监控下载进度。
...
以下是 wis2downloader 使用的环境变量：
    ...
    - DOWNLOAD_BROKER_PORT: 要连接的 MQTT Broker 的端口。默认为 443（用于 WebSocket 的 HTTPS）
    - DOWNLOAD_BROKER_USERNAME: 连接 MQTT Broker 时使用的用户名。默认为 everyone
    - DOWNLOAD_BROKER_PASSWORD: 连接 MQTT Broker 时使用的密码。默认为 everyone
    - DOWNLOAD_BROKER_TRANSPORT: 连接 MQTT Broker 时使用的传输机制（websockets 或 tcp）。默认为 websockets
    - DOWNLOAD_RETENTION_PERIOD_HOURS: 下载数据的保留时间（小时）。默认为 24
    - DOWNLOAD_WORKERS: 使用的下载工作线程数量。默认为 8，决定并行下载的数量。
...
要访问 wis2box 中 wis2downloader 的命令行界面，您可以使用以下命令登录到 **wis2downloader** 容器：
...
在 Grafana 中查看 wis2downloader 仪表板，查看新添加的订阅。等待几分钟，您应该会看到开始下载的第一个数据。确认下载开始后，继续下一次练习。

wis2box 中的 wis2downloader 服务将数据下载到您在 `wis2box.env` 文件中定义的 `WIS2BOX_HOST_DATADIR` 目录中的 `downloads` 目录。要查看下载目录的内容，请使用以下命令：
...
??? success "点击查看答案"
    您应该会看到一个目录结构，从 `cache/a/wis2/de-dwd-gts-to-wis2/` 开始，在其下方是以下载数据的 GTS 公报头命名的更多目录。

    根据您开始订阅的时间，您可能会或可能不会在此目录中看到任何下载的文件。如果您尚未看到任何文件，请再等待几分钟并再次检查。

让我们在继续下一次练习之前清理订阅和下载的数据。

登录回 wis2downloader 容器：
...
在 Grafana 中查看 wis2downloader 仪表板，确认订阅已被删除。您应该会看到下载停止。
...
!!! note "origin 与 cache 主题"
    ...
    当订阅以 `origin/` 开头的主题时，您将收到带有规范 URL 的通知，该 URL 指向由发布数据的 WIS 中心提供的数据服务器。

    当订阅以 `cache/` 开头的主题时，您将为同一数据收到多个通知，每个 Global Cache 都会发送一个通知。每个通知都包含一个规范 URL，指向相应 Global Cache 的数据服务器。wis2downloader 将从它能够访问的第一个规范 URL 下载数据。

## 从 WIS2 Training Broker 下载示例数据

在本次练习中，您将订阅 WIS2 Training Broker，该 Broker 发布用于培训的示例数据。

### 更改 wis2downloader 配置

此操作演示如何订阅非默认的 Broker，并允许您下载由 WIS2 Training Broker 发布的一些数据。
...
检查 wis2downloader 的日志，查看是否成功连接到新的 Broker：
...
现在我们将设置一个新订阅，订阅 WIS2 Training Broker 发布的气旋轨迹数据。
...
等待直到您在 Grafana 中的 wis2downloader 仪表板中看到下载开始。

再次检查 wis2downloader 的日志，确认数据已下载：
...
您应该会看到一个名为 `origin/a/wis2/int-wis2-training/data/core/weather/prediction/forecast/medium-range/probabilistic/trajectory` 的新目录，其中包含下载的数据。
...
接下来，尝试添加另外两个订阅，分别下载月度地表温度异常数据和全球集合预报数据，主题如下：
...
!!! success "恭喜！"
    ...
    - 在 Grafana 仪表板中查看下载状态
    - 更改 wis2downloader 的默认配置以订阅不同的 Broker
    - 在本地系统上查看下载的数据