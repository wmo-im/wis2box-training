...
!!! note "关于 wis2downloader"
     ...
     wis2downloader 也可以作为独立服务运行在与发布 WIS2 通知的系统不同的系统上。有关作为独立服务使用 wis2downloader 的更多信息，请参阅 [wis2downloader](https://pypi.org/project/wis2downloader/)。
...
- DOWNLOAD_BROKER_USERNAME: 连接 MQTT Broker 时使用的用户名。默认值为 everyone
- DOWNLOAD_BROKER_PASSWORD: 连接 MQTT Broker 时使用的密码。默认值为 everyone
...
要访问 wis2box-stack 中 wis2downloader 的命令行界面，可以使用以下命令登录到 **wis2downloader** 容器：
...
然后通过键入 `exit` 退出 **wis2downloader** 容器：
...
!!! note "origin 与 cache 主题"
    ...
    当您订阅以 `origin/` 开头的主题时，您将收到带有指向由发布数据的 WIS Centre 提供的数据服务器的规范 URL 的通知。

    当您订阅以 `cache/` 开头的主题时，您将为同一数据收到多个通知，每个 Global Cache 都会发送一条通知。每条通知都包含一个指向相应 Global Cache 数据服务器的规范 URL。wis2downloader 将从它能够访问的第一个规范 URL 下载数据。
...
这部分内容展示了如何订阅一个非默认的代理，以便下载从 WIS2 Training Broker 发布的一些数据。
...
检查 `wis2downloader` 的日志，确认是否成功连接到新的代理：
...
现在我们将为订阅一个主题进行配置，以从 WIS2 Training Broker 下载气旋轨迹数据。
