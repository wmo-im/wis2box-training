...
wis2downloader 作为 wis2box 中的一个独立容器包含在 Docker Compose 文件中。wis2box 中的 Prometheus 容器被配置为从 wis2downloader 容器中抓取指标，这些指标可以通过 Grafana 仪表板可视化。
...
以下是 wis2downloader 使用的环境变量：
    ...
    - DOWNLOAD_BROKER_USERNAME: 用于连接 MQTT broker 的用户名。默认值为 everyone
    - DOWNLOAD_BROKER_PASSWORD: 用于连接 MQTT broker 的密码。默认值为 everyone
    - DOWNLOAD_BROKER_TRANSPORT: websockets 或 tcp，用于连接 MQTT broker 的传输机制。默认值为 websockets
    - DOWNLOAD_RETENTION_PERIOD_HOURS: 下载数据的保留时间（小时）。默认值为 24
    - DOWNLOAD_WORKERS: 使用的下载工作线程数量。默认值为 8，决定并行下载的数量。
    - DOWNLOAD_MIN_FREE_SPACE_GB: 保留在存储下载的卷上的最小空闲空间（GB）。默认值为 1。
...
如果您保留了 wis2downloader 的默认配置，它当前已连接到由 Météo-France 托管的 WIS2 Global Broker。
...
在 Grafana 中检查 wis2downloader 仪表板，查看新添加的订阅。等待几分钟，您应该会看到开始下载的第一个数据。确认下载开始后，进入下一个练习。

wis2box 中的 wis2downloader 服务将数据下载到 `wis2box.env` 文件中定义的 `WIS2BOX_HOST_DATADIR` 目录下的 'downloads' 目录中。要查看 downloads 目录的内容，请使用以下命令：
...
查看 Grafana 中的 wis2downloader 仪表板以查看下载进度，您将在仪表板左上角看到您添加的订阅，以及随着数据下载数量的增加而变化的下载数量：
...
!!! note "Origin 与 Cache 主题"
    ...
    当订阅以 `cache/` 开头的主题时，您将为相同的数据收到多个通知，每个通知对应一个 Global Cache。每条通知将包含一个指向相应 Global Cache 数据服务器的规范 URL。`wis2downloader` 将从它能够访问的第一个规范 URL 下载数据。
...
此操作演示如何订阅非默认代理，并允许您下载从 WIS2 Training Broker 发布的一些数据。
...
通过再次检查 wis2downloader 的日志确认数据是否已下载：
...
您应该会看到与您订阅的主题对应的新目录，其中包含已下载的数据。
