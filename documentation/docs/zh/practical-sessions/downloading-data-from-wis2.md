...
wis2downloader 作为 wis2box 中的一个独立容器包含在 Docker Compose 文件中。wis2box 中的 Prometheus 容器被配置为从 wis2downloader 容器中抓取指标，这些指标可以通过 Grafana 仪表板进行可视化。
...
!!! note "更新 wis2downloader 的配置"
    ...
    您将看到 wis2downloader 服务以新的配置重新启动。
...
在 Grafana 中检查 wis2downloader 仪表板，查看新添加的订阅。等待几分钟，您应该会看到开始下载的第一个数据。确认下载开始后，继续下一个练习。

wis2box 中的 wis2downloader 服务将数据下载到您在 `wis2box.env` 文件中定义的 `WIS2BOX_HOST_DATADIR` 目录下的 `downloads` 目录中。要查看下载目录的内容，可以使用以下命令：
...
??? success "点击查看答案"
    ...
    根据您开始订阅的时间，您可能会或可能不会在该目录中看到任何下载的文件。如果尚未看到文件，请再等待几分钟后再次检查。
...
!!! note
    ...
    wis2downloader 容器中的目录 `app/data/downloads` 映射到 `wis2box.env` 文件中定义的 `WIS2BOX_HOST_DATADIR` 中的 `downloads` 目录。上述命令会删除所有下载的数据。
...
!!! note "origin 与 cache 主题的区别"
    ...
    当订阅以 `cache/` 开头的主题时，您将为相同的数据收到多个通知，每个 Global Cache 都会发送一条通知。每条通知将包含一个规范 URL，指向相应 Global Cache 的数据服务器。`wis2downloader` 将从它能够访问的第一个规范 URL 下载数据。
...
编辑 `wis2box.env` 文件，并将 `DOWNLOAD_BROKER_HOST` 更改为 `wis2training-broker.wis2dev.io`，`DOWNLOAD_BROKER_PORT` 更改为 `1883`，`DOWNLOAD_BROKER_TRANSPORT` 更改为 `tcp`：
...
接下来尝试添加另外两个订阅，以从以下主题下载月度地表温度异常数据和全球集合预报数据：
