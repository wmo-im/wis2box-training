...
!!! note "关于 wis2downloader"
     ...
     wis2downloader 也可以作为独立服务运行，可以部署在与发布 WIS2 通知的系统不同的系统上。有关作为独立服务使用 wis2downloader 的更多信息，请参阅 [wis2downloader](https://pypi.org/project/wis2downloader/)。
...
- DOWNLOAD_BROKER_HOST: 要连接的 MQTT Broker 主机名。默认为 globalbroker.meteo.fr
- DOWNLOAD_BROKER_PORT: 要连接的 MQTT Broker 端口。默认为 443（用于 WebSocket 的 HTTPS）
- DOWNLOAD_BROKER_USERNAME: 连接 MQTT Broker 时使用的用户名。默认为 everyone
- DOWNLOAD_BROKER_PASSWORD: 连接 MQTT Broker 时使用的密码。默认为 everyone
- DOWNLOAD_BROKER_TRANSPORT: 连接 MQTT Broker 使用的传输机制，可选值为 websockets 或 tcp。默认为 websockets
- DOWNLOAD_RETENTION_PERIOD_HOURS: 下载数据的保留时间（小时）。默认为 24
- DOWNLOAD_WORKERS: 使用的下载工作线程数。默认为 8，决定并行下载的数量。
- DOWNLOAD_MIN_FREE_SPACE_GB: 保留在存储下载数据的卷上的最小可用空间（GB）。默认为 1。
...
在 Grafana 中检查 wis2downloader 仪表板，查看新添加的订阅。等待几分钟，您应该会看到开始下载的第一批数据。确认下载开始后，继续下一步练习。
...
??? success "点击查看答案"
    您应该会看到一个目录结构，从 `cache/a/wis2/de-dwd-gts-to-wis2/` 开始，在其下方是更多以下载数据的 GTS 公报头命名的目录。

    根据您开始订阅的时间，您可能会或可能不会在此目录中看到任何下载的文件。如果您尚未看到任何文件，请再等待几分钟并再次检查。

在继续下一步练习之前，让我们清理订阅和下载的数据。
...
!!! note "原始主题 vs 缓存主题"
    ...
    当订阅以 `cache/` 开头的主题时，您将为相同的数据收到多个通知，每个通知对应一个 Global Cache。每个通知都包含指向相应 Global Cache 数据服务器的规范 URL。wis2downloader 将从它能够访问的第一个规范 URL 下载数据。

## 从 WIS2 Training Broker 下载示例数据

在本练习中，您将订阅 WIS2 Training Broker，该 Broker 发布用于培训目的的示例数据。

### 更改 wis2downloader 配置

此示例演示了如何订阅一个非默认的代理，以便下载从 WIS2 Training Broker 发布的一些数据。

编辑 `wis2box.env` 文件，将 `DOWNLOAD_BROKER_HOST` 修改为 `wis2training-broker.wis2dev.io`，`DOWNLOAD_BROKER_PORT` 修改为 `1883`，`DOWNLOAD_BROKER_TRANSPORT` 修改为 `tcp`：
...
等待直到你在 Grafana 的 wis2downloader 仪表板中看到下载开始。
...
接下来，尝试添加另外两个订阅，以下载以下主题的月度地表温度异常数据和全球集合预报数据：
