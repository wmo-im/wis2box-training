...
在本课程中，您将学习如何设置 WIS2 Broker 的订阅，并使用 wis2box 中包含的 "wis2downloader" 服务自动将数据下载到本地系统。

!!! note "关于 wis2downloader"
     
     wis2downloader 也可以作为独立服务运行，且可以部署在与发布 WIS2 通知的系统不同的系统上。有关作为独立服务使用 wis2downloader 的更多信息，请参阅 [wis2downloader](https://pypi.org/project/wis2downloader/)。
...
wis2downloader 作为 wis2box 中的一个独立容器包含在 Docker Compose 文件中定义的服务中。wis2box 中的 Prometheus 容器配置为从 wis2downloader 容器中抓取指标，这些指标可以通过 Grafana 仪表板可视化。

### 在 Grafana 中查看 wis2downloader 仪表板

打开一个网页浏览器，导航到您的 wis2box 实例的 Grafana 仪表板，访问 `http://YOUR-HOST:3000`。
...
- DOWNLOAD_BROKER_TRANSPORT: 连接 MQTT Broker 时使用的传输机制，可选值为 websockets 或 tcp。默认为 websockets
- DOWNLOAD_RETENTION_PERIOD_HOURS: 下载数据的保留时间（小时）。默认为 24
- DOWNLOAD_WORKERS: 使用的下载工作线程数。默认为 8，决定并行下载的数量。
- DOWNLOAD_MIN_FREE_SPACE_GB: 保留在存储下载数据的卷上的最小空闲空间（GB）。默认为 1。
...
!!! note "更新 wis2downloader 配置"
    ...
    要更新 wis2downloader 的配置，您可以编辑 `wis2box.env` 文件。应用更改后，可以重新运行 wis2box 堆栈的启动命令：

    ```bash
    python3 wis2box-ctl.py start
    ```

    您将看到 wis2downloader 服务使用新配置重新启动。
...
在 Grafana 中检查 wis2downloader 仪表板，查看新添加的订阅。等待几分钟，您应该会看到开始下载的第一批数据。确认下载开始后，继续下一个练习。
...
??? success "点击查看答案"
    您应该会看到一个目录结构，从 `cache/a/wis2/de-dwd-gts-to-wis2/` 开始，在其下方可以看到更多以下载数据的 GTS 公报头命名的目录。

    根据您开始订阅的时间，您可能会或可能不会在此目录中看到任何下载的文件。如果尚未看到任何文件，请再等待几分钟并再次检查。
...
检查 Grafana 中的 wis2downloader 仪表板，确认订阅已被移除。您应该会看到下载停止。
...
!!! note "origin 与 cache 主题"
    ...
    当订阅以 `origin/` 开头的主题时，您将收到包含指向由发布数据的 WIS 中心提供的数据服务器的规范 URL 的通知。

    当订阅以 `cache/` 开头的主题时，您将收到针对相同数据的多个通知，每个通知对应一个 Global Cache。每个通知将包含指向相应 Global Cache 数据服务器的规范 URL。wis2downloader 将从其能够访问的第一个规范 URL 下载数据。

## 从 WIS2 Training Broker 下载示例数据

在本练习中，您将订阅 WIS2 Training Broker，该 Broker 发布用于培训的示例数据。

### 修改 wis2downloader 配置

这演示了如何订阅一个非默认的代理，并允许您下载从 WIS2 Training Broker 发布的一些数据。

编辑 `wis2box.env` 文件，将 `DOWNLOAD_BROKER_HOST` 更改为 `wis2training-broker.wis2dev.io`，`DOWNLOAD_BROKER_PORT` 更改为 `1883`，`DOWNLOAD_BROKER_TRANSPORT` 更改为 `tcp`：
...
现在我们将设置一个新的订阅，以从 WIS2 Training Broker 下载气旋轨迹数据。
...
??? success "点击查看答案"
    ...
    下载的数据是 BUFR 格式的，文件扩展名为 `.bufr`。

接下来，尝试添加另外两个订阅，以下载以下主题的月度地表温度异常和全球集合预报数据：
...
您应该看到两个新目录，分别为 `origin/a/wis2/int-wis2-training/data/core/weather/prediction/forecast/medium-range/probabilistic/global` 和 `origin/a/wis2/int-wis2-training/data/core/climate/experimental/anomalies/monthly/surface-temperature`，其中包含下载的数据。
...
??? success "点击查看答案"
    ...
    `../prediction/forecast/medium-range/probabilistic/global` 主题的下载数据是 GRIB2 格式的，文件扩展名为 `.grib2`。

    `../climate/experimental/anomalies/monthly/surface-temperature` 主题的下载数据是 NetCDF 格式的，文件扩展名为 `.nc`。
...
!!! success "恭喜！"
    ...
    - 在 Grafana 仪表板中查看下载状态
    - 更改 wis2downloader 的默认配置以订阅不同的代理
    - 在本地系统上查看下载的数据