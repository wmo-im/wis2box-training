...
在 WIS2 中，所有数据集都有一个元数据文件，可以在 **Global Discovery Catalogues** 中找到。因此，用户应始终查询这些服务以找到在 WIS2 上共享的数据。

WIS2 Downloader 采用这一原则，通过查找这些 GDC 中可用的所有记录并在内部组合它们，使用户能够浏览 WIS2 上可用的数据。由于记录数量庞大，必须提供一种方法让用户通过筛选找到正确的记录。即使找到了正确的记录并订阅了它，某些数据集中的文件数量可能仍然超出用户当前的需求。因此，需要第二级过滤——在决定是否下载文件时进行操作。

## 在目录视图中使用

**目录视图** 是在 WIS2 Downloader 中查找和订阅数据集的两种方式之一。它汇总来自 Global Discovery Catalogues 的记录，并以可搜索、可筛选的界面呈现——类似于直接浏览 GDC 门户。

在左侧边栏导航到 **目录视图**。

![WIS2 Downloader 目录视图](../assets/img/wis2-downloader-catalogue-view.png)

在页面顶部，您会看到一个搜索栏和一组过滤器。您可以使用这些工具通过关键字、Centre ID 或数据策略（核心 vs 推荐）缩小可用记录的列表。

您还可以通过定义一个 **边界框** 来进行空间过滤，使用四个坐标输入框——**北**、**西**、**南** 和 **东**——以十进制纬度和经度值表示。当设置边界框时，您可以选择两种匹配模式：

- **交集** — 返回空间范围与边界框有任何重叠的记录。
...
??? success "点击查看答案"
    ...
    任何主题包含 `surface-based-observations` 且数据策略为 `核心` 的记录都是有效选择。应用关键字过滤 `temperature, precipitation` 将进一步缩小与这些变量相关的数据集的结果。

    一旦订阅，**管理订阅** 视图将显示活动订阅及其主题和目标文件夹。文件将在代理上收到新通知时开始下载。
...
**树视图** 以可折叠树的形式呈现 WIS2 主题层次结构，允许您逐级浏览可用主题——类似于在 MQTT Explorer 中导航主题。它设计用于从高层次、从上到下探索 WIS2 上可用的数据，从层次结构的根开始逐步深入。这与目录视图形成对比，后者直接将您带到单个数据集记录，更适合当您已经知道自己在寻找什么时使用。

在左侧边栏导航到 **树视图**。

![WIS2 Downloader 树视图](../assets/img/wis2-downloader-tree-view.png)

树按照 WIS2 主题层次结构组织。通过点击节点展开每一级以显示其子节点。在任何级别，您都可以通过选择一个节点并点击 **订阅** 来订阅——使用通配符 (`#`) 捕获该节点下的所有主题。

!!! note "在不同级别订阅"

    在树中较高的级别订阅（例如在 Centre ID 级别）将捕获该中心发布的所有数据集。在较低级别订阅可以提供更细粒度的控制。使用 WIS2 Downloader 在树视图订阅时自动附加的通配符 `#` 后缀。
...
**手动订阅** 视图允许您直接输入主题创建订阅，而无需依赖 Global Discovery Catalogues。与目录视图和树视图——两者都从 GDC 中获取其主题——不同，手动订阅在您已经知道确切的主题并希望设置它时非常有用，无需浏览目录，并且可以更自由地选择使用的 WTH。
...
- **过滤器** — 控制下载哪些通知的可选过滤对象，以文本形式提供。
- **优先队列** — 控制从该订阅接收的通知的下载优先级。
- **身份验证** — 访问受控数据集所需的凭据。

!!! note "何时使用手动订阅"

    当您已经知道确切的主题并希望快速设置它时使用手动订阅；当主题未包含在目录中时，或当您需要为受访问控制的数据集提供凭据时。
...
- **Bearer token** — 提供由数据发布者颁发的令牌，以代替用户名和密码。
...
!!! question "设置并订阅受访问控制的数据集"
    ...
    **步骤 1 — 在 wis2box 上创建受访问控制的数据集**
    ...
    在您的 wis2box 实例上创建一个启用了访问控制的数据集，并记录生成的主题和 Bearer token。如果尚未完成，请参考 [Datasets with access control](datasets-with-access-control.md) 实践课程以获取完整的设置步骤。

**步骤 2 — 配置 WIS2 Downloader 监听 wis2box broker**

默认情况下，WIS2 Downloader 监听 Global Broker。为了直接接收来自您的 wis2box 实例的通知，您需要在 WIS2 Downloader 的 compose 文件中添加一个订阅者，指向 wis2box 内部的 MQTT broker。
...
1. 进入 WIS2 Downloader UI 的 **Manual Subscribe** 页面。
...
在您的 wis2box 实例上，将文件发布到受控访问的数据集。有关数据发布的步骤，请参考 [Ingesting data for publication](ingesting-data-for-publication.md) 实践课程。
...
使用有效的 bearer token 时，WIS2 Downloader 会在下载受限主题的文件时进行身份验证。步骤 4 中发布的文件应在被 wis2box 接收后不久出现在 `restricted-data` 文件夹中。

如果身份验证失败，即使订阅在 **Manage Subscriptions** 页面中显示为活跃状态，文件也不会被下载。请仔细检查 bearer token 是否与 wis2box 数据集配置的 token 匹配。

!!! note "取消订阅并删除已下载文件"

进入 **Manage Subscriptions** 页面，点击 **Unsubscribe** 取消订阅主题，然后清理下载文件夹：
...
在 **Catalogue View** 或 **Tree View** 中选择数据集后，右侧屏幕会出现一个过滤器面板。在订阅之前，您可以填写想要应用的过滤器值。WIS2 Downloader 会根据您的输入自动生成过滤器对象。

在 **Manual Subscribe** 页面中，您需要手动输入此过滤器对象，填写表单中的 `Filter (JSON)` 输入字段。

!!! note "可用过滤器输入"

- **媒体类型** — 限制下载为特定内容类型（例如 `application/bufr`）。
- **数据集** — 根据元数据标识符限制下载为特定数据集。
- **边界框** — 限制下载为数据位于特定空间区域的通知，定义范围包括 `north`、`south`、`east` 和 `west` 值。
- **日期和时间范围** — 限制下载为在特定时间范围内发布的通知。
- **自定义过滤器** — 根据元数据记录中定义的通知属性值过滤，例如通过 `wigos_station_identifier` 过滤，仅下载来自特定站点的数据。
...
1. 进入 **Catalogue View**，搜索您选择的地面观测数据集。
2. 选择数据集以在右侧面板中展开其详细信息。
3. 在过滤器输入中，为您选择的区域设置 **边界框**。
4. 可选：设置 **媒体类型** 过滤器以限制下载为 BUFR 文件。
...
只有符合您定义的所有条件的通知才会被接受并下载。所有其他通知将被默认的兜底规则拒绝。
...
- 如何直接使用 Manual Subscribe 页面订阅主题
- 如何应用过滤器控制从订阅中下载的文件
- 如何使用身份验证从受控访问数据集中下载文件