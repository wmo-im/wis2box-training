...
在 WIS2 中，所有数据集都包含一个元数据文件，可以在 **Global Discovery Catalogues** 中找到。因此，建议用户始终使用这些服务来查找在 WIS2 上共享的数据。

WIS2 Downloader 利用这一原则，通过查找这些 GDC 中的所有可用记录并在内部进行整合，使用户能够浏览 WIS2 上的数据。由于记录数量庞大，必须提供一种方式让用户能够过滤这些记录并找到正确的记录。即使找到了正确的记录并订阅了它，某些数据集中的文件数量可能仍然超出用户当前的需求。因此，需要第二级过滤——在决定是否下载文件时进行操作。

## 在目录视图中使用

**目录视图** 是在 WIS2 Downloader 中查找和订阅数据集的两种方式之一。它从 Global Discovery Catalogues 中聚合记录，并以可搜索、可过滤的界面呈现——类似于直接浏览 GDC 门户。

在左侧边栏导航到 **目录视图**。

![WIS2 Downloader Catalogue View](../assets/img/wis2-downloader-catalogue-view.png)

页面顶部有一个搜索栏和一组过滤器。您可以使用这些工具通过关键字、Centre ID 或数据策略（核心 vs 推荐）来缩小可用记录的范围。
...
??? success "点击显示答案"
    ...
    任何主题包含 `surface-based-observations` 且数据策略为 `核心` 的记录都是有效选择。应用关键字过滤器 `temperature, precipitation` 将进一步缩小结果范围到与这些变量相关的数据集。

    订阅后，**管理订阅** 视图将显示活动订阅及其主题和目标文件夹。文件将在代理上收到新通知时开始下载。

!!! note "取消订阅并删除下载的文件"
    
    前往 **管理订阅** 视图并从上一练习中选择的主题中 `取消订阅`。
...
**树视图** 以可折叠树的形式呈现 WIS2 主题层级，允许您逐级浏览可用主题——类似于在 MQTT Explorer 中导航主题。它设计用于更高层次的、自上而下的探索 WIS2 上的数据，从层级根部开始逐步深入。这与目录视图形成对比，后者直接带您到具体的数据集记录，更适合当您已经知道自己在寻找什么时使用。

在左侧边栏导航到 **树视图**。

![WIS2 Downloader Tree View](../assets/img/wis2-downloader-tree-view.png)

树结构按照 WIS2 主题层级组织。点击节点展开每一级以显示其子节点。在任何级别，您都可以通过选择一个节点并点击 **订阅** 来订阅——使用通配符 (`#`) 捕获该节点下的所有主题。

!!! note "在不同层级订阅"

    在树结构较高层级订阅（例如在 Centre ID 层级）将捕获该中心发布的所有数据集。在较低层级订阅则提供更精细的控制。使用 WIS2 Downloader 在树视图中订阅时自动附加的通配符 `#` 后缀。
...
**手动订阅** 视图允许您直接输入主题创建订阅，而无需依赖 Global Discovery Catalogues。与目录视图和树视图不同——它们都从 GDC 中获取主题——手动订阅在您已经知道确切的主题并希望设置订阅时非常有用，无需浏览目录，同时对 WTH 的使用有更多自由。
...
- **目标文件夹** — 下载文件将保存的本地子目录。
- **过滤器** — 一个可选的文本形式过滤对象，用于控制下载哪些通知。
- **优先队列** — 控制从该订阅接收的通知的下载优先级。
- **身份验证** — 访问受控数据集所需的凭据。

!!! note "何时使用手动订阅"

    当您已经知道确切的主题并希望快速设置订阅时使用手动订阅；当主题未包含在目录中，或需要为受访问控制的数据集提供凭据时使用。
...
!!! question "设置并订阅受访问控制的数据集"
    ...
    **步骤 1 — 在 wis2box 上创建受访问控制的数据集**
    ...
    在您的 wis2box 实例上创建一个启用访问控制的数据集，并记录其主题和为其生成的 Bearer token。如果尚未完成，请参考 [Datasets with access control](datasets-with-access-control.md) 实践课程以完成完整设置步骤。
...
使用有效的 bearer token 时，WIS2 Downloader 会在下载受限主题的文件时进行身份验证。步骤 4 中发布的文件应在被 wis2box 处理后不久出现在 `restricted-data` 文件夹中。

如果身份验证失败，即使订阅在 **Manage Subscriptions** 视图中显示为活跃，文件也不会被下载。请仔细检查 bearer token 是否与 wis2box 数据集上配置的 token 匹配。

!!! note "取消订阅并删除下载的文件"

进入 **Manage Subscriptions** 视图，点击 **Unsubscribe** 取消订阅该主题，然后清理下载文件夹：
...
过滤器允许您在通知级别控制从订阅下载的文件——这是介绍中提到的第二级过滤。您可以定义过滤器，使只有符合特定条件的通知触发下载，而不是下载主题上发布的所有文件。

在 **Catalogue View** 或 **Tree View** 中选择数据集后，过滤器面板会出现在屏幕右侧。在订阅之前，您可以填写想要应用的过滤器值。WIS2 Downloader 会根据您的输入自动生成过滤器对象。
...
- **边界框** — 限制下载为数据位于特定空间区域的通知，定义方式为 `north`、`south`、`east` 和 `west` 值。
- **日期和时间范围** — 限制下载为在特定时间范围内发布的通知。
- **自定义过滤器** — 根据元数据记录中定义的任何其他通知属性进行过滤，通过指定属性值（例如，通过 `wigos_station_identifier` 过滤，仅下载来自特定站点的数据）。
...
3. 在过滤器输入中，为您选择的区域设置一个 **边界框**。
4. 可选：设置 **媒体类型** 过滤器以限制下载为 BUFR 文件。
5. 将目标文件夹设置为 `filtered-obs`。
6. 点击 **Subscribe** 创建订阅。

等待文件到达，并验证只有符合过滤器条件的文件被下载。

??? success "点击查看答案"

只有符合您定义的所有条件的通知会被接受并下载。所有其他通知都会被默认的兜底规则拒绝。
