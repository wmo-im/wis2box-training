...
WIS2 Downloader 基于这一原则，通过查找这些 GDC 中的所有可用记录并在内部进行整合，使用户能够浏览 WIS2 上的可用数据。由于记录数量庞大，必须为用户提供一种过滤方式，以便找到正确的记录。即使找到了正确的记录并订阅了它，某些数据集中的文件数量可能仍然超出用户当前的需求。因此，需要第二层过滤——在决定是否下载文件时进行操作。
...
??? success "点击查看答案"
    ...
    订阅后，**管理订阅** 视图将显示活动订阅及其主题和目标文件夹。随着代理上出现新通知，文件将开始下载。

!!! note "取消订阅并删除下载的文件"
    
    转到 **管理订阅** 视图并从上一练习中选择的主题 `取消订阅`。
...
树结构按照 WIS2 主题层次组织。点击节点展开每一级以显示其子节点。在任何级别，您都可以通过选择一个节点并点击 **订阅** 来订阅——使用通配符 (`#`) 捕获该节点下的所有主题。

!!! note "在不同级别订阅"

    在树中较高的级别订阅（例如在 Centre ID 级别）将捕获该中心发布的所有数据集。在较低级别订阅可以提供更精细的控制。使用 WIS2 Downloader 在树视图中订阅时自动附加的通配符 `#` 后缀。
...
**手动订阅** 视图允许您直接输入主题创建订阅，而无需依赖 Global Discovery Catalogues。与目录视图和树视图——两者都从 GDC 中获取主题——不同，手动订阅在您已经知道确切的主题并希望设置它时非常有用，无需浏览目录，并且可以更自由地选择 WTH。
...
!!! note "何时使用手动订阅"
    ...
    当您已经知道确切的主题并希望快速设置它时使用手动订阅；当主题未包含在目录中时使用；或者当您需要为受访问控制的数据集提供凭据时使用。
...
!!! question "设置并订阅受访问控制的数据集"
    ...
    **步骤 1 — 在 wis2box 上创建受访问控制的数据集**
    ...
    在您的 wis2box 实例上创建一个启用访问控制的数据集，并记录其主题和为其生成的 Bearer token。如果您尚未完成，请参考 [Datasets with access control](datasets-with-access-control.md) 实践课程以获取完整的设置步骤。
...
在您的 wis2box 实例上，将文件发布到受控访问的数据集。参考 [Ingesting data for publication](ingesting-data-for-publication.md) 实践课程了解数据摄取步骤。
...
使用有效的 bearer token 时，WIS2 Downloader 会在下载受限主题的文件时进行认证。步骤 4 中发布的文件应在被 wis2box 摄取后不久出现在 `restricted-data` 文件夹中。

如果认证失败，即使订阅在 **Manage Subscriptions** 视图中显示为活动状态，文件也不会被下载。请仔细检查 bearer token 是否与 wis2box 数据集配置的 token 匹配。
...
过滤器允许您在通知级别控制从订阅中下载哪些文件——这是介绍中提到的第二级过滤。您可以定义过滤器，以便只有符合特定条件的通知会触发下载，而不是下载主题上发布的所有文件。

在 **Catalogue View** 或 **Tree View** 中选择数据集后，过滤器面板会出现在屏幕右侧订阅之前。在这里，您可以填写想要应用的过滤值。WIS2 Downloader 会根据您的输入自动构建过滤器对象。
...
- **数据集** — 根据其元数据标识符限制下载为特定数据集。
- **边界框** — 限制下载为数据落在由 `north`、`south`、`east` 和 `west` 值定义的空间区域内的通知。
- **日期和时间范围** — 限制下载为在特定时间范围内发布的通知。
- **自定义过滤器** — 根据元数据记录中定义的任何其他通知属性进行过滤，指定属性值（例如通过 `wigos_station_identifier` 过滤，仅下载来自特定站点的数据）。
