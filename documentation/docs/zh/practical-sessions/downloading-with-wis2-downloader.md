...
WIS2 Downloader 基于这一原则，通过查找这些 GDC 中的所有可用记录并在内部组合它们，使用户能够浏览 WIS2 上可用的数据。由于记录数量庞大，必须为用户提供一种过滤方式，以便找到正确的记录。即使找到了正确的记录并订阅了它，某些数据集中的文件数量可能仍然超过用户当前的需求。因此，需要第二级过滤——在决定是否下载文件时进行操作。
...
在页面顶部，您会看到一个搜索栏和一组过滤器。您可以使用这些工具通过关键字、Centre ID 或数据策略（核心 vs 推荐）来缩小可用记录的范围。

您还可以通过定义一个 **边界框** 来进行空间过滤，使用四个坐标输入——**北**、**西**、**南** 和 **东**——以十进制纬度和经度值表示。当设置边界框时，您可以选择以下两种匹配模式之一：
...
**手动订阅** 视图允许您直接输入主题创建订阅，而无需依赖 Global Discovery Catalogues。与目录视图和树视图不同——它们都从 GDC 中获取主题——手动订阅在您已经知道确切的主题并希望快速设置时非常有用，同时可以更自由地选择使用的 WTH。
...
- **目标文件夹** — 下载的文件将保存的本地子目录。
- **过滤器** — 控制下载哪些通知的可选过滤对象，以文本形式提供。
- **优先队列** — 控制从此订阅接收的通知的下载优先级。
- **身份验证** — 访问受控数据集所需的凭据。

!!! note "何时使用手动订阅"

    当您已经知道确切的主题并希望快速设置时使用手动订阅；当主题未包含在目录中时使用；或者当您需要为受访问控制的数据集提供凭据时使用。
...
在本练习中，您将在 wis2box 实例上设置一个受访问控制的数据集，配置 WIS2 Downloader 订阅其代理，并验证在提供 Bearer token 时文件是否正确下载。

!!! question "设置并订阅受访问控制的数据集"

    **步骤 1 — 在 wis2box 上创建受访问控制的数据集**

    在您的 wis2box 实例上创建一个启用访问控制的数据集，并记录其主题和为其生成的 Bearer token。如果尚未完成，请参考 [Datasets with access control](datasets-with-access-control.md) 实践课程以获取完整设置步骤。
...
使用有效的 bearer token 时，WIS2 Downloader 在下载受限主题的文件时会进行身份验证。步骤 4 中发布的文件应在被 wis2box 摄取后不久出现在 `restricted-data` 文件夹中。

如果身份验证失败，即使订阅在 **Manage Subscriptions** 视图中显示为活动状态，文件也不会被下载。请仔细检查 bearer token 是否与 wis2box 数据集配置的 token 匹配。
...
- **边界框** — 限制下载为数据落在空间区域内的通知，区域由 `north`、`south`、`east` 和 `west` 值定义。
