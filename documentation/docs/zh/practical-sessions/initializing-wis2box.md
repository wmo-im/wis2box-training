...
??? success "点击查看答案"
    ...
    该 URL 指向 OpenStreetMap 瓦片服务器。如果您希望使用其他地图提供商，可以将此 URL 更改为指向其他瓦片服务器。
...
!!! note
    ...
    除非您确定所做的更改，否则不要编辑 `wis2box.env` 文件。不正确的更改可能导致 wis2box 无法正常工作。

    不要与任何人共享 `wis2box.env` 文件的内容，因为其中包含密码等敏感信息。
...
登录后，将鼠标移至左侧菜单以查看 wis2box Web 应用程序中的可用选项：
