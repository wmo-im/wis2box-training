...
接下来，您将被要求输入您的 wis2box 的 URL。此 URL 将用于访问 wis2box 的 Web 应用程序、API 和 UI。
...
无需担心记住这些密码，它们将存储在您 wis2box 目录中的 `wis2box.env` 文件中。
...
!!! note "wis2box 和 Docker"
    ...
    Python 脚本 `wis2box-ctl.py` 用于运行底层 Docker Compose 命令以控制 wis2box 服务。

    您无需了解 Docker 容器的详细信息即可运行 wis2box 软件栈，但可以查看 `docker-compose*.yml` 文件以了解服务的定义。如果您对 Docker 感兴趣，可以在 [Docker 文档](https://docs.docker.com/) 中找到更多信息。
...
注意，登录后您的提示符会发生变化，表明您现在位于 wis2box-management 容器内：
...
登录后，将鼠标移到左侧菜单以查看 wis2box Web 应用程序中可用的选项：
...
使用以下连接详细信息，确保将 `<your-host>` 的值替换为您的主机名，并将 `<WIS2BOX_BROKER_PASSWORD>` 的值替换为您的 `wis2box.env` 文件中的值：
...
同样的连接也可以通过 WebSockets 使用 MQTT 在端口 80 上实现，连接详细信息如下：
