...
使用您的用户名和密码登录到指定的虚拟机（VM），并确保您位于 `wis2box` 目录下：
...
- 主机上的一个目录，用于在主机和由 `WIS2BOX_HOST_DATADIR` 环境变量定义的 wis2box 容器之间共享数据

可以使用 `wis2box-create-config.py` 脚本来创建 wis2box 的初始配置。

脚本将引导您回答一系列问题以帮助设置配置。
...
脚本将提示您输入用于 `WIS2BOX_HOST_DATADIR` 环境变量的目录。
...
无需担心记住这些密码，它们将存储在 `wis2box.env` 文件中，位于您的 wis2box 目录下。

### 检查 `wis2box.env`

脚本完成后，检查当前目录下 `wis2box.env` 文件的内容：
...
??? success "点击查看答案"
    ...
    wis2box-management 容器每天运行一个 cronjob，将 `wis2box-public` 存储桶和 API 后端中超过 WIS2BOX_STORAGE_DATA_RETENTION_DAYS 定义天数的数据删除：
...
注意，登录后，您的提示符将发生变化，表示您现在位于 wis2box-management 容器内：
...
打开一个新标签页，访问页面 `http://YOUR-HOST/oapi`。

<img alt="wis2box-api.png" src="/../assets/img/wis2box-api.png" width="800">

这是 wis2box API 的首页（通过 **wis2box-api** 容器运行）。
...
打开浏览器并访问页面 `http://YOUR-HOST/wis2box-webapp`。
...
使用以下连接详细信息，确保将 `<your-host>` 替换为您的主机名，将 `<WIS2BOX_BROKER_PASSWORD>` 替换为您 `wis2box.env` 文件中的值：
...
!!! note 
    ...
    请注意，这是您的 **内部** 代理密码，Global Broker 将使用不同的（只读）凭据订阅您的代理。切勿与任何人分享此密码。
