...
当提示输入 `WIS2BOX_WEBAPP_PASSWORD`、`WIS2BOX_STORAGE_PASSWORD` 和 `WIS2BOX_BROKER_PASSWORD` 时，您可以选择随机生成密码，也可以自定义密码。
...
??? success "点击查看答案"
    ...
    此 URL 指向 OpenStreetMap 瓦片服务器。如果您想使用其他地图提供商，可以将此 URL 更改为指向其他瓦片服务器的地址。
...
!!! note
    ...
    除非您确定更改内容，否则不要编辑 `wis2box.env` 文件。不正确的更改可能导致 wis2box 无法正常工作。

    不要与任何人共享您的 `wis2box.env` 文件内容，因为其中包含密码等敏感信息。
...
使用以下连接详情，确保将 `<your-host>` 的值替换为您的主机名，将 `<WIS2BOX_BROKER_PASSWORD>` 的值替换为您 `wis2box.env` 文件中的值：
...
!!! note 
    ...
    请注意，这是您的 **内部** 代理密码，Global Broker 将使用不同的（只读）凭据订阅您的代理。切勿与任何人共享此密码。
