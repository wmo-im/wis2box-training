...
在 WMO 中不被视为“核心”的数据集可以选择配置访问控制策略。wis2box 提供了一种机制，可以为数据集添加访问令牌，从而防止用户在未提供 HTTP 头中的访问令牌时下载数据。
...
将 WMO 数据策略更改为“推荐”，您会看到表单中新增了一个输入字段，用于提供数据集许可证信息的 URL：
...
或者，您可以使用 `WIS2BOX_URL/data/license.txt` 指向您自己 Web 服务器上托管的自定义许可证文件，其中 `WIS2BOX_URL` 是您在 wis2box.env 文件中定义的 URL：
...
检查 MQTT Explorer，查看是否收到 WIS2 通知消息，宣布主题 `origin/a/wis2/<your-centre-id>/metadata` 上的新发现元数据记录。
...
打开您刚刚创建的数据集的链接，向下滚动到 JSON 响应的“links”部分：

<img alt="wis2box-api-recommended-dataset-links" src="/../assets/img/wis2box-api-recommended-dataset-links.png" width="600">

您应该会看到一个指向您在数据集编辑器中提供的 URL 的“License for this dataset”链接。

如果您使用了 `http://YOUR-HOST/data/license.txt` 作为许可证 URL，则该链接目前无法工作，因为我们尚未向 wis2box 实例添加许可证文件。
...
接下来，进入 wis2box-webapp 的站点编辑器。对于您在 `aws-example.csv` 中使用的每个站点，更新“topic”字段以匹配您在上一个练习中创建的数据集的“topic”。

此站点现在将与两个主题相关联，一个用于“核心”数据集，一个用于“推荐”数据集：
...
检查 WIS2 通知消息中的规范链接，并将链接复制粘贴到浏览器中尝试下载数据。

您应该会看到 *401 Unauthorized*。

## 将访问令牌添加到 HTTP 头以访问数据集

为了演示访问数据集需要访问令牌，我们将使用命令行功能 `wget` 重现您在浏览器中看到的错误。

在学生虚拟机的命令行中，使用 `wget` 命令和您从 WIS2 通知消息中复制的规范链接。
...
要上传本地创建的文件 license.txt，请使用 wis2box 实例 9001 端口上的 MinIO 控制台，通过 Web 浏览器访问 `http://YOUR-HOST:9001`。

wis2box.env 文件中定义的 MinIO 控制台访问凭据由环境变量 `WIS2BOX_STORAGE_USERNAME` 和 `WIS2BOX_STORAGE_PASSWORD` 定义。您可以通过以下方式在 wis2box.env 文件中找到这些变量：
...
登录 MinIO 控制台后，您可以使用“Upload”按钮将许可证文件上传到 **wis2box-public** bucket 的基本路径：
...
!!! note
    ...
    wis2box 中的 Web 代理会代理存储在 "wis2box-public" bucket 中的所有文件，路径为 `WIS2BOX_URL/data/`

包含在推荐数据集元数据中的“License for this dataset”链接现在应该可以工作。
