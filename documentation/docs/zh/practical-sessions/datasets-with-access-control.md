...
根据 WMO 统一数据政策，数据在 WIS2 上共享，该政策定义了两类数据。

**核心**：免费且无条件限制地提供数据，无需付费且对使用没有任何限制。

**推荐**：数据可能附带使用条件和/或需要许可证。
...
!!! note "WIS2 中的航空数据"
    ...
    WIS2 中的航空数据应始终以“推荐”的形式共享，以符合国际民航组织 (ICAO) 规定的航空数据共享法规。
...
确保您已打开一个 web 浏览器并访问您的实例的 wis2box-webapp，网址为 `http://YOUR-HOST/wis2box-webapp`。
...
!!! note "关于许可证 URL"
    ...
    与推荐数据集关联的许可证 URL 告知数据消费者有关此数据集的使用条件。
...
由于您选择了 Template = 'other'，数据集未预填任何关键词。请为数据集添加至少 3 个相关关键词：
...
由于您选择了 Template = 'other'，未定义数据集映射。
...
检查 MQTT Explorer，验证您是否收到在主题 `origin/a/wis2/<your-centre-id>/metadata` 上发布的新发现元数据记录的 WIS2 通知消息。
...
要将文件导入到您的数据集中，请进入 **wis2box-incoming** 存储桶，并创建一个以您的数据集元数据标识符命名的新文件夹，然后使用“Upload”按钮将示例 METAR 数据文件上传到该文件夹中：
...
!!! note "故障排除"
    ...
    如果在上传数据后未收到通知，您可以检查 `wis2box-management` 容器的最近日志来排查问题：
...
为了演示访问数据集需要访问令牌，我们将使用命令行工具 `wget` 重现您在浏览器中看到的错误。
...
然后使用命令 `wis2box auth remove-token` 从数据集中移除访问令牌，并使用 `--metadata-id` 标志指定数据集的元数据标识符，将访问令牌作为参数传入：
...
并通过尝试使用带有 HTTP 头中访问令牌的 `wget` 下载数据，或尝试在浏览器中访问数据，验证访问令牌已被移除。

!!! note "访问控制是可选的，仅适用于推荐数据集"

    WIS2 规定推荐数据集*可以*对数据应用访问控制，但这并非强制要求。您可以仅依赖许可 URL 来告知数据消费者数据的使用条件。如果您确实应用了访问控制，您有责任将访问令牌分享给需要访问此数据的消费者。

    如果您对具有 WMO 数据政策“核心”的数据集应用了 `wis2box auth add-token`，由于 Global Caches 无法下载数据，Global Monitoring 将报告错误，您的数据集将被视为*不符合* WIS2 技术法规。
