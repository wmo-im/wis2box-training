...
!!! note "WIS2 中的航空数据"
    ...
    在本次练习中，您需要创建一个数据集以共享 METAR 数据，这是一种用于报告航空气象观测的标准格式。

    由于航空气象数据受到使用限制，因此适用**推荐**数据策略。
...
确保您已打开一个 Web 浏览器并访问您的实例的 wis2box-webapp，地址为 `http://YOUR-HOST/wis2box-webapp`。

## 创建一个数据策略为“推荐”的新数据集

进入 wis2box-webapp 的“数据集编辑器”页面并创建一个新数据集。

对于“Centre ID”，请使用您在之前实践课程中使用的相同值。
...
由于您选择了模板 = 'other'，因此数据集未预填任何关键词。请为数据集添加至少 3 个相关关键词：
...
使用 MQTT Explorer 检查是否收到 WIS2 通知消息，通知主题为 `origin/a/wis2/<your-centre-id>/metadata` 上的新发现元数据记录。
...
您应该会看到一个指向数据集许可证的链接，链接地址为数据集编辑器中提供的 URL。

如果您点击该链接，由于许可证文件尚未添加到您的 wis2box 实例中，您将收到错误。

## 将许可证文件添加到您的 wis2box 实例

确保推荐数据集元数据中“License for this dataset”的链接正常工作。
...
要上传此文件，请使用 wis2box 实例 9001 端口上的 MinIO 控制台，通过 Web 浏览器访问 `http://YOUR-HOST:9001`。
...
登录 MinIO 控制台后，使用“Upload”按钮将许可证文件上传到 **wis2box-public** 存储桶的根路径：
...
推荐数据集元数据中包含的“License for this dataset”链接现在应能正常工作。
...
要将文件导入到您的数据集中，请进入 **wis2box-incoming** 存储桶，并使用您的数据集的元数据标识符名称创建一个新文件夹，然后使用“上传”按钮将示例 METAR 数据文件上传到此文件夹中：

![minio-wis2box-incoming-metar-data-uploaded](../assets/img/minio-wis2box-incoming-metar-data-uploaded.png)

确保文件夹名称与您的数据集的元数据标识符相同，并检查您是否在 MQTT Explorer 中接收到 WIS2 数据通知，主题为 `origin/a/wis2/<your-centre-id>/data/recommended/aviation/metar`：
...
如果您已正确将访问令牌添加到数据集中，则您**不应**能够在浏览器中访问数据，而是会看到错误 *401 Authorization Required*。
...
!!! note "访问控制是可选的，仅适用于推荐数据集"
    ...
    WIS2 规定推荐数据集*可以*对数据应用访问控制。这并不是强制性的，您可以仅依赖许可证 URL 来告知数据使用者数据的使用条件。如果您应用了访问控制，则有责任将访问令牌共享给需要访问此数据的使用者。

    如果您对 WMO 数据政策为“核心”的数据集应用了 `wis2box auth add-token`，由于全局缓存无法下载数据，全球监控将报告错误，并且您的数据集将被视为*不符合* WIS2 技术法规。
