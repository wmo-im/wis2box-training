...
WMO二进制格式（如BUFR和GRIB）在气象领域广泛用于观测和模型数据的交换，通常需要专门的工具来解码和可视化这些数据。
...
我们将使用 `~/exercise-materials/notebook-examples` 中包含的示例笔记本来解码热带气旋轨迹的下载数据。
...
- `-v ~/wis2-downloads:/root/downloads` 将您的虚拟机上的 `~/wis2-downloads` 目录挂载到容器中的 `/root/downloads`。这是您在之前实践课程中使用WIS2 Downloader下载数据后存储数据的地方。
...
容器启动后，您可以通过浏览器访问您的学生虚拟机上的Jupyter笔记本服务器，地址为 `http://YOUR-HOST:8888`。
...
双击 `example-notebooks` 目录以打开它。您应该看到以下屏幕列出示例笔记本：
...
执行所有单元格后，您应该看到一个关于 "850hPa温度异常低于-1.5标准差的概率" 的可视化图：
...
阅读笔记本中的说明并运行单元格以解码热带气旋轨迹的下载数据。通过点击单元格然后点击工具栏中的运行按钮或按 `Shift+Enter` 来运行每个单元格。

最后，您应该看到一个关于热带气旋轨迹的罢工概率的图表：
...
??? success "点击查看答案"
    ...
    要更新笔记本以显示不同距离范围内的热带风暴轨迹预测概率，您可以更新计算罢工概率的代码块中的 `distance_threshold` 变量。
...
!!! note "解码NetCDF数据"
    ...
    NetCDF是一种灵活的格式，在本示例中报告了变量 'anomaly' 的值，并沿 'lat' 和 'lon' 维度报告。不同的NetCDF数据集可能使用不同的变量名称和维度。
...
- [Panoply](https://www.giss.nasa.gov/tools/panoply/) - 一个跨平台应用程序，用于绘制来自NetCDF、HDF、GRIB和其他数据集的地理参考和其他数组
