...
!!! abstract "学习目标！"
    ...
    - 运行一个基于 "demo-decode-eccodes-jupyter" 镜像的Docker容器
    - 运行示例Jupyter笔记本以解码GRIB2、NetCDF和BUFR格式的数据
    - 了解其他工具以解码和可视化WMO表驱动代码格式（TDCF）

## 介绍

BUFR和GRIB等WMO二进制格式在气象领域广泛用于交换观测和模型数据，通常需要专门的工具来解码和可视化这些数据。
...
- BUFR：包含ECMWF集合预测系统生成的热带气旋轨迹数据
- NetCDF：包含月度温度异常数据

## 在Jupyter笔记本中解码下载的数据

为了演示如何解码下载的数据，我们将使用 'decode-bufr-jupyter' 镜像启动一个新的容器。

该容器将在您的实例上启动一个Jupyter笔记本服务器，其中包括 [ecCodes](https://sites.ecmwf.int/docs/eccodes) 库，您可以使用它来解码BUFR数据。

我们将使用 `~/exercise-materials/notebook-examples` 中的示例笔记本来解码下载的气旋轨迹数据。
...
- `docker run -d --name demo-decode-eccodes-jupyter` 启动一个新的容器，并以分离模式（`-d`）运行，同时命名为 `demo-decode-eccodes-jupyter`
- `-v ~/wis2-downloads:/root/downloads` 将您的虚拟机上的 `~/wis2-downloads` 目录挂载到容器中的 `/root/downloads`。这是您从WIS2下载的数据存储位置，前一节课程中介绍了WIS2 Downloader的使用方法。
...
!!! note "关于 demo-decode-eccodes-jupyter 镜像"
    ...
    `demo-decode-eccodes-jupyter` 是为本次培训开发的镜像，基于包含ecCodes库的基础镜像，并添加了Jupyter笔记本服务器以及用于数据分析和可视化的Python包。

    此镜像的源代码，包括示例笔记本，可以在 [wmo-im/demo-decode-eccodes-jupyter](https://github.com/wmo-im/demo-decode-eccodes-jupyter) 找到。
    
启动容器后，您可以通过浏览器访问学生虚拟机上的Jupyter笔记本服务器，地址为 `http://YOUR-HOST:8888`。

您将看到一个屏幕，要求输入 "Password or token"。
...
双击 `example-notebooks` 目录以打开它。您应该会看到以下屏幕，列出了示例笔记本：
...
阅读笔记本中的说明，并运行单元格以解码下载的全球集合预测数据。通过点击单元格并点击工具栏中的运行按钮或按 `Shift+Enter` 来运行每个单元格。
...
阅读笔记本中的说明，并运行单元格以解码下载的热带气旋轨迹数据。通过点击单元格并点击工具栏中的运行按钮或按 `Shift+Enter` 来运行每个单元格。

最后，您应该会看到一个关于热带气旋轨迹的打击概率的图表：
...
??? success "点击查看答案"
    ...
    要更新笔记本以显示不同距离内的热带风暴轨迹预测概率，您可以更新计算打击概率的代码块中的 `distance_threshold` 变量。
...
阅读笔记本中的说明，并运行单元格以解码下载的月度温度异常数据。通过点击单元格并点击工具栏中的运行按钮或按 `Shift+Enter` 来运行每个单元格。
...
!!! note "解码NetCDF数据"
    ...
    NetCDF是一种灵活的格式，在本示例中报告了沿 'lat' 和 'lon' 维度的变量 'anomaly' 的值。不同的NetCDF数据集可能使用不同的变量名称和维度。
...
- [Panoply](https://www.giss.nasa.gov/tools/panoply/) - 一个跨平台应用程序，可绘制来自NetCDF、HDF、GRIB和其他数据集的地理参考和其他数组
