...
在本课程中，您将学习如何在提供的学生虚拟机上设置 WIS2 Downloader 实例，以及如何浏览其不同的服务。

!!! note "关于 WIS2 Downloader"
     
     WIS2 Downloader 可作为一个独立的 Docker Compose 项目使用，建议运行在与 wis2box 分开的服务器上，以避免下载过程干扰消息的发布。

     如果您希望开发自己的服务以订阅 WIS2 通知并下载数据，可以参考 [WIS2 Downloader 源代码](https://github.com/World-Meteorological-Organization/wis2downloader)。
...
在开始之前，请登录到您的学生虚拟机，并确保通过 SSH 隧道以下端口：
...
!!! note "在 Linux 和 macOS 上设置隧道"
    ...
    在 Linux 和 macOS 上，您可以直接从终端使用 SSH 命令中的 `-L` 参数设置隧道：
...
从 GitHub 下载最新的发布包，并在您的学生虚拟机上解压：
...
!!! note "检查运行中的容器"
    ...
    docker compose ps
    ```
    您应该会看到订阅管理器、MQTT 订阅者、UI、Celery 工作进程、Redis、Prometheus、Grafana 和 Loki 的服务。

## 访问 WIS2 Downloader UI

打开浏览器并导航到您的 WIS2 Downloader 实例的 UI，地址为 `http://<WIS2DOWNLOADER_BASE_URL>:8080`。

您会看到默认的登录页面，该页面显示的是 `Dashboard` 视图，其中嵌入了 Grafana 仪表板。
...
- **Dashboard** — 默认登录页面，嵌入的 Grafana 仪表板显示下载活动、队列状态以及运行服务的指标。也可以通过 `http://<WIS2DOWNLOADER_BASE_URL>:3000` 访问。
- **Catalogue View** — 通过搜索或过滤全局目录浏览可用的 WIS2 数据集。选择一个主题和保存目录，然后点击 *Subscribe* 开始下载。
- **Tree View** — 以可折叠树的形式浏览 WIS2 主题层次结构。适合在订阅之前探索可用主题。
- **Manual Subscribe** — 直接输入主题详细信息创建订阅，而无需依赖全局发现目录。适合更自由地使用通配符订阅主题，并允许访问未在 GDC 中找到的主题，例如 GTS 网关和非默认配置中发布在私有代理上的主题。
- **Manage Subscriptions** — 查看和管理所有活动订阅。在这里，您可以查看正在监控的主题并删除不再需要的订阅。
...
- 在 **Tree View** 中，通过像 MQTT Explorer 一样探索主题，从 GDC 目录中选择主题。
...
WIS2 Downloader 在 `<WIS2DOWNLOADER_BASE_URL>:5002` 上提供 REST API。确认服务是否准备就绪：
...
要创建订阅，请发送一个包含 MQTT `topic` 和可选 `target` 子目录的 `POST` 请求，文件将保存到该子目录中：
...
有关可用端点的完整列表（列出、获取、更新订阅等），请参阅 `<WIS2DOWNLOADER_BASE_URL>:5002/swagger` 上的交互式 Swagger 文档。
