...
!!! note "关于 WIS2 Downloader"
     ...
     WIS2 Downloader 作为一个独立的 Docker Compose 项目提供，建议运行在与 wis2box 不同的服务器上，以避免下载过程干扰消息发布。
...
从 GitHub 下载最新的发布压缩包，并在您的学生虚拟机上解压：
...
使用以下下载路径 `/home/{USER}/wis2-downloads`，然后按 Enter 使用用户和组的默认值。
...
- **Dashboard** — 一个嵌入式 Grafana 仪表板，显示下载活动、队列状态以及运行服务的指标。也可通过 `http://<WIS2DOWNLOADER_BASE_URL>:3000` 访问。
- **Catalogue View** — 通过搜索或过滤全局目录浏览可用的 WIS2 数据集。选择一个主题和保存目录，然后点击 *Subscribe* 开始下载。
- **Tree View** — 以可折叠树的形式浏览 WIS2 主题层次结构。适合在订阅之前探索可用主题。
- **Manual Subscribe** — 直接输入主题详细信息创建订阅，而无需依赖全局发现目录。适用于更自由地使用通配符订阅主题，并允许访问未在 GDC 中找到的主题，例如 GTS 网关和非默认配置下发布在私有代理上的主题。
...
如上例所示，您可以通过访问 `http://<WIS2DOWNLOADER_BASE_URL>:8080` 来进入运行实例的 UI。

从那里有三种方式设置订阅：

- 在 **Catalogue View** 中，通过类似于 GDC 门户的方式浏览可用主题。
- 在 **Tree View** 中，通过探索主题（类似于 MQTT Explorer）从 GDC 目录中选择一个主题。
- 在 **Manual Subscribe** 中，直接输入您想要的主题、过滤器和其他参数。

在接下来的练习中，我们将订阅来自所有 WIS2 节点的所有 synop 通知：
...
要查看当前的 WIS2 Downloader 配置，您可以使用以下命令：
...
WIS2 Downloader 在 `<WIS2DOWNLOADER_BASE_URL>:5002/api` 上公开了一个 REST API。确认服务已准备好：
...
要创建订阅，请发送一个包含 MQTT `topic` 和可选 `target` 子目录的 `POST` 请求，文件将保存到该目录中：
...
有关可用端点（列出、获取、更新订阅等）的完整列表，请参考 `<WIS2DOWNLOADER_BASE_URL>:5002/swagger` 上的交互式 Swagger 文档。
