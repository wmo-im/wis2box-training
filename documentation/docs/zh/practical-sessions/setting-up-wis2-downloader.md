...
您应该看到 `loki:latest` 列出并且 `ENABLED` 设置为 `true`。

创建一个专用的 `wis2` 用户组，将您的用户添加到该组，并相应地配置 `.env` 文件和下载目录：
...
打开浏览器并导航到您的 WIS2 Downloader 实例的 UI，地址为 `http://localhost:8080`。

您将进入默认设置为 `Help` 部分的登录页面，显示相关文档。
...
- **Dashboard** — 一个嵌入式 Grafana 仪表板，显示下载活动、队列状态以及运行服务的指标。也可通过 `http://localhost:3000` 访问。
- **Catalogue View** — 通过搜索或过滤全局目录浏览可用的 WIS2 数据集。选择一个主题和保存目录，然后点击 *Subscribe* 开始下载。
- **Tree View** — 以可折叠树的形式浏览 WIS2 主题层次结构。适用于在订阅之前探索可用主题。
- **Manual Subscription** — 通过直接输入主题和代理详细信息创建订阅，而无需依赖全局发现目录。适用于订阅特定 WIS2 节点或私有代理的主题。
- **Subscriptions** — 查看和管理所有活动订阅。从这里您可以看到正在监控的主题，并删除不再需要的订阅。
- **Settings** — 当前允许您从全局发现目录重新加载数据集目录。此部分将在未来版本中扩展，以涵盖 WIS2 Downloader 的一般配置和管理。
...
响应中包括分配给新订阅的 UUID。使用它在不需要时删除订阅：

```bash
curl -X DELETE <WIS2DOWNLOADER_BASE_URL>:5002/api/subscriptions/{id}
```

有关可用端点的完整列表（列出、获取、更新订阅等），请参考 `<WIS2DOWNLOADER_BASE_URL>:5002/api/openapi` 上的交互式 Swagger 文档。
