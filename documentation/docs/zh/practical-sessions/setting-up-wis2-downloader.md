...
!!! note "关于 WIS2 Downloader"
     ...
     WIS2 Downloader 是一个独立的 Docker Compose 项目，建议运行在与 wis2box 分开的服务器上，以避免下载过程干扰消息发布。
...
!!! note "在 Linux 和 macOS 上设置隧道"
    ...
    在 Linux 和 macOS 上，您可以直接在终端使用 SSH 命令的 `-L` 参数设置隧道：
...
这将根据默认值创建 `.env` 文件，并为 `FLASK_SECRET_KEY` 和 `REDIS_PASSWORD` 生成随机值。您可以使用 `cat .env` 查看文件内容——默认配置适用于单机部署。
...
- **Tree View** — 以可折叠树的形式浏览 WIS2 主题层次结构。适用于在订阅前探索可用主题。
- **Manual Subscribe** — 直接输入主题详细信息创建订阅，而无需依赖全局发现目录。适用于更自由地使用通配符订阅主题，并允许访问在非默认配置中未在 GDC 上找到的主题，例如 GTS 网关和私人代理发布的主题。
- **Manage Subscriptions** — 查看和管理所有活动订阅。从这里可以查看正在监控的主题，并删除不再需要的订阅。
- **Settings** — 当前允许从全局发现目录重新加载数据集目录。未来版本将扩展此部分以涵盖 WIS2 Downloader 的一般配置和管理。
- **Documentation** — 显示 WIS2 Downloader 的内置文档。

## 在 UI 中管理订阅

如上例所示，您可以通过访问 `http://<WIS2DOWNLOADER_BASE_URL>:8080` 进入运行实例的 UI。

设置订阅有三种方式：

- 在 **Catalogue View** 中，通过类似 GDC 门户的方式浏览可用主题。
- 在 **Tree View** 中，通过探索主题层次结构选择来自 GDC 目录的主题，类似于 MQTT Explorer。
...
有关可用端点的完整列表（列出、获取、更新订阅等），请参阅 `<WIS2DOWNLOADER_BASE_URL>:5002/swagger` 提供的交互式 Swagger 文档。
