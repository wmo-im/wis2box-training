...
!!! note "关于 WIS2 Downloader"
     ...
     WIS2 Downloader 是一个独立的 Docker Compose 项目，建议在与 wis2box 不同的服务器上运行，以避免下载过程干扰消息发布。
...
!!! note "如果不是在培训期间"
    ...
    以下步骤仅适用于服务器默认未开放所述端口的情况。在任何配置中，这些是使用 WIS2 Downloader 堆栈全部功能所需访问的唯一端口。

开始之前，请登录到您的学生虚拟机，并通过 SSH 隧道以下端口：
...
从 GitHub 下载最新版本的 tarball 并在您的学生虚拟机上解压：
...
这将根据默认值创建 `.env` 文件，并为 `FLASK_SECRET_KEY` 和 `REDIS_PASSWORD` 生成随机值。您可以使用 `cat .env` 查看文件内容——默认值适用于单机部署。
...
创建专用的 `wis2` 用户组，将您的用户添加到该组，并相应地配置 `.env` 文件和下载目录：
...
!!! note "检查运行中的容器"
    ...
    docker compose ps
    ```
    您应该看到订阅管理器、MQTT 订阅者、UI、Celery 工作线程、Redis、Prometheus、Grafana 和 Loki 的服务。

## 访问 WIS2 Downloader UI

打开网页浏览器，导航到您的 WIS2 Downloader 实例的 UI，地址为 `http://<WIS2DOWNLOADER_BASE_URL>:8080`。

您将看到默认的登录页面，该页面设置为 `Help` 部分，显示文档内容。
...
- **Dashboard** — 一个嵌入式 Grafana 仪表板，显示下载活动、队列状态以及运行服务的指标。也可以通过 `http://<WIS2DOWNLOADER_BASE_URL>:3000` 访问。
- **Catalogue View** — 通过搜索或筛选全局目录浏览可用的 WIS2 数据集。选择一个主题和保存目录，然后点击 *Subscribe* 开始下载。
- **Tree View** — 以可折叠树的形式浏览 WIS2 主题层次结构。适用于在订阅之前探索可用主题。
- **Manual Subscribe** — 直接输入主题和代理详细信息创建订阅，而无需依赖全局发现目录。适用于订阅来自特定 WIS2 节点或私有代理的主题。
- **Manage Subscriptions** — 查看和管理所有活动订阅。从这里可以看到正在监控的主题，并移除不再需要的订阅。
...
- 在 **Catalogue View** 中，通过类似于 GDC 门户的方式浏览可用主题。
- 在 **Tree View** 中，通过探索主题（类似于 MQTT Explorer）从 GDC 目录中选择主题。
- 在 **Manual Subscribe** 中，直接输入您希望订阅的主题、过滤器和其他参数。
...
接下来，通过以下命令进入您的学生虚拟机中的下载文件夹：

```bash
ls -R wisdownloader/downloads
```

现在，您应该看到由您的实例下载的一系列文件。
...
WIS2 Downloader 在 `<WIS2DOWNLOADER_BASE_URL>:5002/api` 处公开了一个 REST API。确认服务已准备好：
...
要创建订阅，请发送一个 `POST` 请求，其中包含 MQTT `topic` 和一个可选的 `target` 子目录，文件将保存到该目录中：
...
有关可用端点的完整列表（列出、获取、更新订阅等），请参阅位于 `<WIS2DOWNLOADER_BASE_URL>:5002/api/openapi` 的交互式 Swagger 文档。
