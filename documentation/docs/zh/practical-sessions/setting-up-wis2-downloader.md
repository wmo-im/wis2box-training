...
!!! note "关于 WIS2 Downloader"
     ...
     WIS2 Downloader 可作为独立的 Docker Compose 项目使用，建议运行在与 wis2box 分开的服务器上，以避免下载过程干扰消息的发布。
...
!!! note "如果不是在培训期间"
    ...
    以下步骤仅适用于服务器默认情况下无法访问所需端口的情况。在任何配置中，以下端口是使用 WIS2 Downloader 堆栈全部功能所需的唯一端口。
...
!!! note "在 Linux 和 macOS 上设置隧道"
    ...
    在 Linux 和 macOS 上，您可以直接从终端使用 SSH 命令的 `-L` 参数设置隧道：
...
!!! note "管理用户权限"
    您可以通过修改 `.env` 文件中的 `WIS2DWONLOADER_UID` 和 `WIS2DWONLOADER_GID` 来设置不同的用户和组值。
...
您将进入默认设置为 `Dashboard` 视图的登录页面，该页面显示 Grafana 仪表板。
...
- **Manual Subscribe** — 直接输入主题详细信息创建订阅，而无需依赖全局发现目录。适用于更自由地使用通配符订阅主题，并允许访问 GDC 中未找到的主题，例如 GTS 网关和非默认配置中发布在私有代理上的主题。
...
要查看 WIS2 Downloader 的当前配置，可以使用以下命令：
...
WIS2 Downloader 在 `<WIS2DOWNLOADER_BASE_URL>:5002` 上暴露了一个 REST API。确认服务已准备好：
