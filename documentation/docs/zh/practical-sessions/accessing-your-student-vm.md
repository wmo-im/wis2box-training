...
为了确保用户能够运行 Docker 命令，用户已被添加到 `docker` 组。

测试用户是否能够运行 Docker 的 hello-world 镜像，执行以下命令：
```bash
...
如果你的本地电脑安装了 WinSCP，可以使用它连接到学生虚拟机，检查主目录的内容，并在虚拟机和本地电脑之间上传或下载文件。
