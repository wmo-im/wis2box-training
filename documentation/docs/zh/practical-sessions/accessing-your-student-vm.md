...
!!! note
    ...
    如果你希望在本地培训课程之外运行本次培训，你可以通过任何云服务提供商提供自己的实例，例如：
    ...
    创建虚拟机后，请确保已安装 python、docker 和 docker compose，具体安装说明请参考 [wis2box-software-dependencies](https://docs.wis2box.wis.wmo.int/en/latest/user/getting-started.html#software-dependencies)。
...
测试你的用户是否可以运行 Docker 的 hello-world，执行以下命令：
```bash
...
如果你的本地电脑安装了 WinSCP，你可以使用它连接到你的学生虚拟机，检查主目录的内容，并在虚拟机和本地电脑之间上传或下载文件。

WinSCP 不是培训的必需工具，但如果你希望使用本地电脑上的文本编辑器编辑虚拟机上的文件，它会很有用。

以下是使用 WinSCP 连接到学生虚拟机的方法：

打开 WinSCP 并点击 "New Site"。你可以创建一个新的 SCP 连接，具体如下：
