...
WIS2 使用 MQTT 协议来宣传天气/气候/水文数据的可用性。WIS2 Global Broker 订阅网络中所有的 WIS2 Node，并重新发布其接收到的消息。Global Cache 订阅 Global Broker，下载消息中的数据，然后在 `cache` 主题上重新发布该消息，并附上一个新的 URL。Global Discovery Catalogue 从 Broker 发布发现元数据，并提供搜索 API。
...
MQTT Explorer 是一个有用的工具，用于浏览和查看特定 MQTT Broker 的主题结构以及正在发布的数据。
...
!!! note "关于 MQTT"
    ...
    要以编程方式使用 MQTT（例如，在 Python 中），您可以使用 MQTT 客户端库，例如 [paho-mqtt](https://pypi.org/project/paho-mqtt)，连接到 MQTT Broker 并处理接收到的消息。根据您的需求和技术环境，还存在许多 MQTT 客户端和服务器软件。
...
??? success "点击查看答案"
    ...
    告诉我们数据是由 WIS 中心 `br-inmet` 发布的，这是巴西国家气象研究所（Instituto Nacional de Meteorologia - INMET）的中心 ID。
...
??? success "点击查看答案"
    ...
    告诉我们数据是由德国气象局（Deutscher Wetterdienst, DWD）托管的 GTS-to-WIS2 网关发布的。GTS-to-WIS2 网关是一种特殊类型的数据发布者，它将来自全球电信系统（GTS）的数据发布到 WIS2。主题结构由 GTS 消息的 TTAAii CCCC 标头组成。
...
!!! question
    ...
    我们如何识别数据发布的时间戳？以及如何识别数据采集的时间戳？

??? success "点击查看答案"

    数据发布的时间戳包含在消息的 `properties` 部分，其键为 `pubtime`。

    数据采集的时间戳包含在消息的 `properties` 部分，其键为 `datetime`。
...
??? success "点击查看答案"
    ...
    URL 包含在 `links` 部分，其 `rel="canonical"`，由 `href` 键定义。

    您可以复制该 URL 并将其粘贴到浏览器中下载数据。

## 练习 3：查看 'origin' 和 'cache' 主题的区别

确保您仍然连接到 Global Broker，并使用练习 2 中描述的主题订阅 `origin/a/wis2/+/data/core/weather/surface-based-observations/synop` 和 `cache/a/wis2/+/data/core/weather/surface-based-observations/synop`。
...
??? success "点击查看答案"
    ...
    在 WIS2 网络中有多个 Global Cache，因此您将收到来自每个下载了该消息的 Global Cache 的一条消息。

    Global Cache 仅下载并重新发布发布在 `../data/core/...` 主题层级下的消息。
