...
WIS2 使用 MQTT 协议来宣传天气/气候/水文数据的可用性。WIS2 Global Broker 订阅网络中所有 WIS2 Nodes，并重新发布其接收到的消息。Global Cache 订阅 Global Broker，下载消息中的数据，然后在 `cache` 主题上以新 URL 重新发布该消息。Global Discovery Catalogue 从 Broker 发布发现元数据，并提供搜索 API。
...
MQTT Explorer 是一个浏览和查看指定 MQTT Broker 的主题结构以及发布数据的有用工具。
...
!!! note "关于 MQTT"
    ...
    要以编程方式使用 MQTT（例如，在 Python 中），您可以使用 MQTT 客户端库，例如 [paho-mqtt](https://pypi.org/project/paho-mqtt)，以连接到 MQTT Broker 并处理接收到的消息。根据您的需求和技术环境，还存在许多 MQTT 客户端和服务器软件。
...
点击 'BACK'，然后点击 'SAVE' 保存您的连接和订阅详情。然后点击 'CONNECT'：
